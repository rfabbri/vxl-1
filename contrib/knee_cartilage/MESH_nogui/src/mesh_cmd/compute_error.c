/* $Id: compute_error.c,v 1.1 2007-02-28 16:41:20 ntrinh Exp $ */
                           ...
                           int **dcl_buf, int *dcl_buf_sz)
{
  ...
  double dmin_sqr;      /* minimum distance squared */
  //int dmin_sign;         /* sign of minimum distance */

  double dist_sqr;      /* current distance squared */
  //int dist_sign;         /* signed of current distance, added by Nhon*/
  ...
  do {
    ...
    for (cur_cell = dcl[cell_idx].list[k].cell,
      ...
      do { /* cell has always one triangle at least, so do loop is OK */
        ...
        //// determine which side of the triangle is te
        //t = &triags[t_idx];
        //dist_sign = (scalprod_dv(&p,&(t->normal))-t->a_n) < 0 ? -1 : 1;
                    ...
                    struct prog_reporter *prog)
{
  ...
  //added by Nhon
  // allocate memory for errors at vertices
  me1->verror = xa_realloc(me1->verror,m1->num_vert*sizeof(*(me1->verror)));
  for (i=0; i<me1->mesh->num_vert; ++i)
  {
    me1->verror[i] = -1;
  }
  ...
  for (k=0, kmax=m1->num_faces; k<kmax; k++) {
    ...
    // added by Nhon
    // compute distance to other surface at end vertex
    // check for case no sample was done.
    if (tse.n_samples > 0)
    {
      me1->verror[m1->faces[k].f0] = (float)tse.err_lin[0];
      me1->verror[m1->faces[k].f1] = (float)tse.err_lin[n-1];
      me1->verror[m1->faces[k].f2] = (float)tse.err_lin[tse.n_samples-1];
    }
...
void calc_vertex_error(struct model_error *me, int *nv_empty, int *nf_empty)
{
  ...
  *nv_empty = 0;
  ...
  //// added by Nhon
  //// attemp to assign error at vertices that don't have samples using nearby triangles
  //// this is a hack. need to fix: find average of connected faces instead of just the first one
  //if (*nv_empty > 0)
  //{
  //  for (i=0; i<me->mesh->num_faces; i++) {
  //    n = me->fe[i].sample_freq;
  //    // only consider triangles that have only 1 sample
  //    if (n == 1) { 
  //      if (me->verror[me->mesh->faces[i].f0] == -2) {
  //        me->verror[me->mesh->faces[i].f0] = (float)me->fe[i].serror[0];
  //        (*nv_empty)--;
  //      }
  //      if (me->verror[me->mesh->faces[i].f1] == -2) {
  //        me->verror[me->mesh->faces[i].f1] = (float)me->fe[i].serror[n*(n+1)/2-1];
  //        (*nv_empty)--;
  //      }
  //      if (me->verror[me->mesh->faces[i].f2] == -2) {
  //        me->verror[me->mesh->faces[i].f2] = (float)me->fe[i].serror[n-1];
  //        (*nv_empty)--;
  //      }
  //    } 
