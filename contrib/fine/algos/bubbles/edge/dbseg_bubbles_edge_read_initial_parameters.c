     ...
     
{
  ...
  pars->intensity_intervals[0] = 0;
  pars->intensity_intervals[1] = 99999.0;
  pars->intensity_intervals[2] = -1.0;
  pars->height = 128;
  pars->width  = 128;
  pars->userinit  = 0;
  pars->dam = 0;
  ...
  for (--argc, ++argv; argc; --argc, ++argv ) {
    ...
    if ( **argv == '-' ) {  
      ...
      if ( keymatch( *argv, "imagetype", 9))
        ...
        ii=0;
        while (1) {
          pars->intensity_intervals[ii] =  atoi(*++argv);
          pars->intensity_intervals[ii+1] =  atoi(*++argv);
          --argc; --argc;  ii+=2;
          if ( **argv != '-' )
            break;
        }       
...

{
  ...
  printf("-band_size          %f\n", pars->band_size);
  printf("-initialize_min     %i\n", pars->intensity_intervals[0]);
  printf("-initialize_max     %i\n", pars->intensity_intervals[1]);
