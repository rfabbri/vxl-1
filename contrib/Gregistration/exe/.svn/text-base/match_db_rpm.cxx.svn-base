...
int main(int argc, char *argv[]) {
  ...
  vul_arg<float> outlierval("-outlier","Value of Outlier",0.001);
  vul_arg<float> lambda1("-lambda1","lamda1",200);
  vul_arg<float> lambda2("-lambda2","lamda2",0.1);
...
//  vcl_cout << " created trees\n";
  ...
  for (unsigned int i = 0; i<D; i++) {
      for (unsigned int j=i+1 ; j<D; j++) {
          vul_timer t;
          dbrl_rpm_tps_params params(lambda1(),lambda2(),Mconvg(),Tinit(),Tend(),outlierval(),annealrate());
