...
int main(int argc, char *argv[]) {
    ...
    vox_third_order_edge_det_params_sptr params =
            new vox_third_order_edge_det_params("vox_third_order_edge_det");
    ...
    for(; fit; ++fit)
    {
        ...
        output_file = output_file + "/" + pyramid_object_name;

        bool write_status(false);

        if ( ( params->output_extension_() == ".cem" ||
                params->output_extension_() == ".cemv" ))
        {
            ...
            dbdet_sel_storage_sptr sel_out;
            sel_out.vertical_cast(sel_results[0]);
            ...
            {
                for(int x = 0; x < img_sptr->ni(); x++)
                {
                    orient_array[y][x] = 0;
                    edgemap_image(x, y, 0) = 0;
                }
