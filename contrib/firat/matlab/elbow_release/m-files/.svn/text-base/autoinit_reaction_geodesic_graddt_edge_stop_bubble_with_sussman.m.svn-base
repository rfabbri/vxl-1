...
function phi = autoinit_reaction_geodesic_graddt_edge_stop_bubble_with_sussman(I, slice_index, beta0, alpha, delta_t, max_num_iter, hx, hy, narrow_band, edg, edge_length, debug_mode)	
	I_orig = I;
	I = [I(1,:);I];
	I = [I;I(end,:)];
	I = [I(:,1), I];
	...
	%figure;imagesc(phi);
	phi = sussman2d(phi, hx, narrow_band);
	%figure;imagesc(phi);
	%pause
	%%evolution
	beta1 = 0;
	...
	for i = 1:max_num_iter		
		...
		phi = reinitlevelset_narrow(phi,narrow_band);
		phi = sussman2d(phi, hx, narrow_band);
		if check_stop_condition(phi_prev, phi, 0.05)
			break
		end		
	end
