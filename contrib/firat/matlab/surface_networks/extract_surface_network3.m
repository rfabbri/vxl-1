% This is extract_surface_network3.m
...
function [mins, maxs, saddles, assignments, slope_lines] = extract_surface_network3(I, sigma)
	[nrows,ncols] = size(I);
	...
	%tic;[saddles, S] = detect_saddles_wc_ws(I, sigma);toc
	[saddles, S] = hexsaddle(I, sigma);
	%bsaddles = get_boundary_saddles(S, nrows, ncols);
	%saddles = [saddles; bsaddles];
	inc = 2*pi/60;
	theta = (0:inc:2*pi+inc)';
	num_angles = length(theta);
	A = 1.2*[cos(theta) sin(theta)];
	...
	for i = 1:size(assignments, 1)
		...
		plot([sad(1)], [sad(2)],['c' shapes(3)], 'MarkerSize', 10, 'LineWidth', 2);
		plot([p(1)], [p(2)],['c' shapes(T)], 'MarkerSize', 10, 'LineWidth', 2);
	end	
	hold off
...
function bsaddles = get_boundary_saddles(S, nrows, ncols)
	[pks, pos1] = findpeaks(-S(1,:));
	[pks, pos2] = findpeaks(-S(end,:));
	[pks, pos3] = findpeaks(-S(:,end));
	[pks, pos4] = findpeaks(-S(:,1));
	[pks, pos5] = findpeaks(S(1,:));
	[pks, pos6] = findpeaks(S(end,:));
	[pks, pos7] = findpeaks(S(:,end));
	[pks, pos8] = findpeaks(S(:,1));
	
	bsaddles = [pos1' ones(length(pos1),1)      ; pos2' nrows*ones(length(pos2),1);...
		    ncols*ones(length(pos3),1) pos3; ones(length(pos4),1) pos4;...
		    pos5' ones(length(pos5),1)      ; pos6' nrows*ones(length(pos6),1);...
		    ncols*ones(length(pos7),1) pos7; ones(length(pos8),1) pos8];
end
