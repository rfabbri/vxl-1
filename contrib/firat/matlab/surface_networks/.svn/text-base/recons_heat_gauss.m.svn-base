% This is recons_heat_gauss.m

% \file
% \author Firat Kalaycilar (firat_kalaycilar@brown.edu)
% \date May 7, 2012

function R = recons_heat_gauss(I, mins, maxs, saddles, slope_lines, assignments, mode)
	[nrows,ncols] = size(I);
	...
	Aold = A;
	first_time = true;
	while first_time || max(abs(Aold(:)-A(:))) > 1e-7
		Aold = A;
		first_time = false;		
		Lap = imfilter(A, L, 'replicate', 'conv');		
		A = A + speed*Lap;
		A(index) = value;	
