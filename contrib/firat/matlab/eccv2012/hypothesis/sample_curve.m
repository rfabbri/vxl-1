...
function sampled_curve_coords = sample_curve(curve_coords, N, is_closed, iteration)
    ...
    if isempty(curve_coords)
        sampled_curve_coords = [];
        return
    end
    ...
    if std(V,1) > 1e-10 && iteration < 400
        iteration = iteration + 1;
        sampled_curve_coords = sample_curve(sampled_curve_coords, N, is_closed, iteration);
    end
end
