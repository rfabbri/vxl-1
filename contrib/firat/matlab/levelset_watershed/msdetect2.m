...
function [saddles, mins, WS, saddle_ws, J, K, H, W] = msdetect2(I, sigma, do_plot)
	...
	while true
	   ...
	   for i = 1:Event_count
	        ...
	        if false&&cp_type == 3
    			...
    			if all(cp_loc-K == [24 9])
    			vp'
    				imagesc(Visited_pixels + saddle_mask);hold on; plot(cp_loc(1),cp_loc(2),'r.'); hold off
    				pause
    			end
    		end	
            ...
            elseif cp_type == 3 % collision: saddle creation
            	%saddle_mask(cp_loc(2), cp_loc(1)) = 1;
                saddles.add(cp_loc);            
            end		       	        
       end	   
