% This is /lemsvxl/contrib/firat/matlab/levelset_watershed/evolving_basin_critical_point_detector.m.

% \file
% \author Firat Kalaycilar (firat_kalaycilar@brown.edu)

function [saddles, mins, maxs, J, K, H, W] = evolving_basin_critical_point_detector(I, sigma, do_plot)
	...
	while true
	   ...
	   for i = 1:Event_count
	        ...
	        neig = imdilate(ith_event_mask, strel_square_3);
	        Unique_labels_min = unique(Label_curr_min(neig));        
	        Unique_labels_min(Unique_labels_min <= 0) = [];
	        num_unique_min = length(Unique_labels_min);	 
	        
	        Unique_labels_max = unique(Label_curr_max(neig));
	        Unique_labels_max(Unique_labels_max <= 0) = [];
	        num_unique_max = length(Unique_labels_max);	  
	        
	        subplot(1,2,1); imagesc(Label_curr_min);
			axis([K+1 W-K K+1 H-K]);
			subplot(1,2,2); imagesc(Label_curr_max);
			axis([K+1 W-K K+1 H-K]);
			pause 
        
	        if num_unique_min == 1 && num_unique_max == 1 % regular point
	            continue
	        elseif num_unique_min == 0	            
	            cp_type = 1; %min
	        elseif num_unique_max == 0    
	            cp_type = 2; %max	                  
            elseif num_unique_min > 1
				cp_type = 3; %saddle (min-min)
				%figure
				%subplot(1,2,1); imagesc(Label_curr_min);
				%axis([K+1 W-K K+1 H-K]);
				%subplot(1,2,2); imagesc(Label_curr_max);
    ...
    [tbd1, ign] = find(maxs < 0.5);
    tbd2 = find(maxs(:,1) > W-2*K+.5);
    tbd3 = find(maxs(:,2) > H-2*K+.5);
    maxs([tbd1; tbd2; tbd3],:) = [];
    
    saddles3 = reshape(cell2mat(cell(saddles3.toArray(saddles3))), 2, saddles3.size())'-K;
    [tbd1, ign] = find(saddles3 < 0.5);
    tbd2 = find(saddles3(:,1) > W-2*K+.5);
    tbd3 = find(saddles3(:,2) > H-2*K+.5);
    saddles3([tbd1; tbd2; tbd3],:) = [];
