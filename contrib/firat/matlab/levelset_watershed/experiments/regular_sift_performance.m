function TP = regular_sift_performance(sift_folder, save_decision_histogram)
	...
	for i = 1:num_files
		...
		desc{i} = double(X.reg_d);
		counts(i) = size(X.reg_d, 2);
	...
	for i = 1:num_files
		...
		compute_h = true;
		X = load([sift_folder '/' files(i).name], 'reg_h');
		if isfield(X, 'reg_h') && length(X.reg_h) == num_unique_classes
			h = X.reg_h;
		else 
		    %%
		    q = desc{i}'; %query
			...
			if save_decision_histogram 
				reg_h = h;           
			    save([sift_folder '/' files(i).name], 'reg_h', '-append');
    		end
		end
		[ign, max_id] = max(h);
