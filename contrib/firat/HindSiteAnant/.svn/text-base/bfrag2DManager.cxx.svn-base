...
void bfrag2DManager::displayCurve( bfrag_curve *theCurve )
{
...
void bfrag2DManager::newGrid( int w, int h, int numPairs, int numPuzzles )
{
  ...
  for (unsigned index = 0; index < theGrid->size(); index++)
  {
    int cols = theGrid->cols();
    int col = index / cols;
    int row = index % cols;
  ...
  vgui_image_tableau_sptr newImageTab;
  vgui_composite_tableau_sptr newCompTab;

  vgui_bfrag2D_tableau_sptr newBfragTab;
  vgui_viewer2D_tableau_sptr newViewerTab;
  vgui_text_tableau_sptr newTextTab;
  ...
  for( int yy = 0; yy < w; yy++ )
  {
    ...
    {
      //newImageTab = vgui_image_tableau_new();
      /*newTextTab = vgui_text_tableau_new();
      newImageTab = vgui_image_tableau_new(newTextTab);
      newBfragTab = vgui_bfrag2D_tableau_new(newImageTab);
      ...
      newTextTab = vgui_text_tableau_new();
      newImageTab = vgui_image_tableau_new();
      newCompTab = vgui_composite_tableau_new(newImageTab, newBfragTab, newTextTab);
      newViewerTab = vgui_viewer2D_tableau_new(newCompTab);
...
void bfrag2DManager::addCurveAt( bfrag_curve *theCurve, int gridIndex,
                                 int offset_x, int offset_y, const vgui_style_sptr& style)
{
  gridBfrag2DTableaux[gridIndex]->addCurve( theCurve, offset_x, offset_y, style );
}
...
void bfrag2DManager::show_cost(double cost, int index)
{
  int cols = theGrid->cols();
  int col = index / cols;
  int row = index % cols;
  vgui_viewer2D_tableau *tab = static_cast<vgui_viewer2D_tableau *>(theGrid->get_tableau_at(row, col).ptr());
  vgui_composite_tableau *comp_tab = static_cast<vgui_composite_tableau *>(tab->get_child(0).ptr());
  vgui_text_tableau *text_tab;
  for(unsigned i=0; i < 3; i++)
  {
    vgui_tableau *check_tab = comp_tab->get_child(i).ptr();
...
void bfrag2DManager::center_image(vgl_box_2d<double> &box, int index)
{
  int cols = theGrid->cols();
  int col = index / cols;
  int row = index % cols;
  vgui_viewer2D_tableau *tab = static_cast<vgui_viewer2D_tableau*>(theGrid->get_tableau_at(row, col).ptr());
//  tab->center_event();
//  tab->center_image(box.centroid_x(), box.centroid_y());
//  tab->center_image(box.width(), box.height());
//  tab->zoomout(3.0, 0, 0);
}

void bfrag2DManager::show_image(vil_image_view<vxl_byte> &image, int index)
{
  int cols = theGrid->cols();
  int col = index / cols;
  int row = index % cols;
  ...
  {
    vgui_tableau *check_tab = comp_tab->get_child(i).ptr();
    if(check_tab->type_name() == "vgui_image_tableau")
      image_tab = static_cast<vgui_image_tableau *>(check_tab);
  }

//  vgui_image_tableau_sptr new_tab = vgui_image_tableau_new();
//  comp_tab->add(new_tab);
//  comp_tab->remove(image_tab);
//  new_tab->set_image_view(image);

  image_tab->set_image_view(image);

//  tab->post_idle_request();
//  tab->center_event();
...
//  tab->zoomout(4.0, 0, 0);
/*
  int grid_w = theGrid->get_w();
  int grid_h = theGrid->get_h();
  double x_factor = double(grid_w) / image.ni();
  double y_factor = double(grid_h) / image.nj();
  double factor = x_factor < y_factor ? x_factor : y_factor;
  if(factor < 1.0)
    tab->zoomout(1/factor, 0, 0);
  else
    tab->zoomin(factor, 0, 0);*/
}
