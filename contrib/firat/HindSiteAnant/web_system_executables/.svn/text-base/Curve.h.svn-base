...
//forward declaration b/c bfrag2D includes this header (Curve.h)
class bfrag2D;
...
public:
    ...
    // return the point of the given index from the coarse points list
...
void Curve<ptType,floatType>::smooth(floatType win)
{
  ...
  else
  {
    ...
    {
      ec=_extrema[k%num_extr]; //current extrema
      ep=_extrema[k-1]; //previous extrema
...
// CAN : The computed box is not used at all! Candidate for deletion.
template <class ptType,class floatType>
void Curve<ptType,floatType>::computeBox() 
{
  int i;
  floatType min_x, min_y, max_x, max_y;
  _box.clear();
  ...
  _box.push_back(min_x);
  _box.push_back(min_y);
  _box.push_back(max_x);
  _box.push_back(max_y);
}
...
void Curve<ptType,floatType>::computeCurvatures(){
  ...
  for (i=1;i<_numPoints;i++){
    ...
    cdx=_dx[i];
    cdy=_dy[i];
//    dL=_arcLength[i]-_arcLength[i-1]; // CANARAS: THIS IS WRONG!!!!!!!
    dL=_arcLength[i];
...
void Curve<ptType,floatType>::computeAngles()
{
  ...
  _angle.clear();
  _angle.push_back(0.0); // for closed curve, this will be dealt with later
  _totalAngleChange=0.0;
...
{  
  ...
  for (int i=0;i<size;i++) 
  {
    theta = atan2(_ptArray[(i+1)%size].y()-_ptArray[i].y(), _ptArray[(i+1)%size].x()-_ptArray[i].x());
    // make sure the value of theta is greater than zero
    if (theta<0)
      theta +=2*M_PI;
...
{  
  ...
  {
    temp.first  = aveTangent(i, -VICINITY); // go negative direction
    temp.second = aveTangent((i+1)%_tangent.size(), VICINITY); // go positive direction
...
//    outfp << i << " "  << _ptArray[i].x() << " " << _ptArray[i].y() << " " << angle_diff[i] << vcl_endl;
    ...
    {
      if ((angle_diff[i]<M_PI) && (angle_diff[i]>MIN_TAN_TURN))
...
{ 
  ...
  for(int i=1;i<_numPoints;i++)
  {
    ...
    if(length>=dS) 
    {
      ...
      // CAN : Below could be changed with length -= dS, which makes more sense
      length=0;
    }
  }
  ...
  empty();
  // fill the points vector with the new set of sampled points
...
void Curve<ptType,floatType>::coarseResample(floatType dS)
{
  ...
  computeExtrema();
  // Here, the extrema indices are according to the fine sampled representation
...
//  stati++;
  ...
  empty();
  // now fill them such that the first extrema is the first point
  for(i=0;i<temp_size;i++)
    append(temp_pts[i]);
  // put the first point to the end of the contours point list
  // so that the circularity is achieved
  append(point(0));
  ...
  for(i=0;i<extrema_size_int;i++)
    _extrema[i]-=_offset;
  // add the last point as one of the extrema
  ...
  for(i=1;i<extrema_size_int;i++)
  {
    ...
    // Here, the extrema indices are changed to be according to the coarse sampling
