...
void bfrag_Manager::pairMatch()
{
  ...
  _puzzleSolver->setTopContours( bfragContours );
...
void bfrag_Manager::load_xml(vcl_string filename, int chosen, vcl_string filename_images, vcl_string image_files)
{
  if(chosen == 0)
  {
    SHOW_IMAGES = false;
  ...
  if(SHOW_IMAGES)
  {
    ...
    for( unsigned w = 0; w < states_.size(); w++ )
    {
	 ...
	 filename<<image_files<<w<<".jpg";
	 vcl_string fillename=filename.str();
	  vil_save(temp_image,fillename.c_str());
