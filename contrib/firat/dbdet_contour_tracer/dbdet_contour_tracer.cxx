...
#include <dbnl/algo/dbnl_eno_image_with_shocks.h> //added by FIRAT
...
bool dbdet_contour_tracer::
trace_sedt_image_with_shock_capturing(const vil_image_view<double >& s_edt_double)
{
	unsigned nj = s_edt_double.nj();
	unsigned ni = s_edt_double.ni();
	...
	// curvature smoothing
	//curvature_smoothing(s_edt_double.top_left_ptr(), curvature_smooth_beta_ , curvature_smooth_nsteps_, 0, nj, ni);

	// trace_eno_zero_xings
	//Tracer trac;

	unsigned num_of_contours = trace_eno_zero_xings_with_shock_capturing(s_edt_double, contours_);
...
trace_eno_zero_xings_with_shock_capturing(const vil_image_view<double> &img /*surface*/, vcl_vector<vcl_vector<vsol_point_2d_sptr> >& contours)
{
	...
	{ // begin block
		...
		dbnl_eno_image_with_shocks *eno_image = new dbnl_eno_image_with_shocks();
		eno_image->interpolate(&img);
...
#undef label_assign
	...
	dbdet_contour_tracer_grid tr_grid(width, height, &xings);
	dbdet_trace_all_contours_on_grid(tr_grid, contours);
	return contours.size();
}

int
trace_eno_zero_xings_with_shock_capturing_old(const vil_image_view<double> &img /*surface*/, Tracer *ptrac)
{
	void allocate_data_tracer(Tracer *ptrac, int size);
	void subpixel_contour_tracer_all(Tracer *ptrac, Xings *xings, int height, int width);
