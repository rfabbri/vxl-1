                                   ...
                                   )
{
  ...
  if (active_&gesture_close_polyg_(e))
  {
          active_=false;
          //adding last point
      drawnvert.push_back( vgl_point_2d<double> (last_x , last_y) );
      drawnvert_so.push_back( new vsol_point_2d(last_x , last_y) );
     ...
     //Creating a tableeu for the drawn polygon
      ...
      for (int k=0;k<pts_in_stor.size();k++)
      {
          ...
          tableau_->add_vsol_polygon_2d( drawn_so, polyg_style);
          drawnvert.clear();
