// This is brl/bpro/bpro_spatemp_process_manager.cxx

//:
// \file

#include "bpro_spatemp_process_manager.h"
...
//: Constructor
bpro_spatemp_process_manager::bpro_spatemp_process_manager()
{
...
bool
bpro_spatemp_process_manager::recording_macro()
{
  return recording_macro_;
}

void
bpro_spatemp_process_manager::start_recording_macro()
{
  recording_macro_ = true;
}

void
bpro_spatemp_process_manager::stop_recording_macro()
{
  recording_macro_ = false;
}

void
bpro_spatemp_process_manager::set_window_size(int size)
{
  window_size_ = size;
}

int
bpro_spatemp_process_manager::window_size()
{
  return window_size_;
}
...
void
bpro_spatemp_process_manager::set_costum_window(int size)
{
...
void 
bpro_spatemp_process_manager::unset_costum_window()
{
  allow_costum_window_=false;
}

bool
bpro_spatemp_process_manager::check_process_ostreams(bpro_process_sptr const &process,
                                                ...
                                                vcl_string const & ostream_type) 
{
 ...
 //   {
 //     DATABASE_MANAGER->update_selected_tuple_value(selec, "ostream", ostream_name);
 //     DATABASE_MANAGER->update_selected_tuple_value(selec, "type", ostream_type);

 //   }*/

 //   DATABASE_MANAGER->add_tuple("ostream-process", new brdb_tuple(process->name(),ostream_name,ostream_type));
...
bool
bpro_spatemp_process_manager::set_process_ostreams (const bpro_process_sptr& process, 
...
void
bpro_spatemp_process_manager::set_repository(const vidpro_repository_sptr& rep)
{
...
bpro_spatemp_process_manager::get_repository_sptr() const
{
...
bool
bpro_spatemp_process_manager::get_process_input_names(vcl_map<vcl_string,vcl_vector<vcl_string> > &input_names,
...
//takes care of seeing streams as images for processes
bool
bpro_spatemp_process_manager::get_process_output_names(vcl_map<vcl_string,vcl_vector<vcl_string> > &output_names,
                                                  const bpro_process_sptr& process )
{
...
bool 
bpro_spatemp_process_manager::add_missing_input_names(vcl_vector<vcl_string> &input_names, const bpro_process_sptr& process)
{
...
bool
bpro_spatemp_process_manager::go_to_next_frame()
{
  absolute_frame_++;
...
bool
bpro_spatemp_process_manager::go_to_prev_frame()
{
    absolute_frame_--;
...
bool
bpro_spatemp_process_manager::load_frame_from_istream( int frame, vidpro_istream_storage_sptr& vistream_sto)
{
...
bool
bpro_spatemp_process_manager::load_object_from_stream(int global_frame, int local_frame, 
...
bool 
bpro_spatemp_process_manager::new_ostream(const vcl_string& directory,
                                     ...
                                     const unsigned int init_index)
{
  ...
  //add to database
  DATABASE_MANAGER->add_tuple("global_data", new brdb_tuple(storage_name, sto->type(), sto));

  return true;
}

bool 
bpro_spatemp_process_manager::new_obj_stream(const vcl_string& directory,
                                     const vcl_string& storage_name)
{
    ...
    DATABASE_MANAGER->add_tuple("global_data", new brdb_tuple(storage_name,vcl_string("object stream"),sto));

  return true;
}

bool 
bpro_spatemp_process_manager::write_to_object_ostream(const bpro_storage_sptr& sto,
                                                 vcl_string const & ostream_name)
...
bool 
bpro_spatemp_process_manager::write_to_ostream(const bpro_process_sptr& process,
                                         const bpro_storage_sptr& sto)
{

  if(!DATABASE_MANAGER->exists("ostream-process"))
    return false;
...
bool
bpro_spatemp_process_manager::run_process_on_current_frame( const bpro_process_sptr& process,
                                                      vcl_set<bpro_storage_sptr>* modified )
{
  ...
  if( to_return ){
    for(int a = process->min_frame(); a < process->max_frame(); ++a){
...
bool
bpro_spatemp_process_manager::close_ostreams()
{
  ...
  for (; it != process_queue.end(); ++it, i++){
      ...
      mit!=ostream_names.end(); mit++)
    {
      ...
      if(type=="ostream")
      {
        ...
        DATABASE_MANAGER->add_tuple("global_data", new brdb_tuple(mit->second,vcl_string("istream"),is_sto));
...
bool
bpro_spatemp_process_manager::run_process_queue_on_current_frame( vcl_set<bpro_storage_sptr>* modified )
{
  vcl_vector< bpro_process_sptr >::iterator it = process_queue.begin();
  unsigned i = 0;
  for (; it != process_queue.end(); ++it, i++){
...
bool
bpro_spatemp_process_manager::finish_process( int first_frame, int last_frame,
                                        const bpro_process_sptr& process,
                                        vcl_set<bpro_storage_sptr>* modified )
{
...
bool
bpro_spatemp_process_manager::finish_process_queue( int first_frame, int last_frame,
                                              vcl_set<bpro_storage_sptr>* modified )
{
  vcl_vector< bpro_process_sptr >::iterator it = process_queue.begin();
