          ...
          m_iterator != model_fragments_.end() ; ++m_iterator)
    {
        ...
        for ( unsigned int y=bbox.min_y(); y <= bbox.max_y(); y=y+stride)
        {
            for ( unsigned int x=bbox.min_x(); x <= bbox.max_x() ; x=x+stride) 
            {
                ...
                {
                    for ( unsigned int c=0; c < ldr_1_descriptor.size() ; ++c)
                    {
                        descriptors.push_back(ldr_1_descriptor[c]);
                    }
                }

                {
                    for ( unsigned int c=0; c < ldr_2_descriptor.size() ; ++c)
                    {
                        descriptors.push_back(ldr_2_descriptor[c]);
                    }
                }

                {
                    for ( unsigned int c=0; c < ldr_3_descriptor.size() ; ++c)
                    {
                        descriptors.push_back(ldr_3_descriptor[c]);
                    }
                }


                {
                    for ( unsigned int c=0; c < ldr_4_descriptor.size() ; ++c)
                    {
                        descriptors.push_back(ldr_4_descriptor[c]);
                    }
                }

                class_labels.push_back((*m_iterator).first);
                class_labels.push_back((*m_iterator).first);
                class_labels.push_back((*m_iterator).first);
                class_labels.push_back((*m_iterator).first);
    ...
    bool debug)
{
    ...
    for ( unsigned int y=bbox.min_y(); y <= bbox.max_y(); y=y+stride)
    {
        for ( unsigned int x=bbox.min_x(); x <= bbox.max_x() ; x=x+stride) 
        {
            ...
            double model_rgb_1 = (*model_channel1)(model_pt.x(),model_pt.y());
            double model_rgb_2 = (*model_channel2)(model_pt.x(),model_pt.y());
            double model_rgb_3 = (*model_channel3)(model_pt.x(),model_pt.y());
            ...
            double query_rgb_1 = (*query_channel1)(query_pt.x(),query_pt.y());
            double query_rgb_2 = (*query_channel2)(query_pt.x(),query_pt.y());
            double query_rgb_3 = (*query_channel3)(query_pt.x(),query_pt.y());
