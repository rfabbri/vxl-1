//this is /contrib/bm/dsm2/io/dsm2_io_manager_base.cxx
#include<dsm2/io/dsm2_io_manager_base.h>
...
void vsl_b_read(vsl_b_istream &is, dsm2_manager_base* &p)
{
	...
	if(not_null_ptr)
	{
		if( p = dynamic_cast<dsm2_manager_pixels<unsigned,
					dsm2_state_machine< dsm2_node_gaussian<bsta_gaussian_sphere<double,1> >,
										dsm2_transition_map<unsigned,unsigned,unsigned> > >*>(p) );
		else if(p = dynamic_cast<dsm2_manager_pixels<unsigned,
					dsm2_state_machine< dsm2_node_gaussian<bsta_gaussian_sphere<double,1> >,
										dsm2_transition_map<unsigned,unsigned,double> > >*>(p));
		else if(p = dynamic_cast<dsm2_manager_pixels<unsigned,
					dsm2_state_machine< dsm2_node_gaussian<bsta_gaussian_full<double,2> >,
										dsm2_transition_map<unsigned,unsigned,unsigned> > >*>(p) );
		else if(p = dynamic_cast<dsm2_manager_pixels<unsigned,
					dsm2_state_machine< dsm2_node_gaussian<bsta_gaussian_full<double,2> >,
										dsm2_transition_map<unsigned,unsigned,double> > >*>(p) );
