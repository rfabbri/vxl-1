...
class bsta_gaussian_full : public bsta_gaussian<T,n>
{
 public:
  ...
  //: just a stub 
  typename bsta_distribution<T,n>::vector_type sample(vnl_random& ran_gen) const
  {
    bsta_distribution<T,n>::vector_type d = bsta_gaussian<T,n>::mean_;
   	 vcl_cout<<"Warning: sampling of full covariance gaussian distribution not implemented!"<<vcl_endl;
   return d;
  }

  T entropy() const
  {
  return T(0.5)*T(log(two_pi_e_power<n>::value()*det_covar_));
  } 
  bsta_gaussian_full<T,n> operator + (const bsta_gaussian_full<T,n> &dist) const;
  bsta_gaussian_full<T,n> operator - (const bsta_gaussian_full<T,n> &dist) const;
  bsta_gaussian_full<T,n> operator * (T scale) const;
 protected:
