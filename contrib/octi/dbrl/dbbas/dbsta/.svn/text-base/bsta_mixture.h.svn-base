...
#include <vnl/vnl_random.h>
 ...
 enum compact_type {Heap,Conservative,Prune,Dominant};
                                                       ...
                                                       dist_::dimension>
{
 ...
 private:
  ...
  class sort_weight
  {
   ...
   class sort_entropy
    {
    public:
    bool operator() (const dist_& d1,T w1, const dist_& d2,T w2) const
      { 
		//  vcl_cout<<d1.entropy()<<" , "<<d2.entropy()<<vcl_endl;
		  return d1.det_covar()/w1 > d2.det_covar()/w2; 
	 }
    };
 ...
 public:
  // Default Constructor
  bsta_mixture<dist_>():compactify(Conservative) {}
 ...
 bsta_mixture<dist_>& operator= (const bsta_mixture<dist_>& rhs)
  {
   ...
   bsta_mixture<dist_> operator + (const bsta_mixture<dist_>& rhs) const;
   void operator += (const bsta_mixture<dist_>& rhs);
 
   bsta_mixture<dist_> operator - (const bsta_mixture<dist_>& rhs) const;
   void operator -= (const bsta_mixture<dist_>& rhs);
   
   bsta_mixture<dist_> operator * (T scale) const ;
   void operator *= (T scale);
   
   bsta_mixture<dist_> operator / (T scale) const; 
 
 //: removes duplicate components from a mixture. Components are considered to be equal if their parameters
 // are near equal within a tolerance
 compact_type compactify;

 bool removeDuplicates();
 //: merge according to a criterium the lowest weighted components
 bool compactifyLowestWeight();
 //: merge according to a criterium consecutive mixture pairs.
 bool compactify2Paired();
 //:delete the lowest weighted components until there are @max_components left
 bool compactifyCrude();
 //:divide the component vector into @max_components partitions and then merge according to a criterium
 //:each partition in one single component
 bool compactifyNPaired();

 bool compactifyHeapWeight(bool reversal=false);
 
 bool compactifyConservativeHeapWeight();

 //pairwise merge the components in the mixture - assumes a sorted vector as an input
 vcl_vector<component*> recursiveMerge(vcl_vector<component*> vec);
