...
bsta_gaussian_indep<T,n>::operator + (const bsta_gaussian_indep<T,n> &dist) const
{
	return bsta_gaussian_indep<T,n>(this->mean()+dist.mean(),this->covar()+dist.covar());
...
bsta_gaussian_indep<T,n>::operator + (const bsta_gaussian_full<T,n> &dist) const
{
	
	vnl_matrix_fixed<T,n,n> tmpMat;
	 for (int i=0;i<n;i++)
		{
			tmpMat[i][i]=this->covar()[i];
		}
	
	return bsta_gaussian_full<T,n>(this->mean()+ dist.mean(),dist.covar()+tmpMat);
...
template <class T, unsigned int n>
bsta_gaussian_indep<T,n> 
bsta_gaussian_indep<T,n>::operator * (T scale)	const
{
	
	bsta_gaussian_indep<T,n>::vector_ v=this->mean();
	bsta_gaussian_indep<T,n>::covar_type cov=this->covar();
	v*=scale;
	cov*=scale*scale;
	bsta_gaussian_indep<T,n> gauss(*this);
	gauss.set_covar(cov);
	gauss.set_mean(v);
	return gauss;
