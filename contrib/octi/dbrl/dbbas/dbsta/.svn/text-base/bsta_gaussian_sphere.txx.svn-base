...
bsta_gaussian_sphere<T,n>::operator + (const bsta_gaussian_sphere<T,n> &dist) const
{
	return bsta_gaussian_sphere<T,n>(this->mean()+dist.mean(),this->covar()+dist.covar());
...
template <class T, unsigned int n>
bsta_gaussian_sphere<T,n> 
bsta_gaussian_sphere<T,n>::operator * (T scale)	const
{
	
	bsta_gaussian_sphere<T,n>::vector_ v=this->mean();
	bsta_gaussian_sphere<T,n>::covar_type cov=this->covar();
	v*=scale;
	cov*=scale*scale;
	bsta_gaussian_sphere<T,n> gauss(*this);
	gauss.set_covar(cov);
	gauss.set_mean(v);
	return gauss;
