...
class bbgm_image_of : public bbgm_image_base
{
 ...
 // void compress_image(vcl_string & name) const;

  //: Binary load self from stream.
  virtual void b_read(vsl_b_istream &is);
  typedef dist pixel_type;
...
template <class dist>
class bbgm_image_of_line
{
public:
	bbgm_image_of_line(int width,dist model):data_(1,width,model),width_(width){}
	const dist operator [] (unsigned int x) const {return data_(1,x);}
	dist& operator[] (unsigned int x) {return data_(1,x);}
	const int width_ ;
private:
