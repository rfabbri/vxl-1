// This is brl/bseg/bbgm/pro/processes/bbgm_load_wavelet_process.cxx
#include <bprb/bprb_func_process.h>
//:
// \file
#include <vcl_iostream.h>
#include <dbbgm/bbgm_wavelet.h>
#include <dbbgm/bbgm_wavelet_sptr.h>
#include <brdb/brdb_value.h>
#include <vbl/io/vbl_io_smart_ptr.h>
#include <dbbgm/bbgm_loader.h>

//: Process construct function
bool bbgm_load_wavelet_process_cons(bprb_func_process& pro)
{
  vcl_vector<vcl_string> in_types(1), out_types(1);
  in_types[0]="vcl_string"; //path to distribution image
  pro.set_input_types(in_types);
  out_types[0]="bbgm_wavelet_sptr"; //loaded distribution image
  pro.set_output_types(out_types);
  return true;
}
//: Process execute function
bool bbgm_load_wavelet_process(bprb_func_process& pro)
{
  // Sanity check
  if (!pro.verify_inputs()) {
    vcl_cerr << "In bbgm_load_wavelet_process::execute - invalid inputs\n";
    return false;
  }

  vcl_string binary_filename = pro.get_input<vcl_string>(0);

  vsl_b_ifstream istr(binary_filename);
  if (!istr) {
    vcl_cerr << "Failed to load wavelet image from "
             << binary_filename << vcl_endl;
    return false;
  }
  //register different distributions for image content
  //the registration will only be done once since new instances of
  //the process are cloned  - maybe later make a separate registration step
  bbgm_loader::register_loaders();
  vsl_b_istream& bis = static_cast<vsl_b_istream&>(istr);
  bbgm_wavelet_sptr wavelet;
  vsl_b_read(bis, wavelet);

  brdb_value_sptr output0 = new brdb_value_t<bbgm_wavelet_sptr>(wavelet);
  pro.set_output(0, output0);
  return true;
}

