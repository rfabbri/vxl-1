...
def wvXform(image_string,image,wv_type,level,nplanes,args):
    Wavelet=dbvalue(0,"");
    forwardp=dbvalue(0,"")
    ...
    dbbgm_batch.set_input_string(5,args);
    dbbgm_batch.run_process();
    (Wavelet.id,Wavelet.type)=dbbgm_batch.commit_output(0);
    (forwardp.id,forwardp.type)=dbbgm_batch.commit_output(1);
    return [Wavelet,forwardp]
...
def updateImageStream(path,w_size,init_var,minimum_stdev,start_frame,end_frame,ext):
    dbbgm_batch.init_process("vidlOpenIstreamProcess")
    print dbbgm_batch.set_input_string(0, path+"*"+ext);
    dbbgm_batch.run_process();
    stream = dbvalue(0,"");
    model = dbvalue(0,"");
...
def measureWvLookup(wavelet,test_image,attribute,tolerance,interp_functor,data_path,frameSuffix,threshold):
    image=dbvalue(0,"");
    ...
    dbbgm_batch.set_input_string(6,frameSuffix);
    dbbgm_batch.set_input_float(7,threshold);
    dbbgm_batch.run_process();
    (image.id,image.type)=dbbgm_batch.commit_output(0); 
    return image

def setModeCompactification(model,type):
    dbbgm_batch.init_process("bbgmSelectMixtureAveragingProcess");
    dbbgm_batch.set_input_int(0,type)
    dbbgm_batch.set_input_from_db(1,model)

def measure(model,frame,attribute,tol):
    out_map=dbvalue(0,"");
    dbbgm_batch.init_process("bbgmMeasureProcess");
    dbbgm_batch.set_input_from_db(0,model)
    dbbgm_batch.set_input_from_db(1,frame)
    dbbgm_batch.set_input_string(2,attribute)
    dbbgm_batch.set_input_float(3,tol)
    dbbgm_batch.run_process()
    (out_map.id,out_map.type)=dbbgm_batch.commit_output(0)
    return out_map

def kduCompressFrame(probability_map,input_frame,threshold,kdu_args,mask_args,roi):
    dbbgm_batch.init_process("bbgmThreshAndEncodeProcess");
    dbbgm_batch.set_input_from_db(0,probability_map)
    dbbgm_batch.set_input_from_db(1,input_frame)
    dbbgm_batch.set_input_float(2,threshold)
    dbbgm_batch.set_input_string(3,kdu_args)
    dbbgm_batch.set_input_int(4,roi)
    dbbgm_batch.set_input_string(5,mask_args)
    dbbgm_batch.run_process()
    
def kduDecompressFrame(background_image,kdu_args,mask_args,foreground_path,frame_path):
    out_img=dbvalue(0,"");
    out_frame=dbvalue(0,"");
    dbbgm_batch.init_process("bbgmDecompressAndGlueProcess");
    dbbgm_batch.set_input_from_db(0,background_image)
    dbbgm_batch.set_input_string(1,kdu_args)
    dbbgm_batch.set_input_string(2,kdu_args)
    dbbgm_batch.run_process()
    (out_img.id,out_img.type)=dbbgm_batch.commit_output(0); 
    (out_frame.id,out_frame.type)=dbbgm_batch.commit_output(1); 
    
    saveImg(out_img,foreground_path)
    saveImg(out_frame,frame_path)
    
def evalROC(truth,test,bg_thresh,path):
    dbbgm_batch.init_process("bbgmROCProcess");
    dbbgm_batch.set_input_from_db(0,truth);
    dbbgm_batch.set_input_from_db(1,test);
    dbbgm_batch.set_input_float(2,bg_thresh);
    dbbgm_batch.set_input_string(3,path);
    dbbgm_batch.run_process();
    return []
