#include "dbvxm_update_multiscale_process.h"
...
dbvxm_update_multiscale_process::dbvxm_update_multiscale_process()
{
...
bool dbvxm_update_multiscale_process::execute()
{
  ...
  brdb_value_t<dbvxm_multi_scale_voxel_world_sptr>* input2 = 
  ...
  for (unsigned scale = curr_scale;scale < max_scale;scale++)
  {
      ...
      //dbvxm_util::slab_to_img(image_slab,scale_img);
      ...
      if(scale!=curr_scale)
      {
          ...
          img=dbvxm_util::downsample_image_by_two(img);
          vil_save(*img.as_pointer(),"d:/scaleup.tiff");
          camera=dbvxm_util::downsample_camera( camera, scale);
...
vpgl_camera_double_sptr dbvxm_update_multiscale_process::downsample_camera_by_two(vpgl_camera_double_sptr camera)
{
