...
void dbvxm_part_hierarchy::generate_map(vcl_vector<dbvxm_part_instance_sptr>& extracted_parts, vil_image_view<float>& map, vil_image_view<unsigned>& type_map) {
  ...
  for (unsigned i = 0; i < extracted_parts.size(); i++) {
    dbvxm_part_instance_sptr p = extracted_parts[i];
...
dbvxm_part_base_sptr dbvxm_part_hierarchy::get_node(unsigned layer, unsigned type) {
...
unsigned dbvxm_part_hierarchy::highest_layer_id()
{
  unsigned highest = 0;
  for (vertex_iterator it = this->vertices_begin(); it != this->vertices_end(); it++) {
...
dbvxm_part_instance_sptr dbvxm_part_hierarchy::exists(dbvxm_part_base_sptr upper_p, 
                                                      ...
                                                      float det_threshold)
{
  ...
  dbvxm_part_instance_sptr pi = new dbvxm_part_instance(upper_p->layer_, upper_p->type_, dbvxm_part_instance_kind::COMPOSED, central_p->x_, central_p->y_, 0.0f); 
  dbvxm_hierarchy_edge_sptr e1 = new dbvxm_hierarchy_edge(pi->cast_to_base(), central_p->cast_to_base());
  ...
  for ( ; eit != upper_p->out_edges_end(); eit++) {
    ...
    dbvxm_part_instance_sptr best_part;
    ...
    if (best_part) {
      dbvxm_hierarchy_edge_sptr e2 = new dbvxm_hierarchy_edge(pi->cast_to_base(), best_part->cast_to_base());
                                               ...
                                               vcl_vector<dbvxm_part_instance_sptr>& extracted_upper_parts)
{
  ...
  for (unsigned i = 0; i < extracted_parts.size(); i++) {
    ...
    dbvxm_part_base_sptr hp = this->get_node(p->layer_, p->type_);
    if (!hp)
      continue;
    ...
    for (edge_iterator eit = hp->in_edges_begin(); eit != hp->in_edges_end(); eit++) {
      if (hp == (*eit)->source()->central_part()) {
        dbvxm_part_base_sptr hp_upper = (*eit)->source();

        // now check for the existence of other primitives wrt to the central part and initiate an instance of it if so
        dbvxm_part_instance_sptr hp_upper_instance = exists(hp_upper, p, map, type_map, part_map, detection_threshold); // p will be its central part and map will tell if all the other parts exist
