...
float 
//dbvxm_hierarchy_edge::prob_density(const vnl_vector_fixed<float,2>& pt)
dbvxm_hierarchy_edge::prob_density(const float dist, const float angle)
{
...
float 
dbvxm_hierarchy_edge::prob_density_dist(const float dist)
{
  //: return non-normalized value
  return (float)(dist_model_.var()*2*vnl_math::pi*dist_model_.prob_density(dist));
}
float 
dbvxm_hierarchy_edge::prob_density_angle(const float angle)
{
  //: return non-normalized value
...
/*void 
dbvxm_hierarchy_edge::update_model(const vnl_vector_fixed<float,2>& sample)
{
  to_central_ = false;
...
//: if the model is updated then the to_central flag is made false since the edge becomes a non-central edge
void 
dbvxm_hierarchy_edge::update_dist_model(const float dist)
{
  to_central_ = false;
...
void 
dbvxm_hierarchy_edge::update_angle_model(const float angle)
{
  to_central_ = false;
...
void 
dbvxm_hierarchy_edge::calculate_dist_angle(dbvxm_part_instance_sptr pi, vnl_vector_fixed<float,2>& dif_to_center, float& dist, float& angle)
{
  // if pi is a composed part we need to get its central part
  dbvxm_part_instance_sptr pp = pi;
  while (pp->kind_ == dbvxm_part_instance_kind::COMPOSED) {
