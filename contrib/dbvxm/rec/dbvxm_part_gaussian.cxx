  ...
  for(unsigned j = 0; j<nj; ++j)
    for(unsigned i = 0; i<ni; ++i)
    {
      if (strength_map(i,j) > strength_threshold) {
        dbvxm_part_gaussian_sptr dp = new dbvxm_part_gaussian((float)i, (float)j, strength_map(i,j), lambda0, lambda1, theta, bright, type);
...
dbvxm_part_gaussian* dbvxm_part_gaussian::cast_to_gaussian(void)
{
  return this;
}

bool dbvxm_part_gaussian::mark_receptive_field(vil_image_view<vxl_byte>& img, unsigned plane)
{
...
bool dbvxm_part_gaussian::mark_receptive_field(vil_image_view<float>& img, float val)
{
  vbl_array_2d<bool> mask;
  vbl_array_2d<float> kernel;
  brip_vil_float_ops::extrema_kernel_mask(lambda0_, lambda1_, theta_, kernel, mask);
...
bool dbvxm_part_gaussian::mark_center(vil_image_view<vxl_byte>& img, unsigned plane)
{
  if (img.nplanes() <= plane)
    return false;
...
dbvxm_part_gaussian::direction_vector(void)  // return a unit vector that gives direction of this instance in the image
{
