...
void test_l2norm()
{
 //define some known mixtures
  ...
  double dist1 = measure.distance(g,f);
  double dist2 = measure.distance(h,j);
  ...
  bvxm_voxel_grid<float> *mask_grid = new bvxm_voxel_grid<float>(grid_size);
  mask_grid->initialize_data(1.0);

 bvxm_voxel_grid_base_sptr dist_base  = new bvxm_voxel_grid<float>(grid_size);

 
  bseg3d_explore_mixtures explorer;
  explorer.calculate_l2distances(apm_grid,mask_grid,dist_base,true,f);
