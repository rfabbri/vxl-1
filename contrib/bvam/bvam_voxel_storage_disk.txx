...
template <class T>
bvam_voxel_storage_disk<T>::bvam_voxel_storage_disk(vcl_string storage_filename, vgl_vector_3d<unsigned int> grid_size)
: bvam_voxel_storage(grid_size), storage_fname_(storage_filename)
{
  ...
  if (vul_file::exists(storage_fname_))  {
    ...
    if (is_dir) {
      vcl_cerr << "error: directory name " << storage_fname_ << " passed to bvam_voxel_storage_disk constructor." << vcl_endl;
      return;
    }
