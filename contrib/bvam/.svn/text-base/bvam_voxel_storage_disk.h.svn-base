...
template<class T>
class bvam_voxel_storage_header
{
public:
  bvam_voxel_storage_header(){};
  bvam_voxel_storage_header(vgl_vector_3d<unsigned> grid_size) 
    : nx_(grid_size.x()), ny_(grid_size.y()), nz_(grid_size.z()), nobservations_(0) {};
  bvam_voxel_storage_header(unsigned nx, unsigned ny, unsigned nz)
