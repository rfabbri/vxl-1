...
MAIN( test_bvam_create_voxel_world_process )
{
  REG_PROCESS(bvam_create_voxel_world_process, bprb_batch_process_manager);
  REGISTER_DATATYPE(bvam_voxel_world_sptr);

  bool good = bprb_batch_process_manager::instance()->init_process("CreateVoxelWorldProcess");
  ...
  TEST("bvam_voxel_world_sptr non-null", non_null ,true);

  brdb_value_t<bvam_voxel_world_sptr>* result = 
    static_cast<brdb_value_t<bvam_voxel_world_sptr>* >(value.ptr());
