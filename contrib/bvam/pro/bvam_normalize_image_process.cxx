...
#include <bvam/bvam_util.h>


bvam_normalize_image_process::bvam_normalize_image_process()
{
  //inputs
...
bool bvam_normalize_image_process::execute()
{
  ...
  brdb_value_t<bvam_voxel_world_sptr>* input2 = 
  ...
  //: create metadata:
  bvam_image_metadata observation(input_img,camera);
  ...
  if (voxel_type == "apm_mog_rgb") {
    typedef bvam_voxel_traits<APM_MOG_RGB>::voxel_datatype mog_type;
    typedef bvam_voxel_traits<APM_MOG_RGB>::obs_datatype obs_datatype;
    ...
    bvam_voxel_traits<APM_MOG_RGB>::appearance_processor apm_processor;  
    if (verbose) {
      bvam_voxel_slab<obs_datatype> exp_img = apm_processor.expected_color(*mog_image_ptr);
      vil_image_view_base_sptr temp_img = new vil_image_view<vxl_byte>(ni, nj, 3);
      bvam_util::slab_to_img(exp_img, temp_img);
    ...
    for( float sa = a_start; sa <= a_end; sa+=a_inc ){
      ...
      for( float sb_inc = b_start; sb_inc > b_end; sb_inc *= b_ratio ) {
        ...
        for( float sb = sb_best-2*sb_inc; sb <= sb_best+2.01*sb_inc; sb+= sb_inc ) {
          ...
          //bvam_util::img_to_slab(nimg_stretched_ptr,image_slab);
          //nimg_stretched_sptr = 0;  // to clear up the space taken by new
          bvam_util::img_to_slab(nimg_sptr,image_slab);
    ...
    if (verbose) {
      bvam_voxel_slab<float> exp_img = apm_processor.expected_color(*mog_image_ptr);
