...
#include "mw_curve_appearance_tracing_tool.h"
...
//#include <cvmatch_appearance/mw_cvmatch_appearance_sptr.h>
...
mw_curve_appearance_tracing_tool()
    : p0_(0), pn_(0), current_curve_id_(RICK_INVALID_CURVE_ID), 
      ...
      display_all_right_epips_(false), 
      ...
      use_DP_(false)
{
    ...
    point_style_ = vgui_style::new_style(1, 0.5, 0, 5.0f, 3.0f);
    ...
    for(unsigned n=0; n<5; ++n)
        top_5_match_soview_.push_back(NULL);
}
...
activate ()
{
  vcl_cout << "mw_curve_appearance_tracing_tool ON\n";
  ...
  fm_.resize(nviews_);
  selected_pts_.resize(nviews_);
  frame_map_.resize(nviews_);
...
deactivate ()
{
  vcl_cout << "mw_curve_appearance_tracing_tool OFF\n";
}
        ...
        const bvis1_view_tableau_sptr& view )
{
    ...
    if (e.type == vgui_KEY_PRESS) {
        ...
        case 'm': {
            ...
            vcl_cout << "Matching first two views using HOGs\n";
            vgui::out << "Matching first two views using HOGs\n";
            match_using_hog();
        ...
        case 'b': {
            vcl_cout << "Computing descriptors for the second frame\n";
            vgui::out << "Computing descriptors for the second frame\n";
            compute_descriptors();
            return true;
            break;
        }
        case 'u': {
            ...
            vcl_cout << "Calibration " << state << vcl_endl;
            vgui::out << "Calibration " << state << vcl_endl;
            break;
        }
        case 'p': {
            vcl_string state;
            if (use_DP_) {
                use_DP_ = false;
                state = "bag of features";
            } else {
                use_DP_ = true;
                state = "dynamic programming";
            }
            vcl_cout << "Matching will be done using " << state << vcl_endl;
            vgui::out << "Matching will be done using " << state << vcl_endl;
            break;
        }
        case 'o': {
            ...
            if(!subcurve_)
            {
                vcl_cout << "You need to select a curve first!" << vcl_endl;
                vgui::out << "You need to select a curve first!" << vcl_endl;
                return true;
                break;
            }
            vcl_cout << "Writing descriptors for the selected curve" << vcl_endl;
            vgui::out << "Writing descriptors for the selected curve" << vcl_endl;
            ...
            sc0.write_all_descriptors();

            vcl_cout << "Finished writing descriptors" << vcl_endl;
            vgui::out << "Finished writing descriptors" << vcl_endl;
        ...
        case 'h': {
            ...
            vcl_cout << state << "marking sample correspondence" << vcl_endl;
            vgui::out << state <<  "marking sample correspondence" << vcl_endl;
            break;
        }
        case 'j': {
            vcl_cout << "Computing & writing descriptors for the selected points\n";
            vgui::out << "Computing & writing descriptors for the selected points\n";

            for (unsigned i=0; i<frame_map_.size(); ++i)
                if(frame_map_[i])
                    frame_numbers_.push_back(i);

            compute_and_write_selected_descriptors();
            break;
        }
        case '1': { 
...
get_images()
{
    // TODO Just get two images as a hack for a project. If you want more than 2, modify here 
    vcl_cout << "Just get two images (1st 2 frames) as a hack for a project. If you want more than 2, modify here\n";
    for (unsigned i=0; i<2; ++i) {
        ...
        img_storage.vertical_cast(p);
        if(!p) {
            vcl_cerr << "Error: tool requires image storages in the first 2 frames" << vcl_endl;
            return;
        }

        vil_image_view_base_sptr bview = img_storage->get_image()->get_view();

        assert(bview);

        img_[i] = vil_image_view<float>(vil_convert_cast(0.0f, vil_convert_to_grey_using_average(bview)));
      
        assert(img_[i]);
        vcl_cout << "Image: " << img_[i] << vcl_endl;
    }
}
...
bool mw_curve_appearance_tracing_tool::
handle_mouse_click( 
    const vgui_event & e, 
    const bvis1_view_tableau_sptr& view)
{
    ...
    {
        unsigned f = view->frame();
        frame_map_[f] = true;
        
        vsol_point_2d_sptr pt = new vsol_point_2d(ix_exact,iy_exact);
        double mindist;
        unsigned near_idx = mw_util::find_nearest_pt_using_double(pt,selected_crv_[f],mindist);
        pt = selected_crv_[f]->vertex(near_idx);
        
        tab_[f]->add_vsol_point_2d(pt,point_style_);
        tab_[f]->post_redraw();

        selected_pts_[f].push_back(near_idx);
...
compute_and_write_selected_descriptors()
{
    if(frame_numbers_.size()==0)
    {
        vcl_cout << "You must establish some point correspondences first!" << vcl_endl;
        return false;
    }
    else if(frame_numbers_.size()==1)
    {
        vcl_cout << "You must establish correspondences in at least two frames!" << vcl_endl;
        return false;
    }

    vcl_cout << "Number of frames: " << frame_numbers_.size() << vcl_endl;

    for(unsigned n=0; n<frame_numbers_.size(); ++n)
    {
        unsigned f = frame_numbers_[n];

        if (!selected_crv_[f]) {
            vcl_cout << "No selected curve in frame 0\n";
            return false;
        }
        else if(selected_pts_[f].size()==0) {
            vcl_cout << "No points selected on the curve\n";
            return false;
        }

        vcl_cout << "Started computing HOGs for the selected curve" << vcl_endl;

        mw_sift_curve sc0;
        vcl_cout << "Initializign SIFT / Gaussian Scale Space." << vcl_endl;
        mw_sift_curve_algo computor(img_[f]);
        computor.print_sigmas();
        vcl_cout << "Done initializign SIFT / Gaussian Scale Space." << vcl_endl;

        vcl_cout << "Computing descriptors for the curve. Also computing the image gradient." << vcl_endl;
        {
            dbdet_edgel_chain curvelet_ec;
            mw_algo_util::extract_edgel_chain(*selected_crv_[f], &curvelet_ec);
            computor.compute(curvelet_ec, &sc0);

            for (unsigned i=0; i < curvelet_ec.edgels.size(); ++i)
                delete curvelet_ec.edgels[i];
        }
        vcl_cout << "Done computing descriptors for the curve." << vcl_endl;

        vcl_cout << "Writing out the descriptors for the selected curve." << vcl_endl;

        for (unsigned p=0; p<selected_pts_[f].size(); ++p) {
            for (unsigned s=0; s < sc0.num_scales(); ++s) {
                unsigned pt_idx = selected_pts_[f][p];
                vnl_vector_fixed<double,128> descriptor = sc0.descriptor(s,pt_idx);

                vcl_stringstream f_ss;
                f_ss << f;
                vcl_string f_s = f_ss.str();

                vcl_stringstream p_ss;
                p_ss << p;
                vcl_string p_s = p_ss.str();

                vcl_stringstream s_ss;
                s_ss << s;
                vcl_string s_s = s_ss.str();

                vcl_string filename = "/home/anilusumezbas/Documents/Research/histogram-files/frame-"+f_s+"-sample-"+p_s+"-scale-"+s_s+".txt";
                vcl_ofstream hists(filename.c_str());

                for (unsigned x=0; x<descriptor.size(); ++x)
                {             
                    if(x%8==7)
                        hists << descriptor[x] << vcl_endl;
                    else
                        hists << descriptor[x] << " ";
                }
    ...
    vcl_cout << "Finished writing out the descriptors." << vcl_endl;
...
compute_descriptors()
{
    ...
    sc_img1_.resize(crv_candidates_ptrs_.size());

    computor1_.compute_many(ec_v, &sc_img1_);

    for (unsigned ii=0; ii < crv_candidates_ptrs_.size(); ++ii)
        for (unsigned kk=0; kk < ec_v[ii].edgels.size(); ++kk)
            delete ec_v[ii].edgels[kk];
...
match_using_hog()
{
    ...
    if(!use_DP_)
    {
        ...
        vcl_vector<unsigned> votes(sc_img1_.size(), 0);
        vcl_vector<double> sigmas;
        computor1_.get_sigmas(&sigmas);
...
//      vcl_cout << "scale s = " << s << "; sample i = " << i << vcl_endl;
                ...
                for (unsigned ic=0; ic < crv_candidates_ptrs_.size(); ++ic) {
                    mw_sift_curve_algo::t_descriptor_float d = mw_sift_curve_algo::unambigous_nneighbor(sc_img1_[ic], sc0.descriptor(s, i));
                    //vcl_cout << "Curve ID in the second frame: " << ic << vcl_endl;
...
//      votes[ic_min]++;
        ...
        vcl_vector <unsigned> top5(5,0);
        vcl_vector <unsigned> top5_votes(5,0);
        unsigned i_best;
        ...
        for(unsigned x=0; x<5; ++x)
        {
            ...
            top5[x] = i_best;
            top5_votes[x] = votes[i_best];

            if(!votes.empty() && votes[i_best]!=0)
                votes.erase(votes.begin()+i_best);
            else
                break;
        }

        vcl_cout << "Best curve has index " << top5[0] << " among candidates, with #votes = " << top5_votes[0] << vcl_endl;
        vcl_cout << "Second best: " << top5[1] << " with #votes: " << top5_votes[1] << vcl_endl;
        vcl_cout << "Third best:  " << top5[2] << " with #votes: " << top5_votes[2] << vcl_endl;
        vcl_cout << "Fourth best: " << top5[3] << " with #votes: " << top5_votes[3] << vcl_endl;
        vcl_cout << "Fifth best:  " << top5[4] << " with #votes: " << top5_votes[4] << vcl_endl;
        ...
        {
            if(top_5_match_soview_[y])
                tab_[1]->remove(top_5_match_soview_[y]);

            top_5_match_soview_[y] = tab_[1]->add_vsol_polyline_2d(crv_candidates_ptrs_[top5[y]], best_match_style_[y]);
            tab_[1]->post_redraw();
        }
    ...
    {
        vcl_cout << "Started feature matching using dynamic programming" << vcl_endl;
