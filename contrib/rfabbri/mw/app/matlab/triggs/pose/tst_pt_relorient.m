% tst_pt_relorient() -- test relative orientation routines

% Generates a random scene and set of camera positions, runs the relative
...
% image points in radians, flatness = thickness/width ratio of scene (0
% for a truly planar scene).

function tst_pt_relorient(n,noise,flatness)
   % global badseeds; 
   % seeds = [rand('seed'); randn('seed')];
   % rand("seed",badseeds(1,1)); randn("seed",badseeds(2,1));
