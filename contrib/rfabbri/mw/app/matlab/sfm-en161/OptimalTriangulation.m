function [X, P1, P2, hat_leftpoint, hat_rightpoint]=OptimalTriangulation(lpoint, rpoint, F_input, w, h)
...
[leftpoint, rightpoint,F]=normalize_points(lpoint,rpoint,F_input,w,h);
...
[hat_leftpoint, hat_rightpoint]=denormalize_points(hat_leftpoint,hat_rightpoint,w,h);
...
function [lpn, rpn, Fn]=normalize_points(lpoints, rpoints, F, w, h)
K    = [ 0.5*(w+h) 0 0.5*w ; 0 0.5*(w+h) 0.5*h; 0 0 1];
invK = inv(K);
n    = size(lpoints,2);
lpnH = invK * [lpoints; repmat(1,1,n)];
lpn  = lpnH(1:2,:);
rpnH = invK * [rpoints; repmat(1,1,n)];
rpn  = rpnH(1:2,:);
Fn   = K' * F * K;



function [lp, rp ] = denormalize_points(lpn, rpn, w, h)
K    = [ 0.5*(w+h) 0 0.5*w ; 0 0.5*(w+h) 0.5*h; 0 0 1];
invK = inv(K);
n    = size(lpn,2);
lpH  = K * lpn;
