function [pts3drawcell, normalrawcell, confrawcell] = struct_from_sil2(data,yfactor,vfactor,datasetname)

DEBUG = 1;
neighbors_rel = -1:1;

pts3drawcell = cell(data.imnum-2,1);
normalrawcell = cell(data.imnum-2,1);
confrawcell = cell(data.imnum-2,1);
fprintf(1,'\n');
fprintf(1,'Matching and Reconstructing ...\n');
for iprior = 1:data.imnum-2
    i = iprior + 1;
    fprintf(1,'[%d]',i); 
...
%        plot(xright,yright,'.b','LineWidth',1); hold off;
    ...
    pts3drawcell{iprior} = zeros(3,numedgepts,vfactor);
    normalrawcell{iprior} = zeros(3,numedgepts,vfactor);
    confrawcell{iprior} = zeros(4,numedgepts,vfactor);
    if vfactor == 1
        vsamples = 0;
    else
...
%             end
        ...
        Clr = Clr/Clr(4);
        for v = 1:vfactor
            [pts3drawcell{iprior}(:,j,v), normal_pseudo, confrawcell{iprior}(:,j,v)] = recover3d(PT(:,v), PN(:,v), dPTdv(:,v), dPNdv(:,v));
            if abs(Clr'*PTmiddle)<1e-3
                normalrawcell{iprior}(:,j,v) = [0 0 0]';
                pts3drawcell{iprior}(:,j,v) = Clr(1:3);
            else
                normalrawcell{iprior}(:,j,v) = sign(Clr'*PTmiddle)*normal_pseudo; %% add the threshold checking in the future
            end
        end
    end
end
