...
%% parameters controlling the output
    resultpath = '../result/david_edge21/';
...
fprintf(1, 'Loading data and computing the edges with gradient and orientation ...\n');
    ...
    for i = 1:data.imnum
        ...
        if DEBUG
            imwrite(full(data.edgemap{i}), ['../debug/david_edge21/' 'edgemap_' int2strfixed(i,4) '.png']);
        end
    ...
    if DEBUG
        save('../debug/david_edge21/programdata.mat');
    end

%% main work
    [pts3drawcell, normalrawcell confrawcell] = struct_from_sil(data,1,10,'david_edge21');
...
%% save the results
    ...
    fname_vrml_raw = 'david_edge21_raw_ni.wrl';
    for i = 1:data.imnum
        pts3drawnicell{i} = pts3drawcell{i}(:,:,1);
    end
    pts3drawni = cell2matrix(pts3drawnicell);
    pts3drawbbni = boundingbox(pts3drawni, [-200 600 -200 600 -200 600]);
