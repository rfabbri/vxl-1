...
fprintf(1, 'Loading data and computing the edges with gradient and orientation ...\n');
    ...
    for i = 1:data.imnum
        ...
        if DEBUG
            imwrite(full(data.edgemap{i}), ['../debug/pig_edge21/' 'edgemap_' int2strfixed(i,4) '.png']);
        end
    ...
    if DEBUG
        save('../debug/pig_edge21/programdata.mat');
    end

%% main work
    [pts3drawcell, normalrawcell confrawcell] = struct_from_sil(data,1,10,'pig_edge21');
...
load('../data/pig/Calib_Results.mat');
load('../data/pig/base_extrinsics_rect.mat');
