...
%% parameters controlling the output
    resultpath = '../result/horsemark_edge7/';
...
fprintf(1, 'Loading data and computing the edges with gradient and orientation ...\n');
    ...
    for i = 1:data.imnum
        fprintf(1,'[%d]',i);
        data.imnamecell{i} = [datapath int2strfixed(i-1,4) '.jpg'];
        im = rgb2gray(imread(data.imnamecell{i}));
        [data.edgemap{i}, data.edgegradmap{i}, data.edgeormap{i}, data.edgeposmap{i}] = edgeGOPmap(im, 3, 0, 20, 10);

        if DEBUG
            imwrite(full(data.edgemap{i}), ['../debug/horsemark_edge7/' 'edgemap_' int2strfixed(i,4) '.png']);
        end
        angle = (i-2)/30*2*pi;
    ...
    for i = 1:data.silnum
        data.silnamecell{i} = [silpath int2strfixed(i-1,4) '.png'];
        angle = (i-2)/30*2*pi;
        data.silMcell{i} = loadpmatrix(angle);
    end

    if DEBUG
        save('../debug/horsemark_edge7/programdata.mat');
    end

%% main work
    [pts3drawcell, normalrawcell confrawcell] = struct_from_sil(data,1,5,'horsemark_edge7');
