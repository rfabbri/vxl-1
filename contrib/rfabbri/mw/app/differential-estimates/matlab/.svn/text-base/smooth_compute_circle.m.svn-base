% SAME AS SMOOTH_COMPUTE, BUT THIS IS A SHORTCUT for running on circle using
% the best parameters.
...
k_el = (1/r)*ones(size(fel_fine));
kdot_el = zeros(size(fel_fine));
...
%mycs = gsmi2d(el,5);
%mycs = el;
s = myarclength(pel,'circular');
mycs = gsmi2d(pel,10,s,0,'circular');
