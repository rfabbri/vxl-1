...
#undef RF_UNINITIALIZED_HISTOGRAM_BIN
    ...
    void write_all_descriptors() const
    {
        vcl_string descriptor_filename = "/home/anilusumezbas/Documents/Research/mw/bench_high_threshold_dir/descriptors.txt";
        vcl_ofstream descriptor(descriptor_filename.c_str());

        unsigned is=0;
        unsigned max_n_prints = 300;
        unsigned num_histograms = 16;

        for (unsigned i=0; i < num_scales(); ++i) {
            //vcl_cout << vcl_setprecision(1);
            descriptor << "\nScale #" << i+1 << vcl_endl;
            if (!num_samples()) {
                descriptor << "No samples\n";
                return;
            }
            for (unsigned k=0; k < num_samples(); ++k) {
                descriptor << "  sample " << k+1 << "\n";
                if (is_valid(i,k)) {
                    // output descriptor
                    assert(num_dims());
                    for (unsigned o=0; o<num_histograms; ++o)
                    {
                        for (unsigned m=0; m < num_dims()/num_histograms; ++m)
                        {
                            if(m!=0)
                                descriptor << ", ";
                            descriptor << descriptors_[i][k][8*o+m];
                        }
                        descriptor << vcl_endl;
                    }

                } else {
                    descriptor << "EMPTY" << vcl_endl;
                }
                descriptor << vcl_endl;
                ++is;
                if (is > max_n_prints)
