...
class views_aggregator : public dbpro_sink
{
 public:
  ...
  typedef enum { CAM_ID, EDG_ID, DT_ID, LBL_ID, CRVS_ID, TGTS_ID } input_id;
  static const unsigned sources_per_view = 6;
      ...
      )
  {
    ...
    tgts->connect_input(0, frag_src, 0);
    ...
    connect_input(view*sources_per_view + TGTS_ID, tgts, 0);
  }
...
MAIN( test_multiview_dbpro_process )
{
  ...
  for (unsigned v=0; v < nviews; ++v) {
    ...
    out_ptr->setup_inputs(v, cam_src, edg_src, edg_dt, frag_src, tgts);
  }

  out->run(1);

  TEST("Tangents size matches curves in nviews", 
