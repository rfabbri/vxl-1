...
:proj_mem_io_(proj_mem_io)
{
  ...
  for(unsigned viewno = 0; viewno < proj_mem_io->nviews_; viewno++)
    cameras_.push_back(proj_mem_io->scan_(viewno));
}
...
bool bioproj_mem_proc::execute(int view_filter_type, int spatial_filter_type, int resize_factor)
{
  if(view_filter_type > 2 || view_filter_type < 0){
          vcl_cout << "PROJECTED FILTER " << view_filter_type << " NOT DEFINED!!!" << vcl_endl;
          return 0;
  }

  view_filter_type_ = view_filter_type;
  spatial_filter_type_ = spatial_filter_type;
  ...
  bioproj_filter* filter = 0;

  if(view_filter_type_ == 0)
  {
    filter = new bioproj_nu_g_filter(sigma, extent, increment);
  }
  ...
  for(unsigned viewno = 0; viewno < proj_mem_io_->nviews_; viewno++)
  {
    ...
    if(!current_view_.is_contiguous()){
      vcl_cerr << "Current_view must be contiguous\n";
      return 0;
    }
        
    if(view_filter_type_ == 1)
    {
            filter = new bioproj_nu_gx_filter(sigma, theta_step*viewno, extent, increment);
    }
    else if(view_filter_type_ == 2)
    {
            filter = new bioproj_nu_gy_filter(sigma, theta_step*viewno, extent, increment);
    }

    // do the convolution on the view
    convolve_current_view(filter, viewno);
    if(view_filter_type_ > 0)
      delete filter;
    ...
    vcl_cout << "Elapsed time is " << ((end - start)) << vcl_endl;
  }
...
void bioproj_mem_proc::convolve_current_view(bioproj_filter* filter, const int& viewno)
{
  double* f = filter->filter_;
  int center = filter->half_kernel_size_;
  ...
  int ni = current_view_.ni();
  vcl_ptrdiff_t jstep = current_view_.jstep();
  double* curptr = current_view_.top_left_ptr() + v_min*jstep;
  double* resptr = current_view_convolved_.top_left_ptr() + v_min*jstep;
  // loop over view data rows
  for(int j = v_min; j <= v_max; j++)
  {
    // convolve it
    vil_convolve_1d<double,double,double,double>(curptr, ni, 1, 
      resptr, 1, 
...
//        proj_mem_io_->grid_(i,j,k) += float(interpolate_convolved_data_at_point(proj_point));
        proj_mem_io_->grid_(i,j,k) += (float)
            (vil_bilin_interp_safe(current_view_convolved_, proj_point.x(), proj_point.y()));
      }
    }
  }
}
...
void bioproj_mem_proc::apply_z_filtering()
{
  ...
  for(int j = 0; j < proj_mem_io_->grid_h_; j++)
  {
    for(int i = 0; i < proj_mem_io_->grid_w_; i++)
    {
      ...
      vcl_cout << index++ / double(num_grid_points) * 10000.0 << "% of z convolution finished" << vcl_endl;
  }
  delete [] z_line;
