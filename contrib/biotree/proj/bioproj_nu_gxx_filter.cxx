...
bioproj_nu_gxx_filter::bioproj_nu_gxx_filter(double sigma, double theta, unsigned extent)
{
  error_functions err;
  ...
  int index = 0;
  double common = -vcl_pow(vcl_cos(theta),2.0) / (vnl_math::pi*vcl_pow(sigma, 4));
...
bioproj_nu_gxx_filter::~bioproj_nu_gxx_filter()
{
  delete [] filter_;
}
