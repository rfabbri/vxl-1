//This is lemsvxl/contrib/dec/psm_opt/pro/processes/psm_update_dcdf_implicit_process.cxx
//:
// \file
...
#include <psm/algo/psm_update_dcdf_implicit.h>

namespace psm_update_dcdf_implicit_process_globals
{
...
bool psm_update_dcdf_implicit_process_cons(bprb_func_process& pro)
{
  using namespace psm_update_dcdf_implicit_process_globals;
...
//: Execute the process
bool psm_update_dcdf_implicit_process(bprb_func_process& pro)
{
  using namespace psm_update_dcdf_implicit_process_globals;
    ...
    case PSM_APM_SIMPLE_GREY:
      {
        ...
        psm_aux_scene_base_sptr aux_scene_base = scene->get_aux_scene<PSM_AUX_DCDF_IMPLICIT_GREY>();
        psm_update_dcdf_implicit<PSM_APM_SIMPLE_GREY, PSM_AUX_DCDF_IMPLICIT_GREY>(*scene, aux_scene_base, camera.ptr(), *img, use_black_background);
    ...
    case PSM_APM_MOG_GREY:
      {
        ...
        psm_aux_scene_base_sptr aux_scene_base = scene->get_aux_scene<PSM_AUX_DCDF_IMPLICIT_GREY>();
        psm_update_dcdf_implicit<PSM_APM_MOG_GREY, PSM_AUX_DCDF_IMPLICIT_GREY>(*scene, aux_scene_base, camera.ptr(), *img, use_black_background);
    ...
    case PSM_APM_SIMPLE_RGB:
      {
        ...
        if (!scene) {
         ...
         psm_aux_scene_base_sptr aux_scene_base = scene->get_aux_scene<PSM_AUX_DCDF_IMPLICIT_RGB>();
         psm_update_dcdf_implicit<PSM_APM_SIMPLE_RGB, PSM_AUX_DCDF_IMPLICIT_RGB>(*scene, aux_scene_base, camera.ptr(), *img, use_black_background);
      ...
      case PSM_APM_MOG_RGB:
      {
        ...
        if (!scene) {
         ...
         psm_aux_scene_base_sptr aux_scene_base = scene->get_aux_scene<PSM_AUX_DCDF_IMPLICIT_RGB>();
         psm_update_dcdf_implicit<PSM_APM_MOG_RGB, PSM_AUX_DCDF_IMPLICIT_RGB>(*scene, aux_scene_base, camera.ptr(), *img, use_black_background);

        break;
      }
    default:
      vcl_cerr << "error - psm_update_dcdf_implicit_process: unsupported appearance model type " << apm_type << vcl_endl;
      return false;
  }
