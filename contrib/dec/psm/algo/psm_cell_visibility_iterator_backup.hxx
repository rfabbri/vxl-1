...
psm_cell_visibility_iterator<APM>::psm_cell_visibility_iterator(hsds_fd_tree<psm_sample<APM>,3> const& block, vpgl_perspective_camera<double> const& cam, unsigned int img_ni, unsigned int img_nj, bool reverse_iterator)
: block_(block), cam_(cam), img_ni_(img_ni), img_nj_(img_nj), vis_graph_(block_.twin_tree(psm_cell_vis_graph_node())), reverse_it_(reverse_iterator)
{
  ...
  for (; nit != block_.end(); ++nit, ++vit) {
    ...
    if (cube_visible(cell_bb,cam,img_ni,img_nj,true)) {
