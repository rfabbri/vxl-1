...
psm_cell_visibility_iterator<APM>::psm_cell_visibility_iterator(hsds_fd_tree<psm_sample<APM>,3> const& block, vpgl_camera<double> const* cam, unsigned int img_i0, unsigned int img_j0, unsigned int img_ni, unsigned int img_nj, bool full_vis_needed, bool reverse_iterator)
: block_(block), cam_(cam), img_ni_(img_ni), img_nj_(img_nj), vis_graph_(block_.twin_tree(psm_cell_vis_graph_node())), reverse_it_(reverse_iterator)
{
  cell_count_ = 0;
  //vgl_point_3d<double> cam_center(cam_.camera_center());

  // determine if each cell is visible in the image.
  hsds_fd_tree<psm_cell_vis_graph_node,3>::iterator vit = vis_graph_.begin();
  for (; vit != vis_graph_.end(); ++vit) {
    if (full_vis_needed) {
      vit->second.visible = full_cube_visible(vis_graph_.cell_bounding_box(vit->first),cam,img_ni,img_nj,true);
    } else {
      vit->second.visible = cube_visible(vis_graph_.cell_bounding_box(vit->first),cam,img_i0, img_j0, img_ni,img_nj,true);
    }
  }

  // add visibility edges to each node. add unoccluded cells to curr_cells_
  typename hsds_fd_tree<psm_sample<APM> ,3>::const_iterator nit = block_.begin();
  //hsds_fd_tree<psm_cell_vis_graph_node,3>::iterator
  vit = vis_graph_.begin();
  for (; nit != block_.end(); ++nit, ++vit) {
    ...
    if (vit->second.visible) {
      ...
      psm_cube_face_list vis_faces = visible_faces(cell_bb, cam);
      ...
      if (vis_faces & psm_cube_face::X_HIGH) {
        ...
        for (; neighbor_it != neighbors.end(); ++neighbor_it) {
          ...
          if (neighbor_cell.visible) {
            ...
            neighbor_cell.outgoing_links.push_back(vit);
            ++visible_neighbor_count;
          }
        }
        if (visible_neighbor_count == 0) {
          vit->second.incoming_count++;
