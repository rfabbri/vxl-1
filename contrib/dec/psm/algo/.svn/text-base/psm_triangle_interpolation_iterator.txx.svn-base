...
//: constructor
template<class T>
psm_triangle_interpolation_iterator<T>::psm_triangle_interpolation_iterator(double *verts_x, double *verts_y, T *vals, unsigned int v0, unsigned int v1, unsigned int v2)
: tri_it_()
{
...
template<class T>
void psm_triangle_interpolation_iterator<T>::reset()
{
  tri_it_.reset();
}
...
template<class T>
bool psm_triangle_interpolation_iterator<T>::next()
{
  return tri_it_.next();
}
...
template<class T>
int psm_triangle_interpolation_iterator<T>::scany() const
{
...
template<class T>
int psm_triangle_interpolation_iterator<T>::startx() const
{
...
template<class T>
int  psm_triangle_interpolation_iterator<T>::endx() const
{
...
template<class T>
T psm_triangle_interpolation_iterator<T>::value_at(int x)
{
  return (T)((s0_*(x+0.5) + s1_*(tri_it_.scany()+0.5) + s2_));
}


#define PSM_TRIANGLE_INTERPOLATION_ITERATOR_INSTANTIATE(T) \
  template class psm_triangle_interpolation_iterator<T >
