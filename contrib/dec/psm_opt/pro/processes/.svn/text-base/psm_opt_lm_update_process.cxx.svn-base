//This is lemsvxl/contrib/dec/psm/pro/processes/psm_opt_lm_update_process.cxx
//:
// \file
// \brief A process for updating a psm_scene with a set of images, using a Levenberg Marquard optimization
...
#include <psm_opt/psm_opt_optimizer.h>

namespace psm_opt_lm_update_process_globals
{
...
bool psm_opt_lm_update_process_cons(bprb_func_process& pro)
{
  using namespace psm_opt_lm_update_process_globals;
...
//: Execute the process
bool psm_opt_lm_update_process(bprb_func_process& pro)
{
  using namespace psm_opt_lm_update_process_globals;
  ...
  // get the parameters 
  // (none)
  float min_appearance_sigma = 0.02f;
    ...
    case PSM_APM_SIMPLE_GREY:
      {
        ...
        psm_opt_optimizer<PSM_APM_SIMPLE_GREY, PSM_AUX_OPT_GREY> optimizer(*scene, image_ids, min_appearance_sigma);
    ...
    case PSM_APM_SIMPLE_RGB:
      {
        ...
        psm_opt_optimizer<PSM_APM_SIMPLE_RGB, PSM_AUX_OPT_RGB> optimizer(*scene, image_ids, min_appearance_sigma);
        optimizer.optimize_cells();

        break;
      }
    default:
      vcl_cerr << "error - psm_opt_lm_update_process: unsupported appearance model type " << apm_type << vcl_endl;
      return false;
  }
