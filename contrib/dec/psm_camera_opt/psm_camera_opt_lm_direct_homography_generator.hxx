#ifndef psm_camera_opt_lm_direct_homography_generator_txx_
#define psm_camera_opt_lm_direct_homography_generator_txx_
...
template<class T>
vimt_transform_2d psm_camera_opt_lm_direct_homography_generator<T>::compute_homography()
{
  int border = 2;
  dbvrl_world_roi roi(img0_.ni()- 2*border, img0_.nj()- 2*border,vgl_point_2d<double>(border,border));
  ...
  if (!use_mask0_ && !use_mask1_) {
    ...
    minimizer = new dbvrl_minimizer(from_img, to_img, roi);
  }
  // one mask
    ...
    if (use_mask0_) {
      ...
      minimizer = new dbvrl_minimizer(from_img, to_img, mask_img, roi, false);
    } 
    else {
      vimt_image_2d_of<float> mask_img(mask1_, vimt_transform_2d());
      minimizer = new dbvrl_minimizer(from_img, to_img, mask_img, roi, true);
    }
  }
  // both masks
  else {
    ...
    minimizer = new dbvrl_minimizer(from_img, to_img, from_mask, to_mask, roi);
  }
...
#define PSM_CAMERA_OPT_LM_DIRECT_HOMOGRAPHY_GENERATOR_INSTANTIATE(T) \
template class psm_camera_opt_lm_direct_homography_generator<T >
