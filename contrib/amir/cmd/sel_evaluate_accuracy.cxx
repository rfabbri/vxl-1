...
#include "sel_evaluate_accuracy.h"
...
//the curve model to use
//typedef dbdet_linear_curve_model cmodel;
typedef dbdet_CC_curve_model cmodel;
//typedef dbdet_ES_curve_model cmodel;
//typedef dbdet_ES_curve_model_perturbed cmodel;
...
int main(int argc, char** argv)
{
  ...
  if (argc < 2) {
    vcl_cout << "USAGE: sel_evaluate_accuracy <expt #> \n";
    return 1;
  }

  //get the image filename
  int expt_num = atoi(argv[1]);
                                       ...
                                       vcl_vector<curve_params>& GT)
{
  ...
  //maximum number of edgels
  int N = 50;

  double l=0;
  for (unsigned i=0; i<N; i++, l+=(1+0.2*vrand.drand32(-1,1)))
  {
    ...
    //check to see if the ES is winding into a loop
    double k2 = k + gamma*2.0*vnl_math::pi/(k+1e-8);

    if (1.0/(k2+1e-8) >= 1.0/(k+1e-8)-1.0) //startin gto loop 
      break;
    ...
    edgels.push_back(new dbdet_edgel(ppt, theta+rand_dt));

    //save the ground truth
    GT.push_back(curve_params(theta, k, gamma));
  }
                      ...
                      curve_params& avg_error, curve_params& std_error)
{
  ...
  for (unsigned i=0; i<edgels.size(); i++)
  {
    ...
    for (; it != edgels[i]->curvelets.end(); it++){
      ...
      theta_spread(cvlet_cnt) = max_estimates[0]-min_estimates[0];
      k_spread(cvlet_cnt)     = max_estimates[1]-min_estimates[1];
      gamma_spread(cvlet_cnt) = max_estimates[2]-min_estimates[2];

      theta_error(cvlet_cnt) = estimates[0]-GT[i].theta;
      k_error(cvlet_cnt)     = estimates[1]-GT[i].k;
      gamma_error(cvlet_cnt) = estimates[2]-GT[i].gamma;
  ...
  pdf1d_calc_mean_var(avg_spread.theta, std_spread.theta, theta_spread);
  pdf1d_calc_mean_var(avg_spread.k, std_spread.k, k_spread);
  pdf1d_calc_mean_var(avg_spread.gamma, std_spread.gamma, gamma_spread);
...
void Experiment1a()
{
  ...
  for (double k=0.02; k<0.4; k+=0.02)
  {
    ...
    dbdet_sel_sptr edge_linker = new dbdet_sel<cmodel>(edgemap, nrad, dt_sel, dx_sel); 
...
void Experiment3a()
{
  ...
  vcl_cout << " gamma and k measurement accuracy without perturbations" << vcl_endl;
  vcl_cout << "------------------------------------------" << vcl_endl;
  ...
  //dbdet_edgemap_sptr edgemap = generate_ES_edgemap(kk, gammam, dx_syn, dt_syn, edgels, GT);
  //saveEDG("EStest.edg", edgemap);
  //return;

  for (double gamma=-0.05; gamma<0.06; gamma+=0.01){
    for (double k=-0.35; k<0.4; k+=0.05)
    {
      //the ground truth and measurements for later comparison   
      vcl_vector<dbdet_edgel*> edgels(0); 
      vcl_vector<curve_params> GT(0);
...
{
  vcl_cout << "------------------------------------------" << vcl_endl;
  vcl_cout << " Accuracy Experiment 3b: " << vcl_endl;
  vcl_cout << " gamma and k measurement accuracy with perturbations" << vcl_endl;
  vcl_cout << "------------------------------------------" << vcl_endl;
  vcl_cout << "| dx_syn | dt_syn | dx_sel | dt_sel | nrad | maxN | GT gamma | avg. gamma | std. gamma | avg. gamma err. | std. gamma err. | avg. gamma spread | std. gamma spread | avg. k err. | std. k err. | avg. k spread | std. k spread | avg. theta err. | std. theta err. | " << vcl_endl; 
  vcl_cout.precision(3);
