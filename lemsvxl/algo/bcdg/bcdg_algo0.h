...
public:
  ...
  //: Constructor
  bcdg_algo0(std::vector<bcdg_frame_sptr>&, double = 0, dbecl_epipole_sptr = NULL, double = 0.5, int = 3000);
  //: Destructor
  ~bcdg_algo0();

  //: Get the result
  std::vector<bcdg_hypothesis_sptr> result() const;
  
private:
  //: Store all frames in the video
  std::vector<bcdg_frame_sptr> _frames;
  //: Store the bag of hypotheses
  std::vector<bcdg_hypothesis_sptr> _hypos;
  ...
  void build_starting_fingers_helper(std::vector< std::vector< bcdg_finger_sptr > >&, std::vector< dbecl_episeg_point >&, bcdg_point_projection&);
  
  //: The recursive crux of the algorithm.
  void trace( const std::vector<bcdg_finger_sptr>&, 
              const bcdg_global_option_sptr&, 
              const bcdg_hypothesis_sptr&);

  //: Figure out global options from a list of local ones in each frame.
  std::list<bcdg_global_option_sptr> figure_out_options() const;
  void figure_out_options_helper(int i, 
              std::vector<bcdg_local_option_sptr> building_options, 
              std::list<bcdg_global_option_sptr>& goptions) const;
  ...
  std::vector<bcdg_hypothesis> choose_from_bag();
  //: shared temp variable
  mutable std::vector< std::vector< bcdg_local_option_sptr > > _loptions;
};
...
class bcdg_algo0::params  {
  ...
  public:
    ...
    double speed() const;
    std::vector< dbecl_episeg_point > neighbors(vgl_point_2d<double>, int) const;
