...
void susan_edges_temp(in,r,mid,bp,max_no,x_size,y_size,A,B,W,do_sym,angle_edges)
...
{
  float z;
  int   do_symmetry, i, j, m, n, a, b, x, y;
  float w;
  uchar c,*p,*cp;
  int xx,yy,sq;
  //int *cgx,*cgy;
  ...
  if (r[i*x_size+j]>0)
    {
      ...
      if ( (m > r[(i+a)*x_size+j+b]) && (m > r[(i-a)*x_size+j-b]) &&
        ...
        {        
          mid[i*x_size+j] = 1;
          printf("\n %d %d %d %d %d %f",r[i*x_size+j],i,j,x,y,atan(z));
    ...
    x=0; y=0; w=0;
       ...
       |        \   */
        ...
        c=*(cp-*p++);x+=c;y+=9*c;w+=sqrt(3*3+1)*c;
        c=*(cp-*p++);y+=9*c;
        c=*(cp-*p);x+=c;y+=9*c;w-=sqrt(3*3+1)*c;
        p+=x_size-3; 
        
        c=*(cp-*p++);x+=4*c;y+=4*c;w+=sqrt(4+4)*c;
        c=*(cp-*p++);x+=c;y+=4*c;w+=sqrt(4+1)*c;
        c=*(cp-*p++);y+=4*c;
        c=*(cp-*p++);x+=c;y+=4*c;w-=sqrt(4+1)*c;
        c=*(cp-*p);x+=4*c;y+=4*c;w-=sqrt(4+4)*c;
        p+=x_size-5;
        
        c=*(cp-*p++);x+=9*c;y+=c;w+=sqrt(9+1)*c;
        c=*(cp-*p++);x+=4*c;y+=c;w+=sqrt(4+1)*c;
        c=*(cp-*p++);x+=c;y+=c;w+=sqrt(1+1)*c;
        c=*(cp-*p++);y+=c;
        c=*(cp-*p++);x+=c;y+=c;w-=sqrt(1+1)*c;
        c=*(cp-*p++);x+=4*c;y+=c;w-=sqrt(4+1)*c;
        c=*(cp-*p);x+=9*c;y+=c;w-=sqrt(9+1)*c;
    ...
    else
    {
      ...
      if ( (m > r[(i+a)*x_size+j+b]) && (m > r[(i-a)*x_size+j-b]) &&
        ...
        {
          mid[i*x_size+j] = 2;        
          printf("\n %d %d %d %d %d %f",r[i*x_size+j],i,j,x,y,z);
