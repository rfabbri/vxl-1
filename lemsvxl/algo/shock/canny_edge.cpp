...
/*******************************************************************************
* PROGRAM: canny_edge
* PURPOSE: This program implements a "Canny" edge detector. The procesvcl_sing
...
*
*   sigma = The svcl_tandard deviation of the gaussian smoothing filter.
...
main(int argc, char *argv[])
{
   ...
   float sigma,              // Svcl_tandard deviation of the gaussian kernel. 
   ...
   fprintf(stderr,"\n<USAGE> %s image sigma tlow thigh [writedirim]\n",argv[0]);
      ...
      fprintf(stderr,"      sigma:      Svcl_tandard deviation of the gaussian");
...
void canny(unsigned char *image, int rows, int cols, float sigma,
         float tlow, float thigh, short int *nmsbuf, unsigned char *edge, float *dir_radians)
{
   ...
   /****************************************************************************
   * Perform gaussian smoothing on the image usingthe input svcl_tandard
   * deviation.
   ****************************************************************************/
   if(VERBOSE) printf("Smoothing the image usinga gaussian kernel.\n");
  ...
  //Amir addition 03/06/03
  //return the smoothed image to the image
  for(r=0;r<rows;r++)
      for(c=0;c<cols;c++){
      //image[r*cols+c] = smoothedim[r*cols+c];
      //image[r*cols+c] = magnitude[r*cols+c];
      //image[r*cols+c] = nms[r*cols+c];
      image[r*cols+c] = edge[r*cols+c];

      //output the magnitude if there is support from nms
      //if ( nms[r*cols+c] < 255)
      //  image[r*cols+c] = magnitude[r*cols+c];
      //else
      //  image[r*cols+c] = 0;
  ...
  //Ming: return the non_max_supp nms to the image
  int max_nms=-100000;
  for(r=0;r<rows;r++)
      for(c=0;c<cols;c++) {
      if (nms[r*cols+c]==POSSIBLE_EDGE)
        nmsbuf[r*cols+c] = magnitude[r*cols+c];
      else
        nmsbuf[r*cols+c] = 0;
      if (max_nms < magnitude[r*cols+c])
        max_nms = magnitude[r*cols+c];
    }
...
* read in from either a file or from svcl_tandard input. The image is only read
* from svcl_tandard input when infilename = NULL. Because the PGM format includes
    ...
    int *cols)
{
   ...
   * filename was provided, set fp to read from svcl_tandard input.
   ***************************************************************************/
...
* written to the file specified by outfilename or to svcl_tandard output if
    ...
    int cols, char *comment, int maxval)
{
   ...
   * filename was provided, set fp to write to svcl_tandard output.
   ***************************************************************************/
...
* read in from either a file or from svcl_tandard input. The image is only read
* from svcl_tandard input when infilename = NULL. Because the PPM format includes
* the number of columns and the number of rows in the image, these are read
* from the file. Memory to store the image is allocated in this function.
* All comments in the header are discarded in the process of reading the
* image. Upon failure, this function returns 0, upon sucess it returns 1.
