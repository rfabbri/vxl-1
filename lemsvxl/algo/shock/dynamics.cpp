...
{
   return _H*std::sin(tau)/(2*std::cos(tau)*std::cos(tau));
}

double SIPointPoint::rpp(TAU_TYPE tau)
{
   return _H/(std::cos(tau)*std::cos(tau)*std::cos(tau)) - _H/(2*std::cos(tau));
}

double SIPointPoint::g  (TAU_TYPE tau)
{
   return _H/(2*std::cos(tau)*std::cos(tau));
}
...
{
   return -1/std::tan(M_PI_2/2 - _thetaL/2);
}
...
{
   return 1/std::cos(M_PI_2/2 - _thetaL/2);
}

double SILineLine::tangent (TAU_TYPE tau)
{
   //tangent vector
   double tx = -(std::cos(_ul) + std::cos(_ur))/2;
   double ty = -(std::sin(_ul) + std::sin(_ur))/2;
   //direction of this shock
   double t = std::atan2(ty,tx);
...
{
   //return 1/std::sin(M_PI/4 - thetaL/2);
  return 1/std::cos(M_PI/4 - _thetaL/2);
}
...
double SILineArc::r (TAU_TYPE tau)
{
  ...
  double one_nud_s_cos_tau = 1+(_nud*_s)*std::cos(tau);
...
double SILineArc::v  (TAU_TYPE tau)
{
  ...
  if (_s*_nud==1)
    ...
    return std::fabs(std::sqrt(2+2*std::cos(tau+M_PI))/std::sin(tau+M_PI)); //std::fabs(std::sqrt(2-2*std::cos(tau))/std::sin(tau));
}
...
{
  double dx = _a/_c;
  double dy = ( _c-_a*std::cos(tau) ) / (_c*std::sin(tau));
  double dir = std::atan2 (dy, dx) + _u;
