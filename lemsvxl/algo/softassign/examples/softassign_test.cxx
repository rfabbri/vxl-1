...
{
  //vul_arg<std::string> in_1("-file1","first cem file");
  //vul_arg<std::string> in_2("-file2","second cem file");
  //vul_arg_parse(argc, argv);

  //static std::string filename1 = in_1();
  //static std::string filename2 = in_2();
  ...
  for (i = 0; i<row_no; i++)
    for (j = 0; j<col_no; j++)
      A[i][j] = ((double)std::rand() / (double)(RAND_MAX+1)) * 100;
  ...
  /*for (i = 0; i<row_no+col_no; i++)
    for (j = 0; j<col_no+row_no; j++)
      AE[i][j] = 0;// + 0.01*((double)std::rand() / (double)(RAND_MAX+1));
  ...
  */
  ...
  while (T > 0) {
    ...
    std::cout << "Matrix of Rangarajan at T: " << T << "\n";
  ...
  std::cout << "assignment matrix as A:\n";
  sa.print_M(A, row_no+1, col_no+1);
