                              ...
                              float curve_ratio )
{
  ...
  bapl_lowe_pyramid_set_sptr pyramid_set = new bapl_lowe_pyramid_set(image,3);
...
bapl_refine_peak( const vil_image_view<float> & neighbors, vnl_double_3& delta)
{
  ...
  return neighbors(1,1,1) + dot_product(D,delta)/2.0f;  
}
                     ...
                     float curve_ratio )
{
  ...
  float max_curve = (curve_ratio+1.0)*(curve_ratio+1.0)/curve_ratio;
  float min_curve = (-curve_ratio+1.0)*(-curve_ratio+1.0)/-curve_ratio;
  ...
  for(int index=1; index<(num_oct*oct_size)-1; ++index){
    ...
    float ps = vcl_pow(2.0f,(index/oct_size)-1);
    ...
    for (unsigned j=2;j<(nj/2-1)*2;++j,row += jstep){
      ...
      for (unsigned i=2;i<(ni/2-1)*2;++i,pixel+=istep){
        ...
        peak_pts.push_back(vgl_point_3d<float>((ri+offset(0))*ps, (rj+offset(1))*ps,
                                                vcl_pow(2.0,((rindex+offset(2))/oct_size)-1) ));
...
{
  return vcl_exp(-((x*x)+(y*y))/(2.0*sigma*sigma));
}
                                  ...
                                  vcl_vector<float> & orientations )
{
  ...
  for (unsigned int i=0; i<peaks.size(); ++i){
    if (histogram[ peaks[i] ] > max){
      ...
      float dy   = (ypos - yneg)/2.0;
      float d2y  = 2.0*histogram[ peaks[i] ] - ypos - yneg;
