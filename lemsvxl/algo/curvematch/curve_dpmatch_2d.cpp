...
void curve_dpmatch_2d::initializeDPCosts()
{
  //Ming: init
  _DPCost.clear();
  _DPMap.clear();
...
double curve_dpmatch_2d::computeIntervalCost
  (int i, int ip, int j, int jp, vcl_vector <double> &pt)
{
...
void curve_dpmatch_2d::computeDPCosts ()
{
  ...
  for (sum = 1; sum<_n+_m-1; sum++) {
    start=curve_maxof(0,sum-_m+1,-10000);
...
void curve_dpmatch_2d::findDPCorrespondence (void)
{
  int i, j, ip, jp;
