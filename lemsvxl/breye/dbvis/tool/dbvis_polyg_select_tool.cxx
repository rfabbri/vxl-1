...
dbvis_polyg_select_tool::dbvis_polyg_select_tool()
...
bool
dbvis_polyg_select_tool::set_tableau( const vgui_tableau_sptr& tableau )
{
...
bool
dbvis_polyg_select_tool::set_storage ( const bpro_storage_sptr& storage)
{
  if (!storage.ptr())
    return false;
...
dbvis_polyg_select_tool::tableau()
{
  return tableau_;
}

vidpro_vsol2D_storage_sptr
dbvis_polyg_select_tool::storage()
{
...
bool
dbvis_polyg_select_tool::handle( const vgui_event & e, 
  ...
  if (active_&gesture_close_polyg_(e))
  {
     ...
     //Creating a tableeu for the drawn polygon
      ...
      for (int k=0;k<pts_in_stor.size();k++)
      {
          ...
          vidpro_vsol2D_storage_sptr selected_pts_stor = vidpro_vsol2D_storage_new();
          ...
          bvis_manager::instance()->repository()->store_data(selected_pts_stor);
          bvis_manager::instance()->add_to_display(selected_pts_stor);
          bvis_manager::instance()->make_tableau(selected_pts_stor);
...
vcl_string 
dbvis_polyg_select_tool::name() const
{
  return "Polygonal Selection";
}

void dbvis_polyg_select_tool::activate()
{
    active_ = true;
}
void dbvis_polyg_select_tool::deactivate()
{
    drawnvert.clear();
    active_ = false;
