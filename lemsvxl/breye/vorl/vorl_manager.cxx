// This is breye/vorl/vorl_manager.cxx
//:
// \file
...
#define PROCESS_MANAGER vidpro_process_manager::instance()

//static manager instance 
vorl_manager* vorl_manager::instance_ = 0;


//: Insure only one instance is created
vorl_manager *vorl_manager::instance()
{
  if (!instance_){
    instance_ = new vorl_manager();
    instance_->initialize();
  }
  return vorl_manager::instance_;
}


//: Initialize the instance
void
vorl_manager::initialize()
{
...
//: Constructor
vorl_manager::vorl_manager()
 : param_file_(    vorl_args_, "--vorl_param",
                     "parameter output file",     ""),
   video_file_(    vorl_args_, "--vorl_video",
                     "video input file",          ""),
   video_assoc_dir_(    vorl_args_, "--vorl_video_assoc",
                     "video input associated dir",          ""),
   status_file_(   vorl_args_, "--vorl_status",
                     "status block file",         ""),
   perf_file_(     vorl_args_, "--vorl_perform",
                     "performance output file",   ""),
   output_dir_(  vorl_args_, "--vorl_out_dir",
                     "output directory",          ""),
   out_video_dir_( vorl_args_, "--vorl_out_video",
                     "results video directory",   ""),
   out_ps_dir_( vorl_args_, "--vorl_out_ps",
                     "results ps directory",   ""),
   out_vrml_dir_( vorl_args_, "--vorl_out_vrml",
                     "results vrml directory",   ""),
   out_svg_dir_( vorl_args_, "--vorl_out_svg",
                     "results svg directory",   ""),
   start_frame_( vorl_args_, "--vorl_start_frame",
                     "Frame to start processing", 0),
   end_frame_( vorl_args_, "--vorl_end_frame",
                     "Frame to end processing",   -1),
   video_id_( vorl_args_, "--vorl_video_id",
                     "Id of input video",   0),
   video_fileid_( vorl_args_, "--vorl_video_fileid",
                     "Id of input video file",   0),
   object_id_( vorl_args_, "--vorl_object_id",
                     "Id of input object",   0),
   skip_frames_( vorl_args_, "--skip_frames",
                     "no of frames to skip",   1),
   params_only_(   vorl_args_, "-print_params_only",
...
void 
vorl_manager::add_process_to_queue(const bpro_process_sptr& process)
{
  PROCESS_MANAGER->add_process_to_queue(process);
}


//: Remove the last process from the queue
void
vorl_manager::delete_last_process()
{
  PROCESS_MANAGER->delete_last_process();
}


//: Remove all processes from the queue
void 
vorl_manager::clear_process_queue()
{
  PROCESS_MANAGER->clear_process_queue();
}


//: Add the parameters of process to the program arguments list
void 
vorl_manager::add_process_to_args(const bpro_process_sptr& process)
{
...
unsigned int
vorl_manager::current_frame() const
{
  return PROCESS_MANAGER->get_repository_sptr()->current_frame();
}
...
bool
vorl_manager::next_frame()
{
  if( PROCESS_MANAGER->get_repository_sptr()->current_frame() >= last_frame_ )
    return false;
  return PROCESS_MANAGER->get_repository_sptr()->go_to_next_frame();
}


bool
vorl_manager::next_nth_frame(unsigned  n)
{
  if( PROCESS_MANAGER->get_repository_sptr()->current_frame()>= last_frame_  ||
      PROCESS_MANAGER->get_repository_sptr()->current_frame()+n>= last_frame_ )
    return false;
  return PROCESS_MANAGER->get_repository_sptr()->go_to_next_n_frame(n);
}
...
vorl_manager::rewind()
{
  PROCESS_MANAGER->get_repository_sptr()->go_to_frame(first_frame_);
}


//: Seek to the given frame number
void
vorl_manager::seek_to_frame(unsigned int frame)
{
  PROCESS_MANAGER->get_repository_sptr()->go_to_frame(frame);
}


//: Parse the command line parameters
void
vorl_manager::parse_params(int argc, char** argv)
{
  vul_arg_include(vorl_args_);
...
vorl_manager::load_video()
{
  ...
  vidpro_repository_sptr rep = PROCESS_MANAGER->get_repository_sptr();
  first_frame_ = 0;
...
vorl_manager::update_process_params()
{
        ...
        a_itr != process_args_.args_.end(); ++a_itr )
  {
    ...
    {
      bpro_parameters_sptr test_params = (*p_itr)->parameters();
    ...
    if (arg_string){
      ...
      if(params->valid_parameter_type(option, bpro_filepath())){
        // prepend the path to associated files to this file name
  //      params->set_value(arg_string->option(), 
   //                       bpro_filepath(get_video_assoc_dir()+"/"+(*arg_string)()) );
                params->set_value(arg_string->option(), bpro_filepath((*arg_string)()) );
...
vorl_manager::init_args(const bpro_parameters_sptr& param)
{
  ...
  for(param_vector::iterator p_itr = params.begin(); p_itr != params.end(); ++p_itr){
    ...
    if( bpro_param_type<int> * param = dynamic_cast<bpro_param_type<int> *>(*p_itr) ) {
      process_args_.add(new vul_arg<int>( param->name().c_str(),
                                          param->description().c_str(),
                                          param->value() ));
    }
    else if( bpro_param_type<unsigned> * param = dynamic_cast<bpro_param_type<unsigned> *>(*p_itr) ) {
      process_args_.add(new vul_arg<unsigned>( param->name().c_str(),
                        param->description().c_str(),
                        param->value() ));
    }
    else if( bpro_param_type<float> * param = dynamic_cast<bpro_param_type<float> *>(*p_itr) ) {
      process_args_.add(new vul_arg<float>( param->name().c_str(),
                                            param->description().c_str(),
                                            param->value() ));
    }
    else if( bpro_param_type<double> * param = dynamic_cast<bpro_param_type<double> *>(*p_itr) ) {
    ...
    else if( bpro_param_type<bool> * param = dynamic_cast<bpro_param_type<bool> *>(*p_itr) ) {
...
int vorl_manager::get_no_skip_frames() {
  return skip_frames_();
}
...
bool
vorl_manager::write_status(float status) const
{    
  if (!status_file_.set())
    return false;
...
vorl_manager::write_params() 
{
  ...
  int n_args = process_args_.args_.size() + vorl_args_.args_.size();
  outstream << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
            << "<vxl>\n"
       ...
       arg_it != all_args.args_.end(); ++arg_it)
  {
    ...
    if (type == "string"){
      ...
      for ( proc_iterator p_itr = processes_.begin(); 
            p_itr != processes_.end(); ++p_itr )  
      {
        bpro_parameters_sptr params = (*p_itr)->parameters();
        if(params->valid_parameter(command)){
          if(params->valid_parameter_type(command, bpro_filepath())){
 //           type = "file";
              type = "assocfile";
          }
          break;
                                    ...
                                    const vcl_set<vcl_string>& names )
{
    ...
    if( names.empty() )
      save_rep = PROCESS_MANAGER->get_repository_sptr();
    else{
      save_rep = new vidpro_repository(*(PROCESS_MANAGER->get_repository_sptr()));
...
int vorl_manager::video_size()
{
 return PROCESS_MANAGER->get_repository_sptr()->num_frames();
}


#undef PROCESS_MANAGER
