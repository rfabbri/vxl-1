...
int main(int argc, char** argv)
{
  ...
  //vorl_manager::instance()->add_process_to_args(regimg); 
    ...
    bpro_process_sptr bgbuilder(new dbbgm_aerial_bg_model_process1()); //dbbgm_distribution_image
    bgbuilder->set_input_names(input_names_bg);
    vorl_manager::instance()->add_process_to_args(bgbuilder);


  bpro_process_sptr savebgmodel(new dbbgm_save_bg_model_process1()); 
  savebgmodel->set_input_names(vcl_vector<vcl_string>(1,"bgmodel"));
  vorl_manager::instance()->add_process_to_args(savebgmodel);
  ...
  int n=vorl_manager::instance()->get_no_skip_frames();
  do {
  ...
  }  while(vorl_manager::instance()->next_nth_frame(n));
  vorl_manager::instance()->finish_process_queue();
  vorl_manager::instance()->run_process_on_current_frame(savebgmodel);
