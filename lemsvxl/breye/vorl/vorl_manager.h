...
class vorl_manager
{
 public:
  ...
  // \param modified is an optional parameter.  If specified, all storage objects
  //                 modified by the process are added to the set
  bool finish_process_queue( std::set<bpro_storage_sptr>* modified = NULL );

  //: Calls the finish function on all processes in the queue on a range of frames
  // \param modified is an optional parameter.  If specified, all storage objects
  //                 modified by the process are added to the set
  bool finish_process_queue( int first, int last, std::set<bpro_storage_sptr>* modified = NULL );
 ...
 protected:
  ...
  std::vector<bpro_process_sptr> processes_;
