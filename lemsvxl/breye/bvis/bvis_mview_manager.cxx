...
{
  std::cout << "Direct Database Usage -\n";
...
bvis_mview_manager::regenerate_all_tableaux()
{
  ...
  if(!cache_tableau_)
    return;
  std::set<bpro_storage_sptr> all_storages = 
...
bvis_mview_manager::display_visible_data(bool clear_old) 
{
  ...
  for( std::vector<bvis_view_tableau_sptr>::iterator v_itr = view_tabs_.begin();        v_itr != view_tabs_.end(); ++v_itr ){
    if(!(*v_itr))
      continue;
    //we know these are mview_view_tabs
...
bvis_mview_manager::make_empty_storage()
{
  ...
  std::set<std::string> typeset = bpro_storage_registry::types();
