...
std::vector< std::string > vidpro_frame_average_process::get_input_type()
{
...
std::vector< std::string > vidpro_frame_average_process::get_output_type()
{
  std::vector< std::string > to_return;
  to_return.push_back( "image" );
  return to_return;
...
vidpro_frame_average_process::execute()
{
  if ( input_data_.size() != 1 )
    {
      std::cout << "In vidpro_frame_average_process::execute() - not exactly one"
               << " input image \n";
      return false;
    }
  clear_output();
