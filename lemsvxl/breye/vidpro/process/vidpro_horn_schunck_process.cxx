...
std::vector< std::string > vidpro_horn_schunck_process::get_input_type()
{
...
std::vector< std::string > vidpro_horn_schunck_process::get_output_type()
{  
  std::vector<std::string > to_return;
  to_return.push_back( "image" );
  return to_return;
...
vidpro_horn_schunck_process::execute()
{
  if ( input_data_.size() != 2 ){
    std::cout << "In vidpro_horn_schunck_process::execute() - not exactly two"
             << " input images \n";
    return false;
  }
