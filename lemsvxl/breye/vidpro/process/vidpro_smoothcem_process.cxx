...
vidpro_smoothcem_process::vidpro_smoothcem_process() : bpro_process()
{
  if(!parameters()->add(  "Sigma" , "-sigma" , (float)1.0 ) ||
     !parameters()->add(  "Number of times" , "-times" , 5 ))
  {
    std::cerr << "ERROR: Adding parameters in vidpro_smoothcem_process::vidpro_smoothcem_process()" << std::endl;
  }
}

//: Clone the process
bpro_process*
vidpro_smoothcem_process::clone() const
{
  return new vidpro_smoothcem_process(*this);
}

std::vector< std::string > vidpro_smoothcem_process::get_input_type()
{
...
std::vector< std::string > vidpro_smoothcem_process::get_output_type()
{
  std::vector< std::string > to_return;
  to_return.push_back( "vsol2D" );
  return to_return;
...
bool vidpro_smoothcem_process::execute()
{
  int times=0;
  float sigma=0;
  ...
  for (int i = 0; i<times; i++) {
    ...
    std::cout << "curve size after smoothing: " << curve.size() << std::endl;
    ...
    std::cout << "curve size after erasing 6 points: " << curve.size() << std::endl;
  }
