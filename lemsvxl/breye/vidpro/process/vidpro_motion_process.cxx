...
std::vector< std::string > vidpro_motion_process::get_input_type()
{
...
std::vector< std::string > vidpro_motion_process::get_output_type()
{  
  std::vector<std::string > to_return;
  to_return.push_back( "image" );
  return to_return;
...
vidpro_motion_process::execute()
{
  if ( input_data_.size() != 2 ){
    std::cout << "In vidpro_motion_process::execute() - not exactly two"
             << " input images \n";
    return false;
  }
  clear_output();
