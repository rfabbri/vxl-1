...
vidpro_load_edg_process::vidpro_load_edg_process() : bpro_process()
{
  if( !parameters()->add( "Input file <filename...>" , "-edginput" , bpro_filepath("","*.edg") ) ||
...
//: Clone the process
bpro_process*
vidpro_load_edg_process::clone() const
{
  return new vidpro_load_edg_process(*this);
}


bool vidpro_load_edg_process::execute()
{
  double x, y;
  ...
  while (infp.getline(lineBuffer,1024)) {
    ...
    if (!strncmp(lineBuffer, " EDGE_COUNT=", sizeof(" EDGE_COUNT=")-1)){
    ...
    if (!strncmp(lineBuffer, "EDGE : ", sizeof("EDGE : ")-1))
