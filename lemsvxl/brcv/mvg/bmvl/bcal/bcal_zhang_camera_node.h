...
{
 private:
  std::vector< vgl_homg_point_2d<double> > *point_lists_ptr_;

 public: // constructor and destructor
  bcal_zhang_camera_node(int id=0);
  virtual ~bcal_zhang_camera_node();
 public:
  int read_data(std::vector< vgl_homg_point_2d<double> > &ppts, int iframe);
  int removeData();

  // set get function
  std::vector< vgl_homg_point_2d<double> >& getPoints(int iview)
  {
  ...
  virtual void set_beat(std::vector<double> const& new_beats);
};
