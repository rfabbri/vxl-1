                                             ...
                                             vil1_memory_image_of<float> & reconst)
{
  ...
  int theta_max = 360;
  int tmax = (int)std::sqrt(double((xmax)*(xmax)+(ymax)*(ymax)));
  int tt = (tmax-1)/2;
  sinogram.resize(theta_max,tmax);
  reconst.resize(xmax, ymax);
  reconst.fill(0.0);
  //construct the cylinders
  std::vector<btom_gauss_cylinder_sptr> cyls;
  double linear_cyl = std::sqrt(double(ncyl_));
    ...
    for (int th = 0; th<theta_max; th++)
      {
        ...
        for (std::vector<btom_gauss_cylinder_sptr>::iterator cit = cyls.begin();
             cit != cyls.end(); cit++)
