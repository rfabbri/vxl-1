...
{
  return px(std::cos(theta/2),
            std::sin(theta/2)*std::sin(psi)*std::cos(phi),
            std::sin(theta/2)*std::sin(psi)*std::sin(phi),
            std::sin(theta/2)*std::cos(psi));
}

static double y(const double theta, const double psi, const double phi)
{
  return py(std::cos(theta/2),
            std::sin(theta/2)*std::sin(psi)*std::cos(phi),
            std::sin(theta/2)*std::sin(psi)*std::sin(phi),
            std::sin(theta/2)*std::cos(psi));
}
...
static void generate_points(std::vector<vgl_point_3d<double> >& pts,
                            std::vector<vgl_point_3d<float> >& color,
  ...
  for (double theta = theta_min; theta<=theta_max; theta+=theta_inc)
    for (double psi = psi_min; psi<=psi_max; psi+=psi_inc)
      for (double phi = phi_min; phi<=phi_max; phi+=phi_inc)
      {
        ...
        //          vgl_point_3d<float> c(std::fabs(0.5+p.x()),std::fabs(0.5+p.y()),
        //                          std::fabs(0.5+p.z()));
...
#if 0 // static function unused
static
void generate_polylines(std::vector<std::vector<vgl_point_3d<double> > >& polys,
                        std::vector<vgl_point_3d<float> >& polycolors,
                        const double theta_min,
                        const double theta_max,
                        const double psi_min,
                        const double psi_max,
...
#if 0 // static function unused
static void
generate_algebra_polylines(std::vector<std::vector<vgl_point_3d<double> > >& pys,
                           std::vector<vgl_point_3d<float> >& polycolors,
                           const double i_min,
                           const double i_max,
                           const double j_min,
                           ...
                           const double k_inc)
{
  ...
  for (double i = i_min; i<=i_max; i+=i_inc)
    for (double j = j_min; j<=j_max; j+=j_inc)
      ...
      {
        std::vector<vgl_point_3d<double> > vts;
        for (double t = 0.0; t<=1.0; t+=0.05)
        {
          if (std::fabs(std::fabs(t*i + t*j + t*k)-1)<0.01)
            continue;
...
static
void generate_d_polylines(std::vector<std::vector<vgl_point_3d<double> > >& polys,
                          std::vector<vgl_point_3d<float> >& polycolors,
                          const double theta_min,
                          const double theta_max,
                          const double psi_min,
...
static
void generate_da_polylines(std::vector<std::vector<vgl_point_3d<double> > >& polys,
                           std::vector<vgl_point_3d<float> >& polycolors,
                           const double theta_min,
                           const double theta_max,
                           const double psi_min,
                           ...
                           const double phi_inc)
{
  ...
  for (double theta =theta_min; theta<=theta_max; theta += theta_inc)
  {
    std::cout << "da algebra theta = " << theta << '\n';
    for (double psi = psi_min; psi<=psi_max; psi+=psi_inc)
    {
      std::vector<vgl_point_3d<double> > temp;
      for (double phi = phi_min; phi<=phi_max; phi+=phi_inc)
      {
        double i = std::sin(psi)*std::cos(phi);
        double j = std::sin(psi)*std::sin(phi);
        double k = std::cos(psi);
...
#if 0 // static function unused
static void write_points(std::ofstream& str,
                         std::vector<vgl_point_3d<double> > const& points,
...
static void write_spheres(std::ofstream& str)
{
  double r = 0.02;
  // identity white
  vgl_sphere_3d<double> sid(0, 0, 0, r);
...
static void
write_polylines(std::ofstream& str,
                std::vector<std::vector<vgl_point_3d<double> > > const& polys,
                std::vector<vgl_point_3d<float> > const& polycolors)
{
  brct_algos::write_vrml_header(str);
  unsigned n = polys.size();
  for (unsigned i = 0; i<n; ++i)
  {
    std::vector<vgl_point_3d<double> > poly = polys[i];
...
int main(int argc,char * argv[])
{
  if (argc<8)
  {
    std::cout<<"Usage : generate_S3.exe file theta_min theta_max\n"
            << "psi_min psi_max phi_min phi_max theta_inc psi_inc phi_inc\n";
    return -1;
  }
  std::string point_file(argv[1]);
  std::ofstream point_ostr(point_file.c_str());
  double theta_min = static_cast<double>(std::atof(argv[2]));
  double theta_max = static_cast<double>(std::atof(argv[3]));
  double psi_min = static_cast<double>(std::atof(argv[4]));
  double psi_max = static_cast<double>(std::atof(argv[5]));
  double phi_min = static_cast<double>(std::atof(argv[6]));
  double phi_max = static_cast<double>(std::atof(argv[7]));
  double theta_inc = 0.05;
  double psi_inc = 0.05;
  double phi_inc = 0.05;
  if (argc >8 )
  {
    theta_inc = static_cast<double>(std::atof(argv[8]));
    psi_inc = static_cast<double>(std::atof(argv[9]));
    phi_inc = static_cast<double>(std::atof(argv[10]));
  }
  std::vector<std::vector<vgl_point_3d<double> > > polylines;
  std::vector<vgl_point_3d<float> > polycolors;
