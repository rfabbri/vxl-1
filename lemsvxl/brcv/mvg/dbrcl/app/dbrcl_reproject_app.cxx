...
{
  if( argc!=5 ){
    std::cout << "Usage : dbrcl_reproject_app constraint_file camera_file image_in_dir image_out_dir\n";
    return -1;
  }

  std::string constraint_file( argv[1] );
  std::string camera_file( argv[2] );
  std::string image_in_dir( argv[3] );
  std::string image_out_dir( argv[4] );
  ...
  std::vector<std::string> images_in;
  ...
  for( int i = 0; i < static_cast<int>(camera_list.size()); i++ ){
    ...
    std::stringstream new_image_name;
    if( i < 10 )
