...
static std::vector<std::string > parseLineForNumbers(std::string s);
...
static double mean(std::vector<double> numbers);
static double std(std::vector<double> numbers);
static void rgb_to_ihs(double r,double g, double b,double& i, double& h, double& s);
//: to create a 3d polar histogram for IHS cone
static bool norm3dpolar(std::vector<double> & dim1,std::vector<double> & dim2,std::vector<double> & dim3, vbl_array_3d<double> &polarhist,int thetabins,int rbins,int valuebins,double r1,double v1);    
