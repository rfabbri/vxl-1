//dbctrk_curve_clustering.cxx : implementation of dbctrk_curve_clustering class
...
dbctrk_curve_cluster::dbctrk_curve_cluster()
{
...
double dbctrk_curve_clustering::compute_euclidean_dist(int i,int j)
{
  ...
  {
    double tempx1=clusters_[j].prototype->get_best_match_prev()->match_curve_set[0]->desc->curve_->point((*iter).first).x();
    double tempy1=clusters_[j].prototype->get_best_match_prev()->match_curve_set[0]->desc->curve_->point((*iter).first).y();
    vgl_point_2d<double> point1(tempx1,tempy1);
    
    curve1.push_back(point1);
    
    double tempx2=clusters_[j].prototype->desc->curve_->point((*iter).second).x();
    double tempy2=clusters_[j].prototype->desc->curve_->point((*iter).second).y();
  ...
  for(unsigned int k=0;k<tcurve1.size();k++)
  {
    double dist=vgl_distance<double>(tcurve1[k],curve2[k]);
    cost+=dist;
...
double dbctrk_curve_clustering::compute_cluster_dist(int i,int j)
{
...
void dbctrk_curve_clustering::init_clusters(vcl_vector<dbctrk_tracker_curve_sptr> * curve_sets)
{
  ...
  {
    dbctrk_curve_cluster temp_cluster;
...
void dbctrk_curve_clustering::build_table()
{
...
bool dbctrk_curve_clustering::return_min(int &mini, int &minj, double & min_cost)
{
  min_cost=1e5;
  for(unsigned i=0;i<clusters_.size();i++)
    {
      for(unsigned j=i;j<clusters_.size();j++)
      {
        if(i!=j)
        {
          double cost=compute_cluster_dist(i,j);
          if(min_cost>cost)
          {
            min_cost=cost;
            mini=i;
...
void dbctrk_curve_clustering::clustering()
{
...
//this variable is not used in the code.  PLEASE FIX!  -MM
  ...
  bool clustering_=1;
  assert(no_of_clusters_>=0);
  while(clusters_.size()>static_cast<unsigned>(no_of_clusters_) && clustering_)
  {
    return_min(mini,minj,min_cost);
  ...
  for (unsigned int i=0; i<(*curve_sets).size(); ++i)
  {
    ...
    if (c1->get_best_match_prev().ptr())
    {
       ...
       if (c2->get_best_match_prev().ptr())
       {
          ...
          double cost2=distance_table[p];
          double cost=cost2>cost1 ? cost2 : cost1;
          c1->neighbors_[cost]=c2->get_id();
          c2->neighbors_[cost]=c1->get_id();
      }
     }
   }
...
void dbctrk_curve_clustering::merge_clusters(int i,int j)
{
   ...
   if(clusters_[i].curve_cluster_.size()>1)
   {
    ...
    if(sum>foreg_backg_threshold_ && sum-std > 0)
    {
      ...
      for(unsigned j=0;j<clusters_[i].curve_cluster_.size();j++)
      {
        ...
        clusters_[i].curve_cluster_[j]->ismovingobject_=true;
