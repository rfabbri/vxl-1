...
std::string dbinfo_load_tracks_process::name()
{
  return "Load Tracks";
}
...
dbinfo_load_tracks_process::execute()
{
  ...
  for(std::vector<dbinfo_track_geometry_sptr>::iterator git=track_geoms.begin();
  ...
  if(start_frame>end_frame)
    {
      std::cout << "In dbinfo_load_tracks_process::execute() -"
  ...
  if(start_frame>0)
    for(int frame=static_cast<int>(start_frame)-1; frame>=0; frame--)
      output_data_.push_back(std::vector< bpro1_storage_sptr >(0));
...
dbinfo_load_tracks_process::finish() 
{
 ...
 vidpro1_repository_sptr res = bvis1_manager::instance()->repository();
   ...
   if(!res->store_data(track_storage_))
    {
      std::cout << "In dbinfo_track_display_tool::display_tracks() "
