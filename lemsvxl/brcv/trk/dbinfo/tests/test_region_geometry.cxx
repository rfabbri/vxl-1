...
static void test_region_geometry(int argc, char* argv[])
{
  ...
  std::cout << "region diameter " << diameter << '\n';
  ...
  dbinfo_region_geometry_sptr trotsc_rg 
    = new dbinfo_region_geometry(*rg, H);
  std::cout << "translated, rotated and scaled points \n";
  for(unsigned i = 0; i<trotsc_rg->size();++i)
    std::cout << trotsc_rg->point(i) << '\n';
  ...
  bp_in.close();
  if (rgin_p)
    std::cout << "Recovered region geometry " << *rgin_p << '\n';
