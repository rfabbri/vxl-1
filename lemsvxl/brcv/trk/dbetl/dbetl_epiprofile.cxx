...
std::vector<dbetl_point_2d_sptr> 
dbetl_epiprofile( const std::vector<becld_episeg_sptr>& episegs, double angle )
{
  std::vector<dbetl_point_2d_sptr> to_return;
  ...
  std::sort(to_return.begin(), to_return.end(), dbetl_cmp_dist);
...
std::vector<dbetl_point_2d_sptr> 
dbetl_epiprofile( const std::vector<becld_episeg_sptr>& episegs, 
                 const vil_image_resource_sptr& image,
                 double angle )
{
  std::vector<dbetl_point_2d_sptr> to_return = dbetl_epiprofile( episegs, angle );
...
std::vector<std::vector<dbetl_point_2d_sptr> >
dbetl_epiprofile( const std::vector<becld_episeg_sptr>& episegs, 
                 const vil_image_resource_sptr& image,
                 double start_angle, double step, int num )
{
  // Compute each epipolar profile
  std::vector<std::vector<dbetl_point_2d_sptr> > results(num);
  for(int i=0; i<num; ++i){
    results[i] = dbetl_epiprofile(episegs, image, start_angle + i*step);
  }

  // Link the resulting points
  typedef std::vector<dbetl_point_2d_sptr>::iterator pnt_iterator;
  for(int i=1; i<num; ++i){
