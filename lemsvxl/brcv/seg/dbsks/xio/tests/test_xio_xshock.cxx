...
void test_xio_xgraph_ccm_model()
{
  ...
  std::string ccm_xml_file = "V:\\projects\\kimia\\shockshape\\symseg\\results\\ETHZ-dataset\\xshock-graph\\applelogos-xgraph\\applelogos_prototype1_ccm_cost-closest_oriented_edge-kovesi_len_10_thresh_15-2009april20_pos.xml";
  dbsks_xgraph_ccm_model_sptr xgraph_ccm = 0;

  x_read(ccm_xml_file, xgraph_ccm);
  if (xgraph_ccm)
  {
    xgraph_ccm->print(std::cout);
  }
  // TEST_NEAR("Read CCM model file", 1, 1.01, 0.01);


  // Test loading a geom_model file
  std::string geom_xml_file = "V:\\projects\\kimia\\shockshape\\symseg\\results\\ETHZ-dataset\\xshock-graph\\applelogos-xgraph\\xgraph_geom_model-applelogos_prototype1.xml";
  ...
  std::cout << ">> Log2 length ratios = ";
  for (std::map<unsigned, dbsks_xnode_geom_model_sptr >::iterator iter = xgraph_geom->map_node2geom().begin();
    iter != xgraph_geom->map_node2geom().end(); ++iter)
  {
    ...
    std::cout << " " << std::log(max_radius / min_radius) / vnl_math::ln2;
    graph_size;
  }

  for (std::map<unsigned, dbsks_xfrag_geom_model_sptr >::iterator iter = xgraph_geom->map_edge2geom().begin();
    iter != xgraph_geom->map_edge2geom().end(); ++iter)
  {
    ...
    xfrag_geom->get_param_range(min_psi_start, max_psi_start,
                ...
                graph_size);
    std::cout << " " << std::log(max_chord / min_chord) / vnl_math::ln2;
    graph_size;
  ...
  std::cout << "a0 = huge_val = " << a0 << std::endl;
  std::cout << "a1 = huge_val = " << a1 << std::endl;
  std::cout << "a2 = " << a2 << std::endl;

  std::cout << "a0 + a1 = " << a0 + a1 << std::endl;
  std::cout << "a0 + a2 = " << a0 + a2 << std::endl;

  std::cout << "a0 - a1 = " << a0 - a1 << std::endl;
  std::cout << "a0 - a2 = " << a0 - a2 << std::endl;

  std::cout << "is a0 > a2 ? answer = " << ((a0 > a2) ? "true" : "false") << std::endl;
  std::cout << "is (a0 + a2) > a0 ? answer = " << ((a0 + a2) > a0 ? "true" : "false") << std::endl;
  std::cout << "is (a0 + a2) >= a0 ? answer = " << ((a0 + a2) >= a0 ? "true" : "false") << std::endl;
  std::cout << "is (a0 + a1) > a0 ? answer = " << ((a0 + a1) > a0 ? "true" : "false") << std::endl;
  std::cout << "is (a0 + a1) >= a0 ? answer = " << ((a0 + a1) >= a0 ? "true" : "false") << std::endl;
  std::cout << "is (2*a0) > a0 ? answer = " << ( 2*a0 > a0 ? "true" : "false") << std::endl;
  std::cout << "is (2*a0) >= a0 ? answer = " << ( 2*a0 >= a0 ? "true" : "false") << std::endl;
