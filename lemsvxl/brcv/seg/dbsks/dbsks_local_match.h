...
public:
  ...
  void set_xnode_states(const std::map<dbsksp_shock_edge_sptr, dbsksp_shapelet_sptr >& shapelet_map);

  //: Get object boundary from the cur_xnode_map
  std::vector<bgld_circ_arc > get_cur_bnd_arc_list();
  ...
  const std::map<dbsksp_shock_node_sptr, dbsksp_xshock_node_descriptor >& get_cur_xnode_map()
    const {return this->cur_xnode_map_; }

  //: graph cost, using shock-edit distance cost as regularization
  double f_graph_w_shock_edit_shape_cost(const std::map<dbsksp_shock_node_sptr, dbsksp_xshock_node_descriptor >& xnode_map,
    double& image_cost,
    double& shape_cost);


  //: graph cost, using shock-edit distance cost as regularization
  double f_graph_w_shock_edit_shape_cost(const std::map<dbsksp_shock_node_sptr, 
  ...
  //: graph cost, using shock-edit distance cost as regularization
  double f_graph_w_bnd_length_shape_cost(const std::map<dbsksp_shock_node_sptr, dbsksp_xshock_node_descriptor >& xnode_map);
  ...
  //void get_bnd_arc_list(std::vector<bgld_circ_arc >& arc_list);
  ...
  std::map<dbsksp_shock_edge_sptr, double > init_image_cost_map_;
  std::map<dbsksp_shock_edge_sptr, double > init_shape_cost_map_;


  double final_graph_cost_;
  double final_graph_image_cost_;
  double final_graph_shape_cost_;
  std::map<dbsksp_shock_edge_sptr, double > final_image_cost_map_;
  std::map<dbsksp_shock_edge_sptr, double > final_shape_cost_map_;
...
protected:
  ...
  std::map<dbsksp_shock_node_sptr, dbsksp_xshock_node_descriptor > cur_xnode_map_;

  //////////////////////////////////////////////////////////////////////////////
  // initial states of the graph's edges
  std::map<dbsksp_shock_edge_sptr, dbsksp_shapelet_sptr > shapelet_map_;
