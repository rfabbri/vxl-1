                                     ...
                                     const std::string& det_record_dirname)
{
  ...
  std::string det_record_dir = output_dir + "/" + det_record_dirname;
  ...
  std::vector<std::string > xgraph_paths(det_list.size(), "" );
  for (unsigned i =0; i < det_list.size(); ++i)
  {
    ...
    std::string xgraph_filename = vul_sprintf("%s.%d.xml", det_group_id.c_str(), i);
    std::string xgraph_file = det_record_dir + "/" + xgraph_filename;
  ...
  std::vector<std::string > screenshot_paths(det_list.size(), "");
  unsigned num_screenshots = vnl_math::min(unsigned(det_list.size()), max_number_screenshots);
  for (unsigned i =0; i < num_screenshots; ++i)
  {
    // screenshot file
    std::string screenshot_filename = vul_sprintf("%s.%d.png", det_group_id.c_str(), i);
    std::string screenshot_file = det_record_dir + "/" + screenshot_filename;
	...
	// save the binary shape, in case to use in latter dynamic programming for optimal sequence of pose tracking
		...
		std::vector<vgl_point_2d<int > > region_pts;
		if(!dbsks_fill_in_silhouette(det_list[i]->xgraph(), source_image, region_pts, screenshot_binary))
			std::cout << "failed in fill the silhouette.\n";
		else
		{
			std::cout << "\n  Number of shape points: " << region_pts.size() << "...";	
			std::string screenshot_filename2 = vul_sprintf("%s.%d_binary.png", det_group_id.c_str(), i);
			std::string screenshot_file2 = det_record_dir + "/" + screenshot_filename2;
	...
	/*
			std::string appear_id_filename1 = vul_sprintf("%s.%d_appearance_1.txt", det_group_id.c_str(), i);
			std::string appear_id_file1 = det_record_dir + "/" + appear_id_filename1;
			std::string appear_id_filename2 = vul_sprintf("%s.%d_appearance_2.txt", det_group_id.c_str(), i);
			std::string appear_id_file2 = det_record_dir + "/" + appear_id_filename2;
			// compute the appearance id matrix
			std::vector<vnl_matrix<double> > appearance_id_matrix = dbsks_compute_appearance_id_matrix( region_pts, source_image);

			std::ofstream ofs1(appear_id_file1.c_str()), ofs2(appear_id_file2.c_str());
  ...
  std::string det_xml_record_file = output_dir + 
    "/" + ("xml_det_record+" + det_group_id + ".xml");

  std::cout << "\n>>Save detection results as XML record file " << det_xml_record_file << std::endl;
  {
    ...
    for (unsigned i =0; i < det_list.size(); ++i)
    {
      ...
      std::string screenshot_str = std::string("&lt;img src=\"") + "." + //<img src=."
      ...
      record->set_value("xgraph_scale", std::sqrt(det_list[i]->xgraph()->area()));
                                       ...
                                       std::vector<dbsks_det_desc_xgraph_sptr >& records)
{
  ...
  std::string regexp = storage_folder + "/" + "xml_det_record+*.xml";
  ...
  for (vul_file_iterator fn = regexp; fn; ++fn)
  {
    ...
    std::vector<dbsks_xshock_det_record_sptr > det_record_list;
    if (x_read(fn(), det_record_list))
    {
      ...
      for (unsigned dd =0; dd < det_record_list.size(); ++dd)
      {
        ...
        std::string det_xgraph_filename = "";
        det_record->get_value("xgraph_xml", det_xgraph_filename);

        // full path to the xgraph file
        std::string det_xgraph_file = storage_folder + "/" + det_xgraph_filename;
