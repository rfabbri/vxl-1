...
#include "dbsks_detect_xgraph_using_pyramid_edgemap.h"
  ...
  for (unsigned i_scale =0; i_scale < xgraph_scales.size(); ++i_scale)
  {
    ...
    else
    {
        ...
        << storage_folder << "\n";
      {
        ...
        dbsks_detect_xgraph_using_pyramid_edgemap::convert_edgemap_to_bw(actual_edgemap, bg_view);
    ...
    std::vector<dbsks_det_desc_xgraph_sptr >& dets)
{
  ...
  for (unsigned iw =0; iw < windows.size(); ++iw)
  {
      ...
      << storage_dir << "\n";
    {
      ...
      // create a binary image from the edgemap
      vil_image_view<vxl_byte > bg_view; 
      dbsks_detect_xgraph_using_pyramid_edgemap::convert_edgemap_to_bw(edgemap, bg_view);
