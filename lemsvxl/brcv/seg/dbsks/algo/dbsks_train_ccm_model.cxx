...
parse_ccm_params(const std::map<std::string, std::string >& ccm_params)
{
  ...
  for (std::map<std::string, std::string >::const_iterator iter = ccm_params.begin();
...
// both key and value are in string format
void dbsks_train_ccm_model_params::
parse_data_params(const std::map<std::string, std::string >& data_params)
{
  // a template of current parameters
  bpro1_parameters_sptr p = new bpro1_parameters();
  ...
  // adjust these parameters using input data
  for (std::map<std::string, std::string >::const_iterator iter = data_params.begin();
...
set_training_data_info(const dbsks_train_ccm_model_params& p)
{
  ...
  {
    std::cout << "\nERROR: couldn't load list of positive images:" << this->params.list_pos_images << std::endl;
    return false;
  }
  ...
  {
    std::cout << "\nERROR: couldn't load list of negative images:" << this->params.list_neg_images << std::endl;
    return false;
  }
...
collect_positive_data()
{
  ...
  for (unsigned i_pos =0; i_pos < pos_image_fnames.size(); ++i_pos)
  {
    ...
    std::string image_name = vul_file::strip_extension(pos_image_fnames[i_pos]);
    std::vector<std::string > xgraph_fnames = map_image_name_to_xgraph_fname[image_name];

    // load the image to get the original size
    std::string image_file = this->params.image_folder + "/" + pos_image_fnames[i_pos];
    ...
    for (unsigned i_xgraph =0; i_xgraph < xgraph_fnames.size(); ++i_xgraph)
    {
      ...
      std::string xgraph_file = this->params.xgraph_folder + "/" + xgraph_fname;
  ...
  //// write out xml file
  //std::ofstream os(this->params.pos_output_file.c_str(), std::ios::out);
  //bxml_write(os, doc);
  return true;
                                   ...
                                   map_image_name_to_xgraph_fname)
{
  ...
  std::vector<std::string > xgraph_filenames;
  if (!buld_parse_string_list(this->params.list_gt_xgraph, xgraph_filenames))
  {
    std::cout << "ERROR: couldn't load xgraph list file:" << this->params.list_gt_xgraph << std::endl;
    return false;
  }
  ...
  {
    std::string xgraph_filename = xgraph_filenames[i];
    ...
    std::string image_name = xgraph_filename;
...
std::ostream& dbsks_train_ccm_model::
                            ...
                            std::vector<vgl_box_2d<int > >& list_edgemap_bboxes)
{
  ...
  std::string edgemap_regexp = this->params.edgemap_folder + "/" + image_name +  "/" + image_name + "*" + this->params.edgemap_ext;
                 ...
                 dbsks_subpix_ccm& ccm, const vgl_box_2d<int >* roi)
{
  ...
  std::string edgemap_file = this->params.edgemap_folder + "/" + image_name + "/" + 
    edgemap_name + this->params.edgemap_ext;
  std::string edgeorient_file = this->params.edgemap_folder + "/" + image_name + "/" + 
...
collect_negative_data_old()
{
  ...
  //> Write header to output file
  std::ofstream os_neg(this->params.neg_output_file.c_str(), std::ios::out);
  ...
  std::vector<std::string > list_gt_xgraph_fnames;
  ...
  {
    std::string image_name = vul_file::strip_extension(pos_image_fnames[i_pos]);
    std::vector<std::string > fnames = map_image_name_to_xgraph_fname[image_name];
  ...
  for (unsigned i_neg =0; i_neg < neg_image_fnames.size(); ++i_neg)
  {
    ...
    // name of image and the groundtruth xgraph in it
    std::string image_fname = neg_image_fnames[i_neg];
    ...
    {
      // Name of the edgemap
      std::string edgemap_name = list_edgemap_names[i_edgemap];
      ...
      {
        // path to the xgraph file
        std::string xgraph_fname = list_gt_xgraph_fnames[i_xgraph];
        std::string xgraph_file = this->params.xgraph_folder + "/" + xgraph_fname;
