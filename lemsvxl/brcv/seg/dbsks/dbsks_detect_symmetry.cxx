...
analyze()
{
  ...
  std::vector<dbdet_edgel* > edgel_list;
  ...
  for (int percent_count=0; percent_count < 100; ++percent_count)
  {
    ...
    for (int i=start_count; i < end_count; ++i)
    {
      ...
      vgl_vector_2d<double > t1(std::cos(edge1->tangent), std::sin(edge1->tangent));
      for (unsigned j=i+1; j < edgel_list.size(); ++j)
      {
        ...
        vgl_vector_2d<double > t2(std::cos(edge2->tangent), std::sin(edge2->tangent));
        ...
        // update distribution
        int index = int(std::floor(symmetry_score / 0.02));
        ...
        if (index >= 100)
        {
          std::cout << "ERROR: symmetry score > 2\n";
          continue;
        }
...
void dbsks_detect_symmetry::
write_sym_score_distribution_to_file(const std::string& filename)
{
  std::cout << "Now save the distribution to a file: \n";
  ...
  std::cout << "Symmetry score distribution file = " << filename << std::endl;
                     ...
                     const vgl_point_2d<double >& pt2, const vgl_vector_2d<double >& t2)
{
  ...
  vgl_point_2d<double > medial_pt = 
    midpoint(pt1, pt2) + medial_tan * (d/2) / std::tan(phi - vnl_math::pi_over_2);

  double radius = (d/2) / std::cos(phi-vnl_math::pi_over_2);

  // Return extrinsic shock node
  return dbsksp_xshock_node_descriptor(medial_pt.x(), medial_pt.y(), 
    std::atan2(medial_tan.y(), medial_tan.x()), phi, radius);
}
