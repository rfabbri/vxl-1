...
MAIN( test_bg_model_speed )
{
  ...
  {
    dbsta_gaussian_indep<float,3> init_gauss(init_mean, init_covar);

    init_gauss.set_data(dbsta_stats_data<float>(1));
    dbsta_mg_window_updater<float> updater(init_gauss,
                                          max_components, window_size);
    dbsta_mixture<float> mixture;
    mixture.set_data(dbsta_stats_data<float>(0));
    dbbgm_distribution_image<float> model(ni,nj,mixture);
...

  {
    ...
    typedef bsta_mixture<gauss_type> mix_t;
    typedef bsta_num_obs<mix_t > mix_gauss_type;

    bsta_gauss_if3 init_gauss( init_mean, init_covar );
    bsta_mg_window_updater<mix_t> updater( init_gauss,
                                           max_components,
                                           window_size);
