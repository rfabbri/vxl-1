...
typedef vbl_array_2d<std::vector<dbdet_edgel*> >::iterator dbdet_edgemap_iter;
typedef vbl_array_2d<std::vector<dbdet_edgel*> >::const_iterator dbdet_edgemap_const_iter;
...
public:
  ...
  vbl_array_2d<std::vector<dbdet_edgel*> > edge_cells; 
  ...
  //: constructor2
  dbdet_edgemap(int width, int height, std::vector<dbdet_edgel*>& edgels) : edgels(0)
  { 
  ...
  //: read only access
  const std::vector<dbdet_edgel*>& cell(int x, int y) const { return edge_cells(y, x); }
    ...
    if (ret) {
      for (unsigned int xx=0; xx<width(); xx++) {
        for (unsigned int yy=0; yy<height(); yy++) {
          std::vector<dbdet_edgel*> cellA = this->cell(xx,yy);
          std::vector<dbdet_edgel*> cellB = map.cell(xx,yy);
