...
public:
  ...
  std::list<dbdet_hyp_tree_node*> children;
...
public:
    ...
    public:
      ...
      void operator++(int) //post increment operator
      {
        ...
        if (ptr_->children.size()==0){
          ...
          std::list<dbdet_hyp_tree_node*>::iterator nit = parent->children.begin();
      ...
      std::vector<dbdet_curvelet*>& get_cur_path() { return cur_path_; }

    protected:
      dbdet_hyp_tree_node* ptr_;             //this is the node that the iterator is currently pointing to
      std::vector<dbdet_curvelet*> cur_path_; // this is the path from point where the iterator was initialized to the current node
  };
  ...
  void print_all_paths()
  {
    ...
    for ( ; pit != end(); pit++){
      if ((*pit)->is_leaf()){
        ...
        std::vector<dbdet_curvelet*> CF_list = pit.get_cur_path();
  ...
  void next_node(dbdet_hyp_tree_node* cur_node, std::list<dbdet_curvelet*> &CF)
  {
    ...
    if (cur_node->children.size()==0){
      //print out the current CF list
      std::cout << ":: ";
      std::list<dbdet_curvelet*>::iterator cvit = CF.begin();
    ...
    //go to its children
    std::list<dbdet_hyp_tree_node*>::iterator nit = cur_node->children.begin();
...
{
public:
  std::vector<dbdet_hyp_tree*> nodes;
...
{
public:
  std::vector<std::set<dbdet_hyp_tree_node*> > labels; //contains the hyp tree nodes
...
public:
  ...
  dbdet_hyp_tree* tgt;
  double cost;
  std::vector<dbdet_curvelet*> cvlets;
