// This is brcv/seg/dbdet/pro/dbdet_compass_color_edge_detector_process.cxx

//:
// \file

#include "dbdet_compass_color_edge_detector_process.h"
...
dbdet_compass_color_edge_detector_process::dbdet_compass_color_edge_detector_process()
{
  ...
  if( !parameters()->add( "Compass Signature"   , "-signature_op"  , compass_signature_choices, 0) ||
      !parameters()->add( "Histogram Distance:" , "-hist_dist_op"  , histogram_distance_choices, 4) ||
...
//: Clone the process
bpro1_process*
dbdet_compass_color_edge_detector_process::clone() const
{
  return new dbdet_compass_color_edge_detector_process(*this);
}
...
int
dbdet_compass_color_edge_detector_process::input_frames()
{
  return 1;
}
...
int
dbdet_compass_color_edge_detector_process::output_frames()
{
  return 1;
}
...
dbdet_compass_color_edge_detector_process::execute()
{
  ...
  vil_image_resource_sptr col_image_sptr = frame_image->get_image();
  vil_image_view<vxl_byte> col_image = col_image_sptr->get_view(0, col_image_sptr->ni(), 0, col_image_sptr->nj() );
  ...
  double sigma, thresh;
  int Norient;
  bool bsubpix_edges, bout_str, bout_ori, bSG_filter;
  ...
  convert_RGB_to_Lab(col_image, L, a, b);

  //Now call the detector with these parameters
  vil_image_view<double> hist_grad;

  dbdet_edgemap_sptr edge_map;
  if (hist_dist_op>2){ //independent distance maps in L, a and b
    //temporary solution 
    if (hist_dist_op==3) hist_dist_op=0; 
    if (hist_dist_op==4) hist_dist_op=2;
    edge_map = dbdet_detect_compass_color_edges_independent(L, a, b, 1, Norient/2, signature_op, sigma, hist_dist_op, bSG_filter, thresh, hist_grad, bout_ori);
  }
  else
...
bool
dbdet_compass_color_edge_detector_process::finish()
{
  return true;
}
