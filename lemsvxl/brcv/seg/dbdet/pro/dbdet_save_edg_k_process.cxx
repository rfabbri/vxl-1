...
std::string dbdet_save_edg_k_process::name() 
{
  return "Save K .EDG File";
}

std::vector< std::string > dbdet_save_edg_k_process::get_input_type() 
{
...
std::vector< std::string > dbdet_save_edg_k_process::get_output_type() 
{
  std::vector< std::string > to_return;
  return to_return;
}
...
bool dbdet_save_edg_k_process::dbdet_save_edg_k(std::string filename, dbdet_edgemap_sptr edgemap)
{
  ...
  outfp << "# EDGE_MAP v2.0" << std::endl << std::endl;
  outfp << "# Format :  [Pixel_Pos]  Pixel_Dir Pixel_Conf  [Sub_Pixel_Pos] Sub_Pixel_Dir Strength D2f" << std::endl;
  outfp << std::endl;
  ...
  for (unsigned row=0; row<edgemap->edge_cells.rows(); row++){
    for (unsigned col=0; col<edgemap->edge_cells.cols(); col++){
      for (unsigned k=0; k<edgemap->edge_cells(row, col).size(); k++){
        ...
        //std::list<dbdet_curvelet* >::iterator cv_it = edgel->curvelets.begin();
        //outfp<<" [";
