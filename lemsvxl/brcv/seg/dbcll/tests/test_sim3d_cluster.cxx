...
std::vector<bgld_similarity_3d<double> > random_xforms(unsigned num)
{
  vnl_random rand;
  std::vector<bgld_similarity_3d<double> > xforms;
  for(unsigned i=0; i<num; ++i){
    double s = std::exp(rand.normal64());
...
{
  {
    std::vector<bgld_similarity_3d<double> > sim(4);
    ...
    std::vector<dbcll_sim3d_cluster> c(4);
    c[0] = dbcll_sim3d_cluster(sim,0);
    c[1] = dbcll_sim3d_cluster(sim,1);
    c[2] = dbcll_sim3d_cluster(sim,2);
    c[3] = dbcll_sim3d_cluster(sim,3);

    std::cout << "sim 0->1: "<<c[0].similarity(c[1]) << std::endl;
    std::cout << "sim 0->2: "<<c[0].similarity(c[2]) << std::endl;
    std::cout << "sim 0->3: "<<c[0].similarity(c[3]) << std::endl;
    std::cout << "sim 1->1: "<<c[1].similarity(c[1]) << std::endl;
    std::cout << "sim 1->2: "<<c[1].similarity(c[2]) << std::endl;
    std::cout << "sim 1->3: "<<c[1].similarity(c[3]) << std::endl;
    std::cout << "sim 2->3: "<<c[2].similarity(c[3]) << std::endl;
    ...
    std::cout << "var = " << c[0].var() << std::endl;
    std::cout << "var = " << c[1].var() << std::endl;
    c[0].merge(c[1]);
    std::cout << "var = " << c[0].var() << std::endl;
  ...
  {
    unsigned num = 100;
    std::vector<bgld_similarity_3d<double> > sim = random_xforms(num);
    std::vector<dbcll_sim3d_cluster> c;
    ...
    std::cout << "var: " << super_c.var() << std::endl;
    ...
    for(unsigned i=0; i<num; ++i){
      D(i,i) = -std::numeric_limits<double>::infinity();
    ...
    while(remaining > 1){
      ...
      std::cout << "using "<<i1<<"("<<c[i1].size()<<") and "<<i2
               << "("<<c[i2].size()<<")"<<std::endl;
      std::cout << "nearest neighbor dist: " << D(i1,i2) << std::endl;
      c[i1].merge(c[i2]);
      std::cout << "variance: " <<c[i1].var() << std::endl;
      for(unsigned i=0; i<remaining; ++i){
        if(i == i1 || i == i2)
          continue;
        double new_s = c[i1].similarity(c[i]);
        double old_s = std::max(D(i1,i),D(i2,i));
        if(new_s > old_s ){
          reducible = false;
          std::cout << "new similarity "<<i<<"("<<c[i].size()<<") is less and is " <<new_s
                  << " ("<<D(i1,i)<<", "<<D(i2,i)<<") ";
          std::cout << "angle diff: "<<(c[i].mean().rotation().inverse()*c[i1].mean().rotation()).angle()<<std::endl;
        }
        D(i1,i) = D(i,i1) = new_s;
        //D(i2,i) = D(i,i2) = -std::numeric_limits<double>::infinity();
      }
