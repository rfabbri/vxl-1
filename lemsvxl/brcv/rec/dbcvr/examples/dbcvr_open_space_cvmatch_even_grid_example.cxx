//: 
// \file    dbcvr_open_space_cvmatch_even_grid_example.cxx
// \brief   
// \author  H. Can Aras
// \date    2007-01-16
// 
...
#include <dbcvr/dbcvr_open_space_cvmatch_even_grid.h>
...
int main(int argc, char** argv)
{
  ...
  while(1)
  {
    ...
    dbcvr_open_space_cvmatch_even_grid matching(&curve1, &curve2,
                                                curve1.size()+1, curve2.size()+1,
                                                R1, R2,
                                                "implicit", "geometric-formulas", 3);

    vcl_pair<double, FinalMapType*> result;
...
int main(int argc, char** argv)
{
  vcl_cout << "OPEN SPACE CURVE MATCHING USING EVEN GRID EXAMPLE" << vcl_endl;
  vcl_string batch_fname = argv[1];
  vcl_ifstream infp(batch_fname.c_str());
  ...
  for(unsigned i=0; i<curves.size(); i++)
  {
    ...
    for(unsigned j=0; j<curves.size(); j++)
    {
      if(1)
      {
        ...
        dbcvr_open_space_cvmatch_even_grid matching1(&(curves[i]), &(curves[j]), curves[i].size()+1, curves[j].size()+1,
                                                      R1, R2, "explicit", "geometric-formulas", 3);

        dbcvr_open_space_cvmatch_even_grid matching2(&(curves[i]), &(curves_inv[j]), curves[i].size()+1, curves_inv[j].size()+1,
                                                      R1, R2, "explicit", "geometric-formulas", 3);

        dbcvr_open_space_cvmatch_even_grid matching3(&(curves_inv[i]), &(curves_inv[j]), curves_inv[i].size()+1, curves_inv[j].size()+1,
                                                      R1, R2, "explicit", "geometric-formulas", 3);

        dbcvr_open_space_cvmatch_even_grid matching4(&(curves_inv[i]), &(curves[j]), curves_inv[i].size()+1, curves[j].size()+1,
                                                      R1, R2, "explicit", "geometric-formulas", 3);

        vcl_pair<double, FinalMapType*> result1, result2, result3, result4;
