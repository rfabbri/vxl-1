...
dbcvr_clsd_cvmatch::dbcvr_clsd_cvmatch(){
  _n1 = 0;
  _n2 = 0;
  setTemplateSize(3);  
}

dbcvr_clsd_cvmatch::dbcvr_clsd_cvmatch(const bsol_intrinsic_curve_2d_sptr c1, 
...
dbcvr_clsd_cvmatch::dbcvr_clsd_cvmatch(const vsol_polygon_2d_sptr p1, 
...
void dbcvr_clsd_cvmatch::setTemplateSize (int temp_size) {
...
void dbcvr_clsd_cvmatch::printCost(){
  int i,j;
...
void dbcvr_clsd_cvmatch::writeCost(std::string fname){
...
void dbcvr_clsd_cvmatch::printMap(){
  int i,j;
...
void dbcvr_clsd_cvmatch::initializeDPMask1(){
...
void dbcvr_clsd_cvmatch::initializeDPMask2(int s1, int s2) {
  int ci=0,cj=1;
...
void dbcvr_clsd_cvmatch::computeDPCosts(int startPoint){
...
void dbcvr_clsd_cvmatch::findDPCorrespondence(int startPoint){
...
void dbcvr_clsd_cvmatch::findOptimalPath(int startPoint){
...
void dbcvr_clsd_cvmatch::computeMiddlePaths(int i, int j){
  int k=(i+j)/2;
  if(i<k){
...
void dbcvr_clsd_cvmatch::Match(){
  ...
  for (int i = 0; i<2*_n1; i++) {
    for (int j = 0; j<_n2; j++) {   // -1 since (0,0) will go to CD
      ...
      fpoo << _curve2->arcLength(j) << std::endl;
    }
  }
  ...
  for (int i=0; i<N0 ; i++) {
    ...
    fpoo << _curve2->arcLength(jj) << std::endl;
  }
...
double dbcvr_clsd_cvmatch::stretchCost (bsol_intrinsic_curve_2d_sptr curve, int i, int ip) {
  return curve->arcLength(i) - curve->arcLength(ip);
}

double dbcvr_clsd_cvmatch::bendCost (bsol_intrinsic_curve_2d_sptr curve, int i, int ip) {
  return curve_angleDiff (curve->angle(i), curve->angle(ip));
}

// Cost of matching the interval [x(i-1),x(i)]  to [y(k),y(j)].
double dbcvr_clsd_cvmatch::computeIntervalCost(int i, int ip, int j, int jp){
...
double dbcvr_clsd_cvmatch::finalBestCostRestrictedStartingPoint(int &index, double ratio, bool get_normalized_cost) {
...
double dbcvr_clsd_cvmatch::finalBestCost(int &index, bool get_normalized_cost)
{
  double minCost = _finalCost[0];
  index = 0;
...
dbcvr_cv_cor_sptr dbcvr_clsd_cvmatch::get_cv_cor(int minIndex) {
  return new dbcvr_cv_cor(_curve1, _curve2, _finalMap[minIndex], _n1); 
}
