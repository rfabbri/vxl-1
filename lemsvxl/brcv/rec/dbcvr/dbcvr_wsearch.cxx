...
bool SRPDPNCS (dbcvr_image* pim, dbcvr_image* image, int sx1, int sy1, int sx2, int sy2);
    ...
    int searchx1, int searchy1, int searchx2, int searchy2)
{
   ...
   dbcvr_image* pim = new dbcvr_image (patternwidth, patternheight, patternpointer);
   dbcvr_image* sim = new dbcvr_image (searchwidth,  searchheight,  searchpointer);
...
dbcvr_image* over2sampling (dbcvr_image* im)
{
   int x, y;
   int t1, t2, t3, t4;
   ...
   int ow = im->width*2;
   int oh = im->height*2;
   dbcvr_image* oim = new dbcvr_image (ow, oh);
...
double submtCF;
dbcvr_image* subsim[15];   //sub-sampling image, subsim[0]=sim
dbcvr_image* subpim[15];   //sub-sampling pattern, subpim[0]=pim
...
bool RPDPNCS (dbcvr_image* pim, dbcvr_image* image, int sx1, int sy1, int sx2, int sy2)
{
   ...
   //
   subsim[0] = new dbcvr_image (sw, sh);
...
{ 
  ...
  //
  subsim[BASE] = new dbcvr_image (sw, sh);
...
bool AutoModel (dbcvr_image* sim, int sx1, int sy1, int sx2, int sy2,
...
void ShowSubSampling (void)
{
  if (!bSearchTeach) {
   ...
   dbcvr_image* sub2 = sub2sampling(patim);
   putimage (bx+200,by+20,sub2->image, COPY_PUT);

   dbcvr_image* sub4 = sub2sampling(sub2);
   putimage (bx+300,by+20,sub4->image, COPY_PUT);

   dbcvr_image* sub8 = sub2sampling(sub4);
   putimage (bx+350,by+20,sub8->image, COPY_PUT);

   dbcvr_image* sub16 = sub2sampling(sub8);
...
void ShowPyramid (void)
{
   ...
   int RLayer;
   dbcvr_image* subsim[10];   //sub-sampling image, subsim[0]=sim
   dbcvr_image* subpim[10];   //sub-sampling pattern, subpim[0]=pim
...
void ShowOversampling (void)
{
   ...
   dbcvr_image* t = over2sampling (patim);
   putimage (bx+100,by,t->image,COPY_PUT);
   dbcvr_image* tt = over2sampling (t);
   putimage (bx+350,by,tt->image,COPY_PUT);
   dbcvr_image* ttt = over2sampling (tt);
...
void testSearch2 (void)
{
  ...
  9,  9,  9,128,128,  9,  9,  9,
   ...
   dbcvr_image* pim = new dbcvr_image (6, 5);
   copyblock (pim->imagedata, pat, pim->width*pim->height);
   dbcvr_image* sim1 = new dbcvr_image (8, 7);
   copyblock (sim1->imagedata, sea1, sim1->width*sim1->height);
  dbcvr_image* sim2 = new dbcvr_image (8, 7);
...
void testSearch3 (void)
{
  ...
  dbcvr_image* pim = new dbcvr_image (pw, ph);
   copyblock (pim->imagedata, pat, pim->width*pim->height);
   dbcvr_image* sim = new dbcvr_image (sw, sh);
