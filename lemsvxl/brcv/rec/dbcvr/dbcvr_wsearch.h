...
class dbcvr_image
{
  public:
    ...
    dbcvr_image (int w, int h);
    dbcvr_image (int w, int h, void* imd);
    dbcvr_image (int w, int h, void* imd, bool bcopy);
    dbcvr_image (int w, int h, void* imd, int sx, int sy);

    ~dbcvr_image () { if (balloc) free(imagedata); }
    
    inline void CopyImage (dbcvr_image* im);
};
