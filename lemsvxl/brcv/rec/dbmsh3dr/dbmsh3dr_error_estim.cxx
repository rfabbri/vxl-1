...
{
  #if DBMSH3D_DEBUG > 2
  vul_printf (std::cout, "  compute_pp_min_dist(): %d points to %d points.\n",
              pts1.size(), pts2.size());
  #endif
  min_dists.clear();
  min_ids.clear();
                          ...
                          std::vector<int>& min_ids)
{
  ...
  const unsigned int nc = 3, na = 0;
  std::vector<rsdl_point> search_pts (total);
  std::map<int, dbmsh3d_vertex*>::iterator it = PS1->vertexmap().begin();
  ...
  std::map<int, dbmsh3d_vertex*>::iterator it2 = PS2->vertexmap().begin();
                          ...
                          std::vector<vgl_point_3d<double> >& closest_pts)
{
  ...
  #if DBMSH3D_DEBUG > 2
  vul_printf (std::cout, "  compute_pf_min_dist(): top_n %d M1: V %d F %d M2: V %d F %d.\n",
                      ...
                      double& min, double& mean, double& median, double& RMS, double& max)
{
  ...
  vul_printf (std::cout, "  get_error_estim(): Among %u dists, %u within th %f\n",
  ...
  //Note that std::nth_element() will distory the order in the vector.  
  std::nth_element (tmp.begin(),
                   tmp.begin() + int(tmp.size()/2), 
                   tmp.end());
  median = *(tmp.begin() + int(tmp.size()/2));
  tmp.clear();  
  vul_printf (std::cout, "  min: %f, mean: %f, median: %f, RMS: %f, max: %f.\n", 
...
{
  vul_printf (std::cout, "  normalize_min_dist(): dist_th %f on %u measurements.\n", 
                             ...
                             double& min, double& mean, double& median, double& RMS, double& max)
{
  ...
  vul_printf (std::cout, "  compute_imprv_of_error(): Among %u dists, %u out of th %f\n",
              min_dists_1.size(), min_dists_1.size()-count, dist_th);
  vul_printf (std::cout, "    min: %f, mean: %f, median: %f,\n", min, mean, median);
  vul_printf (std::cout, "    RMS: %f, max: %f.\n", RMS, max);
}
                                ...
                                vgl_point_3d<double>& pt)
{
  ...
  vul_printf (std::cout, "  Performing test...\n");
    ...
    for (; it != M->vertexmap().end(); it++) {
      ...
      if (bgld_eq_m (d, d_exact) == false) {
        vul_printf (std::cout, "\n  Problematic query point: (%lf, %lf, %lf).\n",
                    pt.x(), pt.y(), pt.z());
        vul_printf (std::cout, "     perturbed from V %d, iter %d, top_n %d.\n", V->id(), i, top_n);
        delete kd_tree;
        return false; //fail.
      }
    }

    vul_printf (std::cout, "  iter %d\n", i);
  }
