...
/*
dbru_osl::dbru_osl(std::vector<std::string> const& classes,
                       std::vector<std::vector<dbinfo_observation_sptr> > const& prototypes)
{
  //osl_ = new std::vector<dbru_object_sptr>();
  this->add_prototypes(classes, prototypes);
}*/

dbru_osl::~dbru_osl()
{
  for (std::map<std::string, std::vector<std::pair<unsigned, unsigned> >* >::iterator
                      ...
                      std::vector<dbinfo_observation_sptr>*& prototypes )
{
  ...
  return false;
   ...
   /*       
void dbru_osl::add_prototype(std::string const& cls,
                               dbinfo_observation_sptr const& proto)
{
  ...
  if (proto->geometry()->n_polys() == 1) {
    ...
    std::cout << "WARNING dbru_osl::add_prototype() - number of polygons in the prototype is not 1, polygon is not added to the object\n";
...
add_object(dbru_object_sptr const& obj)
{
  ...
  std::string cls = obj->category();
  std::map<std::string, std::vector<std::pair<unsigned, unsigned> >* >::iterator iter;
  iter = osl_class_map_.find(cls);
  std::vector<std::pair<unsigned, unsigned> > *vec;
  if (iter == osl_class_map_.end()) {
    vec = new std::vector<std::pair<unsigned, unsigned> >();
    osl_class_map_[cls] = vec;
  } else 
    vec = iter->second;
  
  for (unsigned i = 0; i<obj->n_observations(); i++)
    vec->push_back(std::pair<unsigned, unsigned>(obj_id, i));
}

void dbru_osl::
add_objects(std::vector<dbru_object_sptr> const& objects)
{
...
bool dbru_osl::prototypes(std::string const& cls, 
                            std::vector<dbinfo_observation_sptr>& prototypes)
{
  std::map<std::string, std::vector<std::pair<unsigned, unsigned> >* >::iterator iter;
  iter = osl_class_map_.find(cls);
  ...
  for(std::vector<std::pair<unsigned, unsigned> >::iterator pit = vec->begin(); pit != vec->end(); ++pit) 
  {
...
/*
bool dbru_osl::remove_prototype(std::string const& cls,
...
bool dbru_osl::remove_class(std::string const& cls)
{
  bool done = false;
  while (!done) {
    done = true;
    for (std::vector<dbru_object_sptr>::iterator iter = osl_.begin(); iter != osl_.end(); iter++)
  ...
  std::map<std::string, std::vector<std::pair<unsigned, unsigned> >* >::iterator iterm;
...
bool dbru_osl::remove_object(unsigned i)
{
  if (i < osl_.size()) {
    ...
    std::vector<std::pair<unsigned, unsigned> > *vec;
    if (iter == osl_class_map_.end()) {
      std::cout << "Warning: class " << cls << " is not found\n";
    } else {
      vec = iter->second;
      unsigned nn = 0;
      ...
      std::vector<std::pair<unsigned, unsigned> >::iterator viter_b = vec->begin()+nn;
      std::vector<std::pair<unsigned, unsigned> >::iterator viter_e = viter_b+obj->n_observations()-1;
      vec->erase(viter_b, viter_e);
    }  
    
    std::vector<dbru_object_sptr>::iterator itero = osl_.begin();
    osl_.erase(itero+i);
    return true;
  } else
    return false;
...
std::vector<std::string> dbru_osl::classes() const
{
  std::vector<std::string> clss;
  for (std::map<std::string, std::vector<std::pair<unsigned, unsigned> >* >::const_iterator
         oit = osl_class_map_.begin(); oit != osl_class_map_.end(); oit++)
    clss.push_back((*oit).first);
  return clss;
}
/*
dbinfo_observation_sptr dbru_osl::prototype(std::string const& cls, const unsigned index)
{
...
unsigned dbru_osl::n_protos_in_class(std::string const& cls) const
{
  std::map<std::string, std::vector<std::pair<unsigned, unsigned> >* >::const_iterator iter;
  iter = osl_class_map_.find(cls);
  std::vector<std::pair<unsigned, unsigned> > *vec;
  if (iter == osl_class_map_.end()) {
    return 0;
...
std::vector<unsigned> dbru_osl::n_prototypes() const
{
...
bool dbru_osl::fill_category_id_map(void) {
  ...
  for (unsigned i = 0; i < osl_.size(); i++) {
    ...
    std::map<std::string, int>::iterator iter = category_id_map_.find(obj->category_);
  ...
  std::cout << "categories in the OSL are found to be:\n";
  for (std::map<std::string, int>::iterator iter = category_id_map_.begin(); iter != category_id_map_.end(); iter++) {
    std::cout << "\t" << iter->first << " id: " << iter->second << "\n";
  }
...
void dbru_osl::print(std::ostream& os) const
{
...
void dbru_osl::b_read(vsl_b_istream &is)
{
  ...
  switch (ver)
    ...
    case 1:
      {
        std::vector<dbru_object_sptr> osl;
        vsl_b_read(is, osl);
