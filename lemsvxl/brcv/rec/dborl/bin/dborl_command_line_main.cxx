...
int main(int argc, char *argv[]) {
  ...
  vul_arg<std::string> command_initial(arg_list, "-command-init", "the initial portion of the command, same for all elements", "");
  vul_arg<std::string> command_suffix(arg_list, "-command-suf", "the end portion of the command, will be appended after each elements name, may be empty", "");
  ...
  vul_arg<std::string> con_folder(arg_list,"-cons","the folder that contains cons","");

  vul_arg<std::string> esf_folder(arg_list,"-esfs","the folder that contains esfs","");
  ...
  if (prepare_dataset_folder()) {
    std::cout << "image_folder: " << image_folder() << std::endl;
    std::cout << "image ext: " << image_ext() << std::endl;
    std::cout << "con folder: " << con_folder() << std::endl;
    std::cout << "dataset list: " << dataset_list() << std::endl;
    std::cout << "category_info_file: " << category_info_file() << std::endl;
    ...
    if (!prepare_dataset_folder_from_exts(input_folder(), image_ext(), dataset_list(), output_folder(), category())) {
      std::cout << "usage with -prepare-dataset-folder-exts: \n";
      std::cout << input_folder.option() << " " << input_folder.help() << std::endl;
      std::cout << image_ext.option() << " " << image_ext.help() << std::endl;
      std::cout << dataset_list.option() << " " << dataset_list.help() << std::endl;
      std::cout << output_folder.option() << " " << output_folder.help() << std::endl;
      std::cout << " if want to append category names to the beginning of the object names: " << category.option() << " " << category.help() << std::endl;
    }
  } else if (prepare_flat_index_of_dataset()) {
    std::cout << "input folder: " << input_folder() << std::endl;
    std::cout << "output index file name: " << output_file() << std::endl;
    if (image_ext().compare("") != 0 && input_folder().compare("") != 0 && output_file().compare("") != 0) {
      std::cout << "will prepare flat index of an image dataset with image extensions: " << image_ext() << std::endl;
      if (!prepare_flat_index_of_image_dataset(input_folder(), image_ext(), output_file()))
        std::cout << "usage with -prepare-flat-index: -ext <extension> -input <input dataset folder> -out <output index file>\n";
    } else 
      std::cout << "usage with -prepare-flat-index: -ext <extension> -input <input dataset folder> -out <output index file>\n";
      ...
      if (!copy_files_to_object_folders(input_folder(), image_ext(), dataset_list(), output_folder()))
        std::cout << "usage with -copy-files-to-object-folders: -input <input folder> -ext <file extension> -dataset-list <dataset list> -output <output dataset folder>\n";
      ...
      if (!copy_files_from_to_object_folders(input_folder(), image_ext(), dataset_list(), output_folder()))
        std::cout << "usage with -copy-files-from-to-object-folders: -input <input folder> -ext <file extension> -dataset-list <dataset list> -output <output dataset folder>\n";
  } else if (prepare_model_input_xml_for_detection()) {
      if (!prepare_model_input_param_file(input_xml(), output_xml(), model_name()))
        std::cout << "usage with -prepare-model-input: -inp-xml <input xml> -out-xml <output xml> -model <name of the model>\n";
  } else if (prepare_model_inputs_xml_for_detection()) {
      if (!prepare_model_inputs_param_file(input_xml(), dataset_list(), output_folder()))
        std::cout << "usage with -prepare-model-inputs: -inp-xml <input xml> -dataset-list <list of model names> -output <output folder to save output xml files>\n";
  } else if (prepare_command()) {
    if (!prepare_commands_file(command_initial(), command_suffix(), dataset_list(), output_file())) {
      std::cout << "usage with -prepare-command: \n";
      std::cout << command_initial.option() << " " << command_initial.help() << std::endl;
      std::cout << command_suffix.option() << " " << command_suffix.help()  << std::endl;
      std::cout << dataset_list.option() << " " << dataset_list.help() << std::endl;
    ...
    if (!prepare_ground_truth_from_pascal_annot_ver_one(input_folder(), dataset_list(), output_folder(), category())) {
      std::cout << "usage with -prepare-gt-pascal-annot-ver-one: \n";
      std::cout << input_folder.option() << " " << input_folder.help() << std::endl;
      std::cout << category.option() << " " << category.help() << std::endl;
      std::cout << dataset_list.option() << " " << dataset_list.help() << std::endl;
      std::cout << output_folder.option() << " " << output_folder.help() << std::endl;
    }
    ...
    if (!prepare_flat_index_from_dataset_list(dataset_list(), input_folder(), output_file(), category())) {
      std::cout << "usage with -prepare-flat-index-from-dataset-list: \n";
      std::cout << dataset_list.option() << " " << dataset_list.help() << std::endl;
      std::cout << input_folder.option() << " " << input_folder.help() << std::endl;
      std::cout << output_file.option() << " " << output_file.help() << std::endl;
      std::cout << category.option() << " " << category.help() << " if category name is to be appended to the object names in the list " << std::endl;
    }
  } else if (rename_ground_truth_files_arg()) {
    if (!rename_ground_truth_files(input_folder(), dataset_list(), category())) {
      std::cout << "usage with -rename-gts: \n";
      std::cout << dataset_list.option() << " " << dataset_list.help() << std::endl;
      std::cout << input_folder.option() << " " << input_folder.help() << std::endl;
