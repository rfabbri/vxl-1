...
public:
  ...
  dborl_parameter<std::string> input_edge_folder_;

  dborl_parameter<std::string> output_extension_;

  dborl_parameter<std::string> output_bnd_folder_;  // if written to this folder as opposed to object folder then the edge map gets associated to the input object
                                                    // if nothing is written here, nothing gets associated

  std::string algo_abbreviation_;

  dborl_bnd_generic_params(std::string algo_name) : dborl_algo_params(algo_name) { 
