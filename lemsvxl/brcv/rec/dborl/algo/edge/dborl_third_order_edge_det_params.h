...
public:
  ...
  dborl_parameter<std::string> output_edge_folder_;  // if written to this folder as opposed to object folder then the edge map gets associated to the input object
  ...
  void set_process_parameters(dbdet_third_order_edge_detector_process& pro) {
    ...
    for (unsigned i = 0; i < param_list_.size(); i++) {
      if (param_list_[i]->param_group().compare(algo_abbreviation_) != 0)
        continue;
      std::string name = param_list_[i]->param_name();
      ...
      if (param_list_[i]->type_info() == dborl_parameter_type_info::PATH) {
        ...
        std::string val_str = param_list_[i]->value_str();
