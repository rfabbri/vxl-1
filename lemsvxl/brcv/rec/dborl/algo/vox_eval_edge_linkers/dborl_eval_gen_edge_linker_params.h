//:
// \file
// \brief Parameters for the edge linker evaluator
//
...
#if !defined(_dborl_eval_gen_edge_linker_params_h)
#define _dborl_eval_gen_edge_linker_params_h
...
class dborl_eval_gen_edge_linker_params : public dborl_algo_params
{
public:
  ...
  dborl_parameter<bool> link_edgels_;        //link the edgels or not

  //edge linker parameters
  dborl_parameter<bool> req_app_con_;        //require appearance consistency

  dborl_parameter<double> app_thresh_;       //appearance threshold
  dborl_parameter<double> len_thresh_;       //length threshold

  //: constructor
  dborl_eval_gen_edge_linker_params(vcl_string algo_name) : dborl_algo_params(algo_name) 
  { 
    ...
    link_edgels_.set_values(param_list_, "edge_link", "link_edges", "Link edges? Y/N", true, false);

    // set edge linking parameters 
    req_app_con_.set_values(param_list_, "edge_link", "req_app_con", "Require appearance consistency? Y/N", true, false);
    
    app_thresh_.set_values(param_list_, "edge_link", "link_app_thresh", "appearance threshold", 2.0, 2.0);
...
#endif  //_dborl_eval_gen_edge_linker_params_h
