...
bool dborl_detect_patch_processor::parse_command_line(std::vector<std::string>& argv)
{
  vul_arg_info_list arg_list;
  vul_arg<std::string> input_xml_file(arg_list,"-x","the input file in xml format that sets parameters of the algorithm","");
...
bool dborl_detect_patch_processor::parse(const char* param_file)
{
  ...
  for (float t = params_.min_thres_; t < params_.max_thres_; t += params_.thres_inc_) {
    std::cout << " ";  // dummy line because of the compiler error on smarteyes side, when there is a single line here, it computes size_t_ one more than it should be
    size_t_ = size_t_ + 1;
  }

  std::cout << "there are " << size_t_ << " thresholds in the interval given\n";
  
  return parse_index(index_file_);
}

//: this method is run on each processor
bool dborl_detect_patch_processor::parse_index(std::string index_file)
{
...
//: this method is run on each processor
bool dborl_detect_patch_processor::initialize(std::vector<dborl_detect_patch_processor_input>& input_vec)
{
  std::string storage_end = "patch_strg.bin";
  std::string model_storage_file = params_.model_dir_ + params_.model_name_ + params_.model_patch_params_.output_file_postfix_ + "-" + storage_end;

  if (!vul_file::exists(model_storage_file)) {
    std::cout << "dborl_detect_patch_processor::initialize() - " << model_storage_file << " not found\n";
    return false;
  }
  ...
  for (unsigned iii = 0; iii < model_st->size(); iii++) {
    ...
    std::string patch_esf_name = model_storage_file.substr(0, model_storage_file.length()-storage_end.size());
  ...
  //std::string model_gt_file = params_.model_dir_ + "groundtruth.xml";
  std::string model_gt_file = params_.model_dir_ + params_.model_name_ + ".xml";
  if (!vul_file::exists(model_gt_file)) {
    std::cout << "dborl_detect_patch_processor::initialize() - " << model_gt_file << " not found\n";
    return false;
  }
  ...
  if (!model_id->has_single_category()) {
    std::cout << "dborl_detect_patch_processor::initialize() - model image description has more than one category! image was not a valid map for a model\n";
    return false;
  }
  model_category_ = model_id->get_first_category();
  
  //: load each storage given in the root node of the index
  std::vector<std::string>& names = ind_->root_->cast_to_index_node()->names();
  std::vector<std::string>& paths = ind_->root_->cast_to_index_node()->paths();
  if (!names.size() || names.size() != paths.size()) {
    std::cout << "dborl_detect_patch_processor::initialize() - " << params_.index_file_ << " has problems\n";
  }

  std::vector<dbskr_shock_patch_storage_sptr> test_str;
  std::vector<borld_image_description_sptr> test_desc;
  ...
  for (unsigned i = 0; i < names.size(); i++) {  
    ...
    if (names[i].compare(params_.model_name_) == 0) {
      std::cout << "skipping model instance\n";
      continue;
    }

    std::string st_file = paths[i] + "/" + names[i] + params_.query_patch_params_.output_file_postfix_ + "/" + names[i] + params_.query_patch_params_.output_file_postfix_ + "-" + storage_end;
    //std::string st_file = paths[i] + "/" + names[i] + params_.query_patch_params_.output_file_postfix_ + "-" + storage_end;
    if (!vul_file::exists(st_file)) {
      std::cout << "dborl_detect_patch_processor::initialize() - " << st_file << " not found\n";
      return false;
    }
    ...
    //: load the shock graphs --> assumes each shock graph for each patch is saved in the same folder
    //std::cout << st->size() << " patches in test storage, reading shocks..\n";
    //: load esfs for each patch
    ...
    //std::string gt_file = paths[i] + "groundtruth.xml"; 
    std::string gt_file = paths[i] + names[i] + ".xml"; 
    if (!vul_file::exists(gt_file)) {
      std::cout << "dborl_detect_patch_processor::initialize() - " << gt_file << " not found\n";
      return false;
    }
    
    //std::cout << "query description file: " << gt_file << std::endl;
    dborl_image_desc_parser parser;
    ...
    if (id->category_exists(model_category_)) {
      ...
      if (id->get_category_cnt(model_category_) != 1) {
        std::cout << "WARNING: there are more than one instance of the model category in this test image, but increased the positive_cnt only by one.\n";
        std::cout << "MODIFY this section if you want different behavior in counting the number of positive instances\n";
      }
    } else 
      negative_cnt_++;
    test_desc.push_back(id);
    //if (id->has_single_category())
    //  std::cout << "query description has a single category: " << id->get_first_category() << std::endl;
  }

  std::cout << "there are " << positive_cnt_ << " pos instances and " << negative_cnt_ << " neg instances\n";
...
{
  //std::cout << "query: " << inp.query_name << "\n";
  //: load the match instance which contains model vs query patch matches
  std::string match_name = params_.matches_dir_ + inp.model_name + "-" + inp.query_name + params_.shock_match_params_.output_file_postfix_ + ".bin";
  if (!vul_file::exists(match_name.c_str())) {
    std::cout << "dborl_detect_patch_processor::process() - the file:" << match_name << " not found!\n";
    return false;
  }
  //std::cout << "\t loaded match.. ";
  ...
  if (params_.use_normalized_costs_) {
    if (params_.use_reconst_boundary_length_)
      if (!match->compute_length_norm_costs_of_cors()) { // sorts the matches after recomputation of the normalized cost
        std::cout << "dborl_detect_patch_processor::process() - sorting error for the match file:" << match_name << "!!!!\n";
        return false;
      }
  ...
  if (!match->detect_instance(box, params_.det_params_.top_N_, params_.det_params_.k_, inp.threshold)) {
    std::cout << "dborl_detect_patch_processor::process() - detection error for the match file:" << match_name << "!!!!\n";
    return false;
  }

  //std::cout << " ran detection algo.. \n";
  
  //if (box)
  //  std::cout << "\t\t model: " << inp.model_name << " detected box: " << *box  << "\n";
  //else
  //  std::cout << "\t\t model: " << inp.model_name << " did not detect a box\n";
...
bool dborl_detect_patch_processor::finalize(std::vector<dborl_detect_patch_processor_output>& results)
{
  std::cout << "in finalize method!!!!!!!!!!!!!!!\n";
  std::cout.flush();

  std::cout << "size_t_: " << size_t_ << " query_set_size_: " << query_set_size_ << " results size: " << results.size() << "\n";
  std::cout << "size_t_*query_set_size_ = " << size_t_*query_set_size_ << std::endl;
  std::cout.flush();

  if (! ((size_t_*query_set_size_) == results.size()) )
    return false;

  std::cout << "finalizing..results have: " << results.size() << std::endl;
  std::cout.flush();

  //: collect statistics for each threshold
  std::vector<buld_exp_stat_sptr> thres_stats(size_t_, 0);
  ...
  for (unsigned t = 0; t < size_t_; thres += params_.thres_inc_, t++) {
    std::cout << "stats for threshold: " << thres << std::endl;
    thres_stats[t]->print_stats();
    std::cout << "-------------------------------\n";
  }

  std::string out_file_name = params_.out_file_ + params_.output_file_postfix_ + params_.det_params_.output_file_postfix_ + ".out";
  std::ofstream of(out_file_name.c_str(), std::ios::app);
  if (!of) {
    std::cout << "out file: " << out_file_name << " could not be opened!\n";
    return false;
  }
  ...
  for (unsigned t = 0; t < size_t_; thres += params_.thres_inc_, t++) {
    of << "stats for threshold: " << thres << std::endl;
    thres_stats[t]->print_stats(of);
    of << "-------------------------------\n";
  }
  of.close();

  std::string out_file_plot_name = params_.out_file_ + params_.output_file_postfix_ + params_.det_params_.output_file_postfix_ + "-plots.out";
  std::ofstream ofp(out_file_plot_name.c_str(), std::ios::app);
  if (!ofp) {
    std::cout << "out file plot: " << out_file_plot_name << " could not be opened!\n";
    return false;
  }

  //ofp << "# model name: " << params_.model_name_ << " category: " << model_category_ << std::endl;
  ofp << params_.model_name_ << std::endl;
  ofp << size_t_ << std::endl;
  ...
  ofp << borld_evaluation_ROC_EER(thres_stats) << std::endl;
  //ofp << "# -------------------------------\n";
  ...
  ofp << borld_evaluation_RPC_EER(thres_stats) << std::endl;
  //ofp << "#-------------------------------\n";
