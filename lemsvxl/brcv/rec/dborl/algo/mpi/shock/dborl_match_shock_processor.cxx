...
bool dborl_match_shock_processor::parse_command_line(std::vector<std::string>& argv)
{
...
//: this method is run on each processor
bool dborl_match_shock_processor::initialize(std::vector<dborl_match_shock_processor_input>& t)
{
  ...
  if (params_.db_train_same_ && D1 == D2 && params_.db_dir_.compare(params_.train_dir_) == 0) {
    std::cout << "in db_train_same ON mode - will initialize " << D1 * D2 << " matchings\n";
    ...
    std::cout << "in db_train_same OFF mode - will initialize " << D1 * D2 + D2 * D1 << " matchings\n";
  ...
  std::vector<dbskr_tree_sptr> database_tree, training_set_tree;
  ...
  for (unsigned int i = 0; i<D1; i++) {
    ...
    if (!sg) {
      std::cout << "Problems in getting graphs! "<<database[i]<<std::endl;
      return false;
    } //else
...
{
  //std::cout << "processing: " << i.name1 << " tree n size: " << i.tree1->node_size() << " and " << i.name2 << " tree n size: " << i.tree2->node_size() << std::endl;
  //std::cout.flush();

  std::string out_name = params_.shgm_folder_ + i.name1 + "-" + i.name2 + params_.edit_params_.output_file_postfix_ + ".shgm";
...
bool dborl_match_shock_processor::finalize(std::vector<float>& results)
{

  if (params_.db_train_same_) {
    if (results.size() != D1*D2) {
      std::cout << "result vectors size is: " << results.size() << " should have been: " << D1*D2 << std::endl;
      return false;
    } 
  } else {
    if (results.size() != (D1*D2 + D2*D1)) {
      std::cout << "result vectors size is: " << results.size() << " should have been: " << (D1*D2 + D2*D1) << std::endl;
      return false;
    }
  }

  std::vector<std::vector<float> > shock_database_cost1 ( D1,std::vector<float> ( D2 ));
  for(unsigned int i = 0; i<D1; i++){
    for(unsigned int j = 0; j<D2; j++){
  ...
  if (params_.generate_output_) {
    std::ofstream of((params_.output_file_ + params_.edit_params_.output_file_postfix_ + ".out").c_str());
    ...
    for(unsigned int i = 0; i<D1; i++){
      ...
      of << training_set[bj] << " " << shock_database_cost1[i][bj] << std::endl;
    }
