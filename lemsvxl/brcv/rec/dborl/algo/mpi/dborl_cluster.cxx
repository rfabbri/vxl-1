...
void helper_tokenizer(const std::string& str,
...
bool dborl_cluster<T, P, R>::mpi_initialize(int argc, char *argv[])
{
 ...
 // processor name
  ...
  //std::cout << "Hello World! I am " << name << " " << my_rank_ << " of " << total_processors_ << " size before Bcast: " << size << std::endl;

  //: lead processor should parse the input and update the size
  if (lead_processor()) {
    for (int i = 0; i < argc; i++) {
      dummy += std::string(argv[i]) + " ";
    }
    size = dummy.size();
    //std::cout << "lead processor will send: " << dummy << std::endl;
  }
  ...
  //std::cout << "Hello World! I am " << name << " size after Bcast: " << size << " " << std::endl;
  ...
  buffer[size] = '\0';
  //std::cout << "Hello World! I am " << name << " buffer before Bcast: " << buffer << " " << std::endl;
...
#endif
  //std::cout << "Hello World! I am " << name << " buffer after Bcast: " << buffer << " " << std::endl;

  std::vector<std::string> tokens;
  dummy = buffer; // put buffer into dummy in all processors
  helper_tokenizer(dummy, tokens);
  //std::cout << "Hello World! I am " << name << " tokenizer found: " << tokens.size() << " tokens: \n";
  ...
  if (!p_->parse_command_line(tokens)) {
    std::cout << "cannot parse command line!\n";
    return false;  // --> to exit
  }

  //std::cout << "param_file: " << p_->param_file_ << std::endl;
  if (!p_->parse(p_->param_file_.c_str())) { // param_file_ is set by the previous process
    std::cout << "cannot parse parameter file: " << p_->param_file_ << std::endl;
    return false;  // --> to exit
  }
...
bool dborl_cluster<T, P, R>::distribute(std::vector<T>& t)
{
  ...
  if (!MPI::Is_initialized ()) {
    ...
    std::cout << "MPI must be initialized in main\n";
  }
#else
    {
      int inited = 0;
...
//#endif
 ...
 std::cout << "Hello World! I am " << name << " " << my_rank_ << " of " << total_processors_ << " input size: " << cnt_t << " ";
 ...
 std::cout << "start point: " << my_start_point << " # of points: " << my_number_points << std::endl;
 ...
 if (lead_processor())
 {
    ...
    for (int rank = 0; rank < total_processors_; rank++)
    {
        ...
        std::cout << "Process: " << rank << ", offset: " << point_offsets_[rank] << ", count: " << point_counts_[rank] << "\n";
    }
  }
  ...
  if (lead_processor()) std::cout << "Gather start \n";
...
#endif  // MPI_CPP_BINDING
    if (lead_processor()) std::cout << "Gather end \n";
