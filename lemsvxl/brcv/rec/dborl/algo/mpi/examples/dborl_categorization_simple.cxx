...
bool dborl_categorization_simple::parse_command_line(std::vector<std::string>& argv)
{
...
bool dborl_categorization_simple::parse_index(std::string index_file)
{
  ...
  if (xmlFile == NULL){
    std::cout << "dborl_categorization_simple::parse_index() -- " << index_file << "-- error on opening" << std::endl;
    return false;
  }
...
bool dborl_categorization_simple::parse(const char* param_file)
{
  ...
  if (!vul_file::exists(input_file)) {
    std::cout << "dborl_categorization_simple::parse() -- cannot find the input file: " << input_file << "\n";
    return false;
  }
  ...
  if (params_->use_shock_matching()) {  // load the .esf
    ...
    if (!sg) {
      std::cout << "dborl_categorization_simple::parse() -- Problems in loading shock graph: "<< input_file << std::endl;
      return false;
    } 
    ...
    if (!bsold_load_con_file(input_file.c_str(), points, is_closed) || !points.size()) {
      std::cout << "dborl_categorization_simple::process() -- Problems in loading the con file: "<< input_file << std::endl;
      return false;
    }
  ...
  if (!vul_file::exists(obj_groundtruth)) {
    std::cout << "dborl_categorization_simple::parse() -- cannot find ground truth description for input object: " << obj_groundtruth << std::endl;
    return false;
  }
  ...
  if (!object_desc->has_single_category()) {
    std::cout << "dborl_categorization_simple::parse() - WARNING: image description: " << obj_groundtruth << " has more than one category! ambiguity in category comparison! using the first category\n";
  }
  input_gt_category_ = object_desc->get_first_category();

  std::cout << "input category: " << input_gt_category_ << "\n";
...
bool dborl_categorization_simple::initialize(std::vector<std::string>& t)
{
  if (!current_node_)
    return false;
...
bool dborl_categorization_simple::process(std::string t1, float& f)
{
  if (!vul_file::exists(t1)) {
    std::cout << "In dborl_categorization_simple::process() -- the input file: " << t1 << " is not found!\n";
    f = -1;
    return false;
  }
  ...
  if (params_->use_shock_matching()) {
    ...
    if (!sg) {
      std::cout << "dborl_categorization_simple::process() -- Problems in loading shock graph: "<< t1 << std::endl;
      return false;
    } 
    ...
    if (!vul_file::exists(out_name1)) {
      ...
      if (!edit.edit()) {
        std::cout << "dborl_categorization_simple::process() -- Problems in editing trees for: " << out_name1 << "\n";
        return false;
      }
      f = edit.final_cost();
    ...
    if (!bsold_load_con_file(t1.c_str(), points, is_closed) || !points.size()) {
      std::cout << "dborl_categorization_simple::process() -- Problems in loading the con file: "<< t1 << std::endl;
      return false;
    }
...
bool dborl_categorization_simple::finalize(std::vector<float>& results)
{
  ...
  for (unsigned i = 0; i < current_node_->paths().size(); i++) {
    std::string obj_groundtruth = current_node_->paths()[i] + "/" + current_node_->names()[i] + ".xml";
    dborl_image_desc_parser parser;
    borld_image_description_sptr object_desc = borld_image_description_parse(obj_groundtruth, parser);
    //: add all the categories in this description
    for (std::map<std::string, int>::iterator iter = object_desc->category_list_.begin(); iter != object_desc->category_list_.end(); iter++) 
      category_statistics[iter->first] = 0;   // std::map makes sure that each category appears only once, even if this statement called more than once for the same category
  }

  category_statistics[input_gt_category_] = 0;  // make sure that its inserted as well
  std::cout << "there are: " <<  category_statistics.size() << " categories encountered in this database, or in the input\n";

  //: now we declared an instance from current_category_ --> update the statistics for this category
  std::map<std::string, buld_exp_stat_sptr>::iterator it = category_statistics.find(current_category_);
  it->second = new buld_exp_stat();

  std::map<std::string, buld_exp_stat_sptr>::iterator it_gt = category_statistics.find(input_gt_category_);
  ...
  std::cout << "will write evaluation output to: " << params_->evaluation_file() << std::endl;
  ...
  std::string output_file = params_->categorization_file();
  std::cout << "will write categorization output to: " << params_->categorization_file() << std::endl;
  std::ofstream of(output_file.c_str());
  if (!of) {
    std::cout << "dborl_categorization_simple::finalize() - not able to write output to: " << output_file << std::endl;
  } else {
