...
bool dborl_categorization::parse_command_line(std::vector<std::string>& argv)
{
...
bool dborl_categorization::parse_index(std::string index_file)
{
  ...
  if (xmlFile == NULL){
    std::cout << "dborl_categorization::parse_index() -- " << index_file << "-- error on opening" << std::endl;
    return false;
  }
...
//: this method is run on each processor
bool dborl_categorization::parse(const char* param_file)
{
  ...
  std::string input_file = params_->input_object_path() + "/" + params_->input_object_name() + params_->input_extension();
  if (!vul_file::exists(input_file)) {
    std::cout << "dborl_categorization::parse() -- cannot find the input file: " << input_file << "\n";
    return false;
  }
  ...
  if (params_->use_shock_matching()) {  // load the .esf
    ...
    if (!sg) {
      std::cout << "dborl_categorization::parse() -- Problems in loading shock graph: "<< input_file << std::endl;
      return false;
    } 
    ...
    if (!bsold_load_con_file(input_file.c_str(), points, is_closed) || !points.size()) {
      std::cout << "dborl_categorization::process() -- Problems in loading the con file: "<< input_file << std::endl;
      return false;
    }
    input_poly_ = new vsol_polygon_2d(points);
    std::vector<vsol_point_2d_sptr> flipped_points;
  ...
  std::string obj_groundtruth = params_->input_object_path() + "/" + params_->input_object_name() + ".xml";
  if (!vul_file::exists(obj_groundtruth)) {
    std::cout << "dborl_categorization::parse() -- cannot find ground truth description for input object: " << obj_groundtruth << std::endl;
    return false;
  }
  ...
  if (!object_desc->has_single_category()) {
    std::cout << "dborl_categorization::parse() - WARNING: image description: " << obj_groundtruth << " has more than one category! ambiguity in category comparison! using the first category\n";
  }
...
void dborl_categorization::print_default_file(const char* def_file)
{
...
bool dborl_categorization::initialize(std::vector<std::string>& t)
{
  if (!current_node_)
    return false;
  ...
  if (current_node_->names().size() != current_node_->paths().size()) {
    ...
    current_node_->write_xml(std::cout);
    return false;
  }

  for (unsigned i = 0; i < current_node_->names().size(); i++) {
    std::string input_name = current_node_->paths()[i] + "/" + current_node_->names()[i] + params_->input_extension();
    t.push_back(input_name);
  }
  std::cout << "initialized the node: " << current_node_->name_  << " \n";
  return true;
}

//: this method is run in a distributed mode on each processor on the cluster
//  load the image and trace contour
bool dborl_categorization::process(std::string t1, float& f)
{
  if (!vul_file::exists(t1)) {
    std::cout << "In dborl_categorization::process() -- the input file: " << t1 << " is not found!\n";
    f = -1;
    return false;
  }

  std::string t1_name = vul_file::strip_extension(t1);
  t1_name = vul_file::strip_directory(t1_name);

  if (params_->check_for_same_object()) {
   if (t1_name.compare(params_->input_object_name()) == 0) {
     std::cout << "skipping : " << t1_name << " same as the input object!\n";
  ...
  if (params_->use_shock_matching()) {
    ...
    if (!sg) {
      std::cout << "dborl_categorization::process() -- Problems in loading shock graph: "<< t1 << std::endl;
      return false;
    } 
    ...
    std::string out_name1 = params_->output_directory() + params_->input_object_name() + "-" + t1_name + ".shgm";
    std::string out_name2 = params_->output_directory() + t1_name + "-" + params_->input_object_name() + ".shgm";
    ...
    if (!vul_file::exists(out_name1)) {
      ...
      if (!edit.edit()) {
        std::cout << "dborl_categorization::process() -- Problems in editing trees for: " << out_name1 << "\n";
        return false;
      }
      f = edit.final_cost();
    ...
    if (!bsold_load_con_file(t1.c_str(), points, is_closed) || !points.size()) {
      std::cout << "dborl_categorization::process() -- Problems in loading the con file: "<< t1 << std::endl;
      return false;
    }
...
bool dborl_categorization::finalize(std::vector<float>& results)
{
  ...
  std::string obj_groundtruth = current_node_->paths()[min_i] + "/" + current_node_->names()[min_i] + ".xml";
  if (!vul_file::exists(obj_groundtruth)) {
    std::cout << "dborl_categorization::finalize() -- cannot find ground truth description: " << obj_groundtruth << std::endl;
    return false;
  }
  ...
  borld_image_description_sptr object_desc = borld_image_description_parse(obj_groundtruth, parser);
  if (!object_desc->has_single_category()) {
    std::cout << "dborl_categorization::finalize() - WARNING: image description: " << obj_groundtruth << " has more than one category! not a good model! using the first category\n";
  }
  current_category_ = object_desc->get_first_category();
  //: write the current category 
  //  this will be overwritten if index file has more nodes to be processed
  std::string output_file = params_->output_directory() + "/categorization.xml";
  std::ofstream of(output_file.c_str());
  if (!of) {
    std::cout << "dborl_categorization::finalize() - not able to write output to: " << output_file << std::endl;
  } else {
    of << "<type name = \"image\">\n";
    of << "\t<description>\n";
    of << "\t\t<instance>\n";
  ...
  // now the nearest neighbor at this node is found, if there are more nodes in the index file find the next node
  std::vector<dborl_index_node_base_sptr> children;
  if (!ind_->get_children(current_node_->cast_to_index_node_base(), children) || !children.size()) {
    current_node_ = 0;
    return true;
  }

  // replace the current node with the child that has the same name as the nearest object name
  std::string nearest_object_name = current_node_->names()[min_i];
  current_node_ = 0;
  for (unsigned i = 0; i < children.size(); i++) {
    if (children[i]->is_leaf())
      continue;
    if (children[i]->name_.compare(nearest_object_name) == 0) {
      current_node_ = children[i]->cast_to_index_node();
      break;
    }
  }
...
MPI::Datatype dborl_categorization::create_datatype_for_R()
{
  return MPI::FLOAT;
}
#else
MPI_Datatype dborl_categorization::create_datatype_for_R()
{
  return MPI_FLOAT;
}
#endif
