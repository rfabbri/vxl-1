...
bool dborl_categorization_evaluate::parse_index(std::string index_file)
{
  ...
  if (xmlFile == NULL){
    std::cout << "dborl_categorization_evaluate::parse_index() -- " << index_file << "-- error on opening" << std::endl;
    return false;
  }
...
bool dborl_categorization_evaluate::process()
{
  ...
  for (unsigned i = 0; i < root->paths().size(); i++) {
    std::string cat_file = root->paths()[i] + "categorization.xml";

     bxml_document param_doc = bxml_read(cat_file);
     if (!param_doc.root_element()) {
        std::cout << "dborl_categorization_evaluate::process() -- problems in reading: " << cat_file << std::endl;
        return false;
     }
     ...
     if (!result) {
        std::cout << "dbskr_match_shock_params::parse_from_match_data() - could not find the node category\n";
        return false;
     }
     ...
     std::string category = data_text->data();

     bxml_element query2("gt_category");
     bxml_data_sptr result2 = bxml_find_by_name(param_doc.root_element(), query2);

     if (!result2) {
        std::cout << "dbskr_match_shock_params::parse_from_match_data() - could not find the node input_category\n";
        return false;
     }
     ...
     std::string gt_category = data2_text->data();

     //: find the exp stat counter for this category 
     std::map<std::string, buld_exp_stat_sptr>::iterator cat_iter = category_statistics.find(category);
  ...
  for (std::map<std::string, buld_exp_stat_sptr>::iterator temp_iter = category_statistics.begin(); temp_iter != category_statistics.end(); temp_iter++) {
    int neg_cnt = 0;
    for (std::map<std::string, buld_exp_stat_sptr>::iterator temp_iter2 = category_statistics.begin(); temp_iter2 != category_statistics.end(); temp_iter2++) {
  ...
  for (std::map<std::string, buld_exp_stat_sptr>::iterator temp_iter = category_statistics.begin(); temp_iter != category_statistics.end(); temp_iter++) {
    ...
    std::cout << "category: " << temp_iter->first << " neg cnt: " << current_stat->negative_cnt_ << " pos cnt: " << current_stat->positive_cnt_;
    std::cout << " TPs: " << current_stat->TP_ << " FP: " << current_stat->FP_ << "\n";
