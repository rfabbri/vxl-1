...
int main(int argc, char *argv[]) {
    ...
    if ( params->prune_contours_logistic_())
    {
        ...
        // Before we start the process lets clean input output
        pcl_pro.clear_input();
        pcl_pro.clear_output();
        
        // Use input from vsol object
        pcl_pro.add_input(sel_results[0]);
        pcl_pro.add_input(inp);
        
        bool pcl_status = pcl_pro.execute();
        pcl_pro.finish();
        ...
        if ( pcl_status )
        {
            pcl_results = pcl_pro.get_output();
        }
    ...
    if ( !params->trace_contours_())
    {
        ...
        if ( params->prune_contours_logistic_())
        {
            ec_pro.add_input(pcl_results[0]);
        }
        else
        {
