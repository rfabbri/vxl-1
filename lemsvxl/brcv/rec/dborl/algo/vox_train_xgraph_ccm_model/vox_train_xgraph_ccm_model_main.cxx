...
int main(int argc, char *argv[]) {
    ...
    std::string output_dir = params->output_ccm_dir_();
    std::string pos_output_model_file = output_dir + "/object_ccm_pos_model.xml";
    std::string neg_output_model_file = output_dir + "/object_ccm_neg_model.xml";
    std::string overall_model_file = output_dir + "/object_ccm_overall_model.xml";

    std::cout << "Output pos ccm model file: " << pos_output_model_file << std::endl;
    std::cout << "Output neg ccm model file: " << neg_output_model_file << std::endl;
    ...
    if(!pro_status)
    {
        std::cerr << "Problem occurred while training ccm model!" << std::endl;
        return 1;
    }

    //THIS PART WILL BE CHANGED.
    vox_extract_embedded_ccm_files(params->matlab_file_extraction_path_().c_str());
    std::string matlab_command = "matlab -nodesktop -nosplash -r \"addpath " + params->matlab_file_extraction_path_();
    ...
    std::cout << "MATLAB command: " << matlab_command << std::endl;
    std::system(matlab_command.c_str());
    //
