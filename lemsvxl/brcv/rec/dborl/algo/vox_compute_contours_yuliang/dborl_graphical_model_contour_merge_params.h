...
#if !defined(_dborl_graphical_model_contour_merge_params_h)
#define _dborl_graphical_model_contour_merge_params_h
...
class dborl_graphical_model_contour_merge_params : public dborl_algo_params
{
public:
  ...
  dborl_parameter<std::string> input_cem_suffix_;

  dborl_parameter<std::string> output_extension_;
  dborl_parameter<std::string> output_cem_suffix_;
  dborl_parameter<std::string> output_cem_folder_;  // if written to this folder as opposed to object folder then the edge map gets associated to the input object
                                                    // if nothing is written here, nothing gets associated

  std::string algo_abbreviation_;

  dborl_graphical_model_contour_merge_params(std::string algo_name) : dborl_algo_params(algo_name) { 

    algo_abbreviation_ = "gmcm";
    ...
    output_cem_suffix_.set_values(param_list_, "io", "output_cem_suffix", "output .cem suffix", "_merge", "_merge");
    ...
    dbdet_graphical_model_contour_merge_process pro;
    std::vector<bpro1_param*> pars = pro.parameters()->get_param_list();
