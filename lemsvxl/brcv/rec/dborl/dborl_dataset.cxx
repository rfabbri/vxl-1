...
void dborl_dataset::add_objects(std::vector<dborl_object_base_sptr>& objs)
{
  objects_.insert(objects_.end(), objs.begin(), objs.end());
}
void dborl_dataset::get_category_cnts(std::map<std::string, int>& cnts)
{
  if (!objects_.size())
    return;
  ...
  std::map<std::string, int>& tmp = objects_[0]->desc_->category_list_;
  for (std::map<std::string, int>::iterator i = tmp.begin(); i != tmp.end(); i++) {
    cnts[i->first] = i->second;
  }

  for (unsigned i = 1; i < objects_.size(); i++) {
    std::map<std::string, int>& tmp = objects_[i]->desc_->category_list_;
    for (std::map<std::string, int>::iterator it = tmp.begin(); it != tmp.end(); it++) {
      std::map<std::string, int>::iterator cnts_iter = cnts.find(it->first);
...
void dborl_dataset::get_category_assignments(std::map<std::string, std::vector<dborl_object_base_sptr> >& assign)
{
  assign.clear();
  ...
  for (unsigned i = 0; i < objects_.size(); i++) {
    ...
    for (std::map<std::string, int>::iterator it = tmp.begin(); it != tmp.end(); it++) {
      if (it->second > 0) {
        std::map<std::string, std::vector<dborl_object_base_sptr> >::iterator assign_iter = assign.find(it->first);
        if (assign_iter != assign.end())
          assign_iter->second.push_back(objects_[i]);
        else {
          std::vector<dborl_object_base_sptr> tmp(1, objects_[i]);
          assign[it->first] = tmp;
        }
      }
    }
...
dborl_index_sptr dborl_dataset::create_flat_index(std::string base_dir)
{
