  ...
  if (iter == dart_path_scurve_map_.end()) {  // not found yet
    ...
    if (construct_circular_ends)
      //: note: even if interpolate_ds_ is not properly set by the user std::min takes care of it
        sc = dbskr_compute_scurve(start_node, edges, leaf_[end_dart], true, true, std::min((float)scurve_sample_ds_, interpolate_ds_), scurve_sample_ds_,scale_ratio_);
    else
        sc = dbskr_compute_scurve(start_node, edges, false, true, true, std::min((float)scurve_sample_ds_, interpolate_ds_), scurve_sample_ds_,scale_ratio_);
...
bool dbskr_tree::create_shg(std::string fname)
{
  //create a file
...
bool dbskr_tree::create_shgesg(std::string fname)
{
  //create a file
  std::ofstream outf(fname.c_str(), std::ios::out);
  if (!outf){
