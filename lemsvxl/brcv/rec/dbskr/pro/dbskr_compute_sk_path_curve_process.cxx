...
dbskr_compute_sk_path_curve_process::dbskr_compute_sk_path_curve_process()
{
  if (!parameters()->add( "compute reconst boundary polygon?" , "-reconst_poly" , (bool) true ) ||
      ...
      !parameters()->add( "Path edge list" , "-sh_path" , std::string("164 165 113 114 107 106 96 97 73 72 66 67 38 39") ) ||
...
std::string dbskr_compute_sk_path_curve_process::name()
{
  return "Compute Shock-path Curve";
}

std::vector< std::string > dbskr_compute_sk_path_curve_process::get_input_type()
{
...
std::vector< std::string > dbskr_compute_sk_path_curve_process::get_output_type()
{
  std::vector< std::string > to_return;
...
dbskr_compute_sk_path_curve_process::execute()
{
  ...
  } else if (sg)
  {
    ...
    if (b_coords)
    {
      ...
      for (int r=0; r<=NR; r++){
        std::vector< vsol_point_2d_sptr > plus_pts, minus_pts;
    ...
    else if (pts_only) 
    {
      ...
      std::vector< vsol_spatial_object_2d_sptr > sh_pts, bdry_plus_pts, bdry_minus_pts;
      ...
      // for debugging fragment_pt
      std::vector< vsol_spatial_object_2d_sptr > plus_pts, minus_pts;
      for (int i=0; i<sk_path->num_points(); i++){
      ...
      std::vector< vsol_point_2d_sptr > sh_pts, bdry_plus_pts, bdry_minus_pts;
      for (int i=0; i<sk_path->num_points(); i++){
        sh_pts.push_back(new vsol_point_2d(sk_path->sh_pt(i)));
        bdry_plus_pts.push_back(new vsol_point_2d(sk_path->bdry_plus_pt(i)));
        bdry_minus_pts.push_back(new vsol_point_2d(sk_path->bdry_minus_pt(i)));
...
std::vector<int> 
dbskr_compute_sk_path_curve_process::parseLineForNumbers(std::string s)
{
  std::vector <int> vec;
                        ...
                        int start_node_id, std::string sh_path)
{
  ...
  std::vector<int> edge_ids = parseLineForNumbers(sh_path);
...
{
  std::vector< Point > sh_pts;
  std::vector<double> time, alpha, theta;
  std::vector <int> shock_dir;
  std::vector<double> velocity, acceleration, curvature;
    ...
    if (cur_start_node == cur_edge->source())
    {
      ...
      for (int i=0; i< cur_edge->num_samples(); i++)
      {
        ...
        if (sample->speed != 0 && sample->speed < 99990) //100000 signals infinity
          alpha.push_back(std::asin(1/sample->speed));
        else
          alpha.push_back(0.0);
  ...
  std::vector< vgl_point_2d<double> > intp_bdry_plus, intp_bdry_minus;
  std::vector<double> intp_time, intp_alpha, intp_theta;
  std::vector <int> intp_shock_dir;
...
{
  std::cout << "Comparing Curves point-by-point...\n";
  std::cout << "Curves have same # of points ? " << (c1->num_points() == c2->num_points()) << std::endl;
  ...
  for (int i=0; i < c1->num_points(); i++)
  {
    ...
    if (vgl_distance(c1->sh_pt(i), c2->sh_pt(i))>0.01){
      std::cout << "Sh pt["<<i<<"]: (" << c1->sh_pt(i).x() << ", " << c1->sh_pt(i).y() << ")";
      std::cout << "(" << c2->sh_pt(i).x() << ", " << c2->sh_pt(i).y() << ")" << std::endl;
    }

    if (vgl_distance(c1->bdry_plus_pt(i), c2->bdry_plus_pt(i))>0.01){
      std::cout << "B+ pt["<<i<<"]: (" << c1->bdry_plus_pt(i).x() << ", " << c1->bdry_plus_pt(i).y() << ")";
      std::cout << "(" << c2->bdry_plus_pt(i).x() << ", " << c2->bdry_plus_pt(i).y() << ")" << std::endl;
    }
    
    if (vgl_distance(c1->bdry_minus_pt(i), c2->bdry_minus_pt(i))>0.01){
      std::cout << "B- pt["<<i<<"]: (" << c1->bdry_minus_pt(i).x() << ", " << c1->bdry_minus_pt(i).y() << ")";
      std::cout << "(" << c2->bdry_minus_pt(i).x() << ", " << c2->bdry_minus_pt(i).y() << ")" << std::endl;
    }

    if (std::fabs(c1->time(i)-c2->time(i))>0.01)
      std::cout << "Radius["<<i<<"]: " << c1->time(i) << ", " << c2->time(i) << std::endl;

    if (std::fabs(c1->theta(i)-c2->theta(i))>0.01)
      std::cout << "theta["<<i<<"]: " << c1->theta(i) << ", " << c2->theta(i) << std::endl;
    
    if (std::fabs(c1->alpha(i)-c2->alpha(i))>0.01)
      std::cout << "alpha["<<i<<"]: " << c1->alpha(i) << ", " << c2->alpha(i) << std::endl;
    
    if (c1->shock_dir(i)!=c2->shock_dir(i))
      std::cout << "S-dir["<<i<<"]: " << c1->shock_dir(i) << ", " << c2->shock_dir(i) << std::endl;
  }
