// This is algo/bapl/dbkpr_lowe_cluster.cxx
//:
// \file

#include "dbkpr_lowe_cluster.h"
#include <cmath>
#include <algorithm>
...
//: Constructor
dbkpr_lowe_clusterer::dbkpr_lowe_clusterer( int max_image_dim1, 
                                          int max_image_dim2, 
                                          double max_scale )
{
...
void
dbkpr_lowe_clusterer::hash(const dbdet_keypoint_match& match)
{
...
bool sort_by_size( const std::vector< dbdet_keypoint_match > & lhs,
                   const std::vector< dbdet_keypoint_match > & rhs )
{
  return lhs.size() > rhs.size();
}

//: Fill \param clusters with the clusters sorted by cluster size
std::vector< std::vector< dbdet_keypoint_match > >
dbkpr_lowe_clusterer::get_sorted_clusters() const
{
  std::vector< std::vector< dbdet_keypoint_match > > clusters(hash_);
...
bool
dbkpr_lowe_clusterer::estimate_all_affine( std::vector< bapl_affine_transform >& transforms, 
                                          std::vector< dbdet_keypoint_match >& inliers ) const
{
  std::vector< std::vector< dbdet_keypoint_match > > clusters = this->get_sorted_clusters();
  ...
  for(unsigned c=0; c<clusters.size() && clusters[c].size()>2; ++c){
    ...
    std::vector< dbdet_keypoint_match >& matches = clusters[c];
    ...
    matches.clear();
    for( std::vector< dbdet_keypoint_match >::const_iterator itr = all_matches_.begin();
    ...
    for( std::vector< dbdet_keypoint_match >::iterator m_itr = matches.begin();
         m_itr != matches.end();  ++m_itr ){
      bool already_found = false;
      for( std::vector< dbdet_keypoint_match >::iterator i_itr = inliers.begin();
...
bool 
dbkpr_lowe_clusterer::estimate_affine( bapl_affine_transform& transform, 
                                      std::vector< dbdet_keypoint_match >& matches ) const
{
  ...
  while( num_inliers!=matches.size() ){
    ...
    if( !est.weighted_least_squares_fit(p,covar) ){
      ...
      for( std::vector< dbdet_keypoint_match >::iterator itr = matches.begin();
           itr != matches.end();  ++itr ){
...
bool 
dbkpr_lowe_clusterer::is_inlier( const bapl_affine_transform& A, 
                                const dbdet_keypoint_match& match ) const
{
  ...
  double err_s = std::log(s_tgt/s_map)/std::log(2.0);
  ...
  return std::fabs(err_o) < 0.25 && 
         std::fabs(err_s) < 0.5 &&
         std::fabs(err_x) < trans_step_/4.0 && 
         std::fabs(err_y) < trans_step_/4.0;
}
