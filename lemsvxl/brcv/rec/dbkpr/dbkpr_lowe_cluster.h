...
typedef std::pair< dbdet_keypoint_sptr, dbdet_keypoint_sptr > dbdet_keypoint_match;
...
public:
  ...
  const std::vector< dbdet_keypoint_match >& cluster_at(int b) const { return hash_[b]; }

  //: Fill \param clusters with the clusters sorted by cluster size
  std::vector< std::vector< dbdet_keypoint_match > > get_sorted_clusters() const;
...
private:
  ...
  std::vector< std::vector< dbdet_keypoint_match > > hash_;

  //: A vector of all matches in the hash table
  std::vector< dbdet_keypoint_match > all_matches_;
