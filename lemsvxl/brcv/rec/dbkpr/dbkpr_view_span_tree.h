...
class dbkpr_view_span_link
{
 public:
  ...
  dbkpr_view_span_link(int f, int t)
   : from(f), to(t) {}
  //: Constructor
  dbkpr_view_span_link(int f, int t, const std::map<int,int>& m)
...
class dbkpr_view_span_tree : public vbl_ref_count
{
 public:
  ...
  std::vector<std::vector<dbdet_keypoint_sptr> > global_correspondence() const;

  //: Traverse the tree computing a global set of correspondence points
  std::vector<dbdet_keypoint_corr3d_sptr> global_corr3d_points() const;
  
  //: For each node (view) a vector of links to neighbor nodes
  const std::vector<std::vector<dbkpr_view_span_link> > &links() const {return links_; }
  
  //: All the keypoints for each view
  // keypoints()[view_index][point_index]
  const std::vector<std::vector<dbdet_keypoint_sptr> > & keypoints() const { return keypoints_;}
 ...
 private:
  ...
  std::map<int,int> tentative_matches( unsigned int v1, unsigned int v2 ) const;
  ...
  std::vector<bnld_bbf_tree<double,128> > bbf_trees_;

  //: All the keypoints for each view
  // keypoints_[view_index][point_index]
  std::vector<std::vector<dbdet_keypoint_sptr> > keypoints_;

  //: Labels for each frame to determine connected components
  std::vector<int> labels_;

  //: For each node a vector of links to neighbor nodes
  std::vector<std::vector<dbkpr_view_span_link> > links_;
};
