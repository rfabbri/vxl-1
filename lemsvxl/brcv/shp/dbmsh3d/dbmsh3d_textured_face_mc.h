#ifndef _dbmsh3d_textured_face_mc_h_
#define _dbmsh3d_textured_face_mc_h_

//---------------------------------------------------------------------
// This is brcv/shp/dbmsh3d/dbmsh3d_textured_face_mc.h
//:
// \file
// \brief A Multiply Connected Face with an associated texture map.
//        Note that it is assumed the vertices are of type dbmsh3d_textured_vertex
//
//
// \author
//  Daniel Crispell March 14, 2007
//
// \verbatim
//  Modifications
//   <none>
// \endverbatim
//
//-------------------------------------------------------------------------
#include <vcl_map.h>

#include <dbmsh3d/dbmsh3d_face_mc.h>
#include <vgl/vgl_point_2d.h>

class dbmsh3d_textured_face_mc : public dbmsh3d_face_mc
{
protected:
  //: URI of texture map file
  vcl_string tex_map_uri_; 

  //: Should be one tex_coord per face vertex, corresponding to the ifs_face vertices.
  vcl_map<int, vgl_point_2d<double> > tex_coords_;

public:
  //: ====== Constructor/Destructor ======
  dbmsh3d_textured_face_mc (const int id): dbmsh3d_face_mc(id){}

  //: converts an untextured face to a textured face
  dbmsh3d_textured_face_mc (const dbmsh3d_face* face, vcl_string tex_map_uri);

  vcl_string tex_map_uri(){return tex_map_uri_;}
  void set_tex_map_uri(vcl_string uri){tex_map_uri_ = uri;}

  //: copy constructor
  //dbmsh3d_textured_face_mc (const dbmsh3d_textured_face_mc& face);

  virtual ~dbmsh3d_textured_face_mc () {tex_coords_.clear();}

  //: ====== Data access functions ======
  vgl_point_2d<double> tex_coords (const unsigned int i) {
    ///assert (i < vertices_.size());
    return tex_coords_[i];
  }

  vcl_map<int, vgl_point_2d<double> >&  tex_coords() {
    return tex_coords_;
  }

  void set_tex_coord(int id, vgl_point_2d<double> pt) {
    tex_coords_[id] = pt;
  }

  bool get_texture_image (unsigned char*& bitmap, int& width, int& height, int& plane) const;
  
  //: ====== Connectivity Modification Functions ======
  void _add_vertex(dbmsh3d_vertex* vertex) {
    vgl_point_2d<double> tex_coord(0.0, 0.0);
    _add_vertex(vertex,tex_coord);
  }

  void _add_vertex (dbmsh3d_vertex* vertex, vgl_point_2d<double> tex_coord) {
    vertices_.push_back (vertex);
    tex_coords_[vertex->id()] = tex_coord;
  }
  void print();
};

#endif
