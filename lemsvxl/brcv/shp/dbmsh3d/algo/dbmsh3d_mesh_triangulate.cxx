...
//
bool dbmsh3d_triangulate_face (const std::vector<dbmsh3d_vertex*>& vertices, 
                               std::vector<std::vector<int> >& tri_faces)
{
  ...
  for (unsigned int j=0; j<vertices.size(); j++) {
    const dbmsh3d_vertex* V = vertices[j];
  ...
  for (int i=0; i<int(tcount); i++) {
    ...
    ///vul_printf (std::cout, "Triangle %d => (%f, %f) (%f, %f) (%f, %f)\n",
...
{
  dbmsh3d_mesh* triM = new dbmsh3d_mesh;
  ...
  for (; vit != M->vertexmap().end(); vit++) {
    ...
    dbmsh3d_vertex* newV = triM->_new_vertex ();
    newV->set_id (V->id());
  ...
  for (; it != M->facemap().end(); it++) {
    ...
    if (vertices.size() < 4) {
      ...
      dbmsh3d_face* newF = triM->_new_face ();

      for (unsigned int i=0; i<vertices.size(); i++) {
        dbmsh3d_vertex* V = vertices[i];
        int id = V->id();
        dbmsh3d_vertex* newV = triM->vertexmap (id);
      ...
      for (unsigned int i=0; i<tri_faces.size(); i++) {   
        ...
        dbmsh3d_vertex* V0 = triM->vertexmap (tri_faces[i][0]);
        newF->_ifs_add_bnd_V (V0);
        dbmsh3d_vertex* V1 = triM->vertexmap (tri_faces[i][1]);
        newF->_ifs_add_bnd_V (V1);
        dbmsh3d_vertex* V2 = triM->vertexmap (tri_faces[i][2]);
        newF->_ifs_add_bnd_V (V2);
