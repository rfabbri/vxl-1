...
bool dbmsh3d_load_iv_ifs (dbmsh3d_mesh* M, const char* file)
{
  ...
  if ((fp = std::fopen (file, "r")) == NULL) {
    vul_printf (std::cout, "Can't open input IV file %s\n", file);
    return false; 
  }
  ...
  while (1) {
    ...
    while (std::strcmp (buffer, "vertex")!=0 && std::strcmp (buffer, "point")!=0) {
      end_of_file = std::fscanf (fp, "%s", buffer);
      if (end_of_file==EOF)
        goto READ_IV_DONE;
    }

    //search for "["
    std::fscanf (fp, "%s", buffer);
    ...
    while (!this_round_done) {
      ...
      std::fscanf (fp, "%lf %lf %lf%s", &x, &y, &z, buffer);
...
READ_IV_DONE:
  ...
  vul_printf (std::cout, "\t%u points, %u faces read from %s.\n", 
