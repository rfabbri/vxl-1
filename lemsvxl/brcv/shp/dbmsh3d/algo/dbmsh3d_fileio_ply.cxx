//---------------------------------------------------------------------
// This is brcv/shp/dbmsh3d/dbmsh3d_fileio_ply.cxx 
//:
// \file
...
class dbmsh3d_fileio_parsed_mesh_
{
public:
  dbmsh3d_mesh* mesh;
  double p[3];
...
int dbmsh3d_fileio_face_cb_(p_ply_argument argument);
...
bool dbmsh3d_load_ply (dbmsh3d_mesh* M, const char* file)
{
  ...
  dbmsh3d_fileio_parsed_mesh_ parsed_mesh;
  parsed_mesh.mesh = M;
  ...
  nvertices = ply_set_read_cb(ply, "vertex", "x", 
    dbmsh3d_fileio_vertex_cb_, (void*) (&parsed_mesh), 0);
  ply_set_read_cb(ply, "vertex", "y", 
    dbmsh3d_fileio_vertex_cb_, (void*) (&parsed_mesh), 1);
  ply_set_read_cb(ply, "vertex", "z", 
    dbmsh3d_fileio_vertex_cb_, (void*) (&parsed_mesh), 2);
...
bool dbmsh3d_load_ply_f (dbmsh3d_mesh* M, const char* file)
{
  ...
  dbmsh3d_mesh* tmpM = new dbmsh3d_mesh;
  if (dbmsh3d_load_ply (tmpM, file) == false)
    return false;
  ...
  for (; it != tmpM->facemap().end(); it++) {
    ...
    dbmsh3d_face* F = M->_new_face (id);
    for (unsigned int j=0; j<tmpF->vertices().size(); j++) {
      dbmsh3d_vertex* tmpV = tmpF->vertices(j);
      dbmsh3d_vertex* V = M->vertexmap (tmpV->id());
      assert (V);
      F->_ifs_add_bnd_V (V);
      V->set_meshed (true);
    }
...
int dbmsh3d_fileio_vertex_cb_(p_ply_argument argument) 
{
  ...
  dbmsh3d_fileio_parsed_mesh_* parsed_mesh = 
    (dbmsh3d_fileio_parsed_mesh_*) temp;
...
int dbmsh3d_fileio_face_cb_(p_ply_argument argument) 
{
  ...
  {
    dbmsh3d_mesh* M = parsed_mesh->mesh;
...
bool dbmsh3d_save_ply (dbmsh3d_mesh* M, const char* file, bool ascii_mode)
{
  ...
  // comment
  ply_add_comment(oply, "created by dbmsh3d_save_ply");

  // object info
  ply_add_obj_info(oply, "an dbmsh3d_mesh object");
...
// ----------------------------------------------------------------------------
class dbmsh3d_fileio_parsed_richmesh_: public dbmsh3d_fileio_parsed_mesh_
{
public:
  // for use on vertices
...
//: Call-back function for a "vertex" element
int dbmsh3d_fileio_richvertex_cb_(p_ply_argument argument) 
{
  long index;
  void* temp;
  ply_get_argument_user_data(argument, &temp, &index);

  dbmsh3d_fileio_parsed_richmesh_* parsed_mesh = 
    (dbmsh3d_fileio_parsed_richmesh_*) temp;
    ...
    (3 + parsed_mesh->vertex_property_list.size()))
  {
    // INSERT VERTEX INTO THE MESH
    // i. create new
    dbmsh3d_richvertex* V = (dbmsh3d_richvertex*) 
      parsed_mesh->mesh->_new_vertex ();
...
//: Call-back function for a "face" element
int dbmsh3d_fileio_richface_cb_ (p_ply_argument argument) 
{
  long index;
  void* temp;
  ply_get_argument_user_data(argument, &temp, &index);
  ...
  if (parsed_mesh->num_face_values_read == parsed_mesh->face_property_list.size() &&
    ...
    dbmsh3d_richface* F = (dbmsh3d_richface*)(M->_new_face ());
                      ...
                      const vcl_vector<vcl_string >& face_property_list)
{
  ...
  // vertex
  nvertices = ply_set_read_cb(ply, "vertex", "x", 
    dbmsh3d_fileio_richvertex_cb_, (void*) (&parsed_mesh), 0);
  ply_set_read_cb(ply, "vertex", "y", 
    dbmsh3d_fileio_richvertex_cb_, (void*) (&parsed_mesh), 1);
  ply_set_read_cb(ply, "vertex", "z", 
    dbmsh3d_fileio_richvertex_cb_, (void*) (&parsed_mesh), 2);

  for (unsigned int i=0; i<vertex_property_list.size(); ++i) {
    ply_set_read_cb(ply, "vertex", 
      vertex_property_list[i].c_str(), 
      dbmsh3d_fileio_richvertex_cb_, 
                       ...
                       dbmsh3d_storage_mode mode)
{
  ...
  // object info
  ply_add_obj_info(oply, "an dbmsh3d_richmesh object");
  ...
  for (; fit != mesh->facemap().end(); fit++) {
    ...
    dbmsh3d_richface* f = (dbmsh3d_richface*)(*fit).second;
