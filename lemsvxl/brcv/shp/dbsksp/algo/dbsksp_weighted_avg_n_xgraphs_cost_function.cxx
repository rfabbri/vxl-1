// This is file shp/dbsksp/algo/dbsksp_weighted_avg_n_xgraphs_cost_function.cxx

//:
// \file

#include "dbsksp_weighted_avg_n_xgraphs_cost_function.h"
...
dbsksp_weighted_avg_n_xgraphs_one_node_cost_function(
  ...
  const std::vector<std::vector<pathtable_key > >&       dart_corr_parent_to_model,  // correspondence between parent trees and model tree
...
dbsksp_weighted_avg_n_xgraphs_cost_base(parent_tree, parent_weight, model_tree, dart_corr_parent_to_model, scurve_matching_R),
vnl_cost_function(0),
parent_contract_and_splice_cost_(parent_contract_and_splice_cost),
xgraph_model_(xgraph_model), 
active_vid_(active_vid)
{
  //0) Easy things first
  this->num_parents_ = this->parent_tree_.size();
...
void dbsksp_weighted_avg_n_xgraphs_one_node_cost_function::
determine_active_nodes_and_edges()
{
  //0) Allocate space for flags of active darts
  this->active_corr_.resize(this->num_parents_);

  //1) Determine dart paths affected by this node
  std::set<std::pair<int, int > > affected_dart_pairs;
  for (int idx = 0; idx < this->num_parents_; ++idx)
  {
    std::vector<pathtable_key >& corr = this->dart_corr_parent_to_model_[idx];
    ...
    for (unsigned k =0; k < corr.size(); ++k)
    {
      ...
      std::vector<int > dart_path = this->model_tree_->get_dart_path(model_d1, model_d2);

      if (dart_path.empty())
        continue;

      std::vector<dbsksp_xshock_edge_sptr > list_xe;
      ...
      affected_dart_pairs.insert(std::pair<int, int >(model_d1, model_d2));
    }
  }

  //2) Convert dart paths to list shock edges and nodes
  std::set<dbsksp_xshock_edge_sptr > affected_edges;
  std::set<dbsksp_xshock_node_sptr > affected_nodes;
  ...
  for (std::set<std::pair<int, int > >::iterator iter = affected_dart_pairs.begin();
...
{
  this->fixed_deform_cost_.resize(this->num_parents_);

  for (int idx = 0; idx < this->num_parents_; ++idx)
  {
    std::vector<pathtable_key>& corr = this->dart_corr_parent_to_model_[idx];
    std::vector<bool >& active_corr = this->active_corr_[idx];

    this->fixed_deform_cost_[idx] = 0;
    ...
    for (unsigned i =0; i < corr.size(); ++i)
    {
      ...
      this->fixed_deform_cost_[idx] += edit.get_deform_cost(corr[i]);
    }
  }
}
...
//------------------------------------------------------------------------------
//: Main function
double dbsksp_weighted_avg_n_xgraphs_one_node_cost_function::
f(const vnl_vector<double >& x)
{
  // cost components
  vnl_vector<double > fx(this->num_parents_ +1, 0);
  ...
  vnl_vector<double > deform_cost(this->num_parents_, 0);
  for (int idx =0; idx < this->num_parents_; ++idx)
  {
    ...
    deform_cost[idx] += this->fixed_deform_cost_[idx];
  }
  ...
  for (unsigned i =0; i < this->active_edges_.size(); ++i)
  {
    ...
    total_kmax_cost  += dbsksp_compute_kmax_cost(xfrag);
  }
  fx[this->num_parents_] = total_kmax_cost;
  
  return std::sqrt(fx.squared_magnitude() / this->num_parents_);
}
