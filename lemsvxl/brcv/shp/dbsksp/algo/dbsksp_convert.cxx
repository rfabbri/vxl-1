...
dbsksp_xshock_graph_sptr dbsksp_convert_to_xshock_graph(const dbsksp_shock_graph_sptr& graph)
{
  ...
  std::map<dbsksp_shock_node_sptr, dbsksp_xshock_node_sptr > node_map;
  ...
  std::map<dbsksp_shock_edge_sptr, dbsksp_xshock_edge_sptr > edge_map;
  ...
  for (std::map<dbsksp_shock_node_sptr, dbsksp_xshock_node_sptr >::iterator it = 
    node_map.begin(); it != node_map.end(); ++it)
  {
    xgraph->add_vertex(it->second);
  }

  for (std::map<dbsksp_shock_edge_sptr, dbsksp_xshock_edge_sptr >::iterator it = 
    ...
    vit != graph->vertices_end(); ++vit)
  {
      ...
      v->edges_end(); ++eit)
    {
      ...
      xdesc->psi_ = std::atan2(tangent_v.y(), tangent_v.x());
    }
