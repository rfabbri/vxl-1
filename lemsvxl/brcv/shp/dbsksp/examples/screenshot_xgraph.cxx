...
int main(int argc, char *argv[])
{
  if (argc < 4)
  {
    std::cout << "This program draws a xshock graph on top of an existing (source) image\n" 
      << "Usage: " << argv[0] << " <xgraph xml> <source image> <new image>\n";
    return 0;
  }

  std::string xgraph_file = argv[1];
  std::string source_image_file = argv[2];
  std::string target_image_file = argv[3];
  ...
  if (!image_resource)
  {
    std::cout << "ERROR: couldn't load source image - " << source_image_file << std::endl;
    return 1;
  }
  ...
  {
    std::cout << "ERROR: couldn't save image to target image file - " << target_image_file << std::endl;
    return 1;
  }  
  return 0;
}
