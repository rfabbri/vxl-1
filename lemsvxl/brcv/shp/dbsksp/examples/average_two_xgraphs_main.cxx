...
int main(int argc, char *argv[])
{
  ...
  // Input files
  vul_arg<std::string > xgraph1_file(arg_list,"-xgraph1", "Input file for xgraph1", std::string(""));

  vul_arg<std::string > xgraph2_file(arg_list,"-xgraph2", "Input file for xgraph2", std::string(""));

  // Output file
  vul_arg<std::string > output_file(arg_list,"-out", "Output xgraph file", std::string(""));
  ...
  std::string basename = vul_file::strip_extension(output_file());
  if (!output_file.set())
  {
    std::string name1 = vul_file::strip_extension(vul_file::strip_directory(xgraph1_file()));
    std::string name2 = vul_file::strip_extension(vul_file::strip_directory(xgraph2_file()));
    basename = "./" + name1 + "+" + name2;
  }

  std::string average_xgraph_file = basename + ".xml";
  std::string xgraph1_trimmed_file  = basename + "_1.xml";
  std::string xgraph2_trimmed_file  = basename + "_2.xml";
