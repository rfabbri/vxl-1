...
#if GDT_DEBUG_MSG
  if (n_verbose_>1) {
    vul_printf (std::cout, " Validate an entry in Qs2 Wa %d (%.3f, %.3f) and Wb %d (%.3f, %.3f).\n", //0x%x 
...
void gdt_ws_manager::Propagate_welm (gdt_welm* W)
{
  ...
  if (W->next_event() == WENE_NA) {
    #if GDT_DEBUG_MSG
    if (n_verbose_>3) {
      vul_printf (std::cout, "  No more event for psrcW %d (%.3f, %.3f). Remove from Qw.\n",
    ...
    if (n_verbose_>3) {
      ...
      vul_printf (std::cout, "  Propagate ");
      if (W->is_dege())
        vul_printf (std::cout, "dege");
      else if (W->is_psrc())
        vul_printf (std::cout, "psrc");
      vul_printf (std::cout, "W %d (%.3f, %.3f) to %s, simT %f.\n", 
...
void gdt_ws_manager::Advance_welm (gdt_welm* W)
{
  ...
  case WENE_LE_FPT: //W-eL
    ...
    if (W->_has_nextI_on_edge (eL)) {
      #if GDT_DEBUG_MSG
      if (n_verbose_>3)
        vul_printf (std::cout, "  Skip psrcW %d (%.3f, %.3f) to strike eL %d. W already exists.\n", 
      ...
      if (nW) {
        ...
        #if GDT_DEBUG_MSG
        if (n_verbose_>3)
          vul_printf (std::cout, "  Advance psrcW %d (%.3f, %.3f) to strike eL %d. Create nW (%.3f, %.3f).\n", 
                       W->edge()->id(), W->stau(), W->etau(),
  ...
  case WENE_RE_FPT: //W-eR
    ...
    if (W->_has_nextI_on_edge (eR)) {
      #if GDT_DEBUG_MSG
      if (n_verbose_>3)
        vul_printf (std::cout, "  Skip psrcW %d (%.3f, %.3f) to strike eR %d. W already exists.\n", 
      ...
      if (nW) {
        ...
        #if GDT_DEBUG_MSG
        if (n_verbose_>3)
          vul_printf (std::cout, "  Advance psrcW %d (%.3f, %.3f) to strike eR %d. Create nW (%.3f, %.3f).\n", 
                       W->edge()->id(), W->stau(), W->etau(), 
                       nW->edge()->id(), nW->stau(), nW->etau());
        #endif 
      }
  ...
  case WENE_OV:
    ...
    if (W->_psrc_closer_to_Sv()) {
      ...
      if (W->_has_nextI_on_edge(eR)) {
        #if GDT_DEBUG_MSG
        if (n_verbose_>3)
          vul_printf (std::cout, "  Skip psrcW %d (%.3f, %.3f) to strike vO. nextI already exists.\n", 
                       W->edge()->id(), W->stau(), W->etau());
        #endif
      }
    ...
    #if GDT_DEBUG_MSG
    if (nW && n_verbose_>3)
      vul_printf (std::cout, "  Advance psrcW %d (%.3f, %.3f) to strike vO. Create nW %d (%.3f, %.3f).\n", 
  ...
  case WENE_RF_FPT:
    ...
    if (W->validate_RF_FPT ()) { //Validate it first
      ...
      #if GDT_DEBUG_MSG
      if (n_verbose_>3)
        vul_printf (std::cout, "  Advance RF %d (%.3f, %.3f) to FPT, nW created.\n", 
                     W->edge()->id(), W->stau(), W->etau());
      #endif 
    }
    else {
  ...
  case WENE_SV: //W-V strike at SV. Two cases: 1) WES_RF 2) WES_PROP
    ...
    if (W->is_dege()==false && W->_is_RF()) { //1) WES_RF
      ...
      if (W->validate_RF_SPT ()) { //Validate it first
        ...
        #if GDT_DEBUG_MSG
        if (n_verbose_>3)
          vul_printf (std::cout, "  Advance RF %d (%.3f, %.3f) to SV, nW created.\n", 
                       W->edge()->id(), W->stau(), W->etau());
        #endif 
      }
      else {
  ...
  case WENE_EV: //W-V strike at EV. Two cases:
    ...
    if (W->is_dege()==false && W->_is_RF()) { //1) 
      ...
      if (W->validate_RF_EPT ()) { //Validate it first
        ...
        #if GDT_DEBUG_MSG
        if (n_verbose_>3)
          vul_printf (std::cout, "  Advance RF %d (%.3f, %.3f) to EV, nW created.\n", 
                       W->edge()->id(), W->stau(), W->etau());
        #endif 
      }
      else {
  ...
  if (n_verbose_>3 && W->is_final()) {
    if (W->is_dege())
      vul_printf (std::cout, "  Finalize_I dege");
    else if (W->is_psrc())
      vul_printf (std::cout, "  Finalize_I psrc");
    vul_printf (std::cout, "W %d (%.3f, %.3f).\n", W->edge()->id(), W->stau(), W->etau());
  }
  #endif
}
                                            ...
                                            const bool degeV)
{
  if (degeV) {
    #if GDT_DEBUG_MSG
    if (n_verbose_>1)
      vul_printf (std::cout, "  Qv: Skip the dege S-V vertex %d.\n", v->id());
    #endif
    return;
  }
  ...
  if (Wa != NULL && Wa->he()->pair()) {
    ...
    if (Wb != NULL && Wb->he()->pair()) {
      ...
      n_V_bnd_RF_++;
      if (n_verbose_>3)
        vul_printf (std::cout, "  Qv: No next face to create shock from v %d.\n", v->id());
      #endif
    }
  }
}
