...
bool gdt_ws_manager::_vertex_strike_at_Sv (gdt_welm* W)
{
  #if GDT_DEBUG_MSG
  if (n_verbose_>3)
    vul_printf (std::cout, "  Advanve psrcW %d (%.3f, %.3f) to W-Vs(%d) strike.\n", 
  ...
  if (nextF_he) {
    ...
    if (W->_has_nextI_on_edge (eL)) {
      ...
      #if GDT_DEBUG_MSG
      if (n_verbose_>3)
        vul_printf (std::cout, "  Skip psrcW %d (%.3f, %.3f) to strike eL %d. sV already been propagated.\n", 
                     W->edge()->id(), W->stau(), W->etau(), W->sV()->id());
      #endif
    }
    else {
      ...
      if (nW) {
        ...
        #if GDT_DEBUG_MSG
        if (n_verbose_>3)
          vul_printf (std::cout, "  Create psrcW %d (%.3f, %.3f) with simT %f.\n", 
...
bool gdt_ws_manager::_vertex_strike_at_Ev (gdt_welm* W)
{
  #if GDT_DEBUG_MSG
  if (n_verbose_>3)
    vul_printf (std::cout, "  Advanve psrcW %d (%.3f, %.3f) to W-Ve(%d) strike.\n", 
  ...
  if (nextF_he) {
    ...
    if (W->_has_nextI_on_edge(eR)) {
      ...
      #if GDT_DEBUG_MSG
      if (n_verbose_>3)
        vul_printf (std::cout, "  Skip psrcW %d (%.3f, %.3f) to strike eR %d. eV already been propagated.\n", 
                     W->edge()->id(), W->stau(), W->etau(), W->eV()->id());
      #endif
    }
    else {
...
bool gdt_ws_manager::advance_W_to_endvertex (gdt_welm* W, dbmsh3d_gdt_vertex_3d* v)
{
  ...
  if (it == Qv_.end()) {
    ...
    if (va->Wb_ != NULL) {
      #if GDT_DEBUG_MSG
      if (n_verbose_>3)
        vul_printf (std::cout, "  !! Degenerate S-V detected: W %d (%.3f, %.3f) strikes v %d.\n", 
