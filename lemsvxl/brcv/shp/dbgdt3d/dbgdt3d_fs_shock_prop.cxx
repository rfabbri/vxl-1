...
void gdt_shock_prop::propagate_gdt_shocks ()
{
  ...
  while (shock_queue_.size() != 0) {
    if (n_verbose_>1)
      vul_printf (std::cout, "           s_iter %d \n", n_prop_iter);
    propagate_gdt_shock_one_step ();
    n_prop_iter++;
  }
  vul_printf (std::cout, "     ----> Total s_iter %d.\n", n_prop_iter);
#else
  while (shock_queue_.size() != 0)
    propagate_gdt_shock_one_step ();
#endif
