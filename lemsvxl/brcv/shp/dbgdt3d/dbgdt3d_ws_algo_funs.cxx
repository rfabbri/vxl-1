...
void gdt_ws_manager::add_to_Qw (gdt_welm* W) 
{
  #if GDT_DEBUG_MSG
  if (n_verbose_>4) {
    vul_printf (std::cout, "      Add W simtime = %lf on edge %d to the front Qw.\n", 
                 W->simT(), W->edge()->id());
  }
...
void gdt_ws_manager::add_to_Qv (gdt_active_vertex* va) 
{
  ...
  if (n_verbose_>2) {
    ...
    vul_printf (std::cout, "    Add vertex %d (%lf) with <%s, %s> to Qv.\n", 
...
void gdt_ws_manager::add_to_Qs (gdt_shock* S) 
{
  ...
  if (n_verbose_>2) {
    ...
    vul_printf (std::cout, "    Add S %d (v %d prjE %s) simtime=%f (Ia %s, Ib %s) to Qs.\n", 
...
void gdt_ws_manager::print_Qw ()
{
  ...
  for (unsigned int i=0; it != Qw_.end(); it++, i++) {
    ...
    gdt_welm* W = (*it).second;
    vul_printf (std::cout, "<%f, (%f, %f) on %d> ", 
...
{
  vul_printf (std::cout, "Qs_: %d\n", (int) Qs_.size());

  std::multimap<double, gdt_shock*>::iterator it = Qs_.begin();
  for (unsigned int i=0; it != Qs_.end(); it++, i++) {
    double simtime = (*it).first;
    gdt_shock* S = (*it).second;
    vul_printf (std::cout, "<%f, %d> ", simtime, S->id());
    if (i%4==3) 
      vul_printf (std::cout, "\n");
  }
  vul_printf (std::cout, "\n\n");
