...
dbsk2d_xshock_graph_fileio::load_xshock_graph(std::string filename)
{
...
void dbsk2d_xshock_graph_fileio::load_xshock_header()
{
  ...
  while (!end_of_header)
  {
    ...
    if (!std::strncmp(buffer, "Extrinsic Shock File v1.0", sizeof("Extrinsic Shock File v1.0")-1))
      version = 1;

    //else if (!std::strncmp(buffer, "Begin [GENERAL INFO]", sizeof("Begin [GENERAL INFO]")-1))

    else if (!std::strncmp(buffer, "Number of Nodes: ", sizeof("Number of Nodes: ")-1))
      sscanf(buffer,"Number of Nodes: %d", &(num_nodes));

    else if (!std::strncmp(buffer, "Number of Edges: ", sizeof("Number of Edges: ")-1))
      sscanf(buffer,"Number of Edges: %d", &(num_edges));

    else if (!std::strncmp(buffer, "Number of Sample Points: ", sizeof("Number of Sample Points: ")-1))
      sscanf(buffer,"Number of Sample Points: %d", &(num_samples));

    else if (!std::strncmp(buffer, "End [GENERAL INFO]", sizeof("End [GENERAL INFO]")-1))
      end_of_header = true;
  }
}
...
void dbsk2d_xshock_graph_fileio::load_xshock_node_description()
{
  ...
  while (!end_of_node_description)
  {
    ...
    if (!std::strncmp(buffer, "Begin [NODE DESCRIPTION]", sizeof("Begin [NODE DESCRIPTION]")-1)){
      //read next line
      ...
      for (int i=0; i< num_nodes; i++)
      {
        ...
        std::vector<std::pair<int, int> > adjacency_edge_list;
        assert(adjacency_list.size() == edge_list.size());

        for (unsigned ii = 0; ii < adjacency_list.size(); ii++) {
          std::pair<int, int> p(adjacency_list[ii], edge_list[ii]); 
          adjacency_edge_list.push_back(p);
        }

        //node_adjacency_map.insert(std::pair<int, std::vector<int> >(node_id, adjacency_list));
        node_adjacency_map.insert(std::pair<int, std::vector<std::pair<int, int> > >(node_id, adjacency_edge_list));
    ...
    else if (!std::strncmp(buffer, "End [NODE DESCRIPTION]", sizeof("End [NODE DESCRIPTION]")-1))
...
void dbsk2d_xshock_graph_fileio::load_xshock_node_samples()
{
  ...
  while (!end_of_node_samples)
  {
    ...
    if (!std::strncmp(buffer, "Begin [NODE SAMPLE POINTS]", sizeof("Begin [NODE SAMPLE POINTS]")-1))
    ...
    else if (!std::strncmp(buffer, "Begin SAMPLE", sizeof("Begin SAMPLE")-1))
    {
      dbsk2d_xshock_sample_sptr new_sample = load_xshock_sample();
      samples_map.insert(std::pair<int, dbsk2d_xshock_sample_sptr>(new_sample->id, new_sample));
    }

    else if (!std::strncmp(buffer, "End [NODE SAMPLE POINTS]", sizeof("End [NODE SAMPLE POINTS]")-1))
...
dbsk2d_xshock_graph_fileio::load_xshock_sample()
{
  ...
  while (!end_of_sample)
  {
    ...
    //read the sample_id
    if (!std::strncmp(buffer, "sample_id", sizeof("sample_id")-1))
      sscanf(buffer,"sample_id %d", &(sample_id));

    //read the extrinsic coordinates
    else if (!std::strncmp(buffer, "(x, y, t)", sizeof("(x, y, t)")-1))
      sscanf(buffer,"(x, y, t) (%f, %f, %f)", &(x), &(y), &(t));

    //read the edge_id
    else if (!std::strncmp(buffer, "edge_id", sizeof("edge_id")-1))
      sscanf(buffer,"edge_id %d", &(edge_id));

    //read the label
    else if (!std::strncmp(buffer, "label", sizeof("label")-1))
      sscanf(buffer,"label %s", label);

    //read the sample_type
    else if (!std::strncmp(buffer, "type", sizeof("type")-1))
      sscanf(buffer,"type %c", &(sample_type));

    //read the theta parameter
    else if (!std::strncmp(buffer, "theta", sizeof("theta")-1))
      sscanf(buffer,"theta %f", &(theta));

    //read the speed parameter
    else if (!std::strncmp(buffer, "speed", sizeof("speed")-1))
      sscanf(buffer,"speed %f", &(speed));

    //read the boundary points
    else if (!std::strncmp(buffer, "boundaryPoints", sizeof("boundaryPoints")-1))
      sscanf(buffer,"boundaryPoints [(%f, %f), (%f, %f)]", 
      &(lbndx), &(lbndy), &(rbndx), &(rbndy));

    //read the boundary tangents
    else if (!std::strncmp(buffer, "boundaryTangents", sizeof("boundaryTangents")-1))
      sscanf(buffer,"boundaryTangents [%f, %f]", &(ltheta), &(rtheta));

    //end of sample marker
    else if (!std::strncmp(buffer, "End SAMPLE", sizeof("End SAMPLE")-1))
      end_of_sample = true;
...
void dbsk2d_xshock_graph_fileio::load_xshock_edge_description()
{
  ...
  while (!end_of_edge_description)
  {
    ...
    if (!std::strncmp(buffer, "Begin [EDGE DESCRIPTION]", sizeof("Begin [EDGE DESCRIPTION]")-1)){
      //read next line
      fp_in.getline(buffer,2000); //this is the comment line 
      ...
      for (int i=0; i< num_edges; i++)
      {
        ...
        edge_samples_map.insert(std::pair<dbsk2d_shock_edge_sptr, std::vector<int> >(new_edge, samples_list));
      }
    }

    else if (!std::strncmp(buffer, "End [EDGE DESCRIPTION]", sizeof("End [EDGE DESCRIPTION]")-1))
...
void dbsk2d_xshock_graph_fileio::load_xshock_edge_samples()
{
  ...
  while (!end_of_edge_samples)
  {
    ...
    if (!std::strncmp(buffer, "Begin [EDGE SAMPLE POINTS]", sizeof("Begin [EDGE SAMPLE POINTS]")-1))
    ...
    else if (!std::strncmp(buffer, "End [EDGE SAMPLE POINTS]", sizeof("End [EDGE SAMPLE POINTS]")-1))
...
void dbsk2d_xshock_graph_fileio::setup_connectivity_between_nodes_and_edges()
{
  ...
  for ( ; v_it != shock->vertices_end(); ++v_it)
  {
    ...
    std::vector<std::pair<int, int> > adjacent_nodes_list = node_adjacency_map.find(cur_node->id())->second;
    ...
    for (unsigned int i=0; i<adjacent_nodes_list.size(); i++)
    {
      dbsk2d_shock_edge_sptr connected_edge = 
        edges_map.find(std::pair<int, std::pair<int, int> >(cur_node->id(), adjacent_nodes_list[i]))->second;
...
{
  fp_out <<"Extrinsic Shock File v1.0"<<std::endl<<std::endl;

  fp_out <<"Begin [GENERAL INFO]"<<std::endl<<std::endl;
  fp_out <<"Other parameters used: Default"<<std::endl;
  fp_out <<"Date of Creation: "<<std::endl<<std::endl;

  fp_out <<"Number of Nodes: "<< shock->number_of_vertices() <<std::endl;
  fp_out <<"Number of Edges: "<< shock->number_of_edges() <<std::endl;
  fp_out <<"Number of Sample Points: "<< std::endl<<std::endl;

  fp_out <<"End [GENERAL INFO]"<<std::endl<<std::endl;

}

void dbsk2d_xshock_graph_fileio::write_xshock_node_description()
{
  fp_out <<"Begin [NODE DESCRIPTION]"<<std::endl;
  fp_out <<"# node_ID  node_type Inside/Outside [CW linked node_IDs] [corresponding sample points]"<<std::endl;
  ...
  fp_out <<"End [NODE DESCRIPTION]"<<std::endl<<std::endl;
}

void dbsk2d_xshock_graph_fileio::write_xshock_sample (dbsk2d_xshock_sample_sptr sample)
{
  fp_out <<"Begin SAMPLE"<<std::endl;
  fp_out <<"sample_id "<< sample->id<<std::endl;
  fp_out <<"(x, y, t) ("<< sample->pt.x() <<", " << sample->pt.y() <<", " << sample->radius <<")"<<std::endl;
  fp_out <<"edge_id "<< sample->edge_id <<std::endl;
  ...
  if (sample->type == dbsk2d_xshock_sample::NORMALSAMPLE)         fp_out <<"type "<< "N"<<std::endl;
  else if (sample->type == dbsk2d_xshock_sample::PRUNEDSAMPLE)    fp_out <<"type "<< "P"<<std::endl;
  else                                                            fp_out <<"type "<< "C"<<std::endl;

  fp_out <<"theta "<< sample->theta*180/vnl_math::pi<<std::endl;
  fp_out <<"speed "<< sample->speed<<std::endl;
  ...
  fp_out <<", (" << sample->right_bnd_pt.x()<<", " << sample->right_bnd_pt.y()<<")]"<<std::endl;
  fp_out <<"boundaryTangents ["<< sample->left_bnd_tangent <<", "<< sample->right_bnd_tangent <<"]"<<std::endl;
  fp_out <<"End SAMPLE"<<std::endl<<std::endl;
}

void dbsk2d_xshock_graph_fileio::write_xshock_node_samples()
{
  fp_out <<"Begin [NODE SAMPLE POINTS]"<<std::endl<<std::endl;
  ...
  fp_out <<"End [NODE SAMPLE POINTS]"<<std::endl<<std::endl;
}

void dbsk2d_xshock_graph_fileio::write_xshock_edge_description()
{
  fp_out <<"Begin [EDGE DESCRIPTION]"<<std::endl;
  fp_out <<"# Edge_ID Inside/Outside [From_NODE To_NODE] [IDs of the sample points]"<<std::endl;
  ...
  fp_out <<"End [EDGE DESCRIPTION]"<<std::endl<<std::endl;
}

void dbsk2d_xshock_graph_fileio::write_xshock_edge_samples()
{
  fp_out <<"Begin [EDGE SAMPLE POINTS]"<<std::endl<<std::endl;
  ...
  fp_out <<"End [EDGE SAMPLE POINTS]"<<std::endl<<std::endl;
}
