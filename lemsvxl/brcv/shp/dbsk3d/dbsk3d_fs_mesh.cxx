...
float dbsk3d_fs_mesh::compute_median_A122_dist ()
{
  ...
  vul_printf (std::cout, "  compute_median_A122_dist(): %u A12 shock sheet elms.\n", facemap_.size());
  ...
  std::vector<double>::iterator medianIter=vals.begin() + vals.size()/2;
  std::nth_element (vals.begin(),medianIter,vals.end());
  median_A122_dist_ = (float) *medianIter;

  vul_printf (std::cout, "\tmedian A12-2 sample dist = %f\n", median_A122_dist_);
...
{
  vul_printf (std::cout, "  detect_valid_FE_type(): totally %u link-elms\n", edgemap_.size());
  ...
  vul_printf (std::cout, "\tInvalid (pruned) fs_edges (with 0 sheet):\t%6u,\n", n_invalid);
  vul_printf (std::cout, "\tRib fs_edges (with 1 sheet):\t\t\t%6u,\n", n_sheet_1);
  vul_printf (std::cout, "\tManifold fs_edges (with 2 sheets):\t\t%6u,\n", n_sheet_2);
  vul_printf (std::cout, "\tAxial fs_edges (with 3 sheets):\t\t\t%6u,\n", n_sheet_3);
  vul_printf (std::cout, "\tDegenerate Axial fs_edges (with 4+ sheets):\t%6u.\n", n_sheet_4p);
}
...
{
  std::set<dbmsh3d_edge*> Eset_to_del;
  ...
  std::set<dbmsh3d_edge*>::iterator eit = Eset_to_del.begin();
...
//
bool dbsk3d_fs_mesh::check_all_G_asgn (std::vector<dbmsh3d_vertex*>& unasgn_genes) 
{
  vul_printf (std::cout, "  check_all_G_asgn(): %d genes.\n", bnd_mesh_->vertexmap().size());

  //Go through all generators and initialize the G_asso_map_[] for all G's to G_ASSO_NONE. 
  std::map<int, char>  G_asso_map;

  std::map<int, dbmsh3d_vertex*>::iterator git = bnd_mesh_->vertexmap().begin();
  ...
  vul_printf (std::cout, "    %u valid fs_faces (total %u), %u FF-G asgn.\n", 
  ...
  for (; eit != edgemap_.end(); eit++) {
    ...
    if (FE->have_asgn_Gs()) {
      ...
      if (r == false) {
        ///assert (0);
        vul_printf (std::cout, "FE %d has G but not valid, ", FE->id());
      }
      ...
      for (dbmsh3d_ptr_node* cur = FE->asgn_G_list(); cur != NULL; cur = cur->next()) {
        ...
        n_G_L_asgn++;
        ///vul_printf (std::cout, "%d ", (*lgit)->id());
      }
    }
  }
  vul_printf (std::cout, "    %u valid fs_edges (total %u), %u FE-G asgn.\n", 
  ...
  for (; vit != vertexmap_.end(); vit++) {
    ...
    if (FV->have_asgn_Gs()) {
      ...
      if (r == false) {
        ///assert (0);
        vul_printf (std::cout, "FV %d has G but not valid, ", FV->id());
      }
  ...
  vul_printf (std::cout, "    %u valid fs_vertices (total %u), %u FV-G asgn.\n", 
              n_valid_N, vertexmap_.size(), n_G_N_asgn);

  //Go through G_asso_map_[] again and check if any G unassociated.
  vul_printf (std::cout, "    Unassociated gene(s) id: ");
  ...
  vul_printf (std::cout, "\t total # of unassociated gene(s): %d.\n", unasgn_genes.size());
...
void dbsk3d_fs_mesh::summary_report ()
{
  ...
  vul_printf (std::cout, "\t# of faces: %u.\n", facemap_.size());
  vul_printf (std::cout, "\t# of edges: %u.\n", edgemap_.size());
  vul_printf (std::cout, "\t# of vertices: %u.\n", vertexmap_.size());
}
...
int IFS_to_HE_print_mem_usage (dbsk3d_fs_mesh* fs_mesh)
{
  int total = 0;
  vul_printf (std::cout, "\n\n");
  vul_printf (std::cout, "     Map            Object            Size    Map Size    Total (bytes)\n");
  vul_printf (std::cout, "-------------------------------------------------------------------------\n");
  int sum = sizeof (dbsk3d_fs_vertex) * fs_mesh->vertexmap().size();
  total += sum;
  vul_printf (std::cout, " vertexmap_  dbsk3d_fs_vertex    %3d    %9d     %9d\n", 
               sizeof (dbsk3d_fs_vertex), fs_mesh->vertexmap().size(), sum);
  sum = sizeof (dbsk3d_fs_face) * fs_mesh->facemap().size();
  total += sum;
  vul_printf (std::cout, " facemap_    dbsk3d_fs_face   %3d    %9d     %9d\n", 
                                ...
                                int V_F_incidence_size, int total)
{
  ...
  total += sum;
  vul_printf (std::cout, " edgemap_    dbsk3d_fs_edge    %3d    %9d     %9d\n", 
               sizeof (dbsk3d_fs_edge), fs_mesh->edgemap().size(), sum);
  vul_printf (std::cout, "\n");

  sum = sizeof(std::pair<std::pair<int,int>, dbmsh3d_edge*>) * seid_edge_map_size;
  total += sum;
  vul_printf (std::cout, " seid_edge_map <<int,int>,pointer>    %3d    %9d     %9d\n", 
               sizeof(std::pair<std::pair<int,int>, dbmsh3d_edge*>),
               seid_edge_map_size, sum);
  vul_printf (std::cout, "-------------------------------------------------------------------------\n");
  vul_printf (std::cout, "                Total                                      %9d\n", total);
}
                          ...
                          std::set<dbmsh3d_face*>& Gfaces2, std::set<dbmsh3d_face*>& Gfaces1)
{
  ...
  std::set<dbmsh3d_vertex*> Gset_add; //Additional generators to add for Gfaces2 and Gfaces1.
  ...
  for (; git != Gset.end(); git++) {
    ...
    std::set<dbmsh3d_face*> inc_faces;
    G->get_incident_Fs (inc_faces);

    std::set<dbmsh3d_face*>::iterator fit = inc_faces.begin();
