...
public:
  ...
  void get_incident_A3ribs (std::set<dbmsh3d_edge*>& A3_MCs) const;
  void get_bnd_A3ribs (std::set<dbmsh3d_edge*>& A3_MCs) const;

  void get_incident_FEs (std::set<dbmsh3d_edge*>& incident_FEs) const;
  void get_incident_FVs (std::set<dbmsh3d_vertex*>& incident_FVs) const;
  
  //: Get all associated generators of this MS.
  int get_asso_Gs (std::map<int, dbmsh3d_vertex*>& asso_genes,
  ...
  void get_axial (std::set<dbmsh3d_edge*>& axial_set) const;

  //: Get the set of boundary axials of this ms_sheet.
  int get_axial_bnd (std::vector<dbsk3d_ms_curve*>& axial_vec) const;

  //: Get the set of i-curve (pairs and loops) axials of this ms_sheet.
  int get_axial_icurve (std::vector<dbsk3d_ms_curve*>& axial_vec) const;

  //: Get the set of non-swallowtail incident axial_set of this ms_sheet.
  void get_axial_nonsw (std::set<dbmsh3d_edge*>& axial_set) const;

  //: Get the set of non-swallowtail boundary axials of this ms_sheet.
  int get_axial_nonsw_bnd (std::vector<dbsk3d_ms_curve*>& axial_vec) const;

  //: Get the set of non-swallowtail i-curve (pairs and loops) axials of this ms_sheet.
  int get_axial_nonsw_icurve (std::vector<dbsk3d_ms_curve*>& axial_vec) const;

    int _get_axial_chain (const dbmsh3d_halfedge* headHE, 
                          std::vector<dbsk3d_ms_curve*>& axial_vec) const;
  ...
  void get_axial_inc_N (const dbmsh3d_vertex* V, 
                        std::set<dbmsh3d_edge*>& axial_N_set) const;
  
  void get_bndN_inc_axial (std::set<dbmsh3d_vertex*>& bnd_N_axial_set) const;

  void find_MCs_sharing_E (dbsk3d_ms_curve* inputMC, std::vector<dbsk3d_ms_curve*>& shared_MC_set) const;
  ...
  bool prune_pass_Gs (std::set<dbmsh3d_vertex*>& asso_genes);
...
//###############################################################
//       dbsk3d_ms_sheet TEXT std::FILE I/O FUNCTIONS
//###############################################################
