...
protected:
  ...
  std::multimap<double, vispt_elm*>  PQ_;
...
public:
  ...
  std::multimap<double, vispt_elm*>& PQ () {
    return PQ_;
  }
  ...
  void add_FF_to_PQ (const double r, const dbsk3d_fs_face* FF) {
    ...
    assert (result == false);
    PQ_.insert (std::pair<double, dbsk3d_fs_face*> (r, (dbsk3d_fs_face*) FF));
  } 
  ...
  void add_FE_to_PQ (const double r, const dbsk3d_fs_edge* FE) {
    PQ_.insert (std::pair<double, dbsk3d_fs_edge*> (r, (dbsk3d_fs_edge*) FE));
  }  
