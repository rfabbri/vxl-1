//: dbsk3d_ms_xform_merge_a1a3ii_c_c.cxx
//: MingChing Chang
//  Jan. 29, 2008
...
//: Given an A3/(A13,A1n) curve C, search for an (A13, A1n)/A3 curve C2 to merge.
//  Goal is to find the transform candidate with smallest cost (closest event).
//
bool find_A1A3II_c_c_merge_C (const dbsk3d_ms_curve* MC1, dbsk3d_ms_curve*& closest_MC2, 
                              ...
                              const int cmxth)
{
  ...
  //Two cases: 
  // - MC1 is a rib: MC2 is either A13 or A1n.
  // - MC1 is A13/A1n: MC2 must be a rib.
  bool MC1_A3 = false;
  if (MC1->c_type() == C_TYPE_RIB)
    MC1_A3 = true;
  else if (MC1->c_type() != C_TYPE_AXIAL && MC1->c_type() != C_TYPE_DEGE_AXIAL)
    return false;  
  if (is_HE_3_incidence (MC1->halfedge()))
    return false; //Return if MC1 is swallow-tail 3-incidence.
  ...
  do {
    ...
    if (MC1_A3) //MC2 is either A13 or A1n.
