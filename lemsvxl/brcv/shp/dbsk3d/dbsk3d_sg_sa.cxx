...
void dbsk3d_sg_sa::compute_median_edge_len ()
{
  ...
  vul_printf (std::cout, "  compute_median_edge_len(): %u fs_edges.\n", edgemap_.size());
  ...
  vul_printf (std::cout, "\tmedian fine-scale link length = %f\n", median_edge_len_);
}
...
void clone_sg_sa (dbsk3d_sg_sa* targetSG, dbsk3d_sg_sa* inputSG)
{
  ...
  std::map<int, dbsk3d_fs_vertex*>::iterator it = inputSG->FV_map().begin();
  ...
  std::map<int, dbmsh3d_vertex*>::iterator vit = inputSG->vertexmap().begin();
  ...
  //More on cloning each ms_curve: type, fs_vertices[], etc.
  std::map<int, dbmsh3d_edge*>::iterator eit = inputSG->edgemap().begin();
