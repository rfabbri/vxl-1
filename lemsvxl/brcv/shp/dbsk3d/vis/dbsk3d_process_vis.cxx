...
{
  vul_printf (std::cerr, "vis_bnd_mesh(): V: %d, E: %d, F: %d\n", 
  ...
  vul_printf (std::cerr, "  # unmeshed pts: %d\n", n_unmeshed_pts);
...
{
  vul_printf (std::cerr, "vis_mesh_anim(): V: %d, E: %d, F: %d\n", 
              mesh_->vertexmap().size(), mesh_->edgemap().size(),
              mesh_->facemap().size());
  SoSeparator* root = new SoSeparator;
...
bool dbsk3d_pro_vis::compute_dx_dy_dz (const float ratio)
{
  ...
  vul_printf (std::cerr, "    compute_dx_dy_dz(): \n");
  vul_printf (std::cerr, "\tBounding box width %f, height %f, depth %f.\n",
               bbox.width(), bbox.height(), bbox.depth());
  vul_printf (std::cerr, "\tSet view dx %f, dy %f, dz %f.\n",
...
{
  vul_printf (std::cerr, "    compute_vertex_node_size(): ");
  ...
  vul_printf (std::cerr, "default: cube %f, ball %f.\n", cube_size_, ms_vis_param_.ball_size_);
}
                                            ...
                                            const int option, const int vis_option)
{
  ...
  vul_printf (std::cerr, "dbsk3dr_pro_vis::vis_list_file(): %s.\n", filename.c_str());

  std::ifstream  in;
  std::string    linestr;
  in.open (filename.c_str());
  if (in == false) {
    vul_printf (std::cerr, "Can't open listfile %s\n", filename.c_str());
    return root; 
  }
  ...
  while (in) {
    ...
    vul_printf (std::cerr, "\t Currently %u points %u faces loaded.\n", 
  ...
  vul_printf (std::cerr, "\n====================================================\n");
  vul_printf (std::cerr, "  vis_list_file(): %d files (%d align. files) shown from %s.\n", 
              file_count, af_count, filename.c_str());
  vul_printf (std::cerr, "\tTotally %u points, %u faces.\n", 
              n_total_pts, n_total_faces);
  return root;
}
