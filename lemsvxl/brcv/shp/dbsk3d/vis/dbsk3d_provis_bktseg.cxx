                                     ...
                                     const float seg_msr, const int seg_topo_opt)
{
  ...
  double si = dc + db;
  double se = dc + dmax;
  vul_printf (std::cerr, "\tEstimated median sampling dist. dm : %f.\n", dm);
  vul_printf (std::cerr, "\t      average dm of buckets dm_avg : %f.\n", dm_avg);
  vul_printf (std::cerr, "\t        maximum triangle size dmax : %f (dm * msr %f).\n", dmax, seg_msr);
  vul_printf (std::cerr, "\t                confident dist. dc : %f (dm * cdr %f).\n", dc, bktseg_cdr);
  vul_printf (std::cerr, "\t overlapping stitching boundary db : %f (dm * bor %f).\n", db, bktseg_bor);
  vul_printf (std::cerr, "\t         stitching sausage int. si : %f (dc + db).\n", si);
  vul_printf (std::cerr, "\t         stitching sausage ext. se : %f (dc+dmax).\n", se);
