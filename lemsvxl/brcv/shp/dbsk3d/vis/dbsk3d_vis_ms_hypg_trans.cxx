                                          ...
                                          const bool show_id)
{
  ...
  if (MC->all_Es_shared()) {
    vul_printf (std::cout, "\tDege. case: all edges are shared_Es.!\n");
    return root;
  }
  ...
  vul_printf (std::cout, "MC %d: <S %d (%d), E %d (%d)>.\n", 
  ...
  vul_printf (std::cout, "SS %d (%d) - CS %d <%d, %d> -- CE %d <%d, %d> - EE %d (%d).\n",  
  ...
  for (unsigned int i=0; i<UM_Evec.size(); i++) {
    ...
    vul_printf (std::cout, "%d ", UM_Evec[i]->id());
  }
  ...
  for (unsigned int i=0; i<VM_Evec.size(); i++) {
    ...
    vul_printf (std::cout, "%d ", VM_Evec[i]->id());
  }
  vul_printf (std::cout, "\n");
  ...
  //Draw the FF_to_trim. 
  std::set<dbmsh3d_face*>::iterator it = FF_to_trim_U.begin();
                                              ...
                                              const bool show_id)
{
  ...
  vul_printf (std::cout, "MC %d: NA14 %d (%d), NA1A3 %d (%d)>.\n", 
              MC->id(), NA14->id(), NA14->V()->id(), NA1A3->id(), NA1A3->V()->id());
  vul_printf (std::cout, "ribMC %d (FE %d, ..., FE %d).\n",
              ribMC->id(), ribMC->E_vec(0)->id(), ribMC->E_vec_last()->id()); 
  vul_printf (std::cout, "aMC %d (FE %d, ..., FE %d).\n",
              aMC->id(), aMC->E_vec(0)->id(), aMC->E_vec_last()->id());

  if (r == false) {
    vul_printf (std::cout, "Can't perform A12A3I_curve_contract_xform()!\n");
    return root;
  }

  if (FF_to_trim.size() == 0) {
    vul_printf (std::cout, "FF_to_trim.size() == 0.\n");
  }
  ...
  vul_printf (std::cout, "M %d, ",  M->id());
  root->addChild (draw_vertex_vispt_SoCube (U, SbColor(0,0.75f,0), R, show_id));
  vul_printf (std::cout, "U %d.\n",  U->id());
  vul_printf (std::cout, "F_M %d, M_on_aMC %s.\n", F_M->id(), M_on_aMC ? "true" : "false");

  //Draw E_aMC.
  vul_printf (std::cout, "E_aMC: %d fs_edges: ", E_aMC.size());
  for (unsigned int i=0; i<E_aMC.size(); i++) {
    ...
    vul_printf (std::cout, "%d ", E_aMC[i]->id());
  }
  vul_printf (std::cout, "\n");
                                   ...
                                   const float curve_width, const float cube_size)
{
  ...
  //Brute-force debug:
  vul_printf (std::cout, "\tms_hypg.check_integrity() ");
  ...
  vul_printf (std::cout, "    %3d A1A3-I or A12A3-II Sheet-Splice xforms.\n", n_tab_splice);
  ...
  vul_printf (std::cout, "    %3d A1A3-II Node-Node Merge xforms.\n", n_a1a3ii_nn_merge);  
  vul_printf (std::cout, "    %3d A12A3-I Node-Curve Merge xforms.\n", n_a12a3i_nc_merge);
  vul_printf (std::cout, "    %3d A14 Curve-Curve Merge xforms.\n", n_a14_cc_merge);
  vul_printf (std::cout, "    %3d A1A3-II Curve-Curve Merge xforms.\n", n_a1a3ii_cc_merge);
  vul_printf (std::cout, "    %3d A1A5 Node-Curve Merge xforms.\n\n", n_a1a5_nc_merge);
                                         ...
                                         const float cost, const float curve_width)
{
  ...
  vul_printf (std::cout, "MN2 %d (%d), MN1 %d (%d).", MN2->id(), MN2->V()->id(), MN1->id(), MN1->V()->id());
  vul_printf (std::cout, "path: ");
  dbmsh3d_vertex* V = MN2->V();
  for (unsigned int i=0; i<shortest_Evec.size(); i++) {
    dbmsh3d_edge* E = shortest_Evec[i];
    vul_printf (std::cout, "V%d - E%d - ", V->id(), E->id());
    V = E->other_V (V);
  }
  assert (V == MN1->V());
  vul_printf (std::cout, "V%d.\n", V->id());
                                         ...
                                         const float curve_width, const float cube_size)
{
  ...
  if (r == false) {
    vul_printf (std::cout, "no MC2new found.\n");
    return root;
  }
  vul_printf (std::cout, "MC2new %d different from MC2 %d.\n\t", MC2new->id(), MC2->id());
  ...
  //Print path info (back tracking).
  vul_printf (std::cout, "MC2 %d (FV %d), MN1 %d (%d).", MC2->id(), closest_V->id(), MN1->id(), MN1->V()->id());
  vul_printf (std::cout, "path: ");
                                      ...
                                      const float curve_width, const float cube_size)
{
  ...
  //Print path info (back tracking).
  vul_printf (std::cout, "MC2 %d (FV2 %d), MC1 %d (FV1 %d).", MC2->id(), closest_V2->id(), MC1->id(), closest_V1->id());
  vul_printf (std::cout, "path: ");
