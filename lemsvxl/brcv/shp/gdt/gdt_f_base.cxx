...
void gdt_f_manager_base::add_wavefront_he (dbmsh3d_halfedge* cur_he) 
{
...
#if GDT_DEBUG_MSG
  if (n_verbose_>2)
    vul_printf (std::cerr, "    Add edge %d(%f) to the front.\n", cur_edge->id(), dist);
#endif
  ...
  wavefront_.insert (std::pair<double, dbmsh3d_halfedge*> (dist, cur_he));
}
...
bool gdt_f_manager_base::remove_wavefront_he (dbmsh3d_halfedge* cur_he) 
{
  ...
  std::multimap<double, dbmsh3d_halfedge*>::iterator it = _find_in_wavefront (cur_he);
  if (it != wavefront_.end()) {
#if GDT_DEBUG_MSG
    if (n_verbose_>2) {
      double dist = (*it).first;
      vul_printf (std::cerr, "    Remove edge %d(%f) from the front.\n", cur_edge->id(), dist);
    }
#endif
    wavefront_.erase (it);
    return true;
