...
#if GDT_DEBUG_MSG
      if (n_verbose_>1)
        vul_printf (std::cerr, "    dege_I on edge %d initialized.\n", edge_d->id());
#endif
    }
...
#if GDT_DEBUG_MSG
      if (n_verbose_>1) {
        vul_printf (std::cerr, "    psrc_I on edge %d initialized.\n", edge_j->id());
        vul_printf (std::cerr, "    dege_I on edge %d initialized.\n", edge_d->id());
        vul_printf (std::cerr, "  wavefront arc on face %d initialized.\n\n", face_j->id());
      }
#endif
    }
  }
