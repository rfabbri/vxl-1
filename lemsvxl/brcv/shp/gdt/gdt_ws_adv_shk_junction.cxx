                                                        ...
                                                        const gdt_shock* S)
{
  ...
  for (i=0; i<SList.size(); i++) {
    Si = SList[i];
    vul_printf (std::cerr, "%d ", Si->id());
  }
  
  vul_printf (std::cerr, "at junction %d, simtime %f.\n", junct->id(), Sl->simT());
#endif
                                               ...
                                               const gdt_shock* S)
{
  ...
  vul_printf (std::cerr, "at sink %d, simtime %f.\n", sink->id(), Sl->simT());
#endif
...
void gdt_ws_manager::advance_2_shocks_to_junction (gdt_shock* Sl, gdt_welm* Wc, gdt_shock* Sr)
{
  ...
  if (n_verbose_>1) {
    ...
    vul_printf (std::cerr, "       to intersection: simtime %f.\n", Sl->simT());
    ...
    vul_printf (std::cerr, "  WL %s%d (%.3f, %.3f), WC %s%d (%.3f, %.3f), WR %s%d (%.3f, %.3f).\n",
  ...
  if (n_verbose_>1) {
    ...
    if (Wc->_is_RF())
      ...
      std::sprintf (wc, " ");
    vul_printf (std::cerr, "  WC %d (%.3f, %.3f) finalized.\n",
