...
void gdt_ws_manager::Create_propagation_of_interior_vertex (dbmsh3d_gdt_vertex_3d* v, gdt_welm* Wa, gdt_welm* Wb)
{
#if GDT_DEBUG_MSG
  if (n_verbose_>3)
    vul_printf (std::cerr, "  interior V %d: Wa e %d (%.3f, %.3f), Wb e %d (%.3f, %.3f).\n", 
  ...
  if (Wa->edge() == Wb->edge()) {
    ...
    #if GDT_DEBUG_MSG
      vul_printf (std::cerr, "  Skip the sink V %d: Wa %d (%.3f, %.3f), Wb %d (%.3f, %.3f).\n", 
                   v->id(), Wa->edge()->id(), Wa->stau(), Wa->etau(), 
                   Wb->edge()->id(), Wb->stau(), Wb->etau());
    #endif
    return;
  ...
  if (v->is_hyperbolic()) {
    ...
    if (faceA == faceB) { // H1) faceA == faceB
      ...
      if (n_verbose_>3) {        
        ...
        if (Sb)
          ...
          vul_printf (std::cerr, "    New contactS %s and %s created from v %d.\n", 
    ...
    // E2) Else, fp_ab = faceA = faceB is the face where the shock is on.
      ...
      if (n_verbose_>3) {
        ...
        if (_eqD (Wa->mu(), Wb->mu()))
          vul_printf (std::cerr, "    New lineS %d (prjE %s) created from v %d.\n", 
                       S->id(), str, v->id());
        else
          vul_printf (std::cerr, "    New hypbS %d (prjE %s) created from v %d.\n", 
                       S->id(), str, v->id());
      }
      #endif
    }
                                                        ...
                                                        const dbmsh3d_face* fp)
{
  ...
  if (_eqT (stau, etau)) {
    ...
    #if GDT_DEBUG_MSG
    if (n_verbose_>3)
      vul_printf (std::cerr, "    New contactS %d (tauE %.3f, prjE %d) created from v %d.\n", 
    ...
    if (n_verbose_>3) {        
      ...
      vul_printf (std::cerr, "    New contactS %s and %s created at v %d.\n", 
                   astring, bstring, v->id());
    }
    #endif
  }
