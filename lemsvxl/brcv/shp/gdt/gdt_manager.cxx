...
{
  vul_printf (std::cerr, "===========================================\n");
  vul_printf (std::cerr, "Summary of GDT on [%s] with source %d via\n  ", prefix, i_source);

  if (method == GDT_METHOD_I)
    vul_printf (std::cerr, "Method 1 (Interval Based)");
  else if (method == GDT_METHOD_F)
    vul_printf (std::cerr, "Method 2 (Face Based)");
  else if (method == GDT_METHOD_FS)
    vul_printf (std::cerr, "Method 3 (Face Based + Shock)");
  else if (method == GDT_METHOD_WS)
    vul_printf (std::cerr, "Method 4 (Wavefront + Shock)");
  ...
  for (; eit != gdt_mesh_->edgemap().end(); eit++) {
    ...
    #if GDT_DEBUG_MSG
    if (n_verbose_==3)
      vul_printf (std::cerr, "edge %d (s%d-e%d) (len: %lf) has %d intervals:\n", 
    ...
    for (int i=0; it != cur_edge->interval_section()->I_map()->end(); it++, i++) {
      ...
      if (n_verbose_==3) {
        switch (I->type()) {
        case ITYPE_PSRC:
          vul_printf (std::cerr, "    regular I %d: (%lf - %lf) vs: %d\n", 
                      i, I->stau(), I->etau(), I->psrc()->id());
        break;
        case ITYPE_DEGE:
          vul_printf (std::cerr, "    dege I %d: (%lf - %lf) vs: %d\n", 
                      i, I->stau(), I->etau(), I->psrc()->id());
        break;
        default:
          break;
  ...
  vul_printf (std::cerr, "mesh #v: %d, #f: %d, #e: %d\n",
               (int) gdt_mesh_->vertexmap().size(), 
               (int) gdt_mesh_->facemap().size(), 
               (int) gdt_mesh_->edgemap().size());

  vul_printf (std::cerr, "# total iterations: %d\n", (int) n_prop_iter_);
  vul_printf (std::cerr, "# propagated faces: %d\n", n_prop_faces);
  vul_printf (std::cerr, "# propagated intervals: %d\n", n_total_intervals);
  vul_printf (std::cerr, "# propagated edges: %d\n", n_prop_edges);
  double d_I_per_edge = (double) n_total_intervals / n_prop_edges;
  vul_printf (std::cerr, "# average interval per edge: %f\n", d_I_per_edge);
}
...
#if GDT_DEBUG_MSG
  vul_printf (std::cerr, "===========================================\n");
  vul_printf (std::cerr, "# 2nd order sources: %d\n", n_W_W_2nd_source_);
  vul_printf (std::cerr, "# shock junctions: %d\n", n_S_junct_);
  vul_printf (std::cerr, "# shock sinks: %d\n", n_S_sink_);
  vul_printf (std::cerr, "# W-E strike at FPT: %d\n", n_WE_FPT_);
  vul_printf (std::cerr, "# W-E strike at SV: %d\n", n_WE_SV_);
  vul_printf (std::cerr, "# W-E strike at EV: %d\n", n_WE_EV_);
  vul_printf (std::cerr, "# S-E intersection: %d\n", n_S_E_);
  vul_printf (std::cerr, "# W-V strikes: %d\n", n_W_V_strike_);
  vul_printf (std::cerr, "# V launch shocks: %d\n", n_V_launch_S_);
  vul_printf (std::cerr, "# interior V launch rarefactions: %d\n", n_V_interior_RF_);
  vul_printf (std::cerr, "# bnd V launch rarefactions: %d\n", n_V_bnd_RF_);
  vul_printf (std::cerr, "# V launch degeI (instead of S): %d\n", n_V_launch_degeI_);
  vul_printf (std::cerr, "# S on E: %d\n", n_S_on_edge_);  
  vul_printf (std::cerr, "# S terminate at V: %d\n", n_S_term_at_V_);
  vul_printf (std::cerr, "# W terminate at V: %d\n", n_W_term_at_V_);
#endif
}
