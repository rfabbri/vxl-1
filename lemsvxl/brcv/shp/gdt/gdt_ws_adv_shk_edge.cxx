...
{ 
  ...
  *newWb = nWb;
  ...
  if (n_verbose_>3) {
    vul_printf (std::cerr, " Advance S %d (v %d) to prjE %d at tauE %f, endtime %f.\n", 
    ...
    vul_printf (std::cerr, "  pWa %s%d (%.3f, %.3f), pWb %s%d (%.3f, %.3f),\n", 
    ...
    vul_printf (std::cerr, "  nWa %s, nWb %s.\n", astring, bstring);
  }
#endif
}
...
void gdt_ws_manager::SE_update_pWa_pWb (const gdt_shock* S)
{
  ...
  if (pWa->_is_RF()) {
    ...
    if (pWa->try_finalize()) {
      ...
      #if GDT_DEBUG_MSG
      if (n_verbose_>3)
        vul_printf (std::cerr, "  Finalize_I psrcW %d (%.3f, %.3f).\n", 
