...
#if GDT_DEBUG_MSG
  if (n_verbose_>1) {
    vul_printf (std::cerr, "Front %d: ", (int) wavefront_.size());
...
#if GDT_DEBUG_MSG
    if (n_verbose_>1)
      vul_printf (std::cerr, "(C%d and L%d -> R%d)\n", cur_edge_->id(), left_edge_->id(), right_edge_->id());
#endif
...
#if GDT_DEBUG_MSG
    if (n_verbose_>1)
      vul_printf (std::cerr, "(C%d and L%d and R%d -> sink)\n", 
                   cur_edge_->id(), left_edge_->id(), right_edge_->id());
#endif
