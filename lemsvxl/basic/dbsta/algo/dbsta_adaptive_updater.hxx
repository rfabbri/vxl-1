// This is brcv/seg/dbsta/algo/dbsta_adaptive_updater.hxx
                                              ...
                                              const vnl_vector<T>& sample ) const
{
  ...
  std::vector<T> probs(mixture.num_components(),T(0));
  T sum_probs = T(0);
                                         ...
                                         const vnl_vector<T>& sample ) const
{
  ...
  std::vector<T> probs(num_components,T(0));
  ...
  if(mixture.weight(mixture.num_components()-1) < std::numeric_limits<T>::epsilon()){
    mixture.remove_last(); 
    T sum = 0;
    for(unsigned int i=0; i<mixture.num_components(); ++i){
                                                 ...
                                                 const vnl_vector<T>& sample ) const
{
  ...
  for(unsigned int i=0; i<mixture.num_components(); ++i){
    ...
    dbsta_gaussian<T>& g = static_cast<dbsta_gaussian<T>& >(mixture.distribution(i));
    T weight = (T(1.0)-alpha) * mixture.weight(i);
    //if(weight < std::numeric_limits<T>::epsilon()*alpha)
    //  std::cout << "weight < "<<std::numeric_limits<T>::epsilon()*alpha <<std::endl;
    if(no_match && dbsta_fits_gaussian_model(g,sample,T(2.5))){
      weight += alpha;
      if(weight == alpha)
