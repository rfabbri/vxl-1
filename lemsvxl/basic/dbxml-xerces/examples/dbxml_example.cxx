...
int main(int argC, char* argV[])
{
    ...
    std::vector<dbxml_generic_ptr> object;
    ...
    // If the parse was successful, output the document data from the DOM tree
        ...
        for (std::vector<dbxml_generic_ptr>::iterator cit = object.begin();
                cit != object.end(); cit++){

                        void *p  = (*cit).get_anyobject();
                           std::cout                                << " startframe: " << ((dbxml_track *)p)->startframe_
           << " numframes: " << ((dbxml_track *)p)->numframes_
               << " width: " << ((dbxml_track *)p)->width_
               << " height: " << ((dbxml_track *)p)->height_
                                   << std::endl;
             std::vector<dbxml_generic_ptr> track = ((dbxml_track *)p)->tracks_;

                        for (std::vector<dbxml_generic_ptr>::iterator citt = track.begin();
                citt != track.end(); citt++){

                        void *p  = (*citt).get_anyobject();
                           std::cout                                << " x: " << ((dbxml_tracks *)p)->x_
           << " y: " << ((dbxml_tracks *)p)->y_
                                   << std::endl;
    }
    ...
    /*
                ...
                cit != object.end(); cit++){
                        ...
                        void *p  = (*cit).get_anyobject();
                           std::cout                                << " startframe: " << ((dbxml_param *)p)->command_
    ...
    */
                           ...
                           std::cout                                << " Error parsing file " << gXmlFile
                                   << std::endl;
