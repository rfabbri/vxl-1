...
class dbxml_param_input_converter : public dbxml_input_converter
{
 public:
  ...
  ~dbxml_param_input_converter();
    //static BXML_DLL_DATA std::vector<bxml_input_converter_sptr> input_converters_;
...
//  virtual bool extract_from_dom(DOMNode * node);
  ...
  virtual dbxml_generic_ptr construct_object_2(std::vector<dbxml_generic_ptr>& objs);
  virtual dbxml_generic_ptr construct_object_3(std::vector<dbxml_generic_ptr>& objs);


  //: construct a new object instance
  virtual dbxml_generic_ptr construct_object_1(std::vector<dbxml_generic_ptr>& objs);
 std::string command_;
 std::string desc_;
  std::string type_;
  std::string value_;
