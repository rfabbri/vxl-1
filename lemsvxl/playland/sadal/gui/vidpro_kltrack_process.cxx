...
#include "vidpro_kltrack_process.h"
...
//: Constructor
vidpro_kltrack_process::vidpro_kltrack_process(void): vidpro_process(),kl_params(),kl_points(NULL)
{
...
//: Destructor
vidpro_kltrack_process::~vidpro_kltrack_process()
{
    delete(kl_points);
}
...
int
vidpro_kltrack_process::input_frames()
{
  int num_frames;
...
int
vidpro_kltrack_process::output_frames()
{
  return input_frames();
}
...
vcl_vector< vcl_string > vidpro_kltrack_process::get_input_type()
{
...
vcl_vector< vcl_string > vidpro_kltrack_process::get_output_type()
{  
  vcl_vector<vcl_string > to_return;
...
vidpro_kltrack_process::execute()
{
 int num_of_frames;
  ...
  vcl_vector<vidpro_image_storage_sptr> frame_image(num_of_frames);
  vcl_vector<vil_image_resource_sptr> images(num_of_frames);
  vcl_vector<vidpro_image_storage_sptr>::const_iterator frame_iter= frame_image.begin();
...
//: Clone the process
vidpro_process*
vidpro_kltrack_process::clone() const
{
  return new vidpro_kltrack_process(*this);
}
    
bool
vidpro_kltrack_process::finish()
{
  return true;
}
