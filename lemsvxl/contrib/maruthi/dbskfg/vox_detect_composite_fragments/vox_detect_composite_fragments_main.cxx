...
// \brief An algorithm to detect bbox within model cgraph fragments to query
//        composite graph fragments
//
//
// \verbatim
// \endverbatim
...
#include "vox_detect_composite_fragments_params_sptr.h"
...
int main(int argc, char *argv[]) 
{
    ...
    vox_detect_composite_fragments_params_sptr params = 
        new vox_detect_composite_fragments_params(
            "vox_detect_composite_fragments");  
    ...
    else
    {
        ...
        vcl_string glob=params->query_object_dir_()+"/"+"*"+
            ...
            params->input_cgraph_match_file_extension_();
    ...
    if (!match_status ) 
    {
        vcl_cerr << "Cannot find match (.xml) file: " << 
            match_file << vcl_endl;

        return 1;
    }

    // Lets create directory of where output detection results will go
    vcl_string output_file;
    if (params->save_to_object_folder_())
    { 
        output_file = params->output_cgraph_fragment_detect_folder_() + "/";
    }
    else 
    {
    ...
    //******************** Detecting Fragments ****************************
    vcl_cout<<"***********  Detecting Cgraph Fragments  ***********"<<vcl_endl;
    
    // Set up process
    dbskfg_detect_fragments_process detect_pro;

    // Set parameters of this process
    bpro1_filepath match_file_path(match_file,".xml");

    detect_pro.parameters()->set_value("-sminput", 
                                       match_file_path);
    ...
    for ( ; thresh_index < num_thresholds ; ++thresh_index )
    {
        ...
        if ( !detect_status )
        {
            vcl_cerr<<" Error in cgraph fragment detection!"<<vcl_endl;
