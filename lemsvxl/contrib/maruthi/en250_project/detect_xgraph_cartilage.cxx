                                ...
                                int& detection_window_height)
{
    ...
    {
        std::cout << "Warning: no xgraph scale was chosen. Default scale will be used.\n";
    }
...
int main(int argc, char *argv[]) 
{
    ...
    std::cout << "\n>>Run object detection on image " << 
        vul_file::strip_directory(image_file)<< " using xgraph model " 
             << vul_file::strip_directory(xgraph_file) << std::endl;
    ...
    /////////////////////////////////////////////////////////////////////////////
    // OUTPUT ///////////////////////////////////////////////////////////////////
    std::string output_file_prefix = params->get_output_folder() + "/";

    //>> Write detection results to a file in VOC2008 format
    std::string det_result_file = output_file_prefix + "VOC_det_result+" 
    ...
    std::vector<std::string > xgraph_filenames(num_sols, "");
    std::vector<std::string > screenshot_filenames(num_sols, "");
    ...
    for (unsigned i =0; i < num_sols; ++i)
    {
        ...
        // shock graph file
        std::string output_xml_filename = vul_sprintf("%s+%s.%d.xml", params->input_xgraph_name_().c_str(),
                                                     params->input_object_name_().c_str(), 
                                                     i);
        std::string output_xml_file = output_file_prefix + output_xml_filename;
        ...
        // // screenshot file
        // std::string output_screenshot_filename = vul_sprintf("%s+%s.%d.png", params->input_xgraph_name_().c_str(),
        //   params->input_object_name_().c_str(), i);
        // std::string output_screenshot_file = output_file_prefix + output_screenshot_filename;
    ...
    std::string det_xml_record_file = output_file_prefix + "xml_det_record+" + params->input_xgraph_name_()
        + "+" + params->input_object_name_() + ".txt";

    std::cout << "\n>>Save detection results as XML record file " << det_xml_record_file << std::endl;
    {

        std::ofstream ofs(det_xml_record_file.c_str(), std::ios::out);
        for (unsigned i =0; i < det_list.size(); ++i)
        {
            ofs << "<xshock_det>\n";
            ...
            ofs << "  <xgraph_scale>" << std::sqrt(det_list[i]->xgraph()->area()) <<"</xgraph_scale>\n";
    ...
    std::cout <<">>EVALUATE DETECTION RESULTS\n";
  
    std::string image_desc_file = params->input_object_dir_() + "/" +
    ...
    std::string eval_file = output_file_prefix + "eval_det+" +
        params->input_xgraph_name_() + "+" + 
        params->input_object_name_() + ".txt";
