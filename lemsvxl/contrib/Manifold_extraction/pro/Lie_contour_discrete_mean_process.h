/*************************************************************************
 *    NAME: pradeep
 *    FILE: Lie_contour_discrete_mean_process.h
 *    DATE: 04 July 2007
 // brief:  process to find out the intrinsic mean curve given a set of 
 //         contour files by searching along the discrete samples in the tangent space
 //         of each of the training shape instances 
 *************************************************************************/

#ifndef Lie_contour_discrete_mean_process_header
#define Lie_contour_discrete_mean_process_header
...
class Lie_contour_discrete_mean_process : public bpro1_process 
{
protected:
  ...
  void save_shape_as_image(std::vector<vsol_point_2d_sptr> new_shape,std::string new_shapes_path,
                                                                 unsigned int i);
  unsigned int find_min_cost_index(std::vector<std::vector<vsol_point_2d_sptr> > search_shapes,
                                                                    std::vector<std::vector<vsol_point_2d_sptr> > given_shapes,
                                                                    double &least_cost,std::string file_name);                                 

public:
  Lie_contour_discrete_mean_process();
  virtual ~Lie_contour_discrete_mean_process() {}

  //: Clone the process
  virtual bpro1_process* clone() const;

  std::string name() {
    return "lie contour discrete mean";
  }
