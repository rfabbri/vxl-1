...
breg3d_ekf_camera_optimizer_state breg3d_ekf_camera_optimizer::optimize(bvxm_voxel_world_sptr const& vox_world,
                                                                    ...
                                                                    breg3d_ekf_camera_optimizer_state &prev_state, std::string apm_type, unsigned bin_idx)
{
  ...
  if (use_expected_) {
    ...
    while(iterate_again) {
      ...
      if (apm_type == "apm_mog_grey") {
        ...
        std::cerr << "error: unsupported appearance model type " << apm_type << " ! " << std::endl;
      }
...
vnl_vector<double> breg3d_ekf_camera_optimizer::img_homography(vil_image_view_base_sptr &base_img_viewb, vil_image_view_base_sptr &img_viewb, vil_image_view<float> &mask_view, vnl_matrix_fixed<double,3,3> K, bool projective)
{
  ...
  switch(base_img_viewb->pixel_format()) {
    ...
    case VIL_PIXEL_FORMAT_BYTE:
      ...
      break;
    default:
      std::cerr << "error: breg3d_ekf_camera_optimizer::img_homography : unsupported pixel type " << base_img_viewb->pixel_format() << std::endl;
  }
  ...
  switch(img_viewb->pixel_format()) {
    ...
    case VIL_PIXEL_FORMAT_BYTE:
      ...
      break;
    default:
      std::cerr << "error: breg3d_ekf_camera_optimizer::img_homography : unsupported pixel type " << img_viewb->pixel_format() << std::endl;
  }
