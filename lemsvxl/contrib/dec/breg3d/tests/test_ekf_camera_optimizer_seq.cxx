...
static void init_ground_plane(double plane_z, double plane_std, bvxm_voxel_world_sptr vox_world)
{
  ...
  bvxm_voxel_grid<ocp_datatype> *grid = dynamic_cast<bvxm_voxel_grid<ocp_datatype>*>(vox_world->get_grid<OCCUPANCY>(0).ptr());
...
static void test_ekf_camera_optimizer_seq()
{
  ...
  for (unsigned n=0; n < ncameras; ++n, a+= total_angle/ncameras) {
    ...
    if (!ofs.is_open()) {
      std::cerr << "Failed to open file " << cam_fname.str() << std::endl;
    }
    ofs << cam_persp->get_calibration().get_matrix() << std::endl;
    ofs << R << std::endl;
  ...
  bvxm_voxel_grid_base_sptr ocp_base = vox_world->get_grid<OCCUPANCY>(0);
  ...
  for (unsigned n=1; n<ncameras; ++n) {
    ...
    std::cout << "camera " << n-1 << " center   : " << cam0->get_camera_center() << std::endl;
    std::cout << "camera " << n-1 << " rotation : " << std::endl << cam0->get_rotation().as_rodrigues() << std::endl;
    std::cout << "camera " << n << " center   : " << cam1->get_camera_center() << std::endl;
    std::cout << "camera " << n << " rotation : " << std::endl << cam1->get_rotation().as_rodrigues() << std::endl;

    std::cout << "est. center      : " << center_est << std::endl;
    std::cout << "est. rotation    : " << std::endl << rot_est.as_rodrigues() << std::endl;
    ...
    vil_save(*expected_true,"c:/research/registration/output/expected_true.tiff");
    ...
    if (!ofs.is_open()) {
      std::cerr << "Failed to open file " << cam_est_fname.str() << std::endl;
    }
    ...
    // save voxel world
    vox_world_reg->save_occupancy_raw(model_dir + "/ocp_reg.raw");
