#ifndef psm_opt_appearance_estimator_txx_
#define psm_opt_appearance_estimator_txx_
...
void psm_opt_appearance_estimator<APM>::compute_appearance(std::vector<typename psm_apm_traits<APM>::obs_datatype> const& obs, std::vector<float> const& pre, std::vector<float> const& vis, typename psm_apm_traits<APM>::apm_datatype &model)
{
  const float min_sigma = 0.04f; 

  // initialize appearance model estimate using visibility probabilities as weights.
  //psm_apm_traits<APM>::apm_processor::compute_appearance(obs,vis,model,min_sigma);
#if 1
  ...
  for (unsigned int i=0; i<max_its; ++i) {
    ...
    for (unsigned int n=0; n<nobs; ++n) {
      ...
      const float P1 = vis[n] * psm_apm_traits<APM>::apm_processor::prob_density(model,obs[n]);
    ...
    if (max_weight_change < min_weight_change) {
      //if (nobs > 1) {
        ...
        //std::cout << "sigma = " << model.sigma() << ",  gauss_weight = " << model.gauss_weight() << std::endl;
      //}
      break;
    }
    // MAXIMIZATION
    //psm_apm_traits<APM>::apm_processor::compute_appearance(obs,obs_weights,model,min_sigma);
  }

  // adjust model to account for small sample sizes.
  //psm_apm_traits<APM>::apm_processor::finalize_appearance(obs,obs_weights,model);
#endif
  return;
}
...
#define PSM_OPT_APPEARANCE_ESTIMATOR_INSTANTIATE(T) \
  template class psm_opt_appearance_estimator<T>
