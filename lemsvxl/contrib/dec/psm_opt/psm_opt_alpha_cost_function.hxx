...
#include "psm_opt_alpha_cost_function.h"
...
template<psm_apm_type APM>
void psm_opt_alpha_cost_function<APM>::f(vnl_vector<double> const& x, vnl_vector<double>& fx)
{
  double alpha = x[0];
  if (alpha < 0.0) {
    alpha = 0.0;
...
template<psm_apm_type APM>
void psm_opt_alpha_cost_function<APM>::gradf(vnl_vector<double> const&x, vnl_matrix<double> &J)
{
  double alpha = x[0];
  if (alpha < 0.0) {
    alpha = 0.0;
...
template <psm_apm_type APM>
void psm_opt_alpha_cost_function<APM>::cell_to_x(psm_sample<APM> const& cell, vnl_vector<double> &x)
{
  x[0] = cell.alpha;
}

template <psm_apm_type APM>
void psm_opt_alpha_cost_function<APM>::x_to_cell(vnl_vector<double> const& x, psm_sample<APM> &cell)
{
...
#define PSM_OPT_ALPHA_COST_FUNCTION_INSTANTIATE(T) \
  template class psm_opt_alpha_cost_function<T>
