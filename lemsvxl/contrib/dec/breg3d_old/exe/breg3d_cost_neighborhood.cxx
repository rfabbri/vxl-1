...
int main(int argc, char* argv[])
{
  ...
  std::vector<unsigned> camera_indices;
  ...
  for (unsigned c=0; c < camera_indices.size(); c++) {
    ...
    std::stringstream outfile_fname;
    outfile_fname << "d:/dec/matlab/reg3d/data/camera_scores_xyz_" << camera_idx <<".txt";
    std::ofstream fout(outfile_fname.str().c_str());
    if (!fout.is_open()) {
    ...
    for (unsigned dim = 0; dim < 6; dim++) {
      for (unsigned xi = 0; xi < nincs[dim]; xi++) {
        ...
        std::cout << "computing score for camera with dim " << dim << " offset by " << offset << "..";
