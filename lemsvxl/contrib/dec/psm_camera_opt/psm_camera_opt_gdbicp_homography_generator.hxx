#ifndef psm_camera_opt_gdbicp_homography_generator_txx_
#define psm_camera_opt_gdbicp_homography_generator_txx_
...
template<class T>
vimt_transform_2d psm_camera_opt_gdbicp_homography_generator<T>::compute_homography()
{
  ...
  vil_convert_stretch_range_limited<T>(img0_,img0_byte,0.0f,255.0f);
  vil_save(img0_byte,img0_fname.c_str());

  // save image1
  vil_image_view<vxl_byte> img1_byte(img1_.ni(),img1_.nj());
  vil_convert_stretch_range_limited<T>(img1_,img1_byte,0.0f,255.0f);
  ...
  if (retval) {
    std::cerr << "std::system(" << command << ") returned " << retval << "." << std::endl;
    std::cerr << "  make sure " << gdbicp_command << " is in your path." << std::endl;
  ...
  if (!vul_file::exists(output_fname)) {
    std::cerr << "error: output file " << output_fname << " does not exist!" << std::endl;
    vul_file::change_directory(og_dir);
    return vimt_transform_2d();
  }
...
template<class T>
vimt_transform_2d psm_camera_opt_gdbicp_homography_generator<T>::parse_gdbicp_output(std::string filename)
{
  ...
  if (compute_projective_) {
    ...
    ifs >> H_centered;
    std::cout << "H_centered = " << std::endl << H_centered << std::endl;
    ifs >> cx >> cy >> dx >> dy;
  }else {
...
#define PSM_CAMERA_OPT_GDBICP_HOMOGRAPHY_GENERATOR_INSTANTIATE(T) \
template class psm_camera_opt_gdbicp_homography_generator<T >
