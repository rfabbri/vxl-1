...
private:
  ...
  // IO functions
  friend std::ostream&  operator<<(std::ostream& s, psm_camera_opt_ekf_state const& ekf_state);

};

//: output description of state to stream.
std::ostream&  operator<<(std::ostream& s, psm_camera_opt_ekf_state const& ekf_state);
