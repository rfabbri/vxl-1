...
class psm_cell_enumerator {
  ...
  typedef std::vector<hsds_fd_tree_node_index<3> > cell_list_type;
  typedef std::map<vgl_point_3d<int>, cell_list_type, vgl_point_3d_cmp<int> > block_map_type;
  typedef std::map<vgl_point_3d<int>, unsigned int, vgl_point_3d_cmp<int> > block_offset_map_type;
...
public:
  ...
  psm_cell_enumerator(psm_scene<APM> &scene)
  {
    ...
    for (; block_it != scene.block_index_end(); ++block_it) {
      ...
      std::vector<hsds_fd_tree_node_index<3> > block_cell_list;
      ...
      // sort cell list
      std::sort(block_cell_list.begin(), block_cell_list.end());
      // insert into map
      enum_map_.insert(std::make_pair<vgl_point_3d<int>, cell_list_type >(*block_it, block_cell_list));
    } 
  }
  ...
  bool index_of(vgl_point_3d<int> block_idx, hsds_fd_tree_node_index<3> cell_idx, unsigned int& cell_index) const
  {
    ...
    for (; block_it != enum_map_.end(); ++block_it) {
      if (block_it->first == block_idx) {
        cell_list_type::const_iterator cell_it = std::lower_bound(block_it->second.begin(), block_it->second.end(), cell_idx);
