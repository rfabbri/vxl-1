...
public:
  static hsds_fd_tree<psm_sample<sample_type>,2> xz_slice(vgl_point_3d<double> pt, psm_scene<sample_type>& scene)
  {
    ...
    std::map<hsds_fd_tree_node_index<2>, psm_sample<sample_type> > alpha_xz;
    ...
    //std::cout << "xz_mask = 0x" << std::hex << xz_mask << std::endl;
    ...
    for (; block_it != block.end(); ++block_it) {
      ...
      //std::cout << "  node index = " << block_it->first << std::endl;
      if ((pt_index.idx & xz_mask) >> level_shift == (block_it->first.idx & xz_mask) >> level_shift) {
        //std::cout << "********* node is a match ***********" << std::endl;
        //std::cout << block_it->first << std::endl;
        ...
        alpha_xz.insert(std::make_pair<hsds_fd_tree_node_index<2>, psm_sample<sample_type>  >(quad_index,block_it->second));
      }
    }
