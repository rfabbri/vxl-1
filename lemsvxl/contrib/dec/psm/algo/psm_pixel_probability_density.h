...
public:
  //: default constructor
  psm_pixel_probability_functor(vpgl_camera<double> const* cam, vil_image_view<typename psm_apm_traits<APM>::obs_datatype> const& image, vil_image_view<float> &prob) 
    : cam_(cam), obs_(image), prob_(prob),
...
template <psm_apm_type APM>
void psm_pixel_probability_density(psm_scene<APM> &scene, vpgl_camera<double> const* cam, vil_image_view<typename psm_apm_traits<APM>::obs_datatype> const& img, vil_image_view<float> &probabilities, bool black_background = false)
{
  ...
  psm_pixel_probability_functor<APM> pix_prob_functor(cam, img, probabilities);
