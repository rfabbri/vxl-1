#ifndef psm_update_2pass_aa_h_
#define psm_update_2pass_aa_h_
...
class psm_update_aa_functor_pass1
{
public:
  //: default constructor
  psm_update_aa_functor_pass1(vpgl_perspective_camera<double> const& cam, vil_image_view<typename psm_apm_traits<APM>::obs_mathtype> &image) 
    : cam_(cam), obs_(image), 
    PI_img_(image.ni(),image.nj(),1), PI_img_weights_(image.ni(), image.nj(),1), alpha_img_(image.ni(),image.nj(),1), alpha_integral_(image.ni(),image.nj(),1), vis_(image.ni(), image.nj(),1), vis_end_(image.ni(),image.nj(),1), 
  ...
  inline bool step_cells(vgl_point_3d<int> const& block_idx, hsds_fd_tree<psm_sample<APM>,3> &block, hsds_fd_tree<psm_aux_traits<PSM_AUX_NULL>::sample_datatype,3> &aux_block, vcl_vector<hsds_fd_tree_node_index<3> > &cells)
  {
    ...
    for (; cell_it != cells.end(); ++cell_it) {
      ...
      alpha_seg_len_aa(xverts_2d, yverts_2d, vert_dists, visible_faces, cell_value.alpha, alpha_img_);       
      // get weighted mean of observation at cell
      typename psm_apm_traits<APM>::obs_datatype cell_mean_obs;
      if (cube_mean_aa(xverts_2d, yverts_2d, vert_dists, visible_faces, obs_, cell_mean_obs)) {
    ...
    // normalize PI_img
    vil_transform2(PI_img_weights_, PI_img_, safe_norm_functor_);
...
private:
  ...
  vil_image_view<float> PI_img_weights_;
...
template <psm_apm_type APM>
class psm_update_aa_functor_pass2
{
public:
  //: default constructor
  psm_update_aa_functor_pass2(vpgl_perspective_camera<double> const& cam, vil_image_view<typename psm_apm_traits<APM>::obs_mathtype> &image, vil_image_view<float> &norm_img) 
  ...
  inline bool step_cells(vgl_point_3d<int> const& block_idx, hsds_fd_tree<psm_sample<APM>,3> &block, hsds_fd_tree<psm_aux_traits<PSM_AUX_NULL>::sample_datatype,3> &aux_block, vcl_vector<hsds_fd_tree_node_index<3> > &cells)
  {
    ...
    for (; cell_it != cells.end(); ++cell_it) {
      ...
      if (cube_mean_aa(xverts_2d, yverts_2d, vert_dists, visible_faces, vis_, cell_mean_vis)) {
    ...
    for (cell_it = cells.begin(); cell_it != cells.end(); ++cell_it) {
      ...
      if (cube_mean_aa(xverts_2d, yverts_2d, vert_dists, visible_faces, update_factor_, mean_update_factor)) {
        // update alpha value
...
template <psm_apm_type APM>
void psm_update_aa(psm_scene<APM> &scene, vpgl_perspective_camera<double> const& cam, vil_image_view<typename psm_apm_traits<APM>::obs_mathtype> &img, bool black_background = false)
{
  ...
  psm_parallel_raytrace_function<psm_update_aa_functor_pass1<APM>, APM> raytrace_fn1(scene, cam, img.ni(), img.nj());
  psm_update_aa_functor_pass1<APM> up_functor1(cam, img);
  up_functor1.set_background_model(background_apm);

  vcl_cout << "update_aa: pass1" << vcl_endl;
  ...
  psm_parallel_raytrace_function<psm_update_aa_functor_pass2<APM>, APM> raytrace_fn2(scene, cam, img.ni(), img.nj());
  psm_update_aa_functor_pass2<APM> up_functor2(cam, img, norm_img);
  up_functor2.set_background_model(background_apm);

  vcl_cout << "update_aa: pass2" << vcl_endl;
