...
bool psm_sample_backprojection(psm_scene<APM> &scene, vil_image_view<typename psm_apm_traits<APM>::obs_datatype> const& img, const vpgl_camera<double>* cam, float i, float j, float ij_sigma, bool use_appearance, std::vector<vgl_point_3d<double>> &samples, unsigned int n_img_samples = 10, unsigned int n_samples_per_ray = 100)
{
  ...
  for (unsigned int n=0; n<n_img_samples; ++n) {
    ...
    std::cout << "i = " << rand_i << " j = " << rand_j << std::endl;
    
    // generate a ray probe at this pixel location
    std::vector<float> depth_vals, alpha_vals, appearance_prob_vals;
    ...
    std::vector<float> cdf(depth_vals.size());
    double alpha_int = 0;
    cdf[0] = 0;
    for (unsigned int x=0; x<cdf.size()-1; ++x) {
      float vis = (float)std::exp(-alpha_int);
      double seg_len = depth_vals[x+1] - depth_vals[x];
      if (seg_len <= 0) {
        std::cout << "error : depth_val[" << x+1 <<"] = " << depth_vals[x+1] << " depth_val[" << x << "] = " << depth_vals[x] << std::endl;
        //return false;
        seg_len = 0;
      }
      alpha_int += alpha_vals[x] * seg_len;
      if (alpha_vals[x] < 0) {
        std::cerr << "error: alpha_vals[" << x << "] = " << alpha_vals[x] << std::endl;
        return false;
      }
      if (use_appearance) {
        cdf[x+1] = (float)(cdf[x] + vis * (1 - std::exp(-alpha_vals[x]*seg_len)) * appearance_prob_vals[x]);
      }
      else {
        cdf[x+1] = (float)(cdf[x] + vis * (1 - std::exp(-alpha_vals[x]*seg_len)));
      }
      if (cdf[x+1] < 0) {
        std::cerr << "error: cdf[" << x+1 << "] = " << cdf[x+1] << std::endl;
        return false;
      }
