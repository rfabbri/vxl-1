...
namespace psm_sample_backprojection_process_globals
{
 ...
 const std::string param_n_image_samples_ =  "n_image_samples";
 const std::string param_n_samples_per_ray_ = "n_samples_per_ray";
}
...
bool psm_sample_backprojection_process(bprb_func_process& pro)
{
  ...
  // get the parameters
   ...
   std::cout << "n_image_samples = " << n_img_samples << std::endl;
   std::cout << "n_samples_per_ray = " << n_samples_per_ray << std::endl;
  ...
  std::string output_filename = pro.get_input<std::string>(7);
  ...
  switch (apm_type) {
    ...
    default:
      std::cerr << "error - psm_sample_backprojection_process: unknown appearance model type " << apm_type << std::endl;
      return false;
  }
  ...
  std::vector<vgl_point_3d<double> >::const_iterator pit = points.begin();
  //ofs << points.size() << std::endl;
  for (; pit != points.end(); ++pit) {
    ofs << pit->x() << " " << pit->y() << " " << pit->z() << std::endl;
  }
