//This is lemsvxl/contrib/dec/psm/pro/processes/psm_pixel_probability_density_process.cxx
//:
// \file
// \brief A process for computing the probability density value of each pixel in an image
...
#include <psm/algo/psm_pixel_probability_density.h>

namespace psm_pixel_probability_density_process_globals
{
...
bool psm_pixel_probability_density_process_cons(bprb_func_process& pro)
{
  using namespace psm_pixel_probability_density_process_globals;
...
//: Execute the process
bool psm_pixel_probability_density_process(bprb_func_process& pro)
{
  using namespace psm_pixel_probability_density_process_globals;
  ...
  switch (apm_type) {
    case PSM_APM_MOG_GREY:
      {
        ...
        psm_pixel_probability_density(*scene, camera.ptr(), *img, *prob_img, use_black_background);
    ...
    case PSM_APM_MOG_RGB:
      {
        ...
        psm_pixel_probability_density(*scene, camera.ptr(), img_rgb, *prob_img, use_black_background);
    ...
    default:
      vcl_cerr << "error - psm_pixel_probability_density_process: unknown appearance model type " << apm_type << vcl_endl;
      return false;
  }
