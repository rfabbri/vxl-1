// This is /lemsvxl/brcv/seg/dbsks/algo/dbsks_detect_xgraph_course_to_fine.cxx.
...
#include "dbsks_detect_xgraph_course_to_fine.h"
...
//------------------------------------------------------------------------------
//:
bool dbsks_detect_xgraph_course_to_fine::
execute()
{
    std::cout << "\n|========================================================|"
    ...
    for (unsigned i_scale =0; i_scale < xgraph_scales.size(); ++i_scale)
    {
        ...
        std::string storage_foldername = "scale_" + vul_sprintf("%d", vnl_math_rnd(xgraph_scales[i_scale]));
        ...
        std::vector<dbsks_det_desc_xgraph_sptr > course_dets; //> List of detections for this scale
        std::vector<dbsks_det_desc_xgraph_sptr > fine_dets; //> List of detections for this scale
        if (dbsks_load_detections_from_folder(storage_folder, course_dets))
        {
        ...
        else
        {
            ...
            course_dets.clear();
            this->run_course_detection_on(actual_edgemap, actual_xgraph, min_accepted_confidence,
                    storage_folder, course_dets);

            //> Dump the detections to a folder for backing up
            std::cout << "\n> Saving course detections of selected scale to dump folder = "
                    << storage_folder << "\n";
            {
                ...
                std::string det_group_id = xgraph_name + "+cdet+" + object_id + "+" + storage_foldername;
                std::string model_category = "";

                // create a binary image from the edgemap
                vil_image_view<vxl_byte > bg_view;
                dbsks_detect_xgraph_course_to_fine::convert_edgemap_to_bw(actual_edgemap, bg_view);

                // save detection to disk
                dbsks_save_detections_to_folder(course_dets, object_id, model_category,
        ...
        //> Detect xgraph with the specified scale
        fine_dets.clear();
        this->run_fine_detection_on(actual_edgemap, actual_xgraph, min_accepted_confidence,
                storage_folder, course_dets, fine_dets);

        //> Dump the detections to a folder for backing up
        std::cout << "\n> Saving fine detections of selected scale to dump folder = "
                << storage_folder << "\n";
        {
            // form a unique id for this group of detections
            std::string xgraph_name = vul_file::strip_extension(vul_file::strip_directory(xgraph_file));
            std::string det_group_id = xgraph_name + "+fdet+" + object_id + "+" + storage_foldername;
            std::string model_category = "";
            ...
            // save detection to disk
            dbsks_save_detections_to_folder(fine_dets, object_id, model_category,
                    det_group_id, bg_view, storage_folder, "");
        }
        ...
        for (unsigned m =0; m < fine_dets.size(); ++m)
        {
            fine_dets[m]->xgraph()->scale_up(0, 0, 1.0 / actual_pyramid_scale);
            fine_dets[m]->compute_bbox();
        }
        ...
        << "\n  # detections:  " << fine_dets.size() << "\n";

        // Save detections to the global list
        raw_dets_all_scales.insert(raw_dets_all_scales.end(), fine_dets.begin(), fine_dets.end());
        std::cout
        << "\n-------------------------------------------------------------------\n";
    ...
    //if (run_nms_based_on_overlap) FIRAT
    if(true)
    {
        std::cout
...
load_params_and_models()
{
    ...
    // ccm model
    dbsks_load_xgraph_ccm_model(xgraph_course_ccm_file, xgraph_course_ccm);

    // Check compatibility between Contour Chamfer Matching model and xgraph (are all edges covered)
    std::cout << ">> Checking compatibility between course CCM model and xgraph...";
    if (!xgraph_course_ccm->is_compatible(xgraph_prototype_))
    {
        std::cout << "Failed\n." << std::endl;
        return false;
    }
    ...
    if (!xgraph_course_ccm->override_cfrag_with_constant_distribution(cfrag_list_to_ignore))
    {
        std::cout << "[ Failed ]\n";
        return false;
    }
    ...
    xgraph_course_ccm->compute_cache_loglike_for_all_edges();

    //: Construct biarc sampler from xgraph_ccm model
    xgraph_course_ccm->build_biarc_sampler(this->course_biarc_sampler);
    this->course_biarc_sampler.compute_cache_sample_points();
    this->course_biarc_sampler.compute_cache_nkdiff();

    //

    // ccm model
    dbsks_load_xgraph_ccm_model(xgraph_fine_ccm_file, xgraph_fine_ccm);

    // Check compatibility between Contour Chamfer Matching model and xgraph (are all edges covered)
    std::cout << ">> Checking compatibility between fine CCM model and xgraph...";
    if (!xgraph_fine_ccm->is_compatible(xgraph_prototype_))
    {
        std::cout << "Failed\n." << std::endl;
        return false;
    }
    ...
    // Set distributions of user-selected boundary fragments to constant
    std::cout << "\n>> Overriding 'ignored' edges with constant distribution...";
    if (!xgraph_fine_ccm->override_cfrag_with_constant_distribution(cfrag_list_to_ignore))
    {
        std::cout << "[ Failed ]\n";
        return false;
    }
    ...
    //: Compute cache values for ccm models
    xgraph_fine_ccm->compute_cache_loglike_for_all_edges();

    //: Construct biarc sampler from xgraph_ccm model
    xgraph_fine_ccm->build_biarc_sampler(this->fine_biarc_sampler);
    this->fine_biarc_sampler.compute_cache_sample_points();
    this->fine_biarc_sampler.compute_cache_nkdiff();
        ...
        dbsksp_xshock_graph_sptr& actual_xgraph)
{
    ...
    double target_pyramid_scale = this->xgraph_course_ccm->base_xgraph_size() / target_xgraph_size; //
    ...
    double actual_edgemap_scale;
    {
        ...
        {
            scale_diff[k] = vnl_math_abs(std::log(this->list_edgemap_scale[k] / target_edgemap_scale));
        }
...
bool dbsks_detect_xgraph_course_to_fine::
run_course_detection_on(const dbdet_edgemap_sptr& edgemap,
        ...
        std::vector<dbsks_det_desc_xgraph_sptr >& dets)
{
    ...
    ccm_like.set_biarc_sampler(&this->course_biarc_sampler);
    ccm_like.set_ccm_model(xgraph_course_ccm);
    std::cout << " [ OK ]\n";
    ...
    for (unsigned iw =0; iw < windows.size(); ++iw)
    {
        ...
        //> Name of folder to store detect records from this scale
        std::string storage_dirname = "cdet_ROI" +
                vul_sprintf("_%d_%d_%d_%d", window.min_x(), window.min_y(), window.max_x(), window.max_y());
        std::string storage_dir = work_dir + "/" + storage_dirname;
        ...
        {
            std::cout << "  This window has been processed. Loading back detection records...";
            raw_dets_all_windows.insert(raw_dets_all_windows.end(), dets_window.begin(), dets_window.end());
            std::cout << " Done. Move on to next window.\n";
            continue;
        }
                ...
                << storage_dir << "\n";
        {
            // form a unique id for this group of detections
            std::string xgraph_name = vul_file::strip_extension(vul_file::strip_directory(xgraph_file));
            std::string det_group_id = xgraph_name + "+cdet+" + object_id + "+" + storage_dirname;
            std::string model_category = "";

            // create a binary image from the edgemap
            vil_image_view<vxl_byte > bg_view;
            dbsks_detect_xgraph_course_to_fine::convert_edgemap_to_bw(edgemap, bg_view);
...
//: Perform detection on a specific edgemap
bool dbsks_detect_xgraph_course_to_fine::
run_fine_detection_on(const dbdet_edgemap_sptr& edgemap,
        const dbsksp_xshock_graph_sptr& xgraph,
        double confidence_lower_threshold,
        const std::string& work_dir,
        ...
        std::vector<dbsks_det_desc_xgraph_sptr >& dets)
{
    ...
    int width_edgmap = edgemap->width();
    int height_edgmap = edgemap->height();
    for (unsigned m =0; m < course_dets.size(); ++m)
    {
        vsol_box_2d_sptr bb = course_dets[m]->bbox();

        int min_x = bb->get_min_x() - 0.05*bb->width();
        int max_x = bb->get_max_x() + 0.05*bb->width();
        int min_y = bb->get_min_y() - 0.05*bb->height();
        int max_y = bb->get_max_y() + 0.05*bb->height();
        if (min_x < 0)
        {
            min_x = 0;
        }
        else if(min_x >= width_edgmap)
        {
            min_x = width_edgmap - 1;
        }
        ...
        else if(max_x >= width_edgmap)
        {
            max_x = width_edgmap - 1;
        }
        ...
        else if(min_y >= height_edgmap)
        {
            min_y = height_edgmap - 1;
        }
        ...
        else if(max_y >= height_edgmap)
        {
            max_y = height_edgmap - 1;
        }
        vgl_box_2d<int > detection_bb(min_x, max_x, min_y, max_y);
        windows.push_back(detection_bb);
    }
    ...
    dbsks_xshock_ccm_likelihood ccm_like;
    ccm_like.set_edgemap(edgemap);
    ccm_like.set_biarc_sampler(&this->fine_biarc_sampler);
    ccm_like.set_ccm_model(xgraph_fine_ccm);
    std::cout << " [ OK ]\n";
    ...
    std::cout << "\n> Total #course detection windows = " << windows.size() << "\n";
    std::cout << "\n> List of windows: \n";
    for (unsigned iw =0; iw < windows.size(); ++iw)
    {
        vgl_box_2d<int > window = windows[iw];
    ...
    for (unsigned iw =0; iw < windows.size(); ++iw)
    {
        ...
        //> Name of folder to store detect records from this scale
        std::string storage_dirname = "fdet_ROI" +
                vul_sprintf("_%d_%d_%d_%d", window.min_x(), window.min_y(), window.max_x(), window.max_y());
        std::string storage_dir = work_dir + "/" + storage_dirname;
        ...
        for (unsigned i =0; i < engine.list_solutions_.size(); ++i)
        {
            ...
            // only consider detetion with at least minimal confidence level
            if (real_confidence >= course_dets[iw]->confidence())
            {
                dbsks_det_desc_xgraph_sptr det = new dbsks_det_desc_xgraph(sol_xgraph, real_confidence);
                det->compute_bbox();
                dets_window.push_back(det);
                ...
                << storage_dir << "\n";
        {
            // form a unique id for this group of detections
            std::string xgraph_name = vul_file::strip_extension(vul_file::strip_directory(xgraph_file));
            std::string det_group_id = xgraph_name + "+fdet+" + object_id + "+" + storage_dirname;
            std::string model_category = "";
