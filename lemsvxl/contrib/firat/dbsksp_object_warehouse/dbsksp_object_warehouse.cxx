...
dbsksp_object_warehouse::dbsksp_object_warehouse(const std::string& exemplar_xgraph_folder, const std::string& exemplar_xgraph_list_file)
{
	...
	{
		std::string xml_file = exemplar_xgraph_folder + "/" + xml_filenames[i];
		dbsksp_xshock_graph_sptr xg;
		if(!x_read(xml_file, xg))
		{
			std::cout << "Reading exemplar file failed!";
			continue;
		}
		dbsksp_xshock_graph::vertex_iterator vit = xg->vertices_begin();
		dbsksp_xshock_node_sptr root_node = xg->node_from_id((*vit)->id());
		xg->similarity_transform(root_node->pt(), 0, 0, 0, 100 / std::sqrt(xg->area()));
		this->exemplar_xgraphs_.push_back(xg);
		this->child_xgraphs_.push_back(new std::vector<dbsksp_xshock_graph_sptr>);
	}
...
bool dbsksp_object_warehouse::save(const std::string& folder)
{
	for(int i = 0; i < this->exemplar_xgraphs_.size(); i++)
	{
		std::stringstream ss1 (std::stringstream::in | std::stringstream::out);
		ss1 << folder << "/exemplar_" << i+1 << ".xml";
		std::string exemplar_file_path;
		ss1 >> exemplar_file_path;
		std::cout << "Saving:" << exemplar_file_path << std::endl;
		x_write(exemplar_file_path, this->exemplar_xgraphs_[i]);
		std::vector<vsol_point_2d_sptr> point_list =  dbsksp_get_boundary_point_list(this->exemplar_xgraphs_[i], 400);
		std::string exemplar_contour_file = exemplar_file_path;
		exemplar_contour_file.replace(exemplar_contour_file.length()-3, 3, "txt");
		std::cout << "Saving:" << exemplar_contour_file << std::endl;
		...
		for(int j = 0; j < this->child_xgraphs_[i]->size(); j++)
		{
			...
			ss2 >> child_file_path;
			std::cout << "Saving:" << child_file_path << std::endl;
...
dbsksp_object_warehouse* dbsksp_object_warehouse::load(const std::string& folder)
{
	return 0;
}
...
void dbsksp_object_warehouse::save_contour_file_(std::vector<vsol_point_2d_sptr>& point_list, const std::string& filename)
{
	std::ofstream ofs(filename.c_str());
	ofs << point_list.size() << " 0" << std::endl;
	for(int i = 0; i < point_list.size(); i++)
	{
		ofs << point_list[i]->x() << " " << point_list[i]->y() << std::endl;
	}
	ofs.close();
}
