...
//BotContour methods added by AVP.
bfrag_curve bfrag2D::getBotContourAsCurve(){
 return botContourCurve;

}
bfrag2D::bfrag2D(){

  //std::cout << "made a new UNINDEXED bfrag2D" << std::endl;
...
bfrag2D::bfrag2D( int index, std::string s ){

  myName = s;
  myIndex = index;

  //std::cout << "MADE NEW BFRAG2D index: here" << myIndex << std::endl;
  ...
  //bool orange=false;
//  bool &a1=&apple;
//bool &a2=&orange;//bool *a3=&orange;
//popupInfo(*apple,*orange,*orange);

}
//constructor. Not much done. just basic initializations
void bfrag2D::popupInfo(bool &drawTopImage, bool &drawBotCurve, bool &drawBotImage){
std::cout<<"pop up thingy ran"<<std::endl;
...
bfrag2D::~bfrag2D()
{
  ...
  //std::cout << "deleted bfrag2D index: " << myIndex << std::endl;
...
*
*******************************************************************************/
int bfrag2D::loadContour( std::string fn, bool top ){

  //its ok if were tryin to load a blank bottom contour, but we MUST have top
  if( !top && fn.size() == 0 ) return 0;

 // std::cout << "bfrag [" << myIndex << "] loading contour \"" << fn;
  //if(top) std::cout << "\" as TOP contour" << std::endl;
  //else    std::cout << "\" as BOTTOM contour" << std::endl;
  ...
  if( strncmp( fn.c_str() + fn.size() - 4, ".con", 4 ) == 0 || 
    strncmp( fn.c_str() + fn.size() - 4, ".CON", 4 ) == 0 ){
      //std::cout << "Loading a \".con\" file: " << fn << std::endl;
...
//          botContourCurve.setTop(false);
    ...
    strncmp( fn.c_str() + fn.size() - 4, ".CEM", 4 ) == 0 ) {
      //std::cout << "Loading up a \".cem\" file: " << fn << std::cout;
...
//          botContourCurve.setTop(false);
    ...
    std::cout << "ERROR: \"" << fn << "\" is not a .cem or .con file! -Aborting-" << std::endl;
...
*
*******************************************************************************/
int bfrag2D::loadImage( std::string fn, bool top ){

  if( fn.size() == 0 ) return 0;

  //std::cout << "bfrag [" << myIndex << "] loading image \"" << fn;
  if(top){
    //std::cout << "\" as TOP image" << std::endl;

    topRGBImage = vil_load( fn.c_str() );
    if( topRGBImage->size() == 0 ){
      std::cout << "ERROR: image " << fn << " could not be loaded." << std::endl;
      return -1;
    }
    ...
    //std::cout << "Image just loaded: " << fn << " has [" << topRGBImage.nplanes() << "planes." << std::endl;


  } else {
    //std::cout << "\" as BOTTOM image" << std::endl;
...
int bfrag2D::loadCEM( std::string fn, vsol_polygon_2d_sptr &givenContour, bfrag_curve &givenCurve ){

  std::cout << "***WARNING*** loadCEM is not yet supported!" << std::endl;
  ...
  std::ifstream infp(fn.c_str(), std::ios::in);

  if (!infp){
    std::cout << "Error opening file  " << fn.c_str() << std::endl;
    return -1;
  }
...
int bfrag2D::loadCON( std::string fn, vsol_polygon_2d_sptr &givenContour, bfrag_curve &givenCurve ){
...
{
  _maxX = static_cast<int>(std::floor(topContourCurve.max_x()+0.5));
  _minX = static_cast<int>(std::floor(topContourCurve.min_x()+0.5));
  _maxY = static_cast<int>(std::floor(topContourCurve.max_y()+0.5));
  _minY = static_cast<int>(std::floor(topContourCurve.min_y()+0.5));

  _centerX = static_cast<int>(std::floor(((_maxX - _minX) * 0.5) + _minX+0.5));
  _centerY = static_cast<int>(std::floor(((_maxY - _minY) * 0.5) + _minY+0.5));
}
