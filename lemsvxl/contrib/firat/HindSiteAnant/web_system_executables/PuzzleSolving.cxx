...
{ 
  ...
  //_nPieces = _Contours.size();Assuming for rite now that all pieces hv both top nd bottom contours.
  //If this has not been changed so far plz try and implement a better system
};
...
void PuzzleSolving::preProcessCurves()
{
  ...
  for(int i = 0; i < _nPieces; i++)
  {
	...
	/*_BotContours[i].smooth_by_discrete_curvature_algo(float(PSI), NUM_TIMES);
...
//        break;
      ...
      else if(chk1 != chk2 && (p2>=_nPieces)==flag)// OYE YAAD RAKHIYO
      {
        intMap fmap;
...
//So basically all matches are stored in a global vector. Eachstate merely has an index to the match in the global list and its cost.
  _numMatch++;
...
{  
  ...
  {
    p1.push_back(c1->point(map[i].first));//point here refers to index of level3 point
