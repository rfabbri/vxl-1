...
std::string FRAG_PAIRS_OUT_FILENAME;
std::vector<std::string> FRAG_ASSEMBLY_FILENAMES;
...
{//Gets the parameters to run the pair matching and solve the puzzle. Called right at beginning of
  ...
  // open input file
  std::ifstream in(input.c_str());//Create an input file stream for para,s file
  // create xml reader
...
unsigned get_status()
{
  //std::cout<<"get status called"<<std::endl;
  std::string status_filename = OUTPUT_FOLDER;//sets output folder which it had read from params file
  status_filename.append("\\user\\status.txt");
  //std::cout<<status_filename+"crapass"<<std::endl;
  std::ifstream in(status_filename.c_str());
  if(in == NULL)
  {//this means if file not there already. Sometimes it maybe. 
    ...
    std::string pwm_output = OUTPUT_FOLDER;
	std::cout << "entered in==null" << std::endl;
    pwm_output.append("\\user\\iter0\\output.xml");//if no output folder exists create iter0. That is original state of fragments.
    std::ifstream pwm_in(pwm_output.c_str());
    if(pwm_in == NULL)
	{std::cout<<"entered status = -1"<<std::endl;
	return -1;}
	else{
		std::cout<<"entered status = -2"<<std::endl;
		return -2;}
  }
  else
  {
...
void write_status()
{
  // write status file for future use. 
	std::cout<<"crappy"<<std::endl;
  std::string status_filename = OUTPUT_FOLDER;
  status_filename.append("\\user\\status.txt");
  std::ofstream out(status_filename.c_str());
  out << "Number_of_Iterations_Finished: " << FINISHED_STEPS_SO_FAR << std::endl;
  out << "Number_of_Pieces_Added_to_the_Puzzle: " << FINISHED_STEPS_SO_FAR+1 << std::endl;
  out.close();
}
...

{
  for(unsigned i=0; i<FRAG_PATHS.size(); i++)
  {
...

    {
      ...
      {//std::cout<<"main aa gyacon"<<std::endl;
        std::string fname = folder;
        fname.append(f.filename());
...
{//As name suggests basically runs the pairwise matching.
  ...
  std::vector<bfrag_curve> bot_contours;
  ...
  std::vector<searchState> states;//SearchState is basically a name given to a moment in time during puzzle solving. 
  //Each iteration has a puzzle solving associated with it. That is what the visualizer displays.
  // set the contours
 // std::cout<<top_contours.size()<<std::endl;basically all contours(one each per fragment.)
  //std::cout<<"top_contours.size()"<<std::endl;
...
/*  std::vector<searchState> states;
  ...
  std::cout<<"Now do stuff in the GUI and close it."<<std::endl;
...
//char **a;
  ...
  {
    std::cout << "Specified number of iteration steps(" << NUMBER_OF_ITERATION_STEPS;
    std::cout << ") is too large for the given number of fragments(" << FRAG_IDS.size() << ")..." << std::endl;
    std::cout << "Setting it to the maximum possible value(" << FRAG_IDS.size()-1 << ")..." << std::endl;
...
//maingui(argc, argv);
    ...
    {
      //std::cout << "Running Iteration No: " << i+1 << std::endl;
	  ...
	  //vgui::run();
	//  maingui(argc,argv);
	  //std::cout<<"chuut volume 2"<<std::endl;
	// vgui::uninit();
	   //VisualizerGui *gui = new VisualizerGui(argc,argv);

    }
	 //std::cout<<"yyayayaya"<<std::endl;
  }
  else
  {
    ...
    if(FOLDERS.size() == 0)
    {
      ...
      {
        std::stringstream system_command;
		//std::cout << "AARRGGG3: "  << std::endl;
        system_command << OUTPUT_FOLDER << "\\user\\iter" << i;
        //std::cout << system_command.str() << std::endl;
        FOLDERS.push_back(system_command.str());
      }
    }
    std::cout << "Pairwise matching already done and " << status << " iteration(s) already run..." << std::endl;
    std::cout << "Running " << NUMBER_OF_ITERATION_STEPS-status << " iteration(s)..." << std::endl;
  ...
  std::cout << "Algorithm total execution time: " << (double(end)-start) / CLOCKS_PER_SEC << std::endl;
  std::cout << "Pairwise matching execution time: " << (double(end_pwm)-start_pwm) / CLOCKS_PER_SEC << std::endl;
  std::cout << "Puzzle solving iteration(s) execution time: " << total_psi << std::endl;
