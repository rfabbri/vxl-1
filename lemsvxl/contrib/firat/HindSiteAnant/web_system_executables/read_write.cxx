...
void PuzzleSolving::write_experiment_search_state_and_puzzle_solving_objects(std::string fname, std::vector<searchState> &states, bool write_matches)
{
 ...
 // std::cout<<states.size()<<std::endl;
  ...
  {//Basically this always seems to be 1. i.e the soze of the vector
	...
	//std::cout<<dump_state_file_full_path.str()<<std::endl;
...
//std::cout<<output_name.str()+"ppppp"<<std::endl;
...
void PuzzleSolving::read_experiment_search_state_and_puzzle_solving_objects(std::string fname, std::vector<searchState> &states, bool read_matches)
{
  ...
  // open input file
  //std::cout<<fname2+"this is the output file path"<<std::endl;
...
void PuzzleSolving::write_out_matches(std::string fname)//Creates the matches.mtc file.
{
  unsigned size;
  std::ofstream out(fname.c_str());
...
void PuzzleSolving::write_frag_assemblies_in_xml(std::string fname, std::vector<searchState> &states)
{
  if(fname.size() > 0)
  {
	 // std::cout<<"see this does work"<<std::endl;
	  //std::cout<<states.size()<<std::endl;
    bxml_document doc;
...
//    _matches[i].write_out(out);
  ...
  for(unsigned i=0; i < size; i++)
  {
    ...
    out << temp.size() << " / ";
	//std::cout<<"wow i do come here"<<std::endl;
