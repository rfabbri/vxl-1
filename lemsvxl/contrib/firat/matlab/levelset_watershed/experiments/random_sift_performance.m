function TP = random_sift_performance(sift_folder, save_decision_histogram)
	...
	for i = 1:num_files
		...
		desc{i} = double(X.rand_d);
		counts(i) = size(X.rand_d, 2);
	...
	for i = 1:num_files
		...
		compute_h = true;
		X = load([sift_folder '/' files(i).name], 'rand_h');
		if isfield(X, 'rand_h') && length(X.rand_h) == num_unique_classes
			h = X.rand_h;
		else 
		    %%
		    q = desc{i}'; %query
			...
			if save_decision_histogram 
				rand_h = h;           
			    save([sift_folder '/' files(i).name], 'rand_h', '-append');
    		end
		end
		[ign, max_id] = max(h);
