function TP = saddle_sift_performance(sift_folder, top_percent, score_type)
	...
	for i = 1:num_files
		...
		all_desc = double(X.saddle_d);
		num_desc = size(all_desc, 2);
		num_to_keep = round(num_desc * top_percent);
		switch score_type
			case 'min'
				[ign, sort_index] = sort(X.saddle_s_min, 'descend');
			case 'gauss'
				[ign, sort_index] = sort(X.saddle_s_gauss, 'descend');
			case 'mean'
				[ign, sort_index] = sort(X.saddle_s_mean, 'descend');
			otherwise
