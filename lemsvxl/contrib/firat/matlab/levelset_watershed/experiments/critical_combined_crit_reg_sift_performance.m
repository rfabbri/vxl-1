...
function TP = critical_combined_crit_reg_sift_performance(crit_sift_folder, reg_sift_folder, percentage, score_type)
	...
	for i = 1:num_files
		...
		Y = load([reg_sift_folder '/' files(i).name]);
		...
		h4 = Y.reg_h;
		[ign, max_id] = max(h1/sum(h1) + h2/sum(h2) + h3/sum(h3) + h4/ sum(h4));
