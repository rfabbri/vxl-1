% This is /lemsvxl/contrib/firat/matlab/sussman_reinit/sussman3d.m.

% \file
% \author Firat Kalaycilar (firat_kalaycilar@brown.edu)
% \date Nov 21, 2011

function phi = sussman3d(phi0, h, phi_thresh)
    k = h/10;
    S = sign(phi0);
    first_time = true;
    phi = phi0;
    while first_time || continue_condition(phi_prev, phi, phi_thresh, .1)
        first_time = false;
        phi_prev = phi;
        a = (phi - circshift(phi, [0 1 0]))/h;
        a(:,1,:) = a(:,2,:);
		b = (circshift(phi, [0 -1 0]) - phi)/h;
		b(:,end,:) = b(:,end-1,:);
		c =  (phi - circshift(phi, [1 0 0]))/h;
		c(1,:,:) = c(2,:,:);
		d =  (circshift(phi, [-1 0 0]) - phi)/h;
		d(end,:,:) = d(end-1,:,:);
		e = (phi - circshift(phi, [0 0 1]))/h;
        e(:,:,1) = e(:,:,2);
		f = (circshift(phi, [0 0 -1]) - phi)/h;
		f(:,:,end) = f(:,:,end-1);
		[aplus,aminus] = get_parts(a);
		[bplus,bminus] = get_parts(b);
		[cplus,cminus] = get_parts(c);
		[dplus,dminus] = get_parts(d);
		[eplus,eminus] = get_parts(e);
		[fplus,fminus] = get_parts(f);
