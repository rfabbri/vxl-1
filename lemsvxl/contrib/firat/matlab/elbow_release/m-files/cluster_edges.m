...
function clusters = cluster_edges(edg, sthresh)
    Max = round(max(edg(:,1:3)));
    ncols = Max(1);
    nrows = Max(2);
    nbands = Max(3);
    num_entry = size(edg, 2);
    voxel_grid = cell(nrows, ncols, nbands);
	for i = 1:size(edg,1)		
		voxel_r = round(edg(i,2));
		voxel_c = round(edg(i,1));
		voxel_b = round(edg(i,3));
		if (num_entry == 6 || edg(i,7) >= sthresh) && voxel_r > 0 && voxel_c > 0 && voxel_b > 0 && voxel_r <= nrows && voxel_c <= ncols && voxel_b <= nbands
