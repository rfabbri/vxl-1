...
{
    std::cout << "Node id: " << root_node->id() << std::endl;
...
void generate_xgraph(int parent_edge, dbsksp_xshock_node_sptr root_node, dbsks_xgraph_geom_model_sptr xgeom, dbsksp_xshock_graph_sptr xgraph, double graph_size)
{
    ...
    for(;edgelist_iterator != root_node->edge_list().end(); ++edgelist_iterator)
    {
        ...
        std::map<unsigned, dbsks_xfrag_geom_model_sptr > map_edge2geom = xgeom->map_edge2geom();
...
int main()
{
    ...
    random_engine2 = vnl_random(std::time(NULL));
    std::string xgraph_geom_file = "/vision/projects/kimia/shockshape/symseg/results/ETHZ-dataset/xshock-graph-firat/applelogos/applelogos-geom-model-rv29-ppe30-mce31-09232010.xml";
    std::string xgraph_file = "/vision/projects/kimia/shockshape/symseg/results/ETHZ-dataset/xshock-graph/applelogos-xgraph/applelogos_another.xgraph.0.prototype1.xml";
    ...
    generate_xgraph(-1, root_node, xgeom, xgraph, std::sqrt(xgraph->area()));
    if(xgeom->is_compatible(xgraph))
    {
        std::cout << "Compatible" << std::endl;
    }
