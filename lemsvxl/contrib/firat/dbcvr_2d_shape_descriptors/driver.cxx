...
int main()
{
	...
	std::vector<vsol_point_2d_sptr> point_list =  dbsksp_get_boundary_point_list(xg, 1000);
	vsol_digital_curve_2d_sptr curve = new vsol_digital_curve_2d(point_list);
	curve = dbcvr_uniform_sampling_closed(curve, 81);

	/*std::ofstream ofs("/home/firat/Desktop/1.txt");
	ofs << curve->size() << " 0" << std::endl;
	...
	std::cout << alpha_prime(0,0) << " " << alpha_prime(1,42) << " " << alpha_prime(1,80) << std::endl;

	std::vector<vsol_point_2d_sptr> point_list2;
	...
	std::cout << "curve2 size: " << curve2->size() << std::endl;
	std::vector<dbcvr_curve_match> matches;
