...
dbdet_contour_tracer_xing* dbdet_trace_contour_given_first_two_points(dbdet_contour_tracer_grid& grid, dbdet_contour_tracer_xing* prev, dbdet_contour_tracer_xing* curr, std::vector<vsol_point_2d_sptr>& contour)
{
	...
	while(true)
	{
		...
		else
		{
			...
			std::vector<dbdet_contour_tracer_xing*> next_point_candidates;
			...
			{
				//std::cout << "Candidate " << i << " (" << next_point_candidates[i]->x << ", " << next_point_candidates[i]->y << ")" << std::endl;
				angle = find_angle_of_vector(curr, next_point_candidates[i]);
				diff = std::acos(std::cos(angle - src_angle));
