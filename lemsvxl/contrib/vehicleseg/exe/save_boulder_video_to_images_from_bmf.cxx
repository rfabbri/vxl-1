...
int main(int argc,char * argv[])
{
    if(argc!=4)
    {
        std::cout<<"Usage : .exe bmffile outputpath increment\n";
        return -1;
    }
    else
    {
        ...
        std::list<std::string> filenames=pmf.getlist();
        std::list<std::string>::iterator iter;
        vul_file fileprocess;
        std::string dirname=fileprocess.dirname(fname);
        int count=0;    
        int outercount=0;
        for(iter=filenames.begin();iter!=filenames.end();iter++)
        {
            std::string currvideo=dirname+"\\\\"+(*iter);
            ...
            for (int i=0;i<my_movie->length();i++){
                ...
                {    
                    ...
                    std::string filename = vul_sprintf("%s%05d.%s", outname.c_str(),outercount,"tif");
