...
int main (int argc, char *argv[]) 
{
  ...
  std::string output_file=params->input_dir_()+"/";

  if (!vul_file::exists(output_file)) 
    vul_file::make_directory(output_file);

  std::string nm_filename = output_file+"similarity_matrix.txt";
  std::string cm_filename = output_file+"cost_matrix.txt";
  std::string uo_filename = output_file+"unordered.txt";

  std::ofstream nameMatrix(nm_filename.c_str());

  if (!vul_file::exists(nm_filename))
  {
    std::cout << "Cannot create name matrix file" << "\n";
    return 0;
  }

  std::ofstream costMatrix(cm_filename.c_str());

  if (!vul_file::exists(cm_filename)) 
  {
    std::cout << "Cannot create cost matrix file" << "\n";
    return 0;
  }

  std::ofstream unOrdered(uo_filename.c_str());

  if (!vul_file::exists(uo_filename))
  {
    std::cout << "Cannot create unordered similarity matrix file" << "\n";
    return 0;
  }
...
//  unOrdered << std::endl;
  ...
  std::vector<std::vector<std::string> > name_array(number_of_shapes,std::vector<std::string>(number_of_shapes));
  std::vector<std::vector<double> > cost_array(number_of_shapes,std::vector<double>(number_of_shapes));
  ...
  for(int i=0; i<number_of_shapes; i++)
  {
    ...
    {
      std::cout << "Cannot find shock match result (.mshgm) files: " << input_result << "\n";
      return 0;
    }
    ...
    std::string current_cost_string;
    double current_cost;
    ...
    for(int j=0; j<number_of_shapes; j++)
    {
      ...
      std::stringstream temp(current_cost_string);
...
//    unOrdered << root->names()[i] << " ";
  ...
  {    
    ...
    std::map<double, std::string> similarity_row;

    for(int j=0; j<number_of_shapes; j++)
    {
        std::map<double, std::string>::iterator srit = similarity_row.find(cost_array[i][j]);
      ...
      std::pair<double, std::string> current_pair(cost_array[i][j], name_array[i][j]);
      similarity_row.insert(current_pair);
    }

    nameMatrix << root->names()[i] << " ";

    for(std::map<double, std::string>::iterator mit = similarity_row.begin(); mit!=similarity_row.end(); ++mit)
    {
