...
dbrl_rpm_tps_superimpose_process::dbrl_rpm_tps_superimpose_process(void): bpro1_process()
    ...
    if( !parameters()->add( "Initial temperature " , "-initT" , (float)1) ||
        ...
        {
        std::cerr << "ERROR: Adding parameters in dbrl_rpm_tps_superimpose_process::dbrl_rpm_tps_superimpose_process()" << std::endl;
        }
...
std::vector< std::string > dbrl_rpm_tps_superimpose_process::get_input_type()
    {
...
std::vector< std::string > dbrl_rpm_tps_superimpose_process::get_output_type()
    {  
    std::vector<std::string > to_return;
...
dbrl_rpm_tps_superimpose_process::execute()
    ...
    if ( input_data_.size() != 1 ){
        std::cout << "In dbrl_rpm_tps_superimpose_process::execute() - "
