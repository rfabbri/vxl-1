...
#include "dbrl_rpm_affine_process.h"
...
//: Constructor
dbrl_rpm_affine_process::dbrl_rpm_affine_process(void): bpro1_process()
{
    ...
    if( !parameters()->add( "Initial temperature" , "-initT" , (float)1) ||
        ...
        !parameters()->add( "Initial Lambda" , "-initlambda" , (float) 1.0 )||
        !parameters()->add( "Covergence Tolerance" , "-mconvg" , (float) 0.1 )||
        !parameters()->add( "Outlier value" , "-moutlier" , (float) 1e-5 )
        ) 
     {
        vcl_cerr << "ERROR: Adding parameters in dbrl_rpm_affine_process::dbrl_rpm_affine_process()" << vcl_endl;
     }
    
}

//: Destructor
dbrl_rpm_affine_process::~dbrl_rpm_affine_process()
{
...
int
dbrl_rpm_affine_process::input_frames()
{
  return 2;
}
...
int
dbrl_rpm_affine_process::output_frames()
{
  return 1;
}
...
vcl_vector< vcl_string > dbrl_rpm_affine_process::get_input_type()
{
...
vcl_vector< vcl_string > dbrl_rpm_affine_process::get_output_type()
{  
  vcl_vector<vcl_string > to_return;
...
//: Execute the process
bool
dbrl_rpm_affine_process::execute()
{
  if ( input_data_.size() != 2 ){
    vcl_cout << "In dbrl_rpm_affine_process::execute() - "
...
for(int i=0;i<static_cast<int>(list2.size());i++)
  ...
  dbrl_rpm_affine_params params(lambdainit,mconvg,Tinit,Tfinal,moutlier,annealrate);

    dbrl_rpm_affine affinerpm(params,feature_list1,feature_list2);
    dbrl_match_set_sptr outset=affinerpm.rpm();


    vcl_cout<<"Energy : "<<affinerpm.energy();
...
//: Clone the process
bpro1_process*
dbrl_rpm_affine_process::clone() const
{
  return new dbrl_rpm_affine_process(*this);
}
    
bool
dbrl_rpm_affine_process::finish()
{
  return true;
}
