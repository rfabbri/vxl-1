...
std::string dbrl_oriented_edgemap_to_edge_image_process::
name()
{
...
std::vector<std::string> dbrl_oriented_edgemap_to_edge_image_process::
get_input_type()
{
...
execute()
{
  ...
  std::vector<dbdet_edgel*> edgels = input_edgemap->edgels;
  ...
  for(unsigned i=0; i<num_edges; ++i)
  {
      ...
      dir_img((unsigned int)std::floor(current_pt.x()),(unsigned int)std::floor(current_pt.y()),0)=current_pt.x();
      dir_img((unsigned int)std::floor(current_pt.x()),(unsigned int)std::floor(current_pt.y()),1)=current_pt.y();
      dir_img((unsigned int)std::floor(current_pt.x()),(unsigned int)std::floor(current_pt.y()),2)=current_orientation;
  }
