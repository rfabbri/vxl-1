...
unsigned int intializemodel(int ni,int nj)
{
    ...
    std::ofstream ofile("d:\\data\\log.txt");
...
{
  std::ofstream ofile("d:/data/v1_data/loadmodel_log.txt");
  ofile<<filename<<"\n";
...
{
  std::cout<<"\n get polygons ...";
  std::ofstream ofile("d:/data/v1_data/trk_log.txt");
  ...
  std::vector<vsol_polygon_2d_sptr > * allpts=new std::vector<vsol_polygon_2d_sptr >();
...
std::vector<dbrl_id_point_2d_sptr> get_foreground_edges_from_observ(dbinfo_observation_sptr obs,dbbgm_image_of<mix_gauss_type> *model,
...
void track_polygons(unsigned int trkhndno,unsigned char *img_data, int ni,int nj,unsigned int polyhandle, float expmotion, int frameno)
{
    ...
    std::ofstream ofile("d:\\data\\trackinglog.txt");
    ...
    if(std::vector<vsol_polygon_2d_sptr > * polys=
        reinterpret_cast<std::vector<vsol_polygon_2d_sptr > *> (polyhandle))
    {

        ofile<<"Size of polys "<<polys->size()<<"\n";
        if(polys->size()<=0)
        {
            std::cout << "no detections in frame no. "<<frameno<<"\n";
            return ;
        }

        std::vector<dbinfo_observation_sptr> curr_obs;
        //: obtain the detected polygons
        for(std::vector< vsol_polygon_2d_sptr >::iterator dit =
        ...
        std::vector<unsigned> assignment=vnl_hungarian_algorithm(cost_matrix);
...
unsigned int load_tracks(char * filename)
{
  ...
  std::vector<dbinfo_track_sptr> tracks = track_storage_->tracks();
  std::vector<dbinfo_mi_track_sptr> * mi_tracks=new std::vector<dbinfo_mi_track_sptr>();
...
{
    std::vector<dbinfo_mi_track_sptr> * tracks=reinterpret_cast<std::vector<dbinfo_mi_track_sptr> *>(trkhandle);
    dbbgm_image_of<mix_gauss_type> *emodel = reinterpret_cast<dbbgm_image_of<mix_gauss_type>*>(edgehandle);

    vil_image_view<unsigned char> grey_img(img_data,ni,nj,1,1,ni,ni*nj);
    vil_image_resource_sptr im=vil_new_image_resource_of_view(grey_img);

    std::ofstream ofile("d:\\data\\clip_info.txt");
    for(std::vector<dbinfo_mi_track_sptr>::iterator trit = tracks->begin();
        trit != tracks->end(); trit++)
    {
        if(frameno>=(*trit)->start_frame() && frameno<=(*trit)->end_frame())
        {
            ...
            std::vector<dbrl_id_point_2d_sptr> edgels= get_foreground_edges_from_observ(ref_obs,emodel,bgdist, bgminweightthresh,
...
std::vector<dbrl_feature_sptr> get_grid_points(std::vector<dbrl_feature_sptr> f2,
                                                                                     double spacing)
{
...
compute_convex_hull( std::vector<vsol_line_2d_sptr > edges)
{
    if(edges.size()<=0)
        return new vsol_polygon_2d();
...
unsigned int superimpose_edges(unsigned int trkhandle, unsigned char *img_data,float Tinit,
                               ...
                               //float bgdist,float bgminweightthresh,int bgrad,float bgthresh, float polydilate)
{
    std::vector<vsol_polygon_2d_sptr> * polygons=new std::vector<vsol_polygon_2d_sptr>();
    std::vector<dbru_multiple_instance_object* > * multiple_objects=new std::vector<dbru_multiple_instance_object* >();

    vil_image_view<unsigned char> grey_img(img_data,ni,nj,1,1,ni,ni*nj);

    //dbbgm_image_of<mix_gauss_type> *emodel = reinterpret_cast<dbbgm_image_of<mix_gauss_type>*>(emodelandle);
    double gridspace=0.2;
    std::ofstream ofile("d:\\data\\superlog.txt");
    if(std::vector<dbinfo_mi_track_sptr> * tracks_
        =reinterpret_cast<std::vector<dbinfo_mi_track_sptr> *>(trkhandle))
    {
        //std::vector<dbinfo_mi_track_sptr> tracks_;
        //tracks_=tracker_->tracks();
        ofile<<"\n Number of tracks are"<<tracks_->size()<<"\n";
        for(std::vector<dbinfo_mi_track_sptr>::iterator trit = tracks_->begin();
            trit != tracks_->end(); trit++)
        {
            ...
            if(frame_no>=(*trit)->start_frame() && frame_no<=(*trit)->end_frame())
            {
                ...
                if(ref_obs.ptr())
                {
                    ...
                    std::vector<dbrl_id_point_2d_sptr>  pivot_pointedges;
                    std::vector<vgl_point_2d<double> > ps=ref_obs->get_edge_points();
                    std::vector<double> ds=ref_obs->get_edge_dirs();
                    for(int icnt=0;icnt<ps.size();icnt++)
                    {
                        pivot_pointedges.push_back(new dbrl_id_point_2d(ps[icnt].x(),ps[icnt].y(),ds[icnt],icnt));
                    }
                    //std::vector<dbrl_id_point_2d_sptr>  pivot_pointedges=get_foreground_edges_from_observ(ref_obs,emodel,bgdist,bgminweightthresh,bgrad,bgthresh,polydilate);
                            ...
                            if(i>=(*trit)->start_frame() && i<=(*trit)->end_frame() && i!=frame_no)
                            {
                                ...
                                if(temp_obs.ptr())
                                {
                                    if(temp_obs->obs_snippet().ptr())
                                    {
                                        ...
                                        std::vector<vgl_point_2d<double> > cps=temp_obs->get_edge_points();
                                        std::vector<double> cds=temp_obs->get_edge_dirs();
                                        for(int icnt=0;icnt<cps.size();icnt++)
                                        {
                                            curr_pointedges.push_back(new dbrl_id_point_2d(cps[icnt].x(),cps[icnt].y(),cds[icnt],icnt));
                                        }

                                        //std::vector<dbrl_id_point_2d_sptr> curr_pointedges=get_foreground_edges_from_observ(temp_obs,emodel,bgdist,bgminweightthresh,bgrad,bgthresh,polydilate);
                                        ...
                                        cubictform->transform();
                                        std::vector<dbrl_feature_sptr> xfomredgridpoints=cubictform->get_to_features();
                                        ...
                                        for(unsigned m=0;m<gridpoints.size();m++)
                                        {
                                            ...
                                            if(dbrl_feature_point* pt=dynamic_cast<dbrl_feature_point*>(xfomredgridpoints[m].ptr()))
                                            {
                                                ...
                                                int p1x=std::floor(p1->x())-exroi->cmin(0);
                                                int p1y=std::floor(p1->y())-exroi->rmin(0);
                                                ...
                                                {
                                                    intensities(std::floor(p->x()),std::floor(p->y()))+=img_view(p1x,p1y);
                                        ...
                                        for(unsigned k=0;k<img_result.ni();k++)
                                            ...
                                            for(unsigned m=0;m<f2xformed.size();m++)
                                            {
                                                ...
                                                {
                                                    vsol_point_2d_sptr p=new vsol_point_2d(pt->location()[0]+xrefc,pt->location()[1]+yrefc);
                                                    vsol_point_2d_sptr p0=new vsol_point_2d(p->x()-0.2*std::sin(pt->dir()),p->y()+0.2*std::cos(pt->dir()));
                                                    vsol_point_2d_sptr p1=new vsol_point_2d(p->x()+0.2*std::sin(pt->dir()),p->y()-0.2*std::cos(pt->dir()));
...
{
  if(std::vector<dbru_multiple_instance_object*> * obs_=reinterpret_cast<std::vector<dbru_multiple_instance_object*> *>(mobs))
    {
        return obs_->size();
  }
  else return 0;
...
{
    std::vector<vsol_polygon_2d_sptr> * polys =new std::vector<vsol_polygon_2d_sptr>();
    if(std::vector<dbru_multiple_instance_object*> * obs_=reinterpret_cast<std::vector<dbru_multiple_instance_object*> *>(mobs))
    {
...
std::vector<dbrl_id_point_2d_sptr> get_foreground_edges_from_observ(dbinfo_observation_sptr obs,dbbgm_image_of<mix_gauss_type> *model,
    ...
    for(unsigned i=0;i<points.size();i++)
    {
        ...
        objimage(exroi->cmin(0)+(unsigned int)std::floor(points[i]->x()),exroi->rmin(0)+(unsigned int)std::floor(points[i]->y()))=dir;
        masked(exroi->cmin(0)+(unsigned int)std::floor(points[i]->x()),exroi->rmin(0)+(unsigned int)std::floor(points[i]->y()))=true;
...
//
//    std::vector<vsol_spatial_object_2d_sptr> vsol_out;
//    std::vector<vgl_point_2d<double> > supportpoints;
//    for(unsigned j=0;j<totsize;j++)
//    {
//        if(weightst[j] >thresh)
//        {
...
{
 if(std::vector<vsol_polygon_2d_sptr > * polys=
     reinterpret_cast<std::vector<vsol_polygon_2d_sptr > *> (handle))
   return polys->size();
 else
   return -1;
}
