...
curve_dpmatch_2d::curve_dpmatch_2d()
{
  _R = 10;
}

curve_dpmatch_2d::curve_dpmatch_2d (bsol_intrinsic_curve_2d_sptr c1, bsol_intrinsic_curve_2d_sptr c2)
{
  _curve1 = c1;
  _curve2 = c2;
}
...
void curve_dpmatch_2d::Match ()
{
  initializeDPCosts(_curve1->size(), _curve2->size());
  computeDPCosts ();
  findDPCorrespondence (0, _curve1->size(), 0, _curve2->size());
}

void curve_dpmatch_2d::Match( int startA, int endA, int startB, int endB ){
    assert( startA == 0 && startB == 0 );
    initializeDPCosts( endA-startA, endB-startB );
  computeDPCosts( startA, endA, startB, endB );
  findDPCorrespondence( startA, endA, startB, endB );
}
...
void curve_dpmatch_2d::initializeDPCosts(int sizeA, int sizeB)
{
  //Ming: init
  _DPCost.clear();
  _DPMap.clear();
...
// CORRECTED: cost of matching  curve1[i,ip] to curve2[j,jp]
...
void curve_dpmatch_2d::computeDPCosts ()
{
    assert(0);

  int XOFFSET[9] = {-1, 0,-1,-1,-2,-2,-3,-1,-3};
  int YOFFSET[9] = {-1,-1, 0,-2,-1,-3,-2,-3,-1};
  //#define TEMPLATE_SIZE 21 //5x5
  //int XOFFSET[TEMPLATE_SIZE] = {-1, 0,-1,-1,-2,-2,-3,-1,-3, -1,-3,-4,-4, -1,-2,-3,-4,-5,-5,-5,-5};
  //int YOFFSET[TEMPLATE_SIZE] = {-1,-1, 0,-2,-1,-3,-2,-3,-1, -4,-4,-3,-1, -5,-5,-5,-5,-1,-2,-3,-4};
  /*#define TEMPLATE_SIZE 93 //5x5
  int XOFFSET[TEMPLATE_SIZE] = {-1, 0,-1,-1,-2,-2,-3,-1,-3, -1,-3,-4,-4, -1,-2,-3,-4,-5,-5,-5,-5, //5x5
  ...
  int YOFFSET[TEMPLATE_SIZE] = {-1,-1, 0,-2,-1,-3,-2,-3,-1, -4,-4,-3,-1, -5,-5,-5,-5,-1,-2,-3,-4, //5x5
  ...
  for (sum = 1; sum<_n+_m-1; sum++) {
    start=curve_maxof(0,sum-_m+1,-10000); //this is stupid as 0 > -10000
    for (i=start; (i<=_n-1 && i<=sum); i++) {
      ...
      for (k=0;k<TEMPLATE_SIZE;k++) { //TEMPLATE_SIZE=9 originally
        ...
        if (ip >= 0 &&  jp >=0) {
                        //compute cost of matching curve1[i,ip] to curve2[j,jp]
            double incCost=computeIntervalCost(i,ip,j,jp,pt1);
...
//SPINNER NEW COST
void curve_dpmatch_2d::computeDPCosts( int startA, int endA, int startB, int endB )
{
    //if start!=0 just adjust the curve and let code run as normal
    //if( startA != 0 ){
        //bsol_intrinsic_curve_2d_sptr oldCurve1 = new bsol_intrinsic_curve_2d(*_curve1);
        //adjust curve1
    //}
    //_curve1 = oldCurve1;
  ...
  ///int count=0;
    ...
    //sanity check to make sure things are okay
    assert( endA <= _curve1->size() );
    assert( endB <= _curve2->size() );
    assert( startA < endA && startB < endB );
    assert( startA >= 0 && startB >= 0 );
...
void curve_dpmatch_2d::findDPCorrespondence( int startA, int endA, int startB, int endB )
{
  int i, j, ip, jp;
  ...
  //int _m = _curve2->size();
    ...
    assert( _n == sizeA && _m == sizeB );

  _finalCost = _DPCost[sizeA-1][sizeB-1];  //The final value of DPMap
...
void curve_dpmatch_2d::GetAverageCurveFromAMatch (bsol_intrinsic_curve_2d_sptr AverageCurve)
{
  AverageCurve->clear();
  double x, y;
...
void curve_dpmatch_2d::GetSumCurveFromAMatch (bsol_intrinsic_curve_2d_sptr SumCurve)
{
  SumCurve->clear();
  double x, y;
...
void curve_dpmatch_2d::GetMorphingCurvesFromAMatch (bsol_intrinsic_curve_2d_sptr MorphingCurves, int NumMorphs)
{
  double w;
...
void curve_dpmatch_2d::ListDPTable (void)
{
  int n = _curve1->size();
  int m = _curve2->size();
...
void curve_dpmatch_2d::ListDPTable (int sizeA, int sizeB)
{
  int n = sizeA-1;
  int m = sizeB-1;
...
void curve_dpmatch_2d::SaveAlignCurve (void)
{
  int n = _curve1->size();
  int m = _curve2->size();
