...
#if 0
static void backproject_rays(std::string filename)
{
...
static void test_backproject_ray()
{
  ...
  std::string cam_filename=root_dir+"/contrib/brl/bseg/boxm/opt/open_cl/tests/cam_0.txt";
  std::ifstream ifs(cam_filename.c_str());
  if (!ifs)
    return;
  else
    ifs >> (*pcam);
  ...
  if (test_driver.init())
  {
    ...
    for (unsigned i=0;i<64;i++)
    {
      for (unsigned j=0;j<64;j++)
      {
        ...
        float dist=std::sqrt((ray_results[inc]  -norm_direction.x())*(ray_results[inc]-norm_direction.x())+
