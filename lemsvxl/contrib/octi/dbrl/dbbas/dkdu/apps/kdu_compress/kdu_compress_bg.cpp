...
static kdu_roi_image *
  create_roi_source(kdu_codestream rv_codestream, kdu_args &args)
{
  ...
  if ((string != NULL) && (*string == '{'))
    {
      ...
      kdu_dims region; rv_codestream.get_dims(-1,region);
      ...
      result = new kdu_roi_rect(rv_codestream,region);
    }
  else
    { // Must be file-name/threshold form.
      ...
      *string = '\0';
      result = new kdu_roi_graphics(rv_codestream,fname,threshold);
    }
  args.advance();
  return result;
}
...
/*****************************************************************************/
/*                     kdc_bg_flow_control::kdc_bg_flow_control                    */
/*****************************************************************************/

kdc_bg_flow_control::kdc_bg_flow_control(kdc_file_binding *files,
                                   kdu_codestream rv_codestream_,
								   kdc_flow_control* param_flow_,
								   int x_tnum,
                                   bool allow_shorts,bbgm_io& bgm_reader_,
                                   kdu_roi_image *roi_source):bgm_reader(bgm_reader_)
{
  int n;

  this->rv_codestream = rv_codestream_;
  this->param_flow=param_flow_;
  this->param_codestream=param_flow->codestream;
  this->x_tnum = x_tnum;
  //this->bgm_reader=bgm_reader_;
  rv_codestream.get_valid_tiles(valid_tile_indices);
  assert((x_tnum >= 0) && (x_tnum < valid_tile_indices.size.x));
  tile_idx = valid_tile_indices.pos;
  tile_idx.x += x_tnum;
  tile = rv_codestream.open_tile(tile_idx);
  num_components = tile.get_num_components();
  assert(num_components > 0);
  roi_image = roi_source;

  // Set up the individual components.
  components = new kdc_bg_component_flow_control[num_components];
  count_delta = 0;
  for (n=0; n < num_components; n++)
    {
      kdc_bg_component_flow_control *comp = components + n;
	  ...
	  kdu_push_ifc* param_ana_network=new kdu_push_ifc[param_flow->num_components];
	  for (int i=0;i<param_flow->num_components;i++)
		  param_ana_network[i]=(this->param_flow->components[i].compressor);
	  if (res.which() == 0)
        comp->compressor =
        ...
        comp->compressor =
          kdu_bg_analysis(res,&(comp->allocator),use_shorts,bgm_reader,1.0F,roi_node,param_flow,param_ana_network);
...
/*****************************************************************************/
/*                  kdc_bg_flow_control::advance_components                     */
/*****************************************************************************/
          ...
          if (comp->remaining_lines > 0)
            {
              ...
              if (comp->ratio_counter < 0)
                {
                  found_line = true;
                  if (comp->bg_reader->exists() && comp->line.get_width())
                    comp->bg_reader->get
					(comp->line,x_tnum);
                }
            }
...
/*****************************************************************************/
/*                  kdc_bg_flow_control::access_compressor_line                 */
/*****************************************************************************/

kdu_line_buf *
  kdc_bg_flow_control::access_compressor_line(int comp_idx)
{
  assert((comp_idx >= 0) && (comp_idx < num_components));
  kdc_bg_component_flow_control *comp = components + comp_idx;
  return (comp->ratio_counter < 0)?(&(comp->line)):NULL;
}

/*****************************************************************************/
/*                    kdc_bg_flow_control::process_components                   */
/*****************************************************************************/
...
/*****************************************************************************/
/*                        kdc_bg_flow_control::advance_tile                     */
/*****************************************************************************/
  ...
  kdc_bg_flow_control::advance_tile()
{
  ...
  tile = rv_codestream.open_tile(tile_idx);
  this->param_flow->tile=this->param_codestream.open_tile(tile_idx);
  for (n=0; n < num_components; n++)
    {
      kdc_bg_component_flow_control *comp = components + n;
      kdu_tile_comp tc = tile.access_component(n);
	  ...
	  if (res.which() == 0)
        ...
        comp->compressor =
			kdu_bg_analysis(res,&(comp->allocator),use_shorts,this->bgm_reader,1.0F,roi_node,this->param_flow);
      comp->allocator.finalize();
      comp->line.create();
      comp->ratio_counter = 0;
      comp->remaining_lines = dims.size.y;
...
bool kdu_compress_bg(const bbgm_image_of<bgm_mix>* from,bbgm_image_of<bgm_mix>* to,int num_comps,int nLevels,int argc, char* argv[])
{
  kdu_customize_warnings(&std::cout);
  char** param_argv=new char*[argc];
  for (int i=0;i<argc;i++)
   {  param_argv[i]=new char[400];
	  strcpy(param_argv[i],argv[i]);
   }
  kdu_customize_errors(&std::cerr);
  kdu_args args(argc,argv,"-s");
  kdu_args param_args(argc,param_argv,"-s");

 
  // Collect simple arguments.
  
  bool transpose=false, vflip=false, hflip=false;
  bool allow_rate_prediction, allow_shorts=false, mem=0, quiet=0, no_weights, no_palette=false;
  int cpu_iterations=0;
  char *ofname;
  std::ostream *record_stream;
  parse_simple_args(param_args,ofname,record_stream,transpose,vflip,hflip,
  ...
  siz_params siz_rv,siz_param;
  char *string,*string_p;
  // Set up input image files.
  kdu_rgb8_palette palette; // To support palettized imagery.
  int num_components=num_comps;
  create_size(*from,siz_rv,1);
  create_size(*from,siz_param,num_comps);
   // Complete `siz' object.

  siz_params *siz_p_rv = &siz_rv; siz_p_rv->finalize(); // Access func via v-table
  siz_params transformed_siz_rv; // Use this one to construct code-stream.
  transformed_siz_rv.copy_from(&siz_rv,-1,-1,-1,0,0,transpose,false,false);
  // Construct the `kdu_codestream' object and parse all remaining arguments.

  siz_params *siz_p_param = &siz_param; siz_p_param->finalize(); // Access func via v-table
  siz_params transformed_siz_param; // Use this one to construct code-stream.
  transformed_siz_param.copy_from(&siz_param,-1,-1,-1,0,0,transpose,false,false);

  kdu_codestream rv_codestream,param_codestream;
  rv_codestream.create(&transformed_siz_rv,0);
  param_codestream.create(&transformed_siz_param,output);
 // rv_codestream.access_siz()->access_cluster(COD_params)->set(Clevels,0,0,7);
  
  for (string=args.get_first(); string != NULL; )
    string = args.advance(rv_codestream.access_siz()->parse_string(string));
  for (string_p=param_args.get_first(); string_p != NULL; )
    string_p = param_args.advance(param_codestream.access_siz()->parse_string(string_p));
  
  int num_layer_specs = 0;
  
  int *layer_bytes_param =
    assign_layer_bytes(param_args,param_codestream.access_siz(),num_layer_specs);
  kdu_uint16 *layer_thresholds =
    assign_layer_thresholds(param_args,num_layer_specs);
  /*
  if ((num_layer_specs > 0) && allow_rate_prediction &&
      (layer_bytes[num_layer_specs-1] > 0))
    rv_codestream.set_max_bytes(layer_bytes[num_layer_specs-1]);
  if ((num_layer_specs > 0) && allow_rate_prediction &&
      (layer_thresholds[num_layer_specs-1] > 0))
    rv_codestream.set_min_slope_threshold(layer_thresholds[num_layer_specs-1]);
  ...
  if ((num_components == 3) && (!no_weights))
    set_default_colour_weights(rv_codestream.access_siz(),quiet);*/

  rv_codestream.access_siz()->finalize_all();
  //rv_codestream.set_textualization(record_stream);

  param_codestream.access_siz()->finalize_all();
  param_codestream.set_textualization(record_stream);
  if (cpu_iterations >= 0)
    rv_codestream.collect_timing_stats(cpu_iterations);
  ...
  kdc_flow_control **param_flows=new kdc_flow_control*[tile_indices.size.x];
  kdc_bg_flow_control **tile_flows = new kdc_bg_flow_control *[tile_indices.size.x];
  for (x_tnum=0; x_tnum < tile_indices.size.x; x_tnum++)
    {
	  param_flows[x_tnum]=new kdc_flow_control(0,param_codestream,x_tnum,false,roi_source);
	  tile_flows[x_tnum] =
      new kdc_bg_flow_control(0,rv_codestream,param_flows[x_tnum],x_tnum,allow_shorts,
	  bg_reader,roi_source);
  }
  bool done = false;
  ...
  param_codestream.flush(layer_bytes_param,num_layer_specs,layer_thresholds);
  // Cleanup
  ...
  if (!quiet)
    {
      double bpp_dims = get_bpp_dims(rv_codestream.access_siz());
      pretty_cout << "\nGenerated " << param_codestream.get_num_tparts()
                  << " tile-part(s) for a total of "
                  << tile_indices.area() << " tile(s).\n";
      pretty_cout << "Code-stream bytes (excluding any file "
                     "format) = " << param_codestream.get_total_bytes()
                  << " = "
                  << 8.0*param_codestream.get_total_bytes() / bpp_dims
                  << " bits/pel.\n";
...
bool create_size(const bbgm_image_of<bgm_mix>& from,siz_params &siz,int num_comps)
{
   ...
   for (int n=0; n < num_comps; n++)
    {
      siz.set(Sdims,first_comp_idx+n,0,(int)from.nj());
      siz.set(Sdims,first_comp_idx+n,1,(int)from.ni());
      siz.set(Ssigned,first_comp_idx+n,0,true);
      siz.set(Sprecision,first_comp_idx+n,0,32);
...
bool create_size_vil(const vil_image_view<vxl_byte>& from,siz_params &siz)
{
   int num_comps=from.nplanes();
   int first_comp_idx = 0;
   for (int n=0; n < num_comps; n++)
    {
      siz.set(Sdims,first_comp_idx+n,0,(int)from.nj());
 ...
 bool kdu_compress_vil_image(const vil_image_view<vxl_byte>& from,
	 int argc, char *argv[])
{
  kdu_customize_warnings(&std::cout);
  kdu_customize_errors(&std::cerr);
  ...
  vil_image_io vil_io(from,vil_image_view<vxl_byte>(),nLevels,allow_shorts,reversible);
  ...
  for (x_tnum=0; x_tnum < tile_indices.size.x; x_tnum++)
    tile_flows[x_tnum] =
      new kdc_flow_control(inputs,codestream,x_tnum,allow_shorts,roi_source,&vil_io);
  bool done = false;
