...
template <class curveType,class floatType>
void FineDPMatch<curveType,floatType>::printCost(){
  int i,j;
...
template <class curveType,class floatType>
void FineDPMatch<curveType,floatType>::writeCost(vcl_string fname){
...
template <class curveType,class floatType>
void FineDPMatch<curveType,floatType>::printMap(){
  int i,j;
  printf("Map Matrix\n");
  for (i = 0; i<_n; i++){
    for (j = 0; j<_m; j++){
...
template <class curveType,class floatType>
void FineDPMatch<curveType,floatType>::printFinalMap(){
  int j;
  COUT << "Final Map" << endl;
...
// Cost of matching the interval [x(i),x(i)]  to [y(k),y(j)].
template <class curveType,class floatType>
floatType FineDPMatch<curveType,floatType>::computeIntervalCost(int i, int ip, 
                int j, int jp){

  floatType cost,dF=0,dK=0;
  double l1,l2,a1,a2;

  l1=_curve1.totalLength(ip,i);
  l2=_curve2.totalLength(jp,j);
...
void FineDPMatch<curveType,floatType>::findDPCorrespondence(){
  ...
  while (ip > 0 || jp > 0){
    ip=_map[i][j].first;
    jp=_map[i][j].second;
    //COUT << _cost[i][j]-_cost[ip][jp] << endl;
