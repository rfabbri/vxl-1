...
Curve<double,double> bfrag2D::getTopContourAsCurve(){
...
bfrag2D::bfrag2D( int index, std::string s ){
    ...
    //std::cout << "MADE NEW BFRAG2D index: " << myIndex << std::endl;
...
int bfrag2D::loadCEM( std::string fn, vsol_polygon_2d_sptr &givenContour, Curve<double,double> &givenCurve ){
...
int bfrag2D::loadCON( std::string fn, vsol_polygon_2d_sptr &givenContour, Curve<double,double> &givenCurve ){
    ...
    if (!strncmp(openFlag,"OPEN",4)){
        givenCurve._isOpen = true;
    } else if (!strncmp(openFlag,"CLOSE",5)){
        givenCurve._isOpen = false;
    }else{
...
void bfrag2D::calcCenter(){
    ...
    //std::cout << "x [min/max] : [" << minX << "," << maxX << "]    y : [" << minY << "," << maxY << "]" << std::endl;
    //std::cout << "center is: (" << _centerX << "," << _centerY << ")" << std::endl;
