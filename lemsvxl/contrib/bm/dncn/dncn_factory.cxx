...
dncn_factory::dncn_factory( std::string& video_glob, point_vector_type& pv, unsigned num_pivot_pixels, unsigned num_particles, unsigned num_neighbors):
...
void dncn_factory::build_target_list()
{
    ...
    {
        std::cout << "Target List " << (pvit-point_vector_.begin())/float(point_vector_.size()) * 100 << "% complete." << std::endl;
        ...
        //feature_map_type == std::map<unsigned, dncn_feature_sptr>
        ...
        for( unsigned t = 0; t < num_frames; ++t )
        {
            ...
            curr_feature_map.insert( std::make_pair<unsigned,dncn_feature_sptr>(feature_sptr->frame(),feature_sptr) );
			curr_target_map.insert( std::make_pair<unsigned, dncn_feature_sptr>(target_feature_sptr->frame(), target_feature_sptr) );
...
void dncn_factory::calculate_temporal_entropy()
{
    ...
    for( unsigned c = 0; c < num_cols; ++c )
    {
        for( unsigned r = 0; r < num_rows; ++r )
        {
            ...
            std::cout << (float(num_rows*c + r)/float(num_cols*num_rows))*float(100) << "% complete " << std::endl;
        }
    }
...
void dncn_factory::sample_pivot_pixels()
{
    ...
    std::map<vgl_point_2d<unsigned>, double, dncn_less_than> particle_map;
    ...
    std::map<vgl_point_2d<unsigned>,double,dncn_less_than >::iterator pmit;
    std::map<vgl_point_2d<unsigned>,double,dncn_less_than >::iterator pmend = particle_map.end();
    ...
    std::pair<std::set<vgl_point_2d<unsigned>,dncn_less_than>::iterator,bool> ret;
    ...
    while( pivot_pixel_set.size() < this->num_pivot_pixels_ )
    {
        ...
        std::vector<double>::iterator search_itr = std::search(cdf.begin(),cdf.end(),target.begin(),target.end(),&dncn_factory::binary_search_predicate);
        unsigned bin;
...
void dncn_factory::extract_neighbors()
{
    ...
    std::map<unsigned, vil_image_view<vxl_byte> > img_seq = this->build_frame_map();
    ...
    for( target_list_itr = this->target_list_2d_sptr_->target_list_begin(); target_list_itr != target_list_end; ++target_list_itr )
    {
        ...
        std::cout << "Extracting Neighborhood for image coordinate: " << curr_target << /*" (" 
    ...
    std::cout << "Done Extracting Neighbors." << std::endl;
...
void dncn_factory::extract_features()
{
    ...
    for( tl_itr = this->target_list_2d_sptr_->target_list_begin(); tl_itr != tl_end; ++tl_itr )
    {
        ...
        std::map<unsigned, vil_image_view<vxl_byte> >::iterator img_seq_itr;
        std::map<unsigned, vil_image_view<vxl_byte> >::iterator img_seq_end = img_seq.end();
        ...
        for( img_seq_itr = img_seq.begin(); img_seq_itr != img_seq_end; ++img_seq_itr )
        {
			...
			//std::cout << "target intensity = " << target_intensity << std::endl;
			//std::cout << "curr_feature_vector_mean = " << curr_feature_vector_mean << std::endl;
			//
			//vnl_vector<double>::iterator cv_itr;
			//vnl_vector<double>::iterator cv_end = curr_feature_vector.end();
			//std::cout << "curr_feature_vector = [";
...
void dncn_factory::extract_sift_features( unsigned const& octave_size, unsigned const& num_oct )
{
	...
	for( tl_itr = this->target_list_2d_sptr_->target_list_begin(); tl_itr != tl_end; ++tl_itr )
	{
		...
		{
			std::cout << "\t Dense Sift on Image: " << t << std::endl;
	...
	std::cout << "Done Extracting Keypoints..." << std::endl;
...
//{
//	std::cout << "Finding Maximal Scale of point (" << location.x() << ", " << location.y() << ")." << std::endl;
...
//
//		float scale = std::pow(float(2.0),float((index/oct_size)-1));
//
...
//		}
//	}//end scale iteration
//	float max_scale = std::pow(float(2.0),float(max_index/oct_size)-1);
//	float actual_scale;
...
// IO
void dncn_factory::save_entropy_bin( std::string const& filename )
{
...
void dncn_factory::load_entropy_bin( std::string const& filename )
{
...
void dncn_factory::save_entropy_dat( std::string const& filename )
{
...
void dncn_factory::save_pivot_pixels_mfile( std::string const& filename )
{
...
void dncn_factory::write_neighborhood_mfile( std::string const& filename )
{
    this->target_list_2d_sptr_->write_neighborhood_mfile( filename );
}//dncn_factory::write_neighborhood_mfile

void dncn_factory::save_factory_bin( std::string const& filename )
{
...
void dncn_factory::read_factory_bin( std::string const& filename )
{
...
std::map<unsigned, vil_image_view<vxl_byte> > dncn_factory::build_frame_map()
{
    //1. Load all images to memory to speed up.
    unsigned num_frames = this->video_stream_.num_frames();
