...
#include<cassert> //the macro is called assert(expression)
#include<iostream>
...
#define DVRML_SETUP if (of_stream_ptr_ == 0){ std::cerr << "dvrml -- std::ostream not set!\n"; return; } std::ofstream& os = *of_stream_ptr_

class dvrml
{
public: 
    //dvrml():of_stream_ptr_(NULL){}
    //dvrml( std::ofstream& os ):of_stream_ptr_(&os){}

    dvrml( std::ofstream& os ):of_stream_ptr_(&os){ preamble(); }
    ...
    //void draw_points( std::vector<vgl_point_3d<double> > const& pts, dvrml_appearance_sptr appearance_sptr = new dvrml_appearance, double const& radius = 1 );
