...
{
	std::string src_path = testlib_root_dir();
	std::cout << src_path << std::endl;

	std::string filename = src_path + "/contrib/brl/lemsvxlsrc/contrib/bm/vlfeat/tests/kermit000.jpg";
	std::string matlab_descr = src_path + "/contrib/brl/lemsvxlsrc/contrib/bm/vlfeat/tests/d.txt";
	std::string matlab_frame = src_path + "/contrib/brl/lemsvxlsrc/contrib/bm/vlfeat/tests/f.txt";
	
	vnl_matrix<float> matlab_descr_matrix(1,128);

	std::ifstream descrFile(matlab_descr.c_str());
	std::ifstream frameFile(matlab_frame.c_str());

	unsigned maxLines = 500;

	std::vector<vgl_point_2d<float> > frames;
	...
	{	
		std::cerr << "ERROR: vlfeat_test_dsift -- FAILED TO OPEN FILE: " << matlab_frame << std::flush;
		testlib_test_perform(false);
	}
	
	std::map<vgl_point_2d<float>, std::vector<float>, vlfeat_vgl_point_2d_coord_compare<float> > matlab_gt_map;
	...
	{
		std::cerr << "ERROR: vlfeat_test_dsift -- FAILED TO OPEN FILE: " << matlab_descr << std::flush;
		testlib_test_perform(false);
	}
	...
	std::map<vgl_point_2d<float>, std::vector<float>, vlfeat_vgl_point_2d_coord_compare<float> > dsift_descriptor_map;
	...
	std::map<vgl_point_2d<float>, std::vector<float>, vlfeat_vgl_point_2d_coord_compare<float> >::const_iterator m_itr, m_end = matlab_gt_map.end();

	for(m_itr = matlab_gt_map.begin(); m_itr != m_end; ++m_itr)
	{
		//look up the matching frame from the wrapper result
		std::vector<float> wrapper_vector = dsift_descriptor_map[m_itr->first];
		std::vector<float> matlab_vector = m_itr->second;
		float accum = 0;
	...
	std::vector<float> wtemp = dsift_descriptor_map[frames[372]];
	std::vector<float> mtemp = matlab_gt_map[frames[372]];

	vgl_point_2d<float> pt2 = frames[233];
	std::vector<float>wtemp2 = dsift_descriptor_map[pt2];
	std::vector<float>mtemp2 = matlab_gt_map[pt2];

	vgl_point_2d<float>pt3 = frames[94];
	std::vector<float>wtemp3 = dsift_descriptor_map[pt3];
	std::vector<float>mtemp3 = matlab_gt_map[pt3];
