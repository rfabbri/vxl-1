...
void vsl_b_write(vsl_b_ostream& os, std::map<T1, std::map<T2, T3, Compare2>, Compare1> const& v)
{
    ...
    for(typename std::map<T1, std::map<T2, T3, Compare2>, Compare1 >::const_iterator itr = v.begin(); itr!=v.end(); ++itr)
    {
        vsl_b_write(os, (*itr).first);
        vsl_b_write(os, (*itr).second.size());
        for( typename std::map<T2,T3,Compare2>::const_iterator itr2=((*itr).second).begin(); itr2!=(*itr).second.end();++itr2)
        {
...
void vsl_b_read(vsl_b_istream& is, std::map<T1, std::map<T2, T3, Compare2>, Compare1>& v)
{
    ...
    default:
        {
            std::cerr << "I/O ERROR: vsl_b_read(vsl_b_istream&, std::map<T1, std::map<T2, T3>&)\n"
...
void vsl_b_read(vsl_b_istream& is,std::map<T1, std::map<T2, std::map<T3, T4, Compare3>, Compare2>, Compare1>& v)
{
    ...
    case 1:
        {
            ...
            for(unsigned i = 0; i < key1_size; ++i)
            {
                ...
                std::map<T2, std::map<T3,T4,Compare3>, Compare2> key2_map;
                for(unsigned j = 0; j < key2_size; ++j)
                {
                    ...
                    std::map<T3,T4,Compare3> key3_map;
    ...
    default:
        {
            std::cerr << "I/O ERROR: std::map<T1, std::map<T2, std::map<T3, T4, Compare3>, Compare2>, Compare1>& v\n"
                << "           Unknown version number "<< ver << '\n';
            is.is().clear(std::ios::badbit); // Set an unrecoverable IO error on stream
            return;
        }//end default
...
void vsl_b_write(vsl_b_ostream& os,std::map<T1, std::map<T2, std::map<T3, T4, Compare3>, Compare2>, Compare1> const& v)
{
    ...
    for( typename std::map<T1, std::map<T2, std::map<T3, T4, Compare3>, Compare2>, Compare1>::const_iterator itr1 = v.begin(); itr1!=v.end(); ++itr1)
    {
        vsl_b_write(os,(*itr1).first);
        vsl_b_write(os,(*itr1).second.size());

        for( typename  std::map<T2, std::map<T3, T4, Compare3>, Compare2>::const_iterator itr2=(*itr1).second.begin(); itr2!=(*itr1).second.end(); ++itr2 )
        {
            vsl_b_write(os,(*itr2).first);
            vsl_b_write(os,(*itr2).second.size());

            for( typename std::map<T3, T4, Compare3>::const_iterator itr3=(*itr2).second.begin(); itr3!=(*itr2).second.end(); ++itr3 )
            {
...
template void vsl_print_summary( std::ostream& os, std::map<T1, std::map<T2, std::map<T3, T4, Compare3>, Compare2>, Compare1> const& v);\
template void vsl_b_read(vsl_b_istream& is,std::map<T1, std::map<T2, std::map<T3, T4, Compare3>, Compare2>, Compare1>& v);\
template void vsl_b_write(vsl_b_ostream& os,std::map<T1, std::map<T2, std::map<T3, T4, Compare3>, Compare2>, Compare1> const& v)
