...
bool dsm_state_machine_classify_pixel_time_series_map_process( bprb_func_process& pro )
{
	...
	{
		std::cout << pro.name() << " dsm_state_machine_classify_pixel_time_series_map_process: The input number should be " << n_inputs_ << std::endl;
		return false;
	}
	...
	case 2:
		{
			...
			for( state_machine_id = 0, t_itr = pixel_time_series_map_sptr->pixel_time_series_map.begin(); t_itr != t_end; ++t_itr, ++state_machine_id )
			{
				...
				std::cout << "Creating State Machine " << state_machine_id << " for target: (" << t_itr->first.x() << "," << t_itr->first.y() << ")" << std::endl;
				...
				std::map<unsigned, dsm_feature_sptr>::const_iterator f_itr, f_end = t_itr->second->time_series.end();
				unsigned time_series_size = t_itr->second->time_series.size();


				std::stringstream output_directory, change_filename, state_filename;
				...
				std::ofstream change_of( change_filename.str().c_str(), std::ios::out ) ;

				state_filename << output_directory.str() << "/state.txt";

				std::ofstream state_of( state_filename.str().c_str(), std::ios::out );
				...
				for( f_itr = t_itr->second->time_series.begin(); f_itr != f_end; ++f_itr )
				{
					...
					std::cout << t_itr->second->time_series.size() - distance(f_itr,f_end) << " out of " << time_series_size << " points updated." << std::endl;

					std::cout << "Writing Graph Dot File: " << std::endl;
					
					std::stringstream dot_filename;

					dot_filename << output_directory.str() << "/frame_"<< std::setfill('0') << std::setw(4) << f_itr->second->t << ".dot";

					std::ofstream of( dot_filename.str().c_str(), std::ios::out );
					...
					std::map<unsigned, dsm_node_base_sptr>::const_iterator g_itr, g_end = state_machine_base_sptr->graph_end();
					...
					for( g_itr = state_machine_base_sptr->graph_begin(); g_itr != g_end; ++g_itr )
					{
						std::map<unsigned, dsm_node_base_sptr>::const_iterator g_itr2, g_end2 = state_machine_base_sptr->graph_end();
					...
					std::stringstream mean_filename, covar_filename, observation_filename, weight_filename;

					mean_filename << output_directory.str() << "/mean_" << std::setfill('0') << std::setw(4) << f_itr->second->t << ".txt";
					covar_filename << output_directory.str() << "/covar_" << std::setfill('0') << std::setw(4) << f_itr->second->t << ".txt";
					observation_filename << output_directory.str() << "/observation_" << std::setfill('0') << std::setw(4) << f_itr->second->t << ".txt";
					weight_filename << output_directory.str() << "/weight_"  << std::setfill('0') << std::setw(4) << f_itr->second->t << ".txt";
					std::string mean_filename_str = mean_filename.str();
					std::string covar_filename_str = covar_filename.str();
					std::string observation_filename_str = observation_filename.str();
					std::ofstream mof(mean_filename.str().c_str());
					std::ofstream cof(covar_filename.str().c_str());
					std::ofstream oof(observation_filename.str().c_str());
					std::ofstream wof(weight_filename.str().c_str());
					oof << std::setprecision(5) << f_itr->second->v[0] << '\t' << f_itr->second->v[1] << '\n';
					oof.close();
					//write the means of all nodes
					std::cout << "Writing Mean, Covariance and Weight Files... " << std::endl;
					for( g_itr = state_machine_base_sptr->graph_begin(); g_itr != g_end; ++g_itr )
					{
						...
						//write mean files
						vnl_vector<double> mean = node_ptr->mean();
						mof << node_id << '\t' << std::setprecision(5) << mean[0] << '\t' << mean [1] << '\n';
						...
						wof << node_id << '\t' << std::setprecision(8) << t_prob << '\n';
	...
	default:
		{
			std::cerr << "---- ERROR PROCESS DSM_STATE_MACHINE_CLASSIFY_PIXEL_TIME_SERIES_MAP_PROCESS:\n"
