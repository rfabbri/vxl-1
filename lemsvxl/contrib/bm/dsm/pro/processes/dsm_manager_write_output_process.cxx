...
bool dsm_manager_write_output_process( bprb_func_process& pro )
{
	...
	{
		std::cout << pro.name() << " dsm_manager_write_output_process: The input number should be " << n_inputs_ << std::endl;
		return false;
	}
	...
	std::string output_directory = pro.get_input<std::string>(i++);
	...
	std::stringstream sm_directory, change_filename, state_filename;
	...
	std::ofstream change_stream(change_filename.str().c_str());

	//std::map<unsigned,bool>::const_iterator c_itr, c_end = sm_base_sptr->frame_change_map().end();
	std::map<unsigned,bool>::const_iterator change_itr, change_end = sm_base_sptr->frame_change_map_.end();
	...
	std::ofstream state_stream(state_filename.str().c_str());

	std::map<unsigned,unsigned>::const_iterator s_itr, s_end = sm_base_sptr->frame_state_map_.end();
	...
	std::map<unsigned, std::vector<unsigned> > temp_frame_graph_map;
	...
	{
		std::cout << "Writing Mean File: " << mean_itr->first << std::endl;

		std::stringstream mean_filename;

		mean_filename << sm_directory.str() << "/mean_" << std::setfill('0') << std::setw(4) << mean_itr->first << ".txt";

		std::ofstream mean_stream(mean_filename.str().c_str());

		std::map<unsigned, vnl_vector<double> >::const_iterator mean_node_itr, mean_node_end = mean_itr->second.end();
	...
	{
		std::cout << "Writing Covariance File: " << frame_cov_itr->first << std::endl;

		std::stringstream covar_filename;

		covar_filename << sm_directory.str() << "/covar_" << std::setfill('0') << std::setw(4) << frame_cov_itr->first << ".txt";

		std::ofstream covar_stream(covar_filename.str().c_str());
	...
	//mixture weights
	std::map<unsigned, std::map<unsigned,double> >::const_iterator
	...
	{
		std::cout << "Writing Mixture Weight File: " << frame_weight_itr->first << std::endl;

		std::stringstream weight_filename;

		weight_filename << sm_directory.str() << "/weight_" << std::setfill('0') << std::setw(4) << frame_weight_itr->first << ".txt";

		std::ofstream weight_stream(weight_filename.str().c_str());
	...
	//std::map<unsigned, std::map<unsigned, std::map<unsigned, unsigned> > >::const_iterator
	...
	//{
	//	std::cout << "Writing Transition Table for Frame: " << frame_transition_itr->first << std::endl;

	//	std::stringstream frame_transition_directory;

	//	frame_transition_directory << sm_directory.str() << "/transition_tables_frame_" 
	//							   << std::setfill('0') << std::setw(4) << frame_transition_itr->first;
	...
	//	{
	//		std::stringstream transition_table_filename;

	//		transition_table_filename << frame_transition_directory.str() << "/node_"
	//			                      << std::setfill('0') << std::setw(4) << node_transition_table_itr->first;

	//		std::ofstream transition_table_of(transition_table_filename.str().c_str());
	...
	//write dot files
	std::map<unsigned, std::vector<unsigned> >::const_iterator frame_graph_itr, frame_graph_end = temp_frame_graph_map.end();

	unsigned rel_time_curr;
	for( frame_graph_itr = temp_frame_graph_map.begin(); frame_graph_itr != frame_graph_end; ++frame_graph_itr )
	{
		std::cout << "Writing Graph File: " << frame_graph_itr->first << std::endl;

		std::stringstream dot_filename;

		dot_filename << sm_directory.str().c_str() << "/frame_" << std::setfill('0') << std::setw(4) << frame_graph_itr->first << ".dot";

		std::ofstream dot_of(dot_filename.str().c_str());
        ...
        //label the nodes
	    std::vector<unsigned>::const_iterator graph_itr, graph_end=frame_graph_itr->second.end();
		...
		for( graph_itr = frame_graph_itr->second.begin(); graph_itr != graph_end; ++graph_itr )
		{
			std::vector<unsigned>::const_iterator graph_itr2;
				...
				for( graph_itr2 = frame_graph_itr->second.begin(); graph_itr2 != graph_end; ++graph_itr2 )
				{
					if( *graph_itr == sm_base_sptr->frame_state_map()[frame_graph_itr->first])
					{
						...
						dot_of << '\t' << '\t' << *graph_itr << "->"
							<< *graph_itr2 
							<< " [label=\"" << std::setprecision(4) << transition_probability << "\""
							<< ",color=red];\n";
					}
					else
					{
	...
	std::map<unsigned, dsm_feature_sptr>::const_iterator 
	...
	{
		std::cout << "Writing Observation File: " << time_series_itr->first << std::endl;
		std::stringstream observation_filename;

		observation_filename << sm_directory.str() << "/observation_" << std::setfill('0') 
							 << std::setw(4) << time_series_itr->first << ".txt";

	    std::ofstream observation_of(observation_filename.str().c_str());

		observation_of << std::setprecision(5) << time_series_itr->second->v[0] << '\t' << time_series_itr->second->v[1];
