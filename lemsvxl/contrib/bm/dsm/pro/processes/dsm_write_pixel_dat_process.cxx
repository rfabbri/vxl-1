...
bool dsm_write_pixel_dat_process( bprb_func_process& pro )
{
	...
	{
		std::cout << pro.name() << "dsm_write_pixel_dat_process: The input number should be " << n_inputs_ << std::endl;
		return false;
	}
	...
	if( ts_itr == ts_end )
	{
		std::cout << pro.name() << "dsm_write_pixel_dat_process:\n" 
				 <<"\tCould not find pixel: (" << x << "," << y <<") in pixel/time-series map." << std::endl;
		return false;
	}
