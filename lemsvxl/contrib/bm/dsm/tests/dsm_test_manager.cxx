...
void test_manager()
{
    ...
    for(unsigned i = 0; i < num_state_machines; ++i)
    {
        ...
        std::vector<vnl_vector<double> >::const_iterator obs_itr, obs_end = observations.end();
    ...
    std::string filename("test_manager_io.bin");
    ...
    vsl_b_ifstream is(filename.c_str(), std::ios::out|std::ios::binary);
    vsl_b_read(is,manager_ptr2);
    is.close();

    dsm_manager_base_sptr manager_sptr2(manager_ptr2);

    std::map<vgl_point_2d<unsigned>, dsm_state_machine_base_sptr, dsm_vgl_point_2d_coord_compare<unsigned> > write_map, read_map;
	...
	std::map<vgl_point_2d<unsigned>, dsm_state_machine_base_sptr, dsm_vgl_point_2d_coord_compare<unsigned> >::const_iterator smitr, smend=write_map.end();
	for(smitr = write_map.begin(); smitr!= smend; ++smitr)
	{
        ...
        {
            std::map<unsigned, dsm_node_base_sptr> graph1 = smp1->graph();
            std::map<unsigned, dsm_node_base_sptr> graph2 = smp2->graph();

            std::map<unsigned, dsm_node_base_sptr>::const_iterator g_itr, g_end = graph1.end();
