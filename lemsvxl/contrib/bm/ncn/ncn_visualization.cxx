...
bool ncn_visualization::neighborhood2dat(target_pixel_type const& target_pixel_list, pivot_pixel_candidate_type const& pivot_pixel_candidates,
					neighborhood_type const& neighborhood, std::ostream& os)
{
	...
	{
		os << "target_pixel" << count << " = [" << neighborhood_itr->first->x() <<  " " << neighborhood_itr->first->y() << "];" << std::endl;

		std::vector<pivot_pixel_candidate_type::const_iterator>::const_iterator pivot_pixel_itr;
		std::vector<pivot_pixel_candidate_type::const_iterator>::const_iterator pivot_pixel_end = neighborhood_itr->second.end();
