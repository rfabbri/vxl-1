...
int main(int argc, char *argv[])
{
  ...
  if(argc != 3){
    std::cout << "Usage: "<< argv[0] << " correspondence_file output_file"<< std::endl;
    return -1;
  }
  ...
  // std::ifstream coors_file( "C:\\Documents and Settings\\Edu\\Desktop\\Research\\calibration\\COORS BASE - DSCN0017 - left1.txt" , std::ifstream::in );
  std::ifstream coors_file( argv[1] , std::ifstream::in );
  ...
  vnl_matrix_fixed<double,3,3> erase; erase=K.get_matrix(); erase.print(std::cout);
  ...
  //std::ofstream of("C:\\Documents and Settings\\Edu\\Desktop\\Research\\calibration\\Pers and Prof cameras.txt",std::ofstream::out);
  std::ofstream of(argv[2],std::ofstream::out);

  camera.print_summary(std::cout);
