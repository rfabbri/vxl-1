...
void merge()
{
 //define some known mixtures
  ...
  merge.all_to_one(g,gauss1);
  merge.all_to_one(f,gauss2);
  merge.all_to_one(h,gauss3);
  merge.all_to_one(j,gauss4);
  ...
  //The sencond part of this test is to test that bseg3d_explore_mixtures::merge_mixtures work

  //Create grids
  vgl_vector_3d<unsigned> grid_size(5,5,2);
  ...
  explorer.merge_mixtures(apm_grid, merged_base);
...
MAIN( test_merge_mixtures)
{
  START ("test l2 norm for gmm");
  merge();
  SUMMARY();
}
