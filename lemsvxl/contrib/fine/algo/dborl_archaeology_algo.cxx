...
bool dborl_archaeology_algo::parse_index(std::string index_file)
{
  ...
  if (root_->names().size() != root_->paths().size()) {
    std::cout << "dborl_archaeology::parse_index() -- " << index_file << "-- number of names not equal number of paths!!" << std::endl;
    return false;
  }
...
bool dborl_archaeology_algo::initialize()
{
  ...
  if (root_->names().size() != root_->paths().size()) {
    std::cout << "dborl_archaeology_algo::initialize() -- names and paths size not equal in index: " << params_->db_index_() << std::endl;
    return false;
  }
...
bool dborl_archaeology_algo::process()
{
  ...
  for (unsigned i = 0; i < D; i++) {
    ...
    if (!vul_file::exists(con_file)) {
      std::cout << "dborl_archaeology_algo::process() -- cannot find: " << con_file << std::endl;
      return false;
    }
    ...
    if (!dbsol_load_con_file(con_file.c_str(), points, is_closed)) { 
      std::cout << "dborl_archaeology_algo::process() -- cannot load: " << con_file << std::endl;
      return false;
    }
    ...
    if (params_->delete_segments_()) {
      ...
      if (params_->add_segments_()) { // both delete and add
        ...
        while (!dbsol_curve_algs::segment_addition_and_deletion(poly, vsol_list, del_pieces, 
          ...
          counter++;
          if (counter > 100) {
            std::cout << "dborl_archaeology_algo::process() -- cannot randomize segment deletion! in: " << con_file << std::endl;
            return false;
          }
        }
      ...
      if (params_->add_segments_()) {  // just add
               ...
               !vsol_list.size()) 
        {
            ...
            counter++;
            if (counter > 100) {
              std::cout << "dborl_archaeology_algo::process() -- cannot randomize segment addition! in: " << con_file << std::endl;
              return false;
            }
        }
