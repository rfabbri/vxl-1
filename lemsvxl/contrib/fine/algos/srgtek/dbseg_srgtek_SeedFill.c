...
fill(x, y, width, win, pixel_array, nv)
int x, y, width;    /* seed point */
Window *win;    /* screen window */
unsigned char *pixel_array;    /*the array which stores the pixels*/
Pixel nv;    /* new pixel value */
{
    ...
    ov = pixelread(x, y, width, pixel_array);        /* read pv at seed point */
     ...
     */
    for (x=x1; x>=win->x0 && pixelread(x, y, width, pixel_array)==ov; x--)
        pixelwrite(x, y, width, nv, pixel_array);
    if (x>=x1) goto skip;
    l = x+1;
    if (l<x1) PUSH(y, l, x1-1, -dy);        /* leak on left? */
    x = x1+1;
    do {
        for (; x<=win->x1 && pixelread(x, y, width, pixel_array)==ov; x++)
        pixelwrite(x, y, width, nv, pixel_array);
        PUSH(y, l, x-1, dy);
        if (x>x2+1) PUSH(y, x2+1, x-1, -dy);    /* leak on right? */
skip:        for (x++; x<=x2 && pixelread(x, y, width,  pixel_array)!=ov; x++);
        l = x;
    } while (x<=x2);
    }
}
