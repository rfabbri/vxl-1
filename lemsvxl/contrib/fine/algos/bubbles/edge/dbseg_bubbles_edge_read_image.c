...
void read_images(Pars *pars, float *image_array, unsigned char *seeds_array)
     
{
  ...
  if (pars->type == 11) {
    ...
    PadIm_UCharToFloat(thePInfo.pic,image_array,(height+2),(width+2));
  }
  ...
  if (pars->type == 1) {
    ...
    fclose(f1);
    PadIm_UCharToFloat(uchar_image,image_array,(height+2),(width+2));
    free(uchar_image);
  }
  else if (pars->type == 2) {
    ...
    fclose(f1);
    PadIm_UShortToFloat(ushort_image,image_array,(height+2),(width+2));
    free(ushort_image);
  }  
    ...
    if (!strcmp(".txt", seedtype)) {
      ...
      for(y=0; y<height; y++) {
        for(x=0; x<width; x++) {
          pos = y*width+x;
          if (thePInfo.pic[pos] == 0)
            thePInfo.pic[pos] = 1;
          else thePInfo.pic[pos] = 0;
        }
      }

      PadIm_UCharToUChar(thePInfo.pic,seeds_array,(height+2),(width+2));
    }
