...
static void test_tracker()
{
  ...
  if( true ){
    ...
    vnl_matrix<int> mi_mask_true( 
      tracker.params.mi_mask_diameter(), tracker.params.mi_mask_diameter(), 0 );
    for( int i = 0; i < tracker.params.mi_mask_diameter(); i++ )
      for( int j = 0; j < tracker.params.mi_mask_diameter(); j++ )
  ...
  // Testing compute_mi.
  if( true ){
    vil_image_view<vxl_byte> image( tracker.params.mi_mask_diameter(), 
      tracker.params.mi_mask_diameter() );
    vnl_matrix<int> mi_mask( tracker.params.mi_mask_diameter(), 
      tracker.params.mi_mask_diameter() );
    for( int i = 0; i < tracker.params.mi_mask_diameter(); i++ ){
      for( int j = 0; j < tracker.params.mi_mask_diameter(); j++ ){
    ...
    float mi = tracker.compute_mi( image, vgl_point_2d<int>(0,0), mi_mask );
  }

  
  // Testing compute_region_mi.
  if( false ){
    vil_image_view<vxl_byte> image1 = vil_load( "f:\\testing\\input\\dbavl1.png" );
    vil_image_view<vxl_byte> image2 = vil_load( "f:\\testing\\input\\dbavl2.png" );
    vgl_point_2d<int> starting_point( 926, 133 );
    vnl_matrix<int> mi_mask( 
      tracker.params.mi_mask_diameter(), tracker.params.mi_mask_diameter() );
    for( int i = 0; i < tracker.params.mi_mask_diameter(); i++ ){
      for( int j = 0; j < tracker.params.mi_mask_diameter(); j++ ){
        mi_mask(j,i) = image1( 
          starting_point.x() - tracker.params.mi_mask_rad + i,
          starting_point.y() - tracker.params.mi_mask_rad + j );
      }
    }
    vnl_matrix<float> region_mi;
    tracker.compute_region_mi( image1, starting_point, mi_mask, region_mi );
    ...
    vil_save( image3, "f:\\testing\\output\\dbavl1.png" );
  }
