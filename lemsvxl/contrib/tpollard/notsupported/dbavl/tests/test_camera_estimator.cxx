...
static void test_camera_estimator()
{
  ...
  if( true ){
    ...
    for( int f = 1; f < num_cameras; f++ ){
      ...
      vgl_h_matrix_3d<double> new_rot;
      new_rot.set_identity();
      new_rot.set_rotation_euler(
        .1*rand()/(double)RAND_MAX, .1*rand()/(double)RAND_MAX, 
        .1*rand()/(double)RAND_MAX );
      new_cam.set_rotation_matrix( new_rot );
