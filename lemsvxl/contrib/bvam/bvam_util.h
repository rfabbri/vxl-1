...
public:
  ...
  static void compute_plane_image_H(vpgl_camera_double_sptr const& cam, bvam_world_params_sptr world_params, unsigned grid_zval, vgl_h_matrix_2d<double> &H_plane_to_image, vgl_h_matrix_2d<double> &H_image_to_plane);


  static bool read_cameras(const std::string filename, std::vector<vnl_double_3x3> &Ks, std::vector<vnl_double_3x3> &Rs, std::vector<vnl_double_3x1> &Ts);

  static bool write_cameras(const std::string filename, std::vector<vnl_double_3x3> &Ks, std::vector<vnl_double_3x3> &Rs, std::vector<vnl_double_3x1> &Ts);

  template<class T>
  static void warp_slab_bilinear(bvam_voxel_slab<T> const& slab_in, vgl_h_matrix_2d<double> invH, bvam_voxel_slab<T> &slab_out);

  template<class T, unsigned N>
  static bool img_to_slab(vil_image_view_base_sptr image, bvam_voxel_slab<vnl_vector_fixed<T,N> > &slab);

  template<class T>
  static bool img_to_slab(vil_image_view_base_sptr image, bvam_voxel_slab<T> &slab);

  template<class T, unsigned N>
  static bool slab_to_img(bvam_voxel_slab<vnl_vector_fixed<T,N> > const &slab, vil_image_view_base_sptr image);

  template<class T>
  static bool slab_to_img(bvam_voxel_slab<T> const& slab, vil_image_view_base_sptr image);

  template<class T>
  static void multiply_slabs(bvam_voxel_slab<T> const& s1, bvam_voxel_slab<T> const& s2, bvam_voxel_slab<T> &product);

  template<class T>
  static void add_slabs(bvam_voxel_slab<T> const& s1, bvam_voxel_slab<T> const& s2, bvam_voxel_slab<T> &sum);

  template<class T>
  static void threshold_slab_above(bvam_voxel_slab<T> const& slab, T const& thresh, bvam_voxel_slab<bool> &mask);

  template<class T>
  static void smooth_gaussian(bvam_voxel_slab<T> &slab, float stdx, float stdy);

  template<class T>
  static void write_slab_as_image(bvam_voxel_slab<T> const& slab_in,std::string filename);

  template<class T, unsigned N>
  static void write_slab_as_image(bvam_voxel_slab<vnl_vector_fixed<T,N> > const& slab_in,std::string filename);

  template<class T>
  static T sum_slab(bvam_voxel_slab<T> const& slab);
...
template<class T>
void bvam_util::warp_slab_bilinear(bvam_voxel_slab<T> const& slab_in, vgl_h_matrix_2d<double> invH, bvam_voxel_slab<T> &slab_out)
{
    ...
    bvam_voxel_slab<T> slab_in_smooth;
    ...
    bvam_voxel_slab<T>::iterator out_it = slab_out.begin();
...
template<class T, unsigned N>
bool bvam_util::img_to_slab(vil_image_view_base_sptr const image, bvam_voxel_slab<vnl_vector_fixed<T,N> > &slab)
{
  // check slab is preallocated to correct size
  if ( (slab.nx() != image->ni()) || (slab.ny() != image->nj()) ) {
    std::cerr << "error: slab and image are different sizes." << std::endl;
    return false;
  }
  ...
  if (image->pixel_format() == VIL_PIXEL_FORMAT_BYTE) {
    if (vil_image_view<unsigned char> *img_view = dynamic_cast<vil_image_view<unsigned char>*>(image.ptr())) {
      if (img_view->nplanes() == N) {
        ...
        bvam_voxel_slab<vnl_vector_fixed<T,N> >::iterator slab_it = slab.begin();
        ...
        std::cerr << "error: img_to_slab (multi-dimensional): nplanes = " << img_view->nplanes() <<", but N = " << N << std::endl;
        return false;
      }
    }
    else {
      std::cerr << "error: failed to cast image_view_base to image_view " << std::endl;
      return false;
    }
  }
  ...
  // take care of pixel format issues.
    ...
    if (vil_image_view<vil_rgb<unsigned char> > *img_view = dynamic_cast<vil_image_view< vil_rgb<unsigned char> >*>(image.ptr())) {
      ...
      if (img_view->nplanes() == 1) {
        ...
        std::cerr << "error: img_to_slab (multi-dimensional): nplanes = " << img_view->nplanes() <<", but N = " << 1 << std::endl;
        return false;
      }
    }
    else {
    ...
    std::cerr << "img_to_slab(scalar): unsupported pixel type " << std::endl;
  }
...
//:  if input image is a float image, assumes that it is already scaled to [0,1] range
template<class T>
bool bvam_util::img_to_slab(vil_image_view_base_sptr const image, bvam_voxel_slab<T> &slab)
{
  // check slab is preallocated to correct size
  if ( (slab.nx() != image->ni()) || (slab.ny() != image->nj()) ) {
    std::cerr << "error: slab and image are different sizes." << std::endl;
  ...
  if (image->pixel_format() == VIL_PIXEL_FORMAT_BYTE) {
    if (vil_image_view<unsigned char> *img_view = dynamic_cast<vil_image_view<unsigned char>*>(image.ptr())) {
      if (img_view->nplanes() > 1) {
        ...
        bvam_voxel_slab<T>::iterator slab_it = slab.begin();
    ...
    if (vil_image_view<vil_rgb<unsigned char> > *img_view_rgb = dynamic_cast<vil_image_view<vil_rgb<unsigned char> >*>(image.ptr())) {
      ...
      if (img_view_rgb->nplanes() == 1) {
        ...
        std::cerr << "error: pixel format RGB, but nplanes = " << img_view_rgb->nplanes() << std::endl;
        return false;
      }
...
template<class T, unsigned N>
bool bvam_util::slab_to_img(bvam_voxel_slab<vnl_vector_fixed<T,N> > const &slab, vil_image_view_base_sptr image)
{
  ...
  case VIL_PIXEL_FORMAT_BYTE:
    if (image->nplanes() ==N)
    {
      if (vil_image_view<unsigned char> *img_view = dynamic_cast<vil_image_view<unsigned char>*>(image.ptr())) {
        ...
        bvam_voxel_slab<vnl_vector_fixed<T,N> >::const_iterator slab_it = slab.begin();
        for (; slab_it != slab.end(); ++slab_it)
        {
          for (unsigned p=0; p<N; ++p) 
          {
      ...
      {
        std::cerr << "error in slab_to_image: failed to cast image_view_base to image_view " << std::endl;
        return false;
      }
    }
    else
    {
      std::cerr << "error in slab_to_image: incorrect number of image planes  " << std::endl;
      return false;
    }
    break;  
  ...
  case VIL_PIXEL_FORMAT_RGB_BYTE:
    ...
    if (vil_image_view<vil_rgb<unsigned char> > *img_view = dynamic_cast<vil_image_view< vil_rgb<unsigned char> >*>(image.ptr()))
    {
      ...
      {
        std::cerr << "error: slab_to_img (multi-dimensional): nplanes = " << img_view->nplanes() <<", but N = " << 1 << std::endl;
        return false;
      }
    }
    else 
  ...
  default:
    std::cerr << "img_to_slab: unsupported pixel type " << std::endl;
    return false;
    break;
  }
...
template<class T>
bool bvam_util::slab_to_img(bvam_voxel_slab<T> const& slab, vil_image_view_base_sptr image) 
{
  // check image is preallocated to correct size
  if ( (slab.nx() != image->ni()) || (slab.ny() != image->nj()) )
  {
  ...
  case VIL_PIXEL_FORMAT_BYTE: 
    if (vil_image_view<unsigned char> *img_view = dynamic_cast<vil_image_view<unsigned char>*>(image.ptr())) 
    {
      ...
      bvam_voxel_slab<T>::const_iterator slab_it = slab.begin();
      for (; img_it != img_view->end(); ++img_it, ++slab_it) {
...
template<class T>
T bvam_util::sum_slab(bvam_voxel_slab<T> const& slab)
{
  T sum = 0;
  bvam_voxel_slab<T>::const_iterator slab_it = slab.begin();
  for (; slab_it != slab.end(); ++slab_it) {
...
template<class T>
static void bvam_util::add_slabs(bvam_voxel_slab<T> const& s1, bvam_voxel_slab<T> const& s2, bvam_voxel_slab<T> &sum)
{
  // check sizes
  if ( (sum.nx() != s1.nx()) || (sum.nx() != s2.nx()) ||
    (sum.ny() != s1.ny()) || (sum.ny() != s2.ny()) ||
    (sum.nz() != s1.nz()) || (sum.nz() != s2.nz()) ) {
      std::cerr << "error: sizes of slabs to multiply do not match." << std::endl;
      return;
  }

  bvam_voxel_slab<T>::const_iterator s1_it = s1.begin(), s2_it = s2.begin();
  bvam_voxel_slab<T>::iterator sum_it = sum.begin();
...
template<class T>
void bvam_util::multiply_slabs(bvam_voxel_slab<T> const& s1, bvam_voxel_slab<T> const& s2, bvam_voxel_slab<T> &product)
{
  // check sizes
  ...
  bvam_voxel_slab<T>::const_iterator s1_it = s1.begin(), s2_it = s2.begin();
  bvam_voxel_slab<T>::iterator prod_it = product.begin();
...
template<class T>
void bvam_util::threshold_slab_above(bvam_voxel_slab<T> const& slab, T const& thresh, bvam_voxel_slab<bool> &mask)
{
  // check sizes
  ...
  bvam_voxel_slab<T>::const_iterator slab_it = slab.begin();
  bvam_voxel_slab<bool>::iterator mask_it = mask.begin();
...
template<class T>
void bvam_util::smooth_gaussian(bvam_voxel_slab<T> &slab, float stdx, float stdy)
{
  if ( (stdx < 0) || (stdy < 0) ) {
    std::cerr << "error: smooth_gaussian called with negative std. deviation!" <<std::endl;
    std::cerr << "stdx = " << stdx << "  " << "stdy = " << stdy << std::endl;
    return;
  }
  ...
  bvam_voxel_slab<T> slab_work(slab.nx(),slab.ny(),slab.nz());
  if (stdx > 0) {
...
// used for debugging
template<class T>
void bvam_util::write_slab_as_image(bvam_voxel_slab<T> const& slab_in,std::string filename)
{
  vil_image_view<T> img(slab_in.nx(),slab_in.ny(),1);
  vil_image_view<T>::iterator img_it = img.begin();
  bvam_voxel_slab<T>::const_iterator slab_it = slab_in.begin();
...
// used for debugging
template<class T, unsigned N>
void bvam_util::write_slab_as_image(bvam_voxel_slab<vnl_vector_fixed<T,N> > const& slab_in,std::string filename)
{
  vil_image_view<T> img(slab_in.nx(),slab_in.ny(),N);
  std::vector<vil_image_view<T>::iterator> img_its;
  for (unsigned p=0; p < N; ++p) {
    vil_image_view<T>::iterator plane_it = img.begin() + (p*img.planestep());
    img_its.push_back(plane_it);
  }
  bvam_voxel_slab<vnl_vector_fixed<T,N> >::const_iterator slab_it = slab_in.begin();
