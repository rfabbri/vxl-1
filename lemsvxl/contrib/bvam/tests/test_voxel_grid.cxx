...
{
  START("bvam_voxel_grid test");

  // we need temporary disk storage for this test.
  std::string storage_fname("bvam_voxel_grid_test_temp.vox");

  vgl_vector_3d<unsigned> grid_size(300,300,120);

  // try test with all types of underlying storage.
  std::vector<bvam_voxel_grid<float>* > grids;
  std::vector<std::string> grid_types; // for labeling tests
  grids.push_back(new bvam_voxel_grid<float>(storage_fname,grid_size)); // disk storage;
  grid_types.push_back("disk_storage");
  grids.push_back(new bvam_voxel_grid<float>(grid_size)); // memory storage;
  grid_types.push_back("memory storage");


  for (unsigned i=0; i<grids.size(); i++) {

    bvam_voxel_grid<float> *grid = grids[i];
    ...
    unsigned count = 0;
    std::cout << "read/write: ";
    bvam_voxel_grid<float>::iterator slab_it;
