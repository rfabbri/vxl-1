...
typedef bvam_voxel_traits<APM_MOG_GREY>::voxel_datatype apm_datatype;
...
generate_cameras_z(vgl_box_3d<double>& world)
{
  ...
  for(unsigned i=0; i<centers.size(); i++) {
    ...
    std::vector<vgl_point_3d<double> > corners = corners_of_box_3d(world);
    for(unsigned i=0; i<corners.size(); i++) {
      ...
      //std::cout << "Perspective [" << u << "," << v << "]" << std::endl;
      //std::cout << "Rational [" << u2 << "," << v2 << "]" << std::endl << std::endl;
    }
...
bool gen_images(vgl_vector_3d<unsigned> grid_size,
                bvam_voxel_world_sptr world,
               // bvam_world_params_sptr world_params,
                bvam_voxel_grid<float>* intensity_grid,
                bvam_voxel_grid<float>* ocp_grid,
                bvam_voxel_grid<apm_datatype>* apm_grid,
                ...
                unsigned int bin_num)
{
  ...
  bvam_voxel_grid<apm_datatype>::iterator apm_slab_it;
  bvam_voxel_grid<float>::iterator obs_it = intensity_grid->begin();
  bool update_status;
  //slab thickness is 1 
  bvam_voxel_slab<float>* weight = new bvam_voxel_slab<float>(grid_size.x(),grid_size.y(),1);  
  weight->fill(1);

  for (apm_slab_it = apm_grid->begin(); apm_slab_it != apm_grid->end(); ++apm_slab_it,++obs_it) {
    bvam_mog_grey_processor processor;
    update_status = processor.update(*apm_slab_it, *obs_it, *weight);
  }
  

 /* bvam_voxel_world_sptr world = new bvam_voxel_world();
  world->set_params(world_params);
  world->set_grid<OCCUPANCY>(0, ocp_grid);
  world->set_grid<APM_MOG_GREY>(0, apm_grid);*/
 // world->save_occupancy_raw("./test_gen_synthetic_world/ocp.raw");

 // world->save_occupancy_raw("./test_gen_synthetic_world/occp.raw");

  std::string path = "./test_gen_synthetic_world/test_img";
  for (unsigned i=0; i<cameras.size(); i++) {
    ...
    bvam_image_metadata camera(img_arg, cameras[i]);
...
bool reconstruct_world( bvam_voxel_world_sptr recon_world, std::vector<vpgl_camera_double_sptr>& cameras, 
                 std::vector <vil_image_view_base_sptr>& image_set,unsigned int bin_num)
{
  ...
  for (unsigned i = 0;i<cameras.size();i++)
  {
    bvam_image_metadata observation(image_set[i],cameras[i]);
                          ...
                          bvam_voxel_grid<float>* intensity_grid)
{
  ...
  bvam_voxel_grid<float>::iterator ocp_slab_it;
  bvam_voxel_grid<float>::iterator intensity_slab_it = intensity_grid->begin();
  ...
  std::ofstream is("test_gen_synthetic_world/intensity_grid.txt");
  unsigned z=nz;
  for (ocp_slab_it = ocp_grid->begin(); 
    ...
    z--;
    is << z << "--->" << std::endl;
    bvam_voxel_slab<float>::iterator vit,iit;
...
static void test_gen_synthetic_world()
{
  ...
  std::string model_dir("./test_gen_synthetic_world");
  ...
  bvam_world_params_sptr world_params = new bvam_world_params();
  ...
  bvam_voxel_grid<float>* ocp_grid = static_cast<bvam_voxel_grid<float>* > 
    (world->get_grid<OCCUPANCY>(0).as_pointer());

  bvam_voxel_grid<apm_datatype>* apm_grid_1 = static_cast<bvam_voxel_grid<apm_datatype>* >
    (world->get_grid<APM_MOG_GREY>(bin_num_1).as_pointer());

    bvam_voxel_grid<apm_datatype>* apm_grid_2 = static_cast<bvam_voxel_grid<apm_datatype>* >
    (world->get_grid<APM_MOG_GREY>(bin_num_2).as_pointer());

  bvam_voxel_grid<float>* intensity_grid = new bvam_voxel_grid<float>
  ...
  bvam_voxel_world_sptr recon_world = new bvam_voxel_world();

  bvam_world_params_sptr recon_world_params = new bvam_world_params();
