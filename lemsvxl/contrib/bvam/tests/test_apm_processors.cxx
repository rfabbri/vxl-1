...
void init_random_slab(bvam_voxel_slab<float>& slab)
{
  vnl_random rand;
...
void add_random_noise(bvam_voxel_slab<float>& slab, float std)
{
  vnl_random rand;
    for(unsigned int y=0; y<slab.ny(); ++y)
      for(unsigned int x=0; x<slab.nx(); ++x){
...
MAIN( test_apm_processors )
{
   ...
   bvam_voxel_slab<float> obs(10,10,1);
   init_random_slab(obs);

   bvam_voxel_slab<float> weight(10,10,1);
   ...
   bvam_voxel_slab<mix_gauss_type> appear(10,10,1);
