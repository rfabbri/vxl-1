...
#include "../bvam_rpc_registration_process.h"
...
MAIN( test_bvam_rpc_registration_process )
{
  ...
  bvam_world_params_sptr voxel_world_params = new bvam_world_params();
  voxel_world_params->set_params("C:\\temp\\voxel_world", vgl_point_3d<float> (-10.0,-10.0,-2.5), num_voxels, voxel_length, lvcs);
  bvam_voxel_world_sptr voxel_world = new bvam_voxel_world();
  ...
  bool good = bprb_batch_process_manager::instance()->init_process("BvamRpcRegistrationProcess");
  good = good && bprb_batch_process_manager::instance()->set_params("rpc_registration_parameters.xml");

  brdb_value_sptr v0 = new brdb_value_t<bvam_voxel_world_sptr>(voxel_world);
  ...
  TEST("run bvam_rpc_registration_process", good ,true);
