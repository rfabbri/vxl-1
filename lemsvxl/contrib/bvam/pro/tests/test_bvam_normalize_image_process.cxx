...
#include "../bvam_normalize_image_process.h"
...
void create_a_synthetic_slab(bvam_voxel_slab<float>& plane_img, unsigned nx, unsigned ny) {
...
bvam_voxel_slab_base_sptr create_mog_image_using_grey_processor(vcl_string model_dir, bvam_voxel_world_sptr& vox_world, vil_image_view_base_sptr& expected_img) {
  ...
  bvam_world_params_sptr params = new bvam_world_params();
  params->set_params(model_dir, corner, num_voxels, voxel_length);
  vox_world = new bvam_voxel_world(params);

  bvam_voxel_grid_base_sptr ocp_grid_ptr = vox_world->get_grid<OCCUPANCY>(0);
  bvam_voxel_grid<float> *ocp_grid = dynamic_cast<bvam_voxel_grid<float>*>(ocp_grid_ptr.ptr());
  // fill in grid with zeros to start
  ocp_grid->initialize_data(0.0f);
  // now make a ground plane
  bvam_voxel_grid<float>::iterator ocp_it = ocp_grid->slab_iterator(nz-1);
  (*ocp_it).fill(1.0f);
  // data not written to disk until iterator is iterated
  ++ocp_it;


  bvam_voxel_slab<float> plane_img(nx,ny,1);
  create_a_synthetic_slab(plane_img, nx, ny);
  
  typedef bvam_voxel_traits<APM_MOG_GREY>::voxel_datatype mog_type;

  bvam_voxel_traits<APM_MOG_GREY>::appearance_processor apm_processor;

  // create a slab of constant weights for update
  bvam_voxel_slab<float> ones(nx,ny,1);
  ones.fill(1.0f);
  ...
  bvam_voxel_grid_base_sptr apm_base = vox_world->get_grid<APM_MOG_GREY>(0);
  bvam_voxel_grid<mog_type> *apm_grid = dynamic_cast<bvam_voxel_grid<mog_type>*>(apm_base.ptr());
  // initialize the appearance model data to get rid of any previous data on disk
  apm_grid->initialize_data(bvam_voxel_traits<APM_MOG_GREY>::initial_val());
  
  bvam_voxel_grid<mog_type>::iterator apm_it = apm_grid->begin();
  ...
  bvam_image_metadata meta1(expected_img,cam1);
  vox_world->expected_image<APM_MOG_GREY>(meta1,expected_img, mask);

  bvam_image_metadata meta2(expected_img,cam1);  // image is passed to get ni and nj basically

  bvam_voxel_slab_base_sptr mog_image;
  vox_world->mixture_of_gaussians_image<APM_MOG_GREY>(meta2, mog_image, 0);
  return mog_image;
}

MAIN( test_bvam_normalize_image_process )
{
  unsigned ni = 640;
  unsigned nj = 480;
  ...
  bvam_voxel_slab<float> weights(ni, nj, 1);
  weights.fill(1.0f/(ni * nj));
  sum = bvam_util::sum_slab(weights);
  TEST_NEAR("check slab sum", sum, 1.0f, 0.01);

  //: test img to slab operation
  typedef bvam_voxel_traits<APM_MOG_RGB>::voxel_datatype mog_type_rgb;
  typedef bvam_voxel_traits<APM_MOG_RGB>::obs_datatype obs_datatype_rgb;

  typedef bvam_voxel_traits<APM_MOG_GREY>::voxel_datatype mog_type;
  typedef bvam_voxel_traits<APM_MOG_GREY>::obs_datatype obs_datatype;

  // convert image to a voxel_slab
  bvam_voxel_slab<obs_datatype> image_slab(ni, nj, 1);
  bvam_util::img_to_slab(input_img_float_stretched_sptr,image_slab);
  ...
  bvam_voxel_slab<obs_datatype_rgb> image_slab_rgb(ni, nj, 1);
  bvam_util::img_to_slab(input_img_rgb_float_stretched_sptr,image_slab_rgb);
  ...
  bvam_voxel_slab_base_sptr mog_image = create_mog_image_using_grey_processor("./test_world_dir", vox_world, expected_image);
  ...
  bvam_voxel_slab<mog_type>* mog_image_ptr = dynamic_cast<bvam_voxel_slab<mog_type>*>(mog_image.ptr());
  ...
  bool good = bprb_batch_process_manager::instance()->init_process("BvamNormalizeImageProcess");
  ...
  TEST("run bvam normalize image process", good ,true);
