...
void
bvam_io_config_parser ::cdataHandler(std::string name, std::string data)
{
...
bvam_io_config_parser::startElement(const char* name, const char** atts)
{
  ...
  if (std::strcmp(name, BVAM_PARAM_CORNER_TAG) == 0) {
  ...
  } else if (std::strcmp(name, BVAM_PARAM_VOXEL_LENGTH_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "l") == 0)
        convert(atts[i+1], voxel_length_);
    }

  } else if (std::strcmp(name, BVAM_APPEAR_MODEL_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "m") == 0)
        convert(atts[i+1], appear_model_);
    }
  } else if (std::strcmp(name, BVAM_APPEAR_NUM_MODES_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "n") == 0)
        convert(atts[i+1], appear_num_modes_);
    }
  } else if (std::strcmp(name, BVAM_NORM_INTENS_TAG) == 0) {
    std::string s;
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "status") == 0)
        convert(atts[i+1], s);
      if (std::strcmp(atts[i], "false")==0)
        norm_intenst_ = false;
      else if (std::strcmp(atts[i], "true")==0)
        norm_intenst_ = true;
      else 
        std::cerr << "Undefined status for " << BVAM_NORM_INTENS_TAG << std::endl;
    }
  } 
  
  else if (std::strcmp(name, BWAM_SCHEDULE_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "order_by_date") == 0)
          convert(atts[i+1], order_by_date_);
      else if (std::strcmp(atts[i], "shuffle") == 0)
          convert(atts[i+1], start_);
    }
  }
  else if (std::strcmp(name, BWAM_VIEW_SCHEDULE_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "order_by_date") == 0)
          convert(atts[i+1], order_by_date_);
  ...
  else if (std::strcmp(name, BWAM_SCHEDULE_SUBSET_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "start") == 0)
          convert(atts[i+1], order_by_date_);
      else if (std::strcmp(atts[i], "end") == 0)
          convert(atts[i+1], end_);
      else if (std::strcmp(atts[i], "inc") == 0)
          convert(atts[i+1], inc_);
    }
  }

  else if (std::strcmp(name, BVAM_PROCESS_RENDER_EXP_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "x") == 0)
  ...
  else if (std::strcmp(name, BVAM_PROCESS_RENDER_VIEW_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "x") == 0)
        convert(atts[i+1], X_);
  ...
  else if (std::strcmp(name, BWAM_VIEW_CAMERA_PATH_TAG) == 0) {
    for (int i=0; atts[i]; i+=2) {
      std::cout << "  Attr=" << atts[i] << "->" << atts[i+1] << std::endl;
      if (std::strcmp(atts[i], "index") == 0)
...
bvam_io_config_parser::endElement(const char* name)
{
   ...
   if (std::strcmp(name, BVAM_PARAM_CORNER_TAG) == 0) {
     ...
     sch_subset_ = false;
   } else if (std::strcmp(name, BVAM_PROCESS_TRAIN_TAG) == 0) {
     bvam_io_process *p = new bvam_io_process_train(image_path_, camera_path_, 
      light_path_, *sch_, output_path_);
     processes_.push_back(p);
      delete sch_;
   } else if (std::strcmp(name, BVAM_PROCESS_DETECT_CHANGE_TAG) == 0) {
     ...
     delete view_sch_;
     processes_.push_back(p);
   } else if (std::strcmp(name, BVAM_PROCESS_WRITE_RAW_TAG) == 0) {
...
void bvam_io_config_parser::trim_string(std::string& s)
{
