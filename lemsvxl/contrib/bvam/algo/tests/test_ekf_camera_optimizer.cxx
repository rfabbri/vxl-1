...
#include <bvam/bvam_voxel_grid.h>
#include <bvam/bvam_voxel_traits.h>

static void test_ekf_camera_optimizer()
{
  START("bvam_ekf_camera_optimizer test");
  ...
  bvam_voxel_world_sptr vox_world = new bvam_voxel_world(params);
 ...
 // now create a couple of cameras and generate the expected images
  ...
  bvam_image_metadata meta0(vil_image_view_base_sptr(0),cam0);
  bvam_image_metadata meta1(vil_image_view_base_sptr(0),cam1);
  bvam_image_metadata meta2(vil_image_view_base_sptr(0),cam2);
  ...
  bvam_ekf_camera_optimizer_state init_state(1e-2,center0,rot0,0.1,0.1);
  bvam_ekf_camera_optimizer optimizer(20.0,20.0,0.01,0.01,0.01,0.01,false,true,false);

  bvam_image_metadata img1_meta(img1,cam1);
  bvam_ekf_camera_optimizer_state end_state = 
  ...
  std::cout << "camera0 rotation : " << std::endl << rot0.as_rodrigues() << std::endl;
  std::cout << "camera1 center   : " << center1 << std::endl;
  std::cout << "camera1 rotation : " << std::endl << rot1.as_rodrigues() << std::endl;
  std::cout << "est. center      : " << center1_est << std::endl;
  std::cout << "est. rotation    : " << std::endl << rot1_est.as_rodrigues() << std::endl;
  ...
  bvam_image_metadata img2_meta(img2,cam2);
  ...
  std::cout << "camera1 center   : " << center1 << std::endl;
  std::cout << "camera1 rotation : " << std::endl << rot1.as_rodrigues() << std::endl;
  std::cout << "camera2 center   : " << center2 << std::endl;
  std::cout << "camera2 rotation : " << std::endl << rot2.as_rodrigues() << std::endl;
  std::cout << "est. center      : " << center2_est << std::endl;
  std::cout << "est. rotation    : " << std::endl << rot2_est.as_rodrigues() << std::endl;
