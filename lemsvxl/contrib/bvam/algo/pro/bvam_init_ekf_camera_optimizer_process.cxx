#include "bvam_init_ekf_camera_optimizer_process.h"
...
bvam_init_ekf_camera_optimizer_process::bvam_init_ekf_camera_optimizer_process()
{
...
bool bvam_init_ekf_camera_optimizer_process::execute()
{
  ...
  if (!parameters()->get_value(vcl_string("position_measurement_variance"), rot_var)) {
    vcl_cout << "bvam_init_ekf_camera_optimizer_process::execute() -- problem in retrieving parameter rotation_variance\n";
    return false;
  }
  if (!parameters()->get_value(vcl_string("position_measurement_variance"), pos_var)) {
    vcl_cout << "bvam_init_ekf_camera_optimizer_process::execute() -- problem in retrieving parameter position_variance\n";
    return false;
  }
    if (!parameters()->get_value(vcl_string("translation_scale"), t_scale)) {
    vcl_cout << "bvam_init_ekf_camera_optimizer_process::execute() -- problem in retrieving parameter translation_scale\n";
    return false;
  }

  bvam_ekf_camera_optimizer_state init_state(t_scale,cam0->camera_center(),cam0->get_rotation(),pos_var,rot_var);
  
  //store output
  brdb_value_sptr output0 = 
    new brdb_value_t<bvam_ekf_camera_optimizer_state>(init_state);
