...
bool bvam_init_ekf_camera_optimizer_process::execute()
{
  ...
  if (!parameters()->get_value(std::string("position_measurement_variance"), rot_var)) {
    std::cout << "bvam_init_ekf_camera_optimizer_process::execute() -- problem in retrieving parameter rotation_variance\n";
    return false;
  }
  if (!parameters()->get_value(std::string("position_measurement_variance"), pos_var)) {
    std::cout << "bvam_init_ekf_camera_optimizer_process::execute() -- problem in retrieving parameter position_variance\n";
    return false;
  }
    if (!parameters()->get_value(std::string("translation_scale"), t_scale)) {
    std::cout << "bvam_init_ekf_camera_optimizer_process::execute() -- problem in retrieving parameter translation_scale\n";
    return false;
  }
