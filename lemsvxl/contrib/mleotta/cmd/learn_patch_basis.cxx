...
int main(int argc, char** argv)
{
  ...
  std::cout << "num features: "<< num_clusters << std::endl;
  ...
  std::cout << "alpha = "<< alpha << " beta = "<<beta << " mix = "<<mix<<std::endl;
  std::cout << "num clusters = "<< clusters_idx.size() << std::endl;

  vnl_vector<double> mean(768,0.0);
  vnl_matrix<double> Sw(768,768,0.0);
  vnl_matrix<double> Sb(768,768,0.0);
  
  std::vector<vnl_vector<double> > c_means(clusters_idx.size(),vnl_vector<double>(768,0.0));
  for(unsigned i=0; i<clusters_idx.size(); ++i){
    const std::vector<unsigned>& c = clusters_idx[i];
    vnl_vector<double>& c_mean = c_means[i];
    const unsigned c_size = c.size();
    //double c_alpha = alpha*c_size + beta;
    std::vector<vnl_vector<double> > c_data(c.size());
    ...
    if(i%100 == 0)
      std::cout << "processed within "<< i << std::endl;
  }
  mean /= num_clusters;
  ...
  for(unsigned i=0; i<clusters_idx.size(); ++i){
    ...
    if(i%100 == 0)
      std::cout << "processed between "<< i << std::endl;
  }
  ...
  std::cout << "Solving Eigensystem" << std::endl;
  ...
  std::vector<vil_image_view<vxl_byte> > basis_patches;
  for(unsigned n=0; n<768; ++n){
    std::cout << sv(768-n-1) << std::endl;
