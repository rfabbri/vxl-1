...
int main(int argc, char** argv)
{
  ...
  bg_modeller->set_input_names(std::vector<std::string>(1,"video"));
  bg_modeller->set_output_names(std::vector<std::string>(1,"model"));
  ...
  std::cout << "Aligning the models" << std::endl;
  // run the process queue on the whole video
  do {
  ...
  std::cout << "Models aligned" << std::endl;
  ...
  std::vector<vil_image_resource_sptr> frames1, frames2, frames3;
  do {
  ...
  std::string outvid_dir = vorl1_manager::instance()->get_output_video_dir();
