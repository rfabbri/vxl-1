...
{
  if (argc < 8) {
    std::cout << "USAGE: sel_cmdline <image file name> <out_prefix> <det type> <sigma> <threshold> <interp> \n";
    std::cout << "                   <cv model> <dx> <dt> <nrad> <N_group> <min_length>\n";
    std::cout << "\n";
    std::cout << " o  image file name : image file \n";
    std::cout << " o  out_prefix      : prefix to the .edg and .cem file \n";
    std::cout << " o  det type        : edge detector type (gen=1, third order=2) \n";
    std::cout << " o  sigma, threshold: edge detector parameters \n";
    std::cout << " o  interp          : interpolation depth (int) \n";
    std::cout << " o  cv model        : curve mdoel for linking (lin=0, cc=1, es=2) \n";
    std::cout << " o  dx, dt          : edge uncertainty parameters \n";
    std::cout << " o  nrad            : size of the neighborhood radius \n";
    std::cout << " o  Ngroup          : min. size of cvlets to group \n";
    std::cout << " o  min_length      : min. size of curves to output \n";
    std::cout << "\n";
  ...
  std::string image_filename(argv[1]); //image filename
  std::string out_prefix(argv[2]);     //output file prefixes
  ...
  std::sprintf(out_cem_filename, "%s_%f_%f_%f_%d_%d.cem", out_prefix.c_str(), nrad, dx_sel, dt_sel, maxN, minLen);
