...
argument_iterator ait = argument_table.begin();
  ...
  // loop through entire std::string in case some flags were grouped
  unsigned int j = 1;
  while (j < s.length()) {
    char name = s[j];
...
void CommandOptions::print_usage(std::string executable)
{
  ...
  for (oi = option_table.begin(); oi != option_table.end(); ++oi)
    if (oi->short_name != '\0')
      std::cerr << ' ' << optionalify(std::string("-") + oi->short_name  + " " + oi->arg_name);
    else
      std::cerr << ' ' << optionalify("--" + oi->long_name + " " + oi->arg_name);
