...
{
  double chordVector = atan2(point2.getY()- point1.getY(), point2.getX()-point1.getX());
  Point2D<double> midChord = (point1+point2)/2;

  double chordL = hypot(point2.getY()- point1.getY(), point2.getX()-point1.getX());
  double d = sqrt(radius*radius - chordL*chordL/4);
    ...
    Point2D<double> &center, double &radius, ARC_NUD &nud) 
{
  ...
  double 
    psi1 = atan2(point2.getY()-point1.getY(),point2.getX()-point1.getX()) + M_PI/2,
    psi2 = atan2(point2.getY()-point3.getY(),point2.getX()-point3.getX()) + M_PI/2;

  double psihat = atan2(s2y-s1y, s2x-s1x);

  if (sin(psi2 - psi1)==0) // collinear
    return false;
  else {
    t = sin(psi2 - psihat )/sin(psi2 - psi1);
    H = hypot(s1y-s2y, s1x-s2x);

    center.setX(s1x + H*t*cos(psi1));
    center.setY(s1y + H*t*sin(psi1));
    ...
    double &radius, ARC_NUD &nud) 
{
  ...
  double u = (x2-x1)*cos(theta) + (y2-y1)*sin(theta),
  v = -(x2-x1)*sin(theta) + (y2-y1)*cos(theta);

  double r = .5*(v*v+u*u)/v;
  double cx = x1 - r*sin(theta), cy = y1 + r*cos(theta);
   // double theta1 = atan2(y1-cy, x1-cx),
   // theta2 = atan2(y2-cy, x2-cx);
...
{
  double thetai = atan2(end.getY()-center.getY(), end.getX()-center.getX());
...
inline double getTangentOfArc(const Point2D<double> &center, double radius, double theta_first, double theta_second,
  ...
  double thetai = atan2(subtangenti.getY(), subtangenti.getX());

  Point2D<double> start_subtan = start - center;
  double theta_start = atan2(start_subtan.getY(), start_subtan.getX());
  if(fabs(fmod(theta_start - theta_first, 2*M_PI)) < 1e-4)
    thetai += M_PI/2;
  else
    thetai -= M_PI/2;
  return thetai;
...
inline ARC_NUD getNudForSmallArc (double sx, double sy, double ex, double ey,
                       double cx, double cy)
{
  return ARC_NUD_CCW;
}
