...
int main(int argc, char *argv[]) {
  ...
  if (argc != 7) {
    ...
    std::cout << "Usage: <program name> <input_osl_name> <out_file> <double p_threshold> <k (number of classes)> <query pairs list> <db pairs list>\n";
    return -1;
  }
  ...
  std::string query_list = argv[5];
  std::string db_list = argv[6];
  ...
  std::map<std::pair<std::pair<unsigned int, unsigned int> , std::pair<unsigned int, unsigned int> >, std::pair<double, double> > matched_instances;
  ...
  if (if3) {
    ...
    while (!if3.eof()) {
      ...
      matched_instances[key] = std::pair<double, double> (shock, info);
      std::pair<std::pair<unsigned int, unsigned int> , std::pair<unsigned int, unsigned int> > key2(d_pair, q_pair);
      matched_instances[key2] = std::pair<double, double> (shock, info);
  ...
  std::map<std::pair<std::pair<unsigned int, unsigned int> , std::pair<unsigned int, unsigned int> >, std::pair<double, double> >::iterator iter;
  for (iter = matched_instances.begin(); iter != matched_instances.end(); iter++) {
    ...
    std::pair<double, double> costs = iter->second;
  ...
  for (unsigned q = 0; q <query_pairs.size(); q++) {
    ...
    for (unsigned d = 0; d <database_pairs.size(); d++) {
      ...
      if (iter == matched_instances.end()) {  // redo the matching, its not saved!!
        ...
        //std::map<std::pair<std::pair<int, int> , std::pair<int, int> >, std::pair<double, double> > matched_instances;
        matched_instances[key] = std::pair<double, double> (shock_matching_cost, info);
        matched_instances[key2] = std::pair<double, double> (shock_matching_cost, info);
