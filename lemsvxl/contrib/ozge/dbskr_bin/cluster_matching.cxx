...
dbskr_cluster_matching::dbskr_cluster_matching (std::string view_str, 
                                                std::string patch_dir, 
                                                std::string match_dir,
                                                std::string sampling_ds_str,
                                                int verbose) 
...
void dbskr_cluster_matching::execute_shock (bool match_with_circular_completions)
{
    ...
    for (unsigned i = 0; i < db_size_; i++) {
      ...
      std::string ins_name = database_ins_names_[i];
      std::string ins_dir = match_dir_+database_cat_names_[i]+"/"+ins_name+"/";

    for (unsigned j = i+1; j < db_size_; j++) 
    { 
      st2 = 0;  // clears scurves and shock graphs
      std::string ins_name2 = database_ins_names_[point];

      std::string output_name = ins_dir+ins_name+"-"+ins_name2+"-sc-"+sampling_ds_str_+"-normal-edit.bin";
      std::string output_name2 = match_dir_+database_cat_names_[j]+"/"+ins_name2+"/"+ins_name2+"-"+ins_name+"-sc-"+sampling_ds_str_+"-normal-edit.bin";
      ...
      if (!st1) {
        //: load str1
        std::string st_file1 = patch_dir_+database_cat_names_[i]+"/"+ins_name+"-patches"+sorting_method_str_+cat_par_ins_strs_[i]+"/"+ins_name+"_patch_storage.bin";
        st1 = load_str_from_file(st_file1);
      }

      if (!st2) {
        //: load str2  
        std::string st_file2 = patch_dir_+database_cat_names_[j]+"/"+ins_name2+"-patches"+sorting_method_str_+cat_par_ins_strs_[j]+"/"+ins_name2+"_patch_storage.bin";        
    ...
    if (LeadProcessor ())
    {
      ...
      for (unsigned int i = 0; i<db_size_; i++)
        for (unsigned int j = i+1; j<db_size_; j++) 
        {
          ...
          if (val != point)
            std::cout << "ERROR: in message passing for point: " << point << " mes received: " << val << " expecting: " << point << std::endl;
          point++;
        }
    }
 }
...
dbskr_shock_patch_storage_sptr dbskr_cluster_matching::load_str_from_file(std::string fname) {
  ...
  std::cout << st1->size() << " patches in str, reading shocks..\n";
  //: load esfs for each patch
  for (unsigned iii = 0; iii < st1->size(); iii++) {
    dbskr_shock_patch_sptr sp = st1->get_patch(iii);
    std::string patch_esf_name = fname.substr(0, fname.length()-str_name_end.size());
    std::ostringstream oss;
    oss << sp->id();
