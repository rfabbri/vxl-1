...
void save_image_poly(dbskr_shock_patch_sptr sp, 
  ...
  if (!poly) {
    std::cout << "Patch graph outer boundary could not be traced from the shock graph!! IMAGE NOT SAVED for " << sp->id() << "\n";
    return; 
  }
                                             ...
                                             bool save_images)
{
   ...
   std::cout << "extracting from an image NOT from a MASK\n";
...
void individual_usage() {
  std::cout << "Usage : <program name> <binary image_file> <output_dir> <min_depth> <max_depth>\n";
}

int main_individual(int argc, char *argv[]) {
  std::cout << "Extracting patches from an individual model image!\n";

  //: out file contains the wrong mathces if any
  std::string image_file, output_dir;
  ...
  std::string::size_type pos2 = image_file.find_last_of(".");
  std::string only_name = image_file.substr(pos+1, pos2);

  std::string out_dir = output_dir + only_name +"-"+argv[3]+"-"+argv[4]+"/";
  vul_file::make_directory_path(out_dir);

  std::string esf_file = out_dir + "/" + only_name;

  std::string kept_dir_name = out_dir+"patches/";
  vul_file::make_directory_path(kept_dir_name);
  std::string output_name = out_dir+only_name+"_patch_storage.bin";
...
void opelt_usage() {
  std::cout << "Usage : <program name> <image_dir> <class_initial> <class_list> <esf_dir (bnd dir)> <output_dir> <min_depth> <max_depth> <depth_int> <pruning_depth> <sorting threshold (0 if no aproiri elimination before sorting> <overlap threshold> <1 or 0>\n";
}
