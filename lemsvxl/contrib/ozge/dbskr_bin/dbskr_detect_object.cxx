...
int main_individual(int argc, char *argv[]) {
  ...
  std::string st_file1_dir, st_file2_dir, match_dir, ins_name1, ins_name2, bbox_file;
  ...
  //std::string st_file1 = st_file1_dir + ins_name1 + "-patches_color__6_18_3_2_0.5_0.8\\"+ins_name1+"_patch_storage.bin";
  std::string st_file1 = st_file1_dir + ins_name1 + "-patches_color__6_6_1_2_0.3_0.7\\"+ins_name1+"_patch_storage.bin";
  ...
  // create the input storage class
  std::string st_file2 = st_file2_dir + ins_name2 + "-map.esf_patch_storage.bin";
  ...
  std::string output_file = match_dir + ins_name1 + "-" + ins_name2 + "-map-match-"+argv[6]+"-no-circ.bin";
  ...
  //std::string first_patch_images_dir = st_file1_dir + ins_name1 + "-patches_color__6_18_3_2_0.5_0.8\\kept\\";
  std::string first_patch_images_dir = st_file1_dir + ins_name1 + "-patches_color__6_6_1_2_0.3_0.7\\kept\\";
  std::string second_patch_images_dir = st_file2_dir + ins_name2 + "-map.esf__kept\\";
  std::string out_html = match_dir + ins_name2 + "-map-" + ins_name1 + "-match-"+argv[6]+"-no-circ-top-10.html";
  ...
  //else
  //  std::cout << "problems in html!\n";

  //: find image patches with best model matches less than given threshold 
  std::vector<std::pair<dbskr_shock_patch_sptr, std::pair<int, dbskr_sm_cor_sptr> > > good_patches; 
  for (unsigned i = 0; i < pv1.size(); i++) {
      std::cout << i << " ";
      std::pair<int, dbskr_sm_cor_sptr> p = match->get_best_match(pv1[i]->id());
      if (p.second->final_norm_cost() < sim_threshold) {
        std::pair<dbskr_shock_patch_sptr, std::pair<int, dbskr_sm_cor_sptr> > pp;
        pp.first = pv1[i];
        pp.second = p;
  ...
  if (!good_patches.size()) {
    std::cout << "No object detected in this image!!\n";
    if (!box_gt)
      std::cout << "CORRECT non-detecion\n";
    else
      std::cout << "WRONG  there was a bbox gt but MISSED the entire object!\n";
    return 0;
  }

  std::sort(good_patches.begin(), good_patches.end(), fine_cost_less);
  std::cout << "there are " << good_patches.size() << " patches passing the threshold\n";
  
  vsol_box_2d_sptr detection_box = new vsol_box_2d();
  //int number_of_different_model_patches = 0;
  //std::map<int> seen_so_far;
  for (unsigned i = 0; i < good_patches.size(); i++) {
    ...
    std::cout << "patch: " << sp->id() << " passed with " << good_patches[i].second.second->final_norm_cost() << " matching to: " << good_patches[i].second.first << "\n";
    detection_box->grow_minmax_bounds(box);
  }
  std::cout << "object detected at: (" << detection_box->get_min_x() << ", " << detection_box->get_min_y() << ") ";
  std::cout << "(" << detection_box->get_max_x() << ", " << detection_box->get_max_y() << ") ";
  ...
  if (box_gt) {
    ...
    if (bsol_algs::intersection(detection_box, box_gt, inters)) {
      ...
      if (ratio >= 0.5) 
        std::cout << "CORRECT detecion box overlap with uni box with: " << ratio << " \n";
      else 
        std::cout << "WRONG detecion box overlap with uni box with: " << ratio << " \n";
    } else 
      std::cout << "WRONG there was bbox gt, but DETECTED a box with NO Intersection!!!\n";
  } else {
    std::cout << "WRONG there was NO bbox gt, but DETECTED something!!!\n";
  }
