...
int main(int argc, char *argv[]) {
  ...
  std::string st_file1_dir, st_file2_dir, match_dir, ins_name1, ins_name2, bbox_file, img_file1, img_file2;
  ...
  std::string sc_samp_ds_str = argv[8];
  int N = atoi(argv[9]);
  ...
  //std::string st_file1 = st_file1_dir + ins_name1 + ".esf_patch_storage.bin";
  std::cout << "storage file 1: " << st_file1 << std::endl;
  if (!vul_file::exists(st_file1)) {
    std::cout << "storge file 1 does not exist!!\n";
    return 0;
  }
  ...
  std::cout << st_test->size() << " patches in str1, reading shocks..\n";
  ...
  // create the input storage class
  std::string st_file2 = st_file2_dir + ins_name2 + ".esf_patch_storage.bin";

  if (!vul_file::exists(st_file2)) {
    std::cout << "storge file 2: " << st_file2 << " does not exist!!\n";
    return 0;
  }
  ...
  //: read the esfs as well
  std::cout << st_model->size() << " patches in str2, reading shocks..\n";
  //: load esfs for each patch
  ...
  std::vector<dbskr_shock_patch_sptr>& pv_test = st_test->get_patches();
  std::vector<dbskr_shock_patch_sptr>& pv_model = st_model->get_patches();

  dbskr_shock_patch_match_sptr match = new dbskr_shock_patch_match();
  patch_cor_map_type& map = match->get_map();

  std::string output_file = match_dir + ins_name1 + "-" + ins_name2 + "-match-"+sc_samp_ds_str+"-no-circ.bin";
  std::cout << "output_file: " << output_file << std::endl;
  //check if the outputfile exists
  if (!vul_file::exists(output_file)) {
    std::cout << "could not find: " << output_file << "!!!\n";
    return 0;
  } else {  // read the file
  ...
  std::map<int, dbskr_shock_patch_sptr> map_test, map_model;
  ...
  std::string model_patch_images_dir = st_file2_dir + ins_name2 + ".esf__kept\\";
  ...
  std::string out_html = match_dir + ins_name2 + "-" + ins_name1 + "-match-"+sc_samp_ds_str+"-no-circ-top-10-placements.html";
  std::string out_html_images_dir = match_dir + ins_name2 + "-" + ins_name1 + "-match-"+sc_samp_ds_str+"-no-circ-top-10-placement-images\\";
