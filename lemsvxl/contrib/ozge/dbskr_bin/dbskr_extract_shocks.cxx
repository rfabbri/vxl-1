...
void shock_usage() {
  std::cout << "Shock from Image Usage: <program name> <image_file> <output_file> <edge_detection_sigma> <pruning color threshold> <avg grad mag thres to keep the curve fragments>\n";
  std::cout << "edge detection sigma is usually 1\n";
  std::cout << "pruning color threshold is the threshold for LAB space color contrast, typically 0.4, used to prune out curve fragments\n";
}
int main_shock(int argc, char *argv[]) {
  std::cout << "extracting shock graph!\n";

  //: out file contains the wrong mathces if any
  std::string image_file, output_file;
...
void shock_from_mask_usage() {
   std::cout << "Shock From Mask Usage: <program name> <image_file> <output_file> <number of smoothing steps for discrete smoothing> \n";
}

int main_from_mask(int argc, char *argv[]) {
  std::cout << "extracting shock graph from a mask!\n";
...
void eth_usage() {
  std::cout << "Shock from ETH Usage: <program name> <image_dir> <output_dir> <view string> <edge_detection_sigma> <pruning_color_threshold>\n";
  std::cout << "view string is e.g. 090-180\n";
  std::cout << "edge detection sigma is usually 1\n";
  std::cout << "pruning color threshold is the threshold for LAB space color contrast, typically 0.4, used to prune out curve fragments\n";
}
int main_eth(int argc, char *argv[]) {
  std::cout << "extracting shock graphs from eth set!\n";

  //: out file contains the wrong mathces if any
  std::string image_dir, output_dir, view_str;
  ...
  for (unsigned i = 0; i < cats.size(); i++) {
    ...
    for (unsigned j = 1; j < 11; j++) {
      ...
      if (!img_sptr) {
        std::cout << "Problems in loading the image: " << (image_dir+ins_name+".png");
        continue;
      }
      /*
      dbsk2d_shock_graph_sptr sg = extract_shock(img_sptr, length_thresh, pruning_region_width, 
...
void opelt_train_usage() {
  std::cout << "Shock from Opelt Usage: <program name> <image_dir z:/images/.../ > <output_dir> <image list> <edge_detection_sigma> <color pruning threshold> <avg grad mag threshold (typically 5.0)> <class initial e.g. Horse_horse>\n";
  std::cout << "edge detection sigma is usually 1\n";
//  std::cout << "pruning color threshold is the threshold for LAB space color contrast, typically 0.4, used to prune out curve fragments\n";
}
int main_opelt_train(int argc, char *argv[]) {
  std::cout << "extracting shock graphs from opelt train set!\n";

  //: out file contains the wrong mathces if any
  std::string image_dir, output_dir, train_images_list, class_initial;
  ...
  std::ifstream fp(train_images_list.c_str());
  if (!fp) {
    std::cout<<" Unable to Open "<< train_images_list <<std::endl;
    return 0;
  }
...
//  double avg_grad_mag_thres = 5.0;
  ...
  //: load the images and extract
  for (unsigned i = 0; i < image_names.size(); i++) {
    std::string ins_name = output_dir+class_initial+image_names[i];
    std::cout << "\t\t" << ins_name << std::endl;
      ...
      if (!img_sptr) {
        std::cout << "Problems in loading the image: " << (image_dir+"image-"+image_names[i]+".png");
        continue;
      }
      //dbsk2d_shock_graph_sptr sg = extract_shock(img_sptr, length_thresh, pruning_region_width, 
      //                                       pruning_color_threshold, rms, smoothing_nsteps,
