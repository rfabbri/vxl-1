...
void individual_usage() {
    std::cout << "Usage : <program name> <image_file> <esf_file> <output_dir> <min_depth> <max_depth> <depth_int> <pruning_depth> <sorting threshold (0 if no aproiri elimination before sorting> <overlap threshold> <1 or 0>\n";
    std::cout << " if last value is 1 then image contrast criteria is used for sorting, otherwise contour ratio is used\n";
    std::cout << "pruning depth is the depth of tree which will be constructed at the root of a patch and all the other patches rooted at a node,\n";
    std::cout << " covered by this pruning tree will be pruned, typically 2. Min depth should be larger than two, otherwise nothing gets pruned at min depth\n";
    std::cout << "polygon area threshold is typically 20\n";
    std::cout << "the less the contour color contrast measure the better, so if the value is less than the given threshold then that patch will be added\n";
    std::cout << "overlap threshold: amount of overlap in terms of number of nodes in v_graphs to prune patches, typically %80, e.g. 0.8f\n";
    std::cout << "contrast measure values are in [0,1], so the threshold should be in this range\n";
}
...
void extract_subgraph_and_find_shock_patches_from_map(std::string image_file, 
                                             std::string esf_file, 
...
int main_individual(int argc, char *argv[]) {
  std::cout << "Extracting patches from an individual image!\n";

  //: out file contains the wrong mathces if any
  std::string image_file, esf_file, output_dir;
  ...
  std::cout << "min_depth: " << min_depth << " max_depth: " << max_depth << " depth int: " << depth_int << std::endl;
  bool contour_ratio = true;
  int iii = atoi(argv[10]);
  if (iii == 1)
    contour_ratio = false;

  std::string::size_type pos = esf_file.find_last_of("//");
  std::string::size_type pos2 = esf_file.find_last_of(".");
  std::string only_esf_name = esf_file.substr(pos+1, pos2);
  std::string boundary_file = esf_file.substr(0, pos2) + "_boundary.bnd";

  std::string kept_dir_name = output_dir+only_esf_name+"__kept/";
  std::string discarded_dir_name = output_dir+only_esf_name+"__discarded/";
  vul_file::make_directory_path(kept_dir_name);
  vul_file::make_directory_path(discarded_dir_name);
  std::string output_name = output_dir+only_esf_name+"_patch_storage.bin";
...
void eth_usage() {
  std::cout << "Usage : <program name> <image_dir> <category name> <view_sptr> <esf_dir (bnd dir)> <output_dir> <min_depth> <max_depth> <depth_int> <pruning_depth> <sorting threshold (0 if no aproiri elimination before sorting> <overlap threshold> <1 or 0>\n";
}

int main_eth(int argc, char *argv[]) {
  std::cout << "extracting shock patches from eth set!\n";
  std::string image_dir, esf_dir, output_dir, view_str, cat_str;
  ...
  std::string sort_type = "_contour_";
  bool contour_ratio = true;
  if (iii == 1) {
    contour_ratio = false;
    sort_type = "_color_";
  ...
  //for (unsigned i = 0; i < cats.size(); i++) {
    ...
    std::string out_dir = output_dir+cats[i]+"/";
    ...
    for (unsigned j = 1; j < 11; j++) {
      ...
      std::string ins_dir = out_dir+cats[i]+oss.str()+"-"+view_str+"-patches"+sort_type+"_"+argv[6]+"_"+argv[7]+"_"+argv[8]+"_"+argv[9]+"_"+argv[10]+"_"+argv[11]+"/";
      vul_file::make_directory_path(ins_dir);

      std::string ins_name = cats[i]+oss.str()+"-"+view_str;
      std::cout << "\t\t" << ins_name << std::endl;

      std::string image_file = image_dir+cats[i]+oss.str()+"/"+ins_name+".png";
      std::string esf_file = esf_dir + cats[i] + "/" + ins_name + ".esf";
      std::string bnd_file = esf_dir + cats[i] + "/" + ins_name + "_boundary.bnd";
      std::string kept_dir_name = ins_dir+"kept/";
      std::string discarded_dir_name = ins_dir+"pruned/";
      vul_file::make_directory_path(kept_dir_name);
      vul_file::make_directory_path(discarded_dir_name);
      std::string output_name = ins_dir+ins_name+"_patch_storage.bin";
...
void opelt_usage() {
  std::cout << "Usage : <program name> <image_dir> <class_initial> ";
  std::cout << "<class_list> <esf_dir (bnd dir)> <output_dir> <min_depth> ";
  std::cout << "<max_depth> <depth_int> <pruning_depth> ";
  std::cout << "<sorting threshold (0 if no aproiri elimination before sorting> <overlap threshold> <1 or 0>\n";
}

int main_opelt_train(int argc, char *argv[]) {
  std::cout << "extracting shock patches!\n";
  std::string image_dir, esf_dir, output_dir, class_initial, class_list, cat_str;
  ...
  std::ifstream fp(class_list.c_str());
  if (!fp) {
    std::cout<<" Unable to Open "<< class_list <<std::endl;
    return 0;
  }
  ...
  while (!fp.eof()) {
    ...
    if (name.size() > 2) {
      ...
      std::string nbr_str = buffer;
      std::cout << nbr << " " << nbr_str << std::endl;
  ...
  /*  std::string::size_type pos = image_names[i].find_last_of(class_initial.c_str());
    std::string::size_type pos2 = image_names[i].find_last_of(".");
    std::string name = image_names[i].substr(pos+1, pos2);
    
    //std::string name = image_names[i].substr(class_initial.length(), image_names[i].length()-4);
    image_names[i] = name;   // actually id
    */
    std::cout << image_names[i] << "\n";
  }

  std::string out_dir = output_dir+class_initial+"/";
  vul_file::make_directory_path(out_dir);
  for (unsigned i = 0; i < image_names.size(); i++) {

    std::string ins_dir = out_dir+class_initial+image_names[i]+"-patches"+sort_type+"_"+argv[6]+"_"+argv[7]+"_"+argv[8]+"_"+argv[9]+"_"+argv[10]+"_"+argv[11]+"/";
    std::cout << "ins_dir: " << ins_dir << std::endl;
    
    vul_file::make_directory_path(ins_dir);

    std::string ins_name = class_initial+image_names[i];
    std::cout << "\t\t" << ins_name << std::endl;

    std::string image_file = image_dir+image_names[i]+".png";
    std::string esf_file = esf_dir + class_initial+image_names[i]+".esf";
    std::string bnd_file = esf_dir + class_initial+image_names[i]+"_boundary.bnd";
    std::string kept_dir_name = ins_dir+"kept/";
    std::string discarded_dir_name = ins_dir+"pruned/";
    vul_file::make_directory_path(kept_dir_name);
    vul_file::make_directory_path(discarded_dir_name);
