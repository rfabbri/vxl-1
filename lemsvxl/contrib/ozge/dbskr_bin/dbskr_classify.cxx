...
void eth_usage() {
  std::cout << "Usage : <program name> <category name> <view_sptr> <patch_storage_dir> <match_output_dir> <scurve_sample_ds> <n> <visualization_n> <id>\n";
  ...
  std::cout << "n: use the top n best patch matches between the two patch sets\n";
}
int main_eth(int argc, char *argv[]) {
  ...
  std::string out_dir = match_dir+cats[outer_i]+"/";
  vul_file::make_directory_path(out_dir);
  std::cout << cats[outer_i] << "\n";
  //for (unsigned j = 1; j < 11; j++) {
  int j = ins_id;
    std::ostringstream oss;
    oss << j;

    std::string ins_name = cats[outer_i]+oss.str()+"-"+view_str;

    std::string out_html_file = match_dir+cats[outer_i]+"/"+cats[outer_i]+"_top_"+argv[7]+"_"+argv[8]+"_matches.html";
    ...
    std::string st_file1 = patch_dir+cats[outer_i]+"/"+ins_name+"-patches"+sorting_method_str+cat_par_strs[outer_i]+"/"+ins_name+"_patch_storage.bin";
    std::string patch_images_dir1 = patch_dir+cats[outer_i]+"/"+ins_name+"-patches"+sorting_method_str+cat_par_strs[outer_i]+"/kept/";
    ...
    std::cout << st1->size() << " patches in str1, skiiping reading shocks..\n";
    ...
    for (unsigned iii = 0; iii < st1->size(); iii++) {
      ...
      //  std::cout << "Bounding box not computed!!!!!!!!\n";
    ...
    //std::vector<dbskr_shock_patch_match_sptr> match_vec;
    std::vector<std::vector<dbskr_shock_patch_match_sptr> > match_vec;
    
    std::vector<std::string> class_names;
    std::vector<std::vector<std::string> > ins_names;
    for (unsigned c = 0; c < cats.size(); c++) {
      
      std::vector<dbskr_shock_patch_match_sptr> match_vec_c;
      std::vector<std::string> ins_names_c;
      for (unsigned cj = 1; cj < 11; cj++) {
        ...
        //: read the esfs as well
        std::cout << st2->size() << " patches in str2, skipping shocks..\n";
        //: load esfs for each patch
        ...
        std::string output_name_fine = ins_dir+ins_name+"-"+ins_name2+"-sc-"+argv[5]+"-normal-edit.bin";
        ...
        std::string output_name2_fine = match_dir+cats[c]+"/"+ins_name2+"/"+ins_name2+"-"+ins_name+"-sc-"+argv[5]+"-normal-edit.bin";
    ...
    /*if (match_vec.size() > 0) {
      ...
      if (best_match_norm_top_n(pv1, match_vec, n, best_id)) {
        std::cout << ins_name << "\n classified as " << class_names[best_id] << " best matching image: " << ins_names[best_id] << std::endl;
      } else {
        std::cout << "matches not complete for " << ins_name << std::endl;
      }
    } else {
      std::cout << "matches not complete for: " << ins_name << std::endl;
    }*/
    ...
    for (unsigned c = 0; c < cats.size(); c++) {
      std::vector<std::string> patch_image_dirs_c;
      for (unsigned cj = 1; cj < 11; cj++) {
        std::ostringstream oss2;
        oss2 << cj;
        std::string ins_name2 = cats[c]+oss2.str()+"-"+view_str;
        ...
        std::string patch_image_file2 = patch_dir+cats[c]+"/"+ins_name2+"-patches"+sorting_method_str+cat_par_strs[c]+"/kept/";
    ...
    if (match_strat1_simple_voting_top_n(pv1, match_vec, best_id, n, visualization_n, best_category_instance_ids)) {
      std::cout << ins_name << " patches voted for category: " << best_id << ": " << cats[best_id] << std::endl;
    } else 
      std::cout << ins_name << " could not be classified yet\n";
