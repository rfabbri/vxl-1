...
void eth_usage() {
  std::cout << "Usage : <program name> <image dir> <category name> <view_sptr> <patch_storage_dir> <match_output_dir> <scurve_sample_ds> <n>\n";
  ...
  for (unsigned j = 1; j < 11; j++) {
    ...
    for (unsigned c = 0; c < cats.size(); c++) {
      ...
      for (unsigned cj = 1; cj < 11; cj++) {
        ...
        std::string output_name = ins_dir+ins_name+"-"+ins_name2+"-sc-"+argv[6]+".bin";
        std::string output_name_fine = ins_dir+ins_name+"-"+ins_name2+"-sc-"+argv[6]+"-fine.bin";
        //std::string output_name_info = ins_dir+ins_name+"-"+ins_name2+"-sc-"+argv[6]+"-info.bin";
        std::string output_name2 = match_dir+cats[c]+"/"+ins_name2+"/"+ins_name2+"-"+ins_name+"-sc-"+argv[5]+".bin";
        std::string output_name2_fine = match_dir+cats[c]+"/"+ins_name2+"/"+ins_name2+"-"+ins_name+"-sc-"+argv[5]+"-fine.bin";


        //: load the image
        std::string image2_file = image_dir+cats[c]+oss2.str()+"/"+ins_name2+".png";
