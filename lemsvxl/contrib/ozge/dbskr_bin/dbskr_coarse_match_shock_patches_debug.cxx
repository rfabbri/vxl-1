...
int main(int argc, char *argv[]) {
  ...
  if (argc != 6) {
    std::cout << "Usage: <program name> <shock_patch_storage1> <shock_patch_storage2> <patch id1> <patch id2> <out_file>\n";
  ...
  std::cout << "matching patch1: " << id1 << " to patch2: " << id2 << std::endl;
  ...
  if (!sg1) {
    if (s1->extract_simple_shock()) 
      ...
      std::cout << "shock graph 1 is not available for patch: " << s1->id() << " skipping\n" << std::endl;
      return false;
    }
  }
  ...
  for (unsigned j = 0; j < pv2.size(); j++) {
    ...
    if (!sg2) {
      if (pv2[j]->extract_simple_shock()) 
        ...
        std::cout << "shock graph 2 is not available for patch: " << pv2[j]->id() << " skipping\n" << std::endl;
        continue;
      }
    }
    ...
    if (!tree2) {
      if (pv2[j]->prepare_tree())
        ...
        std::cout << "Tree 2 is not available for patch: " << pv2[j]->id() << " skipping\n" << std::endl;
        continue;
      }
    }
    std::cout << "\t\tpatch: " << pv2[j]->id() << " d: " << pv2[j]->depth() << " ";
    ...
    std::cout << " cost: " << val << " time: "<< t.real()/1000.0f << " secs.\n";
