...
/*
dbskr_tree_sptr read_esf_from_file(std::string fname, double sampling_ds, bool elastic_splice) {
...
void usage_evaluate() {
  std::cout << "Usage: <program name> <database_list> <shock pruning threshold> <shock curve sampling ds (5.0: coarse, 1.0: finest)> <outfile name> <0 if normal splice 1 if elastic splice cost> <0 if construct_circular_ends, 1 if no circular completions> <0 if original edit distance, 1 if edit distance with combined costs> \n";
}

int main_evaluate(int argc, char *argv[]) {

  std::cout << "Evaluating matched observations\n";

  std::string database_list, out_file;
  ...
  std::string pruning_str = argv[2];
  double sampling_ds = atof(argv[3]);
  std::string sampling_str = argv[3];
  
  bool elastic_splice_cost = false;
  int option = atoi(argv[5]);
  if (option == 1) {
    std::cout << "USING ELASTIC SPLICE COST!!!!!!!!!\n";
  ...
  option = atoi(argv[6]);
  if (option == 1) {
    std::cout << "Matching with NO Circular COMPLETIONS!!!!!!!!!\n";
  ...
  option = atoi(argv[7]);
  if (option == 1) {
    std::cout << "Using edit distance with COMBINED costs!!!!!!!!!\n";
    original_edit = false;
  }

  std::string elastic_str, circular_str, edit_str;
  ...
  out_file = std::string(argv[4]) + "_" + elastic_str + "_" + circular_str + "_" + edit_str + ".out";
  ...
  if (!vul_file::exists(out_file.c_str())) {
    std::cout << " out file does not exist!!\n";
    return -1;
  }
  ...
  std::ifstream ifs((out_file).c_str());
  char buf[1000];
  ...
  ifs >> Df;
  if (Df != D) {
    std::cout << "out file contains " << Df << "input database file size: " << D << " mismatch!! Quiting\n";
    return -1;
  }
  ...
  std::cout << "matching_costs 0 1: " << matching_costs[0][1] << " [D-2][D-1]: " << matching_costs[D-2][D-1] << std::endl;
  std::cout << "matching_costs norm 0 1: " << matching_costs_norm[0][1] << " [D-2][D-1]: " << matching_costs_norm[D-2][D-1] << std::endl;
  ifs.close();

  std::cout << "there are 9 classes with 11 instances each, it is assumed that the input database file was ordered!!\n";
  // find top 15 as in the Sebastian et al. PAMI 06 paper

  std::string out_html = out_file + "-top-15.html";
  ...
  std::vector<unsigned> correct(15, 0);
  std::vector<unsigned> wrong(15, 0);
  ...
  for (unsigned i = 0; i<D; i++) {
    ...
    std::string name = database[i];
    name = name.substr(0, name.length()-4) + ".png";
    std::string image_file = img_dir + name;
    ...
    for (unsigned m = 0; m < 15; m++) {
      ...
      if (print)
        std::cout << database[min_j] << " (val: " << min << ") ";
      ...
      std::string name2 = database[min_j];
      name2 = name2.substr(0, name2.length()-4) + ".png";
      std::string image_file2 = img_dir + name2;
  ...
  for (unsigned m = 0; m < 15; m++) {
    std::cout << "correct: " << correct[m] << " wrong: " << wrong[m] << " total: " << correct[m] + wrong[m] << std::endl;
    ...
    std::cout << "correct in top " << m+1 << ": " << sum_c << "% wrong in top " << m+1 << ": " << sum_w << "%" << std::endl;
  }
  std::cout << "--------- normalized ------------\n";
...
int main_match(int argc, char *argv[]) {
  ...
  std::string database_list, cons_dir, out_file, output_dir_shgm;
  ...
  std::string pruning_str = argv[3];
  double sampling_ds = atof(argv[4]);
  std::string sampling_str = argv[4];
  ...
  out_file = std::string(argv[5]) + "_" + elastic_str + "_" + circular_str + "_" + edit_str + ".out";

  bool save_shgm = false;
  if (argc == 10) {
    save_shgm = true;
    output_dir_shgm = std::string(argv[9])+"/";
  } else {
    output_dir_shgm = "";
  }
  ...
  for (unsigned int i = 0; i<D; i++) {
    std::string con_file = cons_dir + database[i];
  ...
  for (unsigned int i = 0; i<D; i++) {
    std::cout << "extracting shocks from: " << database[i] << std::endl;
  ...
  for (unsigned int i = i_start; i<D; i++) {
    ...
    for ( ; j<D; j++) {
      ...
      float val, norm_val;
      std::string sm_file_name1, sm_file_name2;
      ...
      if (vul_file::exists(sm_file_name1)) {
        ...
        std::cout << "matching " << database[i] << " (nodes: " << database_trees[i]->node_size() << ") and " << database[j] << " (nodes: " << database_trees[j]->node_size() << ")..." << std::endl;
        if (original_edit) {
        ...
        std::cout << "final cost: " << val << " final norm cost: " << norm_val << std::endl;
      ...
      if (vul_file::exists(sm_file_name2)) {
        ...
        std::cout << "matching " << database[j] << " (nodes: " << database_trees[j]->node_size() << ") and " << database[i] << " (nodes: " << database_trees[i]->node_size() << ")..." << std::endl;
        if (original_edit) {
          //instantiate the edit distance algorithm
