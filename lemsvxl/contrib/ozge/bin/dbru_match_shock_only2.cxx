...
int main_shock(int argc, char *argv[]) {
  std::cout << "Matching observations and shock graphs reading pairs from a list!\n";
  ...
  if (argc != 6 && argc != 7) {
    std::cout << "Usage: <program name> <pairs list> <esfs_dir> <shock curve sampling ds> <output_file> <output_dir_shgms>\n";
  ...
  out_file = std::string(argv[4])+"_"+std::string(argv[3])+"_pair_distances.out";
  ...
  while (!fpd.eof()) {
    ...
    if (temp.size() > 1) {
      ...
      std::cout << "pair: " << temp << std::endl;
      std::string::size_type pos = temp.find(" ", 0);
      std::string item1 = temp.substr(0, pos);
      std::string item2 = temp.substr(pos+1, temp.length());
      ...
      pairs_list.push_back(std::pair<std::string, std::string> (item1, item2));
    }
  }
  fpd.close();
  
  unsigned int D = pairs_list.size(); 
  std::cout << " pairs size: " << D << "\n";
#if 1

  std::cout << "printing items and ids: \n";
  for (iter = item_ids.begin(); iter != item_ids.end(); iter++) {
    std::cout << iter->first << " id: " << iter->second << "\n";
  }

  std::cout << "printing pairs list: \n";
  for (unsigned int i = 0; i<D; i++) {
    std::cout << pairs_list[i].first << " " << pairs_list[i].second << "\n";
  }
#endif
  ...
  for (iter = item_ids.begin(); iter != item_ids.end(); iter++) {
    std::string esf_file = esfs_dir + "crop" + iter->first + ".esf";
    std::cout << "reading esf: " << esf_file << std::endl;
  ...
  std::ifstream if3;
  std::map<std::pair<std::string, std::string>, float> pair_dists;

  if3.open(out_file.c_str());
  if (if3) {
    std::string item1, item2; float dist; bool tag = false;
    if3 >> item1;  // read first item
    while (!if3.eof()) {
      if3 >> item2;   // read j
      if (if3.eof()) break;
      if3 >> dist;
      std::pair<std::string, std::string> items(item1, item2);
  ...
  std::map<std::pair<std::string, std::string>, float>::iterator iter2;
  ...
  for (unsigned int is = 0; is<D; is++) {
    ...
    std::cout << pairs_list[is].first << " " << pairs_list[is].second << " ";
    iter2 = pair_dists.find(pairs_list[is]);
    if (iter2 == pair_dists.end()) {  // do the matching
      std::string namei = pairs_list[is].first;
      std::string namej = pairs_list[is].second;
      ...
      of3.open(out_file.c_str(), std::ofstream::app);
      of3 << namei << " " << namej << " " << matching_cost << "\n";
      std::cout << matching_cost << " " << t.real()/1000.0f << " secs\n";
      of3.close();
