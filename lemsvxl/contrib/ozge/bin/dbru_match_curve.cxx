...
int main_curve(int argc, char *argv[]) {
  std::cout << "Matching observations!\n";
  ...
  if (argc != 13) {
    std::cout << "Usage: <program name> <database_list> <images_dir> <cons_dir> <size> <additional_string> <rms> <rest_curve_match_ratio> <ratio (0.01: for dt arclength test)> <0 if line int original implementation, 1 if line int version 4> <increment if line int orig (1), total votes if line int version 4 (15)> <output_dir> <outfile name>\n";
  ...
  output_dir = std::string(argv[11])+std::string(argv[4])+"-"+addition+"/";
  out_file = std::string(argv[12])+"_"+std::string(argv[4])+"_";
  ...
  for (unsigned int i = i_start; i<D; i++) {
    ...
    for ( ; j<D; j++) {
      ...
      if (cost1 < cost2) {
        ...
        std::cout << "i: " << i << " j: " << j << " cost " << cost1 << " time: "<< t.real()/1000.0f << " ";
      } else {
        matching_costs[i][j] = cost2;
        matching_costs[j][i] = cost2;
        std::cout << "i: " << i << " j: " << j << " cost " << cost2 << " time: "<< t.real()/1000.0f << " "; 
      }
