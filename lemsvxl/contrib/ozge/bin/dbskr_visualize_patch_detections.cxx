...
bool dbskr_visualize_patch_detections::initialize()
{
  ...
  //std::string model_storage_file = params_.model_dir_ + params_.model_name_ + params_.model_patch_params_.output_file_postfix_ + "/" + params_.model_name_ + params_.model_patch_params_.output_file_postfix_ + "-" + storage_end;
  ...
  //std::string query_st_file = query_path_ + "/" + query_name_ + "/" + query_name_ + params_.query_patch_params_.output_file_postfix_ + "/" + query_name_ + params_.query_patch_params_.output_file_postfix_ + "-" + storage_end;  
  //std::string query_st_file = query_path_ + "/" + query_name_ + "/" + query_name_ + params_.query_patch_params_.output_file_postfix_ + "-" + storage_end;    
  std::string query_st_file = query_path_ + "/" + query_name_ + params_.query_patch_params_.output_file_postfix_ + "-" + storage_end;  
    
  if (!vul_file::exists(query_st_file)) {
    std::cout << "dborl_detect_patch_processor::initialize() - " << query_st_file << " not found\n";
    return false;
  }
  ...
  //: load the shock graphs --> assumes each shock graph for each patch is saved in the same folder
  std::cout << query_st->size() << " patches in test storage, reading shocks..\n";
  //: load esfs for each patch
  ...
  std::string match_name = params_.matches_dir_ + params_.model_name_ + params_.model_patch_params_.output_file_postfix_ + "-" + query_name_ + params_.query_patch_params_.output_file_postfix_ + params_.shock_match_params_.output_file_postfix_ + ".bin";
...
bool dbskr_visualize_patch_detections::process()
{
  ...
  if (!create_only_html_) {
    ...
    std::string model_shock = params_.model_dir_ + params_.model_name_ + ".esf";
    dbsk2d_xshock_graph_fileio loader;
    dbsk2d_shock_graph_sptr model_sg = loader.load_xshock_graph(model_shock);
    
    //std::string query_shock = query_path_ + query_name_ + "/" + query_name_ + ".esf";
    std::string query_shock = query_path_ + query_name_ + ".esf";
