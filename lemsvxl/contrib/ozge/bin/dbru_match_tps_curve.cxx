...
int main_curve_tps(int argc, char *argv[]) {
  std::cout << "Matching observations using TPS of elastic curve correspondences!\n";
  ...
  if (argc != 11) {
    std::cout << "Usage: <program name> <database_list> <images_dir> <cons_dir> <size> <additional_string> <rms> <rest_curve_match_ratio> <increment> <output_dir> <outfile name>\n";
  ...
  output_dir = std::string(argv[9])+std::string(argv[4])+"-"+addition+"/";
  out_file = std::string(argv[10])+"_"+std::string(argv[4])+"_";
