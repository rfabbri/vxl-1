...
#include <ozge/face_gui/pro/dbru_load_facedb_process.h>
//#include <dbru/pro/dbru_run_facedb_process.h>
...
//  REG_DISPLAYER( dbru_rcor_displayer );
  REG_DISPLAYER( dbru_facedb_displayer );
...
//  REG_STORAGE( dbru_rcor_storage );
  REG_STORAGE( dbru_facedb_storage );
...
//  REG_PROCESS( dbsol_scan_polygon_process );
  ...
  REG_PROCESS( dbru_load_facedb_process );
  REG_PROCESS( dbru_save_facedb_process );
  REG_PROCESS( dbru_create_empty_facedb_process );
  //REG_PROCESS( dbru_run_facedb_process );

  matching_gui_menu menubar;
  vgui_menu menu_holder = menubar.setup_menu(); 
  unsigned w = 512, h = 512;
  std::string title = "Shock 2D GUI";
