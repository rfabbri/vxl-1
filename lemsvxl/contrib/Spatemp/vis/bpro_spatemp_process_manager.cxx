...
bool
bpro_spatemp_process_manager::get_process_input_names(std::map<std::string,std::vector<std::string> > &input_names,
...
//takes care of seeing streams as images for processes
bool
bpro_spatemp_process_manager::get_process_output_names(std::map<std::string,std::vector<std::string> > &output_names,
                                                  const bpro_process_sptr& process )
{
...
bool 
bpro_spatemp_process_manager::add_missing_input_names(std::vector<std::string> &input_names, const bpro_process_sptr& process)
{
...
bool 
bpro_spatemp_process_manager::new_ostream(const std::string& directory,
...
bool 
bpro_spatemp_process_manager::new_obj_stream(const std::string& directory,
                                     const std::string& storage_name)
{
    ...
    DATABASE_MANAGER->add_tuple("global_data", new brdb_tuple(storage_name,std::string("object stream"),sto));
  ...
  for (; it != process_queue.end(); ++it, i++){
      ...
      mit!=ostream_names.end(); mit++)
    {
      ...
      if(type=="ostream")
      {
        ...
        DATABASE_MANAGER->add_tuple("global_data", new brdb_tuple(mit->second,std::string("istream"),is_sto));
...
bool
bpro_spatemp_process_manager::run_process_queue_on_current_frame( std::set<bpro_storage_sptr>* modified )
{
  std::vector< bpro_process_sptr >::iterator it = process_queue.begin();
  unsigned i = 0;
  for (; it != process_queue.end(); ++it, i++){
