...
reconstruct_multiview()
{
  ...
  for (unsigned v=0; v < nviews_; ++v) {
    if (!selected_crv_[v]) {
      std::cout << "Error: You must select a curve in all views by clicking.\n";
      return;
    }
  }
  ...
  fname = prefix + std::string("-2v_kanatani") + ext;
  fcrv_3d_2v_kanatani.open(fname.c_str(),std::ios::out | std::ios::binary);
  std::cout << "Writing 3d curve: " << fname << std::endl;
  ...
  for (unsigned di0=0; di0 + ini_idx <= end_idx; ++di0) {
    ...
    { // for comparison, use Kanatani's optimal triangulation from 2 views: first + last
      std::cout << "reconstruct from 2 views optimal/Kanatani\n";
...
{
  std::cout << "  Reconstructing using Kanatani.\n";
  assert(pt_img.size() > 1);
  assert(views.size() == 1);
