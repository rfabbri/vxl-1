...
update_pn_(const vsol_point_2d_sptr &pt)
{
  ...
  curvelet_ = get_subcurve(selected_crv_[0], p0_idx_, pn_idx_);
  tab_[0]->remove(curvelet_soview_);
  curvelet_soview_ = tab_[0]->add_vsol_polyline_2d(curvelet_,cc_style_);
...
swap_p0_and_pn_()
{
  ...
  p0_idx_ = pn_idx_;
  pn_idx_ = idx_aux;
  curvelet_ = reverse_order(selected_crv_[0]);
...
//  std::cout << "Curve segmt size: " << end_idx - ini_idx + 1 << std::endl;
  ...
  for (unsigned i=ini_idx; i <= end_idx; ++i) {
    ...
    vgl_homg_line_2d<double>  epi_left = l_epipolar_line( fm_[0][1], ep_[0].back());
...
get_images()
{
  ...
  for (unsigned i=0; i<2; ++i) {
    ...
    vidpro1_image_storage img_storage;
    ...
    img_[i] = vil_image_view < float > (bview);
    assert(img_[i]());
...
match_using_hog()
{
  // Selected curve in img_[0] is  selected_crv_[0]; the selected segment is curvelet_
  // The candidate curves in img[1] are crv_candidates_

  // 1 - compute descriptors for curvelet_
