...
for i=1:length(distribs)
  distribs{i} = distribs{i}/100; % sphere radius
end
...
for il=1:size(llines,1)
  ...
  ybase=-0.18e-3;
  if tp == n_sub
    text(x+0.01,0+ybase,sub_tags(tp), 'HorizontalAlignment', 'center', 'Fontsize', 8, 'color', color(tp,:)*0.8)
  elseif tp == n_sub-1
    text(x-0.01,0+ybase,sub_tags(tp), 'HorizontalAlignment', 'center', 'Fontsize', 8, 'color', color(tp,:)*0.8)
  else
    text(x,0+ybase,sub_tags(tp), 'HorizontalAlignment', 'center', 'Fontsize', 8, 'color', color(tp,:)*0.8)
  end
...
if length(sub_axis_label) ~= 0
  yylim = ylim;
  text(min(llines(end).XData)-0.178,0-yylim(1)/2.1,'\Delta_\theta \rightarrow', 'HorizontalAlignment', 'right', 'Fontsize', 12, 'color', 'k')
end
