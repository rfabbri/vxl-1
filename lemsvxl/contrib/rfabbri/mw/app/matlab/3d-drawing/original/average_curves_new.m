...
load intermediate-amsterdam_half.mat;
            ...
            for cc=1:num_curves_in_cluster
                ...
                for vcl=1:numIM
                    ...
                    for cl=1:size(cur_view,2)
                        ...
                        new_mask = cur_mask;
                        for ss=3:numSamples-2
                            if(cur_mask(ss-1,1)==0 && cur_mask(ss+1,1)==0)
                                new_mask(ss,1) = 0;
                            elseif(cur_mask(ss-2,1)==0 && cur_mask(ss+2,1)==0)
                                new_mask(ss-1,1) = 0;
                                new_mask(ss,1) = 0;
                                new_mask(ss+1,1) = 0;
                            end
...
%         end
        ...
        %Write all the merged branches into vrml files
        for br=1:size(curve_graph,1)
            cur_branch = curve_graph{br,1};
            write_curve_to_vrml(cur_branch,round(255.*colors(color_counter,:)),['./ply/curve',num2str(color_counter),'_branch',num2str(br),'.ply']);
        end
        ...
        save('curve_graph_amsterdam_half_3-2.mat','complete_curve_graph','all_junctions');
    end
end
