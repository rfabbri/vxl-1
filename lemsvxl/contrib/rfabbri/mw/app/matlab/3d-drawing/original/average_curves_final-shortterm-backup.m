...
% for v=1:numIM
% 
%     mypath=['./paper-runs/amsterdam_house/half/',num2str(v-1)];
%     recs = cell(0,0);
%     tangs = cell(0,0);
% 
...
%     
%     cons = read_cons(['./curves/',num2str(all_views(1,vv),'%02d'),'.cemv'],[num2str(all_views(1,vv),'%02d'),'.jpg'],0,-1);
%     num_im_contours = size(cons,2);
%     
...
load intermediate-amsterdam-half.mat;
...
%                 end
                ...
                for cc=1:num_curves_in_cluster
                    ...
                    for ms=1:cur_size
                         ...
                         if(dist_to_prev_sample>0.01 && (currently_traversing_branch || ~merged_corresp_mask_cumulative(ms,1)))
                             no_init_flag = 1;
                             if(currently_traversing_branch)
                                 ...
                                 if(isempty(arr_branch_id_last))
                                     if(query_branch_size >= branch_size_threshold && get_length(queryCurve(new_branch_start:ms-1,:)) >= branch_length_threshold)
                         ...
                         if(~merged_corresp_mask_cumulative(ms,1))
                             ...
                             elseif(ms==cur_size)
                                 ...
                                 if(isempty(arr_branch_id_last))
                                     ...
                                     if(query_branch_size < branch_size_threshold || get_length(queryCurve(new_branch_start:ms,:)) < branch_length_threshold)
                                         continue;
                                     end
                                     ...
                                     if(query_branch_size < branch_size_threshold || get_length(queryCurve(new_branch_start-1:ms,:)) < branch_length_threshold)
                                         continue;
                                     end
                         ...
                         %with the relevant branches
                             ...
                             if(currently_traversing_branch)
                                 ...
                                 %If the branch is not long enough, do
                                 %nothing
                                 if(query_branch_size >= branch_size_threshold)
