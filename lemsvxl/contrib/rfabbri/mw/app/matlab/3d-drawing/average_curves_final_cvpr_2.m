...
for v=1:numIM
    ...
    mypath=['./paper-runs/pavilion-midday-half/',num2str(v-1),'/12/'];
    recs = cell(0,0);
    tangs = cell(0,0);
...
load edge-curve-index_yuliang_pavilion_half.mat;
load edge-curve-offset_yuliang_pavilion_half.mat;
...
save intermediate-pavilion-half_12.mat;
...
%                             end
        ...
        for lcs=2:size(longest_curve,1)
            ...
            if(cur_dist>sample_dist_threshold)
...
%                 end
                ...
                for cc=1:num_curves_in_cluster
                    ...
                    for ms=1:cur_size
                         ...
                         if(dist_to_prev_sample>sample_dist_threshold && currently_traversing_branch)
                         ...
                         if(merged_corresp_mask_cumulative(ms,1))
                             ...
                             if(min_branch_dist > sample_dist_threshold)
                                merged_corresp_mask_cumulative(ms,1) = 0;
                             end
                         end
                         
                         if(dist_to_prev_sample>sample_dist_threshold && ~merged_corresp_mask_cumulative(ms,1))
                             no_init_flag = 1;
                         end
...
%                                  end
                    ...
                    if(is_merged)
                        ...
                        all_flags{all_views(1,mrv)+1,1}(1,curveID) = 0;
                    end
                end
...
for v=1:numIM
    ...
    for c=1:num_curves
      if(all_flags{v,1}(c,1)<num_shared_edges)
        curve_graph = [curve_graph; cell(1,1)];
        cg_size = size(curve_graph,1);
        curve_graph{cg_size,1} = all_recs{all_views(1,av)+1,1}{1,c};
      end
    end
end
