...
for v=1:numIM
    ...
    mypath=['./paper-runs/vase-eval/',num2str(v-1),'/11/'];
    recs = cell(0,0);
    tangs = cell(0,0);
...
load edge-curve-index_yuliang_vase.mat;
load edge-curve-offset_yuliang_vase.mat;
...
%Read all the edge support evidence
disp('READING DATA');
fileNames = dir('./curves/vase/*.cemv');
for v=1:numViews
%for v=1:1
    all_views(1,v)
    ...
    cons = read_cons(['./curves/vase/',viewName,'.cemv'],[viewName,'.jpg'],0,-1);
    num_im_contours = size(cons,2);
    [all_links_3d{all_views(1,v)+1,1}, all_offsets_3d{all_views(1,v)+1,1}, all_edge_support_3d{all_views(1,v)+1,1}] = read_association_attributes_11(all_views(1,v),num_im_contours,all_nR(all_views(1,v)+1,1),numIM);
    all_num_im_contours(all_views(1,v)+1,1) = num_im_contours;
...
fileNames = dir('./edges/vase/*.edg');
for v=1:numIM
    v
    fileName = fileNames(v,1).name;
    eloc = strfind(fileName,'.edg');
    viewName = fileName(1,1:(eloc-1));
    [edg edgmap] = load_edg(['./edges/vase/',viewName,'.edg']);
...
for vv=1:numViews
    ...
    fid = fopen(['./curve-edge/vase/',viewName,'.txt']);
...
for vv=1:numViews
    ...
    for crv = 1:all_nR(vview,1)      
        ...
        fileNames = dir('./calibration/vase/*.projmatrix');
        ...
        for v=1:numIM
            ...
            fid = fopen(['./calibration/vase/',viewName,'.projmatrix']);
...
save intermediate-vase_11.mat;
all_clusters = cell(0,0);
num_shared_edges = 5;
junction_dist_threshold = 0.002;
branch_length_threshold = 0.002;
sample_merge_threshold = 0.0003;
...
write_curve_graph_to_vrml_11;
save('curve_drawing_vase_11.mat','complete_curve_graph','all_junctions');
