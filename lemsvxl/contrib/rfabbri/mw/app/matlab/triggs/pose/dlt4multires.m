...
function [u,cond] = dlt4multires(M);
   A = zeros(80,56);
   A(1,47) = M(1,1);
   A(2,39) = M(1,1);
   A(3,38) = M(1,1);
   ...
   % S = diag(S)'
   % sum(abs(A)),sum(abs(A'))
   % fprintf(1,'%d,',sum(A<>0)/4); fprintf(1,'\n');
   % [sum(A<>0)/4;sum(abs(A))]%,[sum(A'<>0);sum(abs(A'))]
   % sqrt(sum(A.^2)) ./ sum(A<>0)
   % D = [1,2,2,3,4,3,3,5,5,3,2,4,5,4,2,1,2,3,3,2,1,2,4,4,5,7,5,4, ...
   %      7,7,4,2,4,5,4,2,3,5,5,5,7,5,3,5,5,3,3,4,4,3,4,3,2,2,2,1];
   % A = A * diag(D);
