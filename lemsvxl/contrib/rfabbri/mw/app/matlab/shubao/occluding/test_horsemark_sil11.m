...
%% parameters controlling the output
    resultpath = '../result/horsemark_sil11/';
...
fprintf(1, 'Loading data and computing the edges with gradient and orientation ...\n');
    data.imnum = 11; 
    datapath = '../data/horsemark/silhouettes/';
    anglearray = -60:12:60;
    for i = 1:data.imnum
        fprintf(1,'[%d]',i);
        if anglearray(i) < 0
            data.imnamecell{i} = [datapath 'm' num2str(abs(anglearray(i))) '.png'];
        elseif anglearray(i) == 0
            data.imnamecell{i} = [datapath 'p00.png'];
        else
            data.imnamecell{i} = [datapath 'p' num2str(abs(anglearray(i))) '.png'];
        end
        ...
        if DEBUG
            imwrite(full(data.edgemap{i}), ['../debug/horsemark_sil11/' 'edgemap_' int2strfixed(i,4) '.png']);
        end
    ...
    if DEBUG
        save('../debug/horsemark_sil11/programdata.mat');
    end

%% main work
    [pts3drawcell, normalrawcell, confrawcell] = struct_from_sil2(data,1,1,'horsemark_sil11');
