...
fprintf(1, 'Loading data and computing the edges with gradient and orientation ...\n');
    ...
    datapath = '../data/hand/silhouettes/';
    for i = 1:data.imnum
        fprintf(1,'[%d]',i);
        data.imnamecell{i} = [datapath int2strfixed((i-1)*25,3) '-segmented.png'];
        ...
        if DEBUG
            imwrite(full(data.edgemap{i}), ['../debug/hand_sil27/' 'edgemap_' int2strfixed(i,4) '.png']);
        end
    ...
    if DEBUG
        save('../debug/hand_sil27/programdata.mat');
    end

%% main work
    [pts3drawcell, normalrawcell confrawcell] = struct_from_sil(data,1,10,'hand_sil27');
