...
% stable cost, using both multiplication by cosines and by average radius.
function [M,v] = four_line_test_cos_radius(e0,e1,p0,p1,ctroid0,ctroid1) 
...
%  r= norm(e0);
  ...
  rbar = norm(e1-ctroid1);
%  rbar = norm(e1);
  rcos_bar = rcos_bar./ ns;
  rsin_bar = rsin_bar./ ns;

%  M = [ rsin.*rsin_bar*r*rbar, rsin.*rcos_bar*r, rsin_bar.*rcos*rbar, rcos.*rcos_bar ];
%  M = [ rsin.*rsin_bar*r*rbar, rsin.*rcos_bar*r*rbar, rsin_bar.*rcos*r*rbar, rcos.*rcos_bar*r*rbar ];
