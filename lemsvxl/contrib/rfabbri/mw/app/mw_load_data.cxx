...
load_rotational_analytic_views(unsigned nviews, unsigned ngrids_wanted, bool ctspheres)
{
  ...
  if (ctspheres) {
    ...
    bdifd_turntable::internal_calib_ctspheres(Kmatrix,x_max_scaled);
  } else {
...
//    double x_max_scaled = 255;
  ...
  for (unsigned i=0; i < nviews; ++i) { // 1 degree per view
    ...
    if (ctspheres)
      P = bdifd_turntable::camera_ctspheres(2*i, K);
    else {
      P = bdifd_turntable::camera_olympus(i, K);
    }
    cam[i].set_p(*P);
...
//      bdifd_data::space_curves_olympus_turntable( crv3d );
    ...
    for (unsigned  i=0; i < crv3d.size(); ++i) {
      bdifd_data::project_into_cams(crv3d[i], cam, crv2d[i]);
    }
  }
