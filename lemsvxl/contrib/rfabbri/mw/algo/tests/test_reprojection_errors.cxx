...
compute_and_print_info(
    const std::vector<double> &err_v,
    const std::vector<unsigned> &valid_idx,
    std::vector<double> &max_err_v,
    std::vector<double> &min_err_v,
    std::vector<double> &mean_err_v,
    std::vector<double> &median_err_v,
    ...
    const std::vector<std::vector<bdifd_3rd_order_point_3d> > &crv3d)
{
  ...
  { //: first, distance to reprojection using ideal cameras should be low for all non-degenerate points:
    ...
    std::cout << "Valid points: " << 100.0*(double)nvalid/(double)crv2d_gt_[0].size() << "% (" << nvalid << "/" << crv2d_gt_[0].size() << ")\n";
  ...
  std::vector<double> perturb_angle_v, 
...
//      const double angle3_perturb = 5+dtheta;
    ...
    for (unsigned i=0; i < err_pos_sq_v.size(); ++i)
      err_pos_sq_v[i] = std::sqrt(err_pos_sq_v[i]);
...
//    if (perturb_angle_v.size() % 10 == 0) {
      std::cout << "Valid points: " << 100.0*(double)valid_idx.size()/(double)crv2d_gt_[0].size() << "% (" << valid_idx.size() << "/" << crv2d_gt_[0].size() << ")\n";
//    }

  }

  // Write to files
  std::string path("../app/dat/synthetic-stereo/perturb/reprojection-precision-tests/");
  mywrite(path+std::string("perturb_angle_v.dat"), perturb_angle_v);
  mywrite(path+std::string("mean_err_pos_v.dat"), mean_err_pos_v);
  mywrite(path+std::string("mean_err_t_v.dat"), mean_err_t_v);
  mywrite(path+std::string("mean_err_k_v.dat"), mean_err_k_v);
  mywrite(path+std::string("mean_err_kdot_v.dat"), mean_err_kdot_v);

  mywrite(path+std::string("max_err_pos_v.dat"), max_err_pos_v);
  mywrite(path+std::string("max_err_t_v.dat"), max_err_t_v);
  mywrite(path+std::string("max_err_k_v.dat"), max_err_k_v);
  mywrite(path+std::string("max_err_kdot_v.dat"), max_err_kdot_v);
}
    ...
    )
{
  ...
  for (unsigned i=0; i < err_v.size(); ++i) {
    assert(err_v[i] >= 0);
    if (std::fabs(err_v[i]) > err_tolerance ) {
      ++n_err;
  ...
  if (n_err_epi > 0) {
    std::cout  << "(" << 100.0*(double)n_err_epi/(double)n_err << "%) max_epi: " << max_epi_angle << "deg," << idx_max_epi_angle;
  }
