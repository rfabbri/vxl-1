...
std::vector<vgl_point_3d<double> > read_form(std::string type, std::ifstream in)
{
...
in.getline(stch,256);
st=stch;
std::string pch=strtok(stch,"(error/count)");
...
std::vector<double>  get_number( std::string line_in,
...
int read_result_txt_file(std::string res_txt,std::vector <vnl_vector <double > >data, bool show=false) {//,float rev,std::vector <vgl_point_3d<double> > & ptl,bool rotation=false){
  ...
  while (!in.eof()) {
    ...
    if (hhh=="search:") {
      ...
      while(hhh!="epi") {
        ...
        {
          //std::cout<<sum_z_norm<<" "<<sum_z<<std::endl;
          vnl_vector_fixed<double,4> a(sum_z,sum_z_norm,epi_x,epi_y);
          data.push_back(a);
          mbl_stats_1d v_stat(v);

          if (show&&CRHM) std::cout<<a(0)<<"  "<<v_stat<<" epi:"<<epi_x<<" "<<epi_y<<std::endl;
          sum_z=0.0;
          sum_z_norm=0.0;
          v.clear();
          w.clear();
 ...
 // exit(1);
  ...
  if (data.size() <3) {
  std::cout<<"too short list of epi search"<<std::endl;
  exit(5);
  }


  std::vector <std::vector <vnl_vector <double > > >subdata_all;
  std::vector <vnl_vector <double > >subdata;
  ...
  std::cout<< "**sub max** "<<subdata[sub_max_index]<<std::endl;
  ...
  std::cout<<"data size: "<<data.size()<<" combination counter "<<combination_counter<<std::endl;
  in.close();



  std::vector <double> data1,data0;
  for (unsigned i=0;i<data.size();i++){
  ...
  {
  if (stats0.max()==data[i][0]) std::cout<<"!! "<<data[i]<<std::endl;
  else std::cout<<"*  normalized:  "<<data[i]<<std::endl;

  }
  //std::cout<<stats1.mean()<<" "<<stats1.max()<<" "<<stats1.min()<<" "<<stats1.variance()<<std::endl;

  //std::cout<<stats0.mean()<<" "<<stats0.max()<<" "<<stats0.min()<<" "<<stats0.variance()<<std::endl;
  std::cout<<"------------------------------"<<std::endl;
  //std::cout<<stats0.variance()<<" "<<stats1.variance()<<" "<<stats1.max()<<" "<<stats1.min()<<std::endl;
  std::cout<<stats0.variance()<<" "<<stats0.max()<<" "<<stats0.min()<<std::endl;
  ...
  if (subdata_all[b][i][0]==0&&subdata_all[b][i][1]==0) std::cout<<"0 0 skip "<<b<<std::endl;
  else {
...
int read_result_txt_file_for_XZdrawing(std::string res_txt,std::vector <vnl_vector <double > >data, bool show=false) {//,float rev,std::vector <vgl_point_3d<double> > & ptl,bool rotation=false){
  std::ifstream in(res_txt.c_str());
    ...
    if (hhh=="search:") {
      ...
      while(hhh!="epi") {
        ...
        else if(hhh=="camera")
        {
          ...
          if (show) std::cout<<a<<"  "<<v_stat<<" "<<epi_x<<" "<<epi_y<<std::endl;
          sum_z=0.0;
          sum_z_norm=0.0;
          v.clear();
          w.clear();
...
int main(int argc, char* argv[])
{
  ...
  for ( ; fn; ++fn) 
  {
    ...
    if (r1.find(input_file.c_str())&&r2.find(input_file.c_str()) )
    {
      ...
      //std::cout<<"reading "<<input_file<<" Done!"<<std::endl;
    }
  }

  exit(1);
  if(argc!=2){
    std::cout << "Usage: "<< argv[0] << " <res_text_file>"<< std::endl;
    //std::cout << "   or: "<< argv[0] << " <wrl_file> <front_wrl_file> <rear_wrl_file>"<< std::endl;
    exit(0);
  }

  //read_result_txt_file(argv[1]);
  std::string in0="C:/CBB_/A3900-3999/3903/sel/res_194c0.txt";

  std::string in1="C:/CBB_/A3800/3863/sel/res_193c0_7relase.txt";
