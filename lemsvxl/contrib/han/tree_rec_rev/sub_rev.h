...
bool table(int ref_num , int recognized_id, int &real_class_id) {
  std::cout<<ref_num<<" "<<recognized_id<<std::endl;
  int class_id;
  switch (ref_num) {
...
defalut:  
  ...
  if (class_id==983) {
    std::cout<<"ref id not registerd"<<std::endl;
    std::cerr<<"ref id not registerd"<<std::endl;
  }
...
/////////////////////////////////////////////////////////////////////
int find_near_point_rev(std::vector <vgl_point_3d<double> > pts,vgl_point_3d <double> pt,
...
////////////////////////////////////////////////////////////////////
read_wrl_file(std::vector <vgl_point_3d<double> > & ptl,std::string argv,bool rotation=false){
 ...
 if (ptl.size()<20) {
  ...
  for (unsigned i=0;i<ptl.size();i++) {
    ...
    pts_z.push_back(R*p);
   // ///##comented out 10-2-2006##/// std::cout<<R*p<<std::endl;
  }
...
pts_f_and_r_from_pts(std::vector <vgl_point_3d<double> > &pts_f,
                     ...
                     char choice)
{
  ///##comented out 10-2-2006##/// std::cout<<choice<<std::endl;
  //exit(1);
  ...
  for (unsigned i=0;i<pts.size();i++) 
  {
    ...
    if (diff_max>diff_min) {
      ...
      ///##comented out 10-2-2006##/// std::cout<<"pts dividing error\n"<<std::endl;
      exit (-2);
    }
    if (pre_flag_r!=flag_r) 
    {///##comented out 10-2-2006##/// std::cout<<"***** switched"<<std::endl;
   ///##xxxxxxxxxxxxxx comented out 10-31-2006##/// std::cerr<<"***** switched"<<std::endl;
        }

    pre_flag_r=flag_r;
    /////##comented out 10-2-2006##/// std::cout<<(pts[i].y()-max_y)*(pts[i].y()-max_y)<<" "<<(pts[i].y()-min_y)*(pts[i].y()-min_y)<<std::endl;
  }


  ///##comented out 10-2-2006##/// std::cout<<pts_f.size()<<"+"<<pts_r.size()<<"="<<pts.size()<<std::endl;
  //exit(1);
}

/////////////////////////////////////////////////////////////////////
read_bb_box_file(std::vector <vgl_point_3d<double> > & ptl,std::string argv){
  std::ifstream in(argv.c_str());
  //in >> cam_matrix;
  if(in.fail()){
    std::cerr << "Failed to read file "<<argv << std::endl;
  ...
  for (unsigned j=0;j<3;j++) {
    in >>hhh;
    ///##comented out 10-2-2006##/// std::cout<<hhh<<std::endl;
  }
  ...
  ///##comented out 10-2-2006##/// std::cout<<"size: "<<ptl.size()<<std::endl;
  in.close();
  //exit(0);
  return 1;
}
