...
vcl_vector< vcl_string > dbcri_accu_process::get_input_type()
{
...
vcl_vector< vcl_string > dbcri_accu_process::get_output_type()
{
  vcl_vector< vcl_string > to_return;
...
dbcri_accu_process::execute()
{
 ...
 // }
  ...
  if ( input_data_.size() != 1 ){
    vcl_cout<<  input_data_.size() <<vcl_endl;
    vcl_cout << "In dbcri_accu_process::execute() - "
...
/*vnl_double_3x4 WC;
 ...
 // return true;
  ...
  vcl_string vrml_filef="delf.wrl";
  vcl_string vrml_filer="delr.wrl";
                              ...
                              dbecl_epipole_sptr &epipole)
{
  ...
  /////  vcl_cout<< ang <<" "<<ratio<<vcl_endl;
   ...
   /// dont have camera yet.. 3d reconstruction is wrong..
    ...
    vcl_cout<<"contour point:"<<point_3d<<vcl_endl;
...
vcl_cout<<ratios_front_list.size()<<vcl_endl;
...
void 
dbcri_accu_process::write_vrml(const vcl_string& filename,
...
void dbcri_accu_process::write_vrml_2(vcl_ofstream& out,//const vcl_string& filename,
...
double 
dbcri_accu_process::histogram_method_for_finding_cross_ratio(vcl_vector <ratios >& ratios_front_list, vcl_vector < ratios >& ratios_rear_list) 
{
...
double 
dbcri_accu_process::histogram_method_for_finding_cross_ratio_finer(vcl_vector <ratios >& ratios_list) 
{
...
void dbcri_accu_process::d3_build(vcl_vector <vsol_digital_curve_2d_sptr> &dcl_,
...
void dbcri_accu_process::d3_build_points(double err, vcl_vector <vgl_point_3d<double> >& point_3d_mean_pts) 
{
pts_3d_a_.clear();
 ...
 // vcl_vector <vgl_point_3d<double> > point_3d_mean_pts;
  ...
  for (double ang=-theta_pos_ ; ang<-theta_neg_ ; ang+=.002) { 
    ...
    for (unsigned fm1=0;fm1<nframes_;fm1++)
      ...
      radius=5;
      vcl_cout<<"(error/count):"<<(error/count)<<" "<<count<<vcl_endl;
...
void dbcri_accu_process::d3_build_points_edge(double err, vcl_vector <vgl_point_3d<double> >& point_3d_mean_pts) 
{
pts_3d_a_.clear();
...
void dbcri_accu_process::d3_build_network(double err, vcl_vector <vgl_point_3d<double> >& point_3d_mean_pts) 
{
...
void  dbcri_accu_process::outliers(vcl_vector <vgl_point_3d<double> > point_3d_list) {
...
void dbcri_accu_process::get_BB_and_scale(vcl_vector <vgl_point_3d<double> > point_3d_list_f,vcl_vector <vgl_point_3d<double> > point_3d_list_r) {
...
if (point_3d_list_f.size()==0) return;
  vcl_sort(pts_z.begin(), pts_z.end(), dbcri_accu_cmp_z);
  ...
  vcl_sort(pts_x.begin(), pts_x.end(), dbcri_accu_cmp_x);
  vcl_sort(pts_y.begin(), pts_y.end(), dbcri_accu_cmp_y);
...
void dbcri_accu_process::left_right_line_bb(vcl_vector <vgl_point_3d<double> > &point_3d_left_list,vcl_vector <vgl_point_3d<double> > &point_3d_right_list) {
...
void dbcri_accu_process::write_vrml_bbox( vcl_ofstream& str,
...
void dbcri_accu_process::statitical_filtering(double &r, vcl_vector <vgl_point_3d <double> > pts) {
...
void dbcri_accu_process::spacial_filtering(double &r, vcl_vector <vgl_point_3d <double> > pts) {
 ...
 // vcl_sort(pts_x.begin(), pts_x.end(), dbcri_accu_cmp_x);
  ...
  for (unsigned i=0;i<pts_y.size(); i++) {
   ...
   vcl_sort(sel_pts_y.begin(), sel_pts_y.end(), dbcri_accu_cmp_x);
...
void dbcri_accu_process::spacial_filtering_x(double &r1,double &r2, vcl_vector <vgl_point_3d <double> > pts) {
...
double dbcri_accu_process::bb_box_contour_measure(vcl_vector<vgl_point_3d<double> > points) {
...
double dbcri_accu_process::val_list_check(vcl_vector <double> val_list)
{
  ...
  for (unsigned i=0;i<val_list.size();i++) {
   ...
   ///vcl_cout<<vcl_endl;
      ...
      vcl_sort(prob_list.begin(), prob_list.end(), dbcri_accu_sort);
...
void dbcri_accu_process::draw_vsol(vcl_vector <vnl_matrix <double> >p8_max_list) {
...
double dbcri_accu_process::bb_box_contour_measure(vcl_vector< vcl_vector<vgl_point_3d<double> > >points) {
...
double dbcri_accu_process::bb_box_contour_measure_clip(vcl_vector<vgl_point_3d<double> > points) {
