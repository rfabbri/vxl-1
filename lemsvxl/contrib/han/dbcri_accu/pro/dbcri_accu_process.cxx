// This is pro/dbcri_accu_process.cxx
...
//: Constructor
dbcri_accu_process::dbcri_accu_process()
: bpro_process()
{
...
//: Clone the process
bpro_process* 
dbcri_accu_process::clone() const
{
  return new dbcri_accu_process(*this);
}


//: Return the name of the process
std::string
dbcri_accu_process::name()
{
  return "ACCURATE Cross Ratio Inv Calibration";
}


//: Returns a vector of strings describing the input types to this process
std::vector< std::string > dbcri_accu_process::get_input_type()
{
...
std::vector< std::string > dbcri_accu_process::get_output_type()
{
  std::vector< std::string > to_return;
...
int
dbcri_accu_process::input_frames()
{
  return 1;
}
...
int
dbcri_accu_process::output_frames()
{
  //return nframes_;
  return 1;
}
...
dbcri_accu_process::execute()
{
 ...
 // }
  ...
  if ( input_data_.size() != 1 ){
    std::cout<<  input_data_.size() <<std::endl;
    std::cout << "In dbcri_accu_process::execute() - "
...
/*vnl_double_3x4 WC;
 ...
 // return true;
  ...
  std::string vrml_filef="delf.wrl";
  std::string vrml_filer="delr.wrl";
...
double dbcri_accu_process::cameras(int fm1, int fm2, int fm3,
                              ...
                              dbecl_epipole_sptr &epipole)
{
  ...
  /////  std::cout<< ang <<" "<<ratio<<std::endl;
   ...
   /// dont have camera yet.. 3d reconstruction is wrong..
    ...
    std::cout<<"contour point:"<<point_3d<<std::endl;
...
double dbcri_accu_process::cameras_accu(int fm1, int fm2, int fm3,
...
std::cout<<ratios_front_list.size()<<std::endl;
...
double dbcri_accu_process::cross_ratio(dbecl_epipole_sptr &epipole,dbecl_episeg_sptr & epi_segl1,
...
void 
dbcri_accu_process::write_vrml(const std::string& filename,
...
void dbcri_accu_process::write_vrml_2(std::ofstream& out,//const std::string& filename,
...
double 
dbcri_accu_process::histogram_method_for_finding_cross_ratio(std::vector <ratios >& ratios_front_list, std::vector < ratios >& ratios_rear_list) 
{
...
double 
dbcri_accu_process::histogram_method_for_finding_cross_ratio_finer(std::vector <ratios >& ratios_list) 
{
...
void dbcri_accu_process::init_cameras(vnl_vector <double> t,double scale)
{
...
void dbcri_accu_process::d3_build(std::vector <vsol_digital_curve_2d_sptr> &dcl_,
                             std::vector< std::vector <dbecl_episeg_sptr> >&episegl)

{
}

void dbcri_accu_process::d3_build()
...
bool dbcri_accu_process::is_inwedge(vgl_point_2d<double> p, int frame) 
{
...
void dbcri_accu_process::d3_build_points(double err, std::vector <vgl_point_3d<double> >& point_3d_mean_pts) 
{
pts_3d_a_.clear();
 ...
 // std::vector <vgl_point_3d<double> > point_3d_mean_pts;
  ...
  for (double ang=-theta_pos_ ; ang<-theta_neg_ ; ang+=.002) { 
    ...
    for (unsigned fm1=0;fm1<nframes_;fm1++)
      ...
      radius=5;
      std::cout<<"(error/count):"<<(error/count)<<" "<<count<<std::endl;
...
void dbcri_accu_process::d3_build_points_edge(double err, std::vector <vgl_point_3d<double> >& point_3d_mean_pts) 
{
pts_3d_a_.clear();
...
void dbcri_accu_process::d3_build_network(double err, std::vector <vgl_point_3d<double> >& point_3d_mean_pts) 
{
    ...
    for (unsigned i=0;i<network_.size();i++) {
      ...
      for (unsigned j=0;j<network_[i].horizon.size();j++) {
        ...
        vgl_point_2d <double> x2pt(network_[i].horizon[j][5],network_[i].horizon[j][6]);
             node n(fm1,fm2,fm3,x0pt,x1pt,x2pt);
...
bool node::compare_and_add(node *a)
{
...
void  dbcri_accu_process::outliers(std::vector <vgl_point_3d<double> > point_3d_list) {
...
bool dbcri_accu_sort(const double& rhs, const double& lhs)
{
  return rhs < lhs;
}
bool dbcri_accu_cmp_x(const vnl_double_3& rhs, const vnl_double_3& lhs)
{
  return rhs[0] < lhs[0];
}

bool dbcri_accu_cmp_y(const vnl_double_3& rhs, const vnl_double_3& lhs)
{
  return rhs[1] < lhs[1];
}

bool dbcri_accu_cmp_z(const vnl_double_3& rhs, const vnl_double_3& lhs)
{
  return rhs[2] < lhs[2];
}
...
void dbcri_accu_process::get_BB_and_scale(std::vector <vgl_point_3d<double> > point_3d_list_f,std::vector <vgl_point_3d<double> > point_3d_list_r) {
...
if (point_3d_list_f.size()==0) return;
  std::sort(pts_z.begin(), pts_z.end(), dbcri_accu_cmp_z);
  ...
  std::sort(pts_x.begin(), pts_x.end(), dbcri_accu_cmp_x);
  std::sort(pts_y.begin(), pts_y.end(), dbcri_accu_cmp_y);
...
void dbcri_accu_process::left_right_line_bb(std::vector <vgl_point_3d<double> > &point_3d_left_list,std::vector <vgl_point_3d<double> > &point_3d_right_list) {
...
void dbcri_accu_process::contour() {
...
void dbcri_accu_process::delete_contour(double thresh) 
{
...
void dbcri_accu_process::write_vrml_bbox( std::ofstream& str,
...
void dbcri_accu_process::print_bb_and_camera()
{
...
void dbcri_accu_process::read_WC()
{
...
void dbcri_accu_process::Estimation_BB_shift_using_convex_hull_in_2D() {
...
vnl_double_4x4 dbcri_accu_process::bb_reform(double x0, double y0, double z0, double x1, double y1, double z1)
{
...
int dbcri_accu_process::points_inside_of_the_box(int index, vnl_double_4x4 bb_xform) {
...
void dbcri_accu_process::statitical_filtering(double &r, std::vector <vgl_point_3d <double> > pts) {
...
void dbcri_accu_process::spacial_filtering(double &r, std::vector <vgl_point_3d <double> > pts) {
 ...
 // std::sort(pts_x.begin(), pts_x.end(), dbcri_accu_cmp_x);
  ...
  for (unsigned i=0;i<pts_y.size(); i++) {
   ...
   std::sort(sel_pts_y.begin(), sel_pts_y.end(), dbcri_accu_cmp_x);
...
void dbcri_accu_process::spacial_filtering_x(double &r1,double &r2, std::vector <vgl_point_3d <double> > pts) {
...
double dbcri_accu_process::bb_contour() {
...
void dbcri_accu_process::read_bb_and_probe(float scale, double &cube_x, double &cube_y, double &cube_z,
...
void dbcri_accu_process::read_bb_and_probe_test(float scale, double &cube_x, double &cube_y, double &cube_z,  //test for same cube_size
                                      vgl_point_3d<double> &X,int  probe_id, int class_id)
{
...
double dbcri_accu_process::Estimation_BB_and_bb_using_convex_hull_in2D(int some_index) {
...
double dbcri_accu_process::bb_box_contour_measure(std::vector<vgl_point_3d<double> > points) {
...
double dbcri_accu_process::val_list_check(std::vector <double> val_list)
{
  ...
  for (unsigned i=0;i<val_list.size();i++) {
   ...
   ///std::cout<<std::endl;
      ...
      std::sort(prob_list.begin(), prob_list.end(), dbcri_accu_sort);
...
void dbcri_accu_process::draw_vsol(std::vector <vnl_matrix <double> >p8_max_list) {
...
void dbcri_accu_process::rotate_bb( vnl_double_3x3 R_in, vnl_double_3x3 & R_out,double theta_x,double theta_y,double theta_z ) {
...
double dbcri_accu_process::generate_mean_contour_model(int class_index) {
...
double dbcri_accu_process::bb_box_contour_measure(std::vector< std::vector<vgl_point_3d<double> > >points) {
...
double dbcri_accu_process::bb_box_contour_measure_clip(std::vector<vgl_point_3d<double> > points) {
