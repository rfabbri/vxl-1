// Some tests for vgl_convex
// Ian Scott, Feb 2004.
#include <testlib/testlib_test.h>
#include <vgl/vgl_convex.h>
#include <iostream>
#include <vnl/vnl_matrix.h>

#include <vnl/vnl_math.h>
#include <vnl/vnl_double_4x4.h>
#include <vnl/vnl_double_3x3.h>
#include <vnl/vnl_vector.h>
#include <vnl/vnl_rotation_matrix.h>
#include <vnl/vnl_inverse.h>
#include <vnl/vnl_double_3x4.h>

#include <vnl/vnl_double_3.h>
#include <vul/vul_file.h>
#include <vector>
#include <string>
#include <iostream>
#include <vnl/vnl_vector.h>
#include <vgl/vgl_point_3d.h>
#include <vgl/vgl_vector_3d.h>
#include <iosfwd>
#include <vgl/algo/vgl_line_2d_regression.h>
#include <vgl/algo/vgl_convex_hull_2d.h>
#include <vgl/vgl_point_3d.h>
#include <vgl/vgl_point_2d.h>
#include <vgl/vgl_distance.h>
#include <vgl/vgl_clip.h>
#include <vgl/vgl_area.h>
vnl_double_3x3 R(0.0);
double scale_=1.0;
float mean_x1_,mean_x2;

///3-21-2007

/*
void print_bb_and_camera(std::string bb_dir)
{

  
  std::vector<vnl_double_3x4> cameras;
  std::vector <int> nums_list;
int counter=0;
    
  
    
    int fram_start=1;
    int fram_end=3;
    char tf[20];char f11[22]; 
  

    std::string input_file_path=".";
    vul_file_iterator fn=input_file_path+"/*.con";//nframes_=5;
    for ( ; fn; ++fn) 
    {
      std::string input_file = fn();
      std::cout<<input_file<<std::endl;
      
      std::string aaa=vul_file::strip_directory(input_file);
      std::string nums1=vul_file::strip_extension(aaa);
      std::cout<<nums1<<std::endl;
      nums_list.push_back(atoi(nums1.c_str()));

      if (counter==0)  fram_start= atoi(nums1.c_str());
      if (counter==nframes_-1) fram_end= atoi(nums1.c_str());

      counter++;
    }

    int how_many_frames=(int) (fram_end-fram_start);
    



  
    std::string bb_file=bb_dir+"/bbox_cam_my.txt";
  
    std::ofstream bbout(bb_file.c_str());
  bbout << "Bounding Box Transform\n" << bb_xform_ << '\n';


  int camera_count=nframes_;
 int search_num;bool search_num_flag;
  for (int i=fram_start;i<=fram_end; i++) {

   
    search_num=false;
    for (unsigned j=0;j<nums_list.size();j++) {
      if (i==nums_list[j]) {
        camera_count--;
        search_num_flag=true;
  
      }
      
    }

    if (search_num_flag) bbout << "Camera " << i << '\n' << cameras_[camera_count] << '\n';
    else bbout << "Camera " << i << '\n' <<  cameras_[camera_count] << '\n';

  }


  bbout<< p7_[0]<<" "<<p7_[1]<<" "<<p7_[2]<<'\n';
  bbout<< p0_[0]<<" "<<p0_[1]<<" "<<p0_[2]<<'\n';
  bbout.close();



return;
}

//
*/


void print_bb_and_camera(std::vector <vgl_point_3d<double> > ppp)
{

  
  //std::vector<vnl_double_3x4> cameras(0,0);
  std::vector <int> nums_list;
  int counter=0;
    
    int fram_start=1;
    int fram_end=3;
    char tf[20];char f11[22]; 

  
    std::string bb_file="./bbox_cam_my.txt";
  
    std::ofstream bbout(bb_file.c_str());

    vnl_double_4x4 bb_xform(0.0);
    bb_xform(0,0)=1.0;
    bb_xform(1,1)=1.0;
    bb_xform(2,2)=1.0;
  bbout << "Bounding Box Transform\n" << bb_xform << '\n';

  vnl_double_3x4 cameras(0.0);

 
  for (int i=1;i<=4; i++) {

   
    
    bbout << "Camera " << i << '\n' <<  cameras << '\n';

  }


  /// 3-21-2007 temporarilry using only x-direction...check it up later..

  if (ppp[0].x()<ppp[1].x()) bbout<< ppp[0].x()<<" "<<ppp[0].y()<<" "<<ppp[0].z()<<'\n';
  else bbout<< ppp[1].x()<<" "<<ppp[1].y()<<" "<<ppp[1].z()<<'\n';
  if (ppp[2].x()<ppp[3].x()) bbout<< ppp[3].x()<<" "<<ppp[3].y()<<" "<<ppp[3].z()<<'\n';
  else bbout<< ppp[2].x()<<" "<<ppp[2].y()<<" "<<ppp[2].z()<<'\n';
  bbout.close();



return;
}

//
















/////////////////////////////////////////////////////////////////////
vnl_vector <int> min_max(std::vector <vgl_point_3d<double> > pts) {

  int max_z_index,min_z_index,max_x_index,min_x_index;
  double max_z=-10000.0,min_z=10000.0,max_x=-100000.0,min_x=100000.0;
  for (unsigned i=0;i<pts.size();i++) {

    if (pts[i].z()>max_z) {max_z_index=i; max_z=pts[i].z();}
    if (pts[i].z()<min_z) {min_z_index=i; min_z=pts[i].z();}
    if (pts[i].x()>max_x) {max_x_index=i; max_x=pts[i].x();}
    if (pts[i].x()<min_x) {min_x_index=i; min_x=pts[i].x();}

  }
  vnl_vector <int> m(4,0);
  m(0)=max_z_index;
  m(1)=min_z_index;
  m(2)=max_x_index;
  m(3)=min_x_index;
  return m;
}

/////////////////////////////////////////////////////////////////////
int find_near_point_SD(std::vector <vgl_point_3d<double> > pts,vgl_point_3d <double> pt,
                             int start_i,int end_i,bool y_flag=true)
{

  double min_t=10000000;
  int min_index=-1;
  if (start_i=-1||end_i==-1) {start_i=0; end_i=pts.size();}
  for (int i=start_i;i<end_i;i++) {

    double tx=pts[i].x()-pt.x();
    double ty=pts[i].y()-pt.y();
    double tz=pts[i].z()-pt.z();

    
    double t=tx*tx+ty*ty+tz*tz;
    if (!y_flag) t=tx*tx+tz*tz;
    if (t<min_t) {
      min_t=t;
      min_index=i;
    }
  }


  return min_index;
}

struct p_list_set 
{
   int probe_number;
   std::vector <vnl_matrix <float>  >p_list;
   int class_id;
   float scale;
};

/////////////////////////////////////////////////////////////////////
void save_points(std::string save_filename,std::vector <struct p_list_set> p_list_set_)
{
  
  std::ofstream out(save_filename.c_str());

  if(!out.is_open()){
    std::cerr<<"Cannot open the write selected probes file.\n";
    exit(2);

  }

  out<<p_list_set_.size()<<"\n";
  for (unsigned i=0;i<p_list_set_.size();i++) {
    out<<i<<" "<<p_list_set_[i].p_list.size()<<std::endl;
    for( unsigned j=0;j<p_list_set_[i].p_list.size();j++){
      out<<p_list_set_[i].p_list[j]<< std::endl ;
    }
  }
  out<<"from 3d detection"<<std::endl;
  out.close();
  
}

////////////////////////////////////////////////////////////////////
//read_wrl_file_SD(float rev,std::vector <vgl_point_3d<double> > & ptl,char* argv,bool rotation=false){
read_wrl_file_SD(float rev,std::vector <vgl_point_3d<double> > & ptl,std::string argv,bool rotation=false){
  std::ifstream in(argv.c_str());
  
  if(in.fail()){
    std::cerr << "Failed to read file "<<argv << std::endl;
    exit(-4);
    return -1;
  }
  
  // in
  std::string hhh="";
  double x,y,z;
  //std::vector<vgl_point_3d<double> >ptl;
  while (!in.eof()) {

    in>>hhh;
    if (hhh=="translation") {
      in>>x>>y>>z;
      //std::cout<<x<<" "<<rev*x<<std::endl;
      vgl_point_3d<double> p3d(rev*x,y,z);
      ptl.push_back(p3d);
      //std::cout<<p3d<<std::endl;
    }
  }
  std::cout<<"size: "<<ptl.size()<<std::endl;

  return 1;
  if (!rotation) return 1;

  std::vector<vnl_double_3> pts_z;
  for (unsigned i=0;i<ptl.size();i++) {
    vnl_double_3 p(ptl[i].x(),ptl[i].y(),ptl[i].z());

    pts_z.push_back(R*p);
   // std::cout<<R*p<<std::endl;
  }

 // std::cout<<"------------------------------------------------------"<<std::endl;
  ptl.clear();

  for (unsigned i=0;i<pts_z.size();i++) {
    vgl_point_3d <double> p(pts_z[i](0),pts_z[i](1),pts_z[i](2));

    ptl.push_back(p);
    //std::cout<<p<<std::endl;
  }
 // std::cout<<"------------------------------------------------------"<<std::endl;
  in.close();

return 1;
}



void write_vrml_2(std::ofstream& out,//const std::string& filename,
                std::vector<vgl_point_3d<double> > pts_3d,float red=0,float green=1,float blue=1)
{

//  std::ofstream out(filename.c_str());
//    if(!out.is_open()){
//      std::cerr<<"Cannot open the input file.\n";
//      return;
//    }
    
    
    
    out <<      "#VRML V2.0 utf8\n";
    out <<      "Background { \n";
    out <<      "    skyColor [ 1 1 1 ]\n";
    out <<      "    groundColor [ 1 1 1 ]\n";
    out <<      "}\n";
    out <<      "PointLight {\n";
    out <<      "    on FALSE\n";
    out <<      "    intensity 1 \n";
    out <<      "ambientIntensity 0 \n";
    out <<      "color 1 1 1 \n";
    out <<      "location 0 0 0 \n";
    out <<      "attenuation 1 0 0 \n";
    out <<      "radius 100  \n";
    out <<      "}\n";
    out <<      "Shape {\n";
    out <<      "   #make the points white\n";
    out <<      "    appearance Appearance {\n";
    out <<      "       material Material { emissiveColor 1 0 0 }\n";
    out <<      "   } \n";
    out <<      "   geometry PointSet {\n";
    out <<      "      coord Coordinate{\n";
    out <<      "         point[\n";
    out <<      "         ]\n";
    out <<      "      }\n";
    out <<      "      color Color { color [ 1 1 1 ] }\n";
    out <<      "   }\n";
    out <<      "}\n";
    
    
    float xx,yy,zz;
   //float red=0,green=1,blue=1;
        
    double radius=0.2;
//    blue=1.0;

  for (unsigned i=0;i <pts_3d.size();i++) {
    //point_3d_mean_pts[i].set(scale_*point_3d_mean_pts[i].x(),scale_*point_3d_mean_pts[i].y(),scale_*point_3d_mean_pts[i].z());
    //std::cout<<point_3d_mean_pts[i]<<std::endl;






   

     out<<"Transform {  \n";
          out<<"translation "<<pts_3d[i].x()<<" "<<pts_3d[i].y()<<" "<<pts_3d[i].z()<< "\n";  
          out<<"Shape { \n";
          out<<"appearance Appearance { \n";
          //out<<" material Material { diffuseColor "<<r*(j+1)/15 <<" "<<g*(13-j)/13 <<" "<<b*(12+j)/24 <<"}\n";

          out<<" material Material { diffuseColor "<<red <<" "<<green <<" "<<blue <<"}\n";


          out<<" }   \n";
          out<<"  geometry  Sphere{  \n"; 
          //out<<" radius "<< radius*(j+5)/7 << "}  \n"; 
          out<<" radius "<< radius*scale_ << "}  \n"; 
          out<<" } \n"; 
          out<<"} \n"; 

  }


  std::cout<<"#### write_vrml_2 end ####"<<std::endl;
}



/////////////////////////////////////////////////////////////////////
pts_f_and_r_from_pts(std::vector <vgl_point_3d<double> > &pts_f,
                     std::vector <vgl_point_3d<double> > &pts_r,
                     std::vector <vgl_point_3d<double> > const &pts,
                     char choice)
{
  std::cout<<choice<<std::endl;
  //exit(1);
  float max_y=-100000,min_y=100000;
  float max_x=-100000,min_x=100000;
  for (unsigned i=0;i<pts.size();i++) 
  {

    if (max_y<pts[i].y()) max_y=pts[i].y();
    if (min_y>pts[i].y()) min_y=pts[i].y();
    if (max_x<pts[i].x()) max_x=pts[i].x();
    if (min_x>pts[i].x()) min_x=pts[i].x();
  }

  int BIN(100);
  bool flag_r=0,flag_f=0,pre_flag_r=0,pre_flag_f=0;
  double diff_max,diff_min;

  for (unsigned i=0;i<pts.size();i++) 
  {

    if (choice=='y') {
      diff_max=(pts[i].y()-max_y)*(pts[i].y()-max_y);
      diff_min=(pts[i].y()-min_y)*(pts[i].y()-min_y);
    } 
    else if (choice=='x') 
    {
      diff_max=(pts[i].x()-max_x)*(pts[i].x()-max_x);
      diff_min=(pts[i].x()-min_x)*(pts[i].x()-min_x);
    }

    if (diff_max>diff_min) {
      pts_r.push_back(pts[i]);
      flag_r=true; flag_f=false;
    }
    else if (diff_max<diff_min) {
      pts_f.push_back(pts[i]);
      flag_r=false; flag_f=true;
    }
    else {
      std::cout<<"pts dividing error\n"<<std::endl;
      exit (-2);
    }
    if (pre_flag_r!=flag_r) 
    {std::cout<<"***** switched"<<std::endl;
    std::cerr<<"***** switched"<<std::endl;}

    pre_flag_r=flag_r;
    //std::cout<<(pts[i].y()-max_y)*(pts[i].y()-max_y)<<" "<<(pts[i].y()-min_y)*(pts[i].y()-min_y)<<std::endl;
  }


  std::cout<<pts_f.size()<<"+"<<pts_r.size()<<"="<<pts.size()<<std::endl;
  //exit(1);
}

/////////////////////////////////////////////////////////////////////
read_bb_box_file_SD(std::vector <vgl_point_3d<double> > & ptl,std::string argv){
  std::ifstream in(argv.c_str());
  //in >> cam_matrix;
  if(in.fail()){
    std::cerr << "Failed to read file "<<argv << std::endl;
    return -1;
  }

  // in
  std::string hhh="";
  double x,y,z;
  double dummy;
  //std::vector<vgl_point_3d<double> >ptl;

  for (unsigned j=0;j<3;j++) {
    in >>hhh;
    std::cout<<hhh<<std::endl;
  }

  double temp;
  vnl_double_4x4 RT(0.0);
  for (unsigned i=0;i<4;i++) {
    for (unsigned j=0;j<4;j++) {
      in >> temp;
      RT[i][j]=temp;
    }
  }



  R[0][0]=RT[0][0];R[0][1]=RT[0][1];R[0][2]=RT[0][2];
  R[1][0]=RT[1][0];R[1][1]=RT[1][1];R[1][2]=RT[1][2];
  R[2][0]=RT[2][0];R[2][1]=RT[2][1];R[2][2]=RT[2][2];

  R.normalize_columns();
  std::cout<<R<<std::endl;

  while (!in.eof()) {

    in>>hhh;
    if (hhh=="Camera") {

      for (unsigned j=0;j<13;j++) in >>dummy;

    }
    else {
      x=atof(hhh.c_str());
      in>>y>>z;
      vgl_point_3d<double> p3d(x,y,z);
      ptl.push_back(p3d);
      std::cout<<p3d<<std::endl; 
      //exit(1);
    }
  }
  std::cout<<"size: "<<ptl.size()<<std::endl;
  in.close();
  //exit(0);
  return 1;
}


//
//find nearest point
//
float fnp(std::vector <vgl_point_3d<double> > pts,vgl_point_3d <double> pt,float shift,
                             bool y_flag=false)
{

  float min_t=10000000;
  int min_index=-1;
  //if (start_i=-1||end_i==-1) {start_i=0; end_i=pts.size();}
  for (int i=0;i<pts.size();i++) {

    double tx=pts[i].x()-pt.x()-shift;
    double ty=pts[i].y()-pt.y();
    double tz=pts[i].z()-pt.z();

    
    double t=tx*tx+ty*ty+tz*tz;
    if (!y_flag) t=tx*tx+tz*tz;
    if (t<min_t) {
      min_t=t;
      min_index=i;
    }
  }


  return min_t;
}

//
float fnp_y(std::vector <vgl_point_3d<double> > pts,vgl_point_3d <double> pt,float shift,
                             bool y_flag=false)
{

  float min_t=10000000;
  int min_index=-1;
  //if (start_i=-1||end_i==-1) {start_i=0; end_i=pts.size();}
  for (int i=0;i<pts.size();i++) {

    double tx=pts[i].x()-pt.x();
    double ty=pts[i].y()-pt.y()-shift;
    double tz=pts[i].z()-pt.z();

    
    double t=tx*tx+ty*ty+tz*tz;
    //if (!y_flag) t=tx*tx+tz*tz;
    if (t<min_t) {
      min_t=t;
      min_index=i;
    }
  }


  return min_t;
}

float shift_error(std::vector <vgl_point_3d<double> > &pts_fix,
                     std::vector <vgl_point_3d<double> > &pts_move,
                     //std::vector <vgl_point_3d<double> > const &pts,
                     float shift)
{
    float error=0;
    for (unsigned i=0;i<pts_move.size();i++) {
        //float del_x=pts_move[fns(pts_fix,pts_move[i])].x()-pts_fix[i].x();
        //float del_z=pts_move[fns(pts_fix,pts_move[i])].z()-pts_fix[i].z();
        //float del_error=del_x*del_x+del_y*del_y;
        float del_error=fnp(pts_fix,pts_move[i],shift);
        error+=del_error;
    }

    return error;
}


float shift_error_y(std::vector <vgl_point_3d<double> > &pts_fix,
                     std::vector <vgl_point_3d<double> > &pts_move,
                     //std::vector <vgl_point_3d<double> > const &pts,
                     float shift)
{
    float error=0;
    for (unsigned i=0;i<pts_move.size();i++) {
        //float del_x=pts_move[fns(pts_fix,pts_move[i])].x()-pts_fix[i].x();
        //float del_z=pts_move[fns(pts_fix,pts_move[i])].z()-pts_fix[i].z();
        //float del_error=del_x*del_x+del_y*del_y;
        float del_error=fnp_y(pts_fix,pts_move[i],shift);
        error+=del_error;
    }

    return error;
}

float mean_shift(std::vector <vgl_point_3d<double> > &pts_fix,
                 std::vector <vgl_point_3d<double> > &pts_move)
{
    float mean_x1=0.0,mean_x2=0.0;
    for (unsigned i=0;i<pts_fix.size();i++) {
       mean_x1+=pts_fix[i].x();
       
    }
    for (unsigned i=0;i<pts_move.size();i++) {
        mean_x2+=pts_move[i].x();
    }
    float shift=mean_x1/(float)pts_fix.size()-mean_x2/(float)pts_move.size();
   

    std::cout<<shift<<" "<<pts_fix.size()<<" "<<pts_move.size()<<std::endl; 
    return shift;
}
 

float mean_shift_y(std::vector <vgl_point_3d<double> > &pts_fix,
                 std::vector <vgl_point_3d<double> > &pts_move)
{
    float mean_y1=0.0,mean_y2=0.0;
    for (unsigned i=0;i<pts_fix.size();i++) {
       mean_y1+=pts_fix[i].y();
       
    }
    for (unsigned i=0;i<pts_move.size();i++) {
        mean_y2+=pts_move[i].y();
    }
    float shift=mean_y1/(float)pts_fix.size()-mean_y2/(float)pts_move.size();
   

    std::cout<<shift<<" "<<pts_fix.size()<<" "<<pts_move.size()<<std::endl; 
    return shift;
}

//rotation of points
void rot_pts_SD(std::vector <vgl_point_3d<double> > &ptl,float rev=1.0) 
{

    vnl_double_3x3 rot=R;
    std::cout<<rot<<std::endl;
    //vnl_double_3x3 inv_rot = rot.transpose();
    vnl_double_3x3 inv_rot = vnl_inverse(rot);
    std::vector<vnl_double_3> pts_z;
    for (unsigned i=0;i<ptl.size();i++) {
      vnl_double_3 p(ptl[i].x(),ptl[i].y(),ptl[i].z());

      std::cout<<ptl[i]<<std::endl;
      pts_z.push_back(rot*p);
      std::cout<<rot*p<<std::endl;
      ptl[i].set(rev*pts_z[i][0],pts_z[i][1],pts_z[i][2]);
    }
}
/////////////////////////////////////////////////////////////////////
// Adding 2 3D smae direction 3-13-2007 Dongjin Han
///////////////////////////////////////////////////////////
int main(int argc, char* argv[])
{

  
  if(argc!=4){
    //std::cout << "Usage: "<< argv[0] << " <wrl_file>"<< std::endl;
   // std::cout << "   or: "<< argv[0] << " <wrl_file> <front_wrl_file> <rear_wrl_file>"<< std::endl;
    //exit(0);
  }

  //=========================== load the files ==========================

  /////////////////////////////////////////
  /////////////////////////////////////////
  std::vector<vgl_point_3d<double> >ptl0;
  std::vector<vgl_point_3d<double> >pts_f0;
  std::vector<vgl_point_3d<double> >pts_r0;
  std::vector<vgl_point_3d<double> >ptl1;
  std::vector<vgl_point_3d<double> >pts_f1;
  std::vector<vgl_point_3d<double> >pts_r1;
  std::vector<vgl_point_3d<double> >ptl2;
  std::vector<vgl_point_3d<double> >pts_f2;
  std::vector<vgl_point_3d<double> >pts_r2;
 std::vector<vgl_point_3d<double> >ptl3;
  std::vector<vgl_point_3d<double> >pts_f3;
  std::vector<vgl_point_3d<double> >pts_r3;


  std::string in0="C:/CBB_/A2/3788/sel/correct_1_10-31-2006-02AM/";
  std::string in1="C:/CBB_/A4000-4099/4071/sel/correct_1_10-31-2006-01PM/";
  

   in0="C:/CBB_/A2/3790/sel/correct_1_10-31-2006-02AM/";
   in1="C:/CBB_/A4000-4099/4073/sel/correct_1_10-31-2006-01PM/";
  
  read_wrl_file_SD(1.0,ptl0,  in0+"del-AEC-all-1.wrl");
  read_wrl_file_SD(1.0,pts_f0,in0+"delf-AEC-1.wrl");
  read_wrl_file_SD(1.0,pts_r0,in0+"delr-AEC-1.wrl");
  std::vector <vgl_point_3d<double> >  ppp0;
  std::vector <vgl_point_3d<double> >  ppp1;
  std::vector <vgl_point_3d<double> >  ppp1s;


  
  /*read_wrl_file_SD(-1.0,ptl0,  "C:/CBB_/A3800/3863/sel/correct_1_12-05-2006-09PM/del-AEC-all-1.wrl");
  read_wrl_file_SD(-1.0,pts_f0,"C:/CBB_/A3800/3863/sel/correct_1_12-05-2006-09PM/delf-AEC-1.wrl");
  read_wrl_file_SD(-1.0,pts_r0,"C:/CBB_/A3800/3863/sel/correct_1_12-05-2006-09PM/delr-AEC-1.wrl");
  read_bb_box_file_SD(ppp,"C:/CBB_/A3800/3863/sel/correct_1_12-05-2006-09PM/bbox_cam_my.txt");
  */

  read_bb_box_file_SD(ppp0,in0+"bbox_cam_my.txt");
  rot_pts_SD(ptl0);
  rot_pts_SD(pts_f0);
  rot_pts_SD(pts_r0);

  rot_pts_SD(ppp0); ///3-21-2007 2 start and end point in bb must rotate, too.

  //read_wrl_file_SD(-1.0,ptl1,  "C:/CBB_/A3800/3863/sel/correct_1_12-05-2006-09PM/del-AEC-all-1.wrl");
  read_wrl_file_SD(1.0,ptl1,  in1+"del-AEC-all-1.wrl");
  read_wrl_file_SD(1.0,pts_f1,in1+"delf-AEC-1.wrl");
  read_wrl_file_SD(1.0,pts_r1,in1+"delr-AEC-1.wrl");
  read_bb_box_file_SD(ppp1,in1+"bbox_cam_my.txt");
  //rot_pts_SD(ptl1,-1.0);
  //rot_pts_SD(pts_f1,-1.0);
  //rot_pts_SD(pts_r1,-1.0);


  rot_pts_SD(ptl1);
  rot_pts_SD(pts_f1);
  rot_pts_SD(pts_r1);

  rot_pts_SD(ppp1); ///3-21-2007 2 start and end point in bb must rotate, too.

  float mean_sh=mean_shift(ptl0,ptl1);

  float min_sh=0.0,min_err=100000000000000.0;
  for (float sh=mean_sh-10.0;sh<mean_sh+10.0;sh+=0.1) {

      float err=shift_error( ptl0,ptl1,sh);

      if (err<min_err) {
          min_err=err;
          min_sh=sh;
      }
      std::cout<<sh<<": "<<err<<std::endl;
  }
                     
  //move it
  for (unsigned i=0;i<ptl1.size();i++) ptl1[i].set(ptl1[i].x()+min_sh,ptl1[i].y(),ptl1[i].z());


  //////3-21-2007
  for (unsigned i=0;i<ppp1.size();i++) ppp1[i].set(ppp1[i].x()+min_sh,ppp1[i].y(),ppp1[i].z()); ///note ppp0 needs not to be shifted
  for (unsigned i=0;i<pts_f1.size();i++) pts_f1[i].set(pts_f1[i].x()+min_sh,pts_f1[i].y(),pts_f1[i].z());
  for (unsigned i=0;i<pts_r1.size();i++) pts_r1[i].set(pts_r1[i].x()+min_sh,pts_r1[i].y(),pts_r1[i].z());

  
  //y alginment
  float mean_sh_y=mean_shift_y(ptl0,ptl1);

  float min_sh_y=0.0,min_err_y=100000000000000.0;
  for (float sh=mean_sh_y-10.0;sh<mean_sh_y+10.0;sh+=0.1) {

      float err=shift_error_y( ptl0,ptl1,sh);

      if (err<min_err_y) {
          min_err_y=err;
          min_sh_y=sh;
      }
      std::cout<<sh<<": "<<err<<std::endl;
  }


////3-21-2007
  for (unsigned i=0;i<ppp1.size();i++) {
      //vgl_point_3d <double> a(ptl1[i].x()+min_sh,ptl1[i].y()+min_sh_y,ptl1[i].z());
      vgl_point_3d <double> a(ppp1[i].x(),ppp1[i].y()+min_sh_y,ppp1[i].z());
      ppp1s.push_back(a);
  }



  ////for all points....

  for (unsigned i=0;i<ptl0.size();i++) {
      ptl2.push_back(ptl0[i]);
  }
  for (unsigned i=0;i<ptl1.size();i++) {
      //vgl_point_3d <double> a(ptl1[i].x()+min_sh,ptl1[i].y()+min_sh_y,ptl1[i].z());
      vgl_point_3d <double> a(ptl1[i].x(),ptl1[i].y()+min_sh_y,ptl1[i].z());
      ptl3.push_back(a);
  }


  std::string vrml_file ="del-AEC-all-1.wrl";
      //std::string vrml_filef=out_file_dir+"//"+vr_dir+"//"+"delf-AEC-"+mode+".wrl";
      //std::string vrml_filer=out_file_dir+"//"+vr_dir+"//"+"delr-AEC-"+mode+".wrl";
      //std::string vrml_file3=out_file_dir+"//"+vr_dir+"//"+"del-AEC-all-"+mode+".wrl";

  std::ofstream vrml(vrml_file.c_str());
  write_vrml_2(vrml,ptl2);
  write_vrml_2(vrml,ptl3,1.0,0.0,0.0);


  ////for front points....
  ptl2.clear();ptl3.clear();
    for (unsigned i=0;i<pts_f0.size();i++) {
      ptl2.push_back(pts_f0[i]);
  }
  for (unsigned i=0;i<pts_f1.size();i++) {
      //vgl_point_3d <double> a(ptl1[i].x()+min_sh,ptl1[i].y()+min_sh_y,ptl1[i].z());
      vgl_point_3d <double> a(pts_f1[i].x(),pts_f1[i].y()+min_sh_y,pts_f1[i].z());
      ptl3.push_back(a);
  }


  std::string vrml_filef ="delf-AEC-1.wrl";
      //std::string vrml_filef=out_file_dir+"//"+vr_dir+"//"+"delf-AEC-"+mode+".wrl";
      //std::string vrml_filer=out_file_dir+"//"+vr_dir+"//"+"delr-AEC-"+mode+".wrl";
      //std::string vrml_file3=out_file_dir+"//"+vr_dir+"//"+"del-AEC-all-"+mode+".wrl";

  std::ofstream vrmlf(vrml_filef.c_str());
  write_vrml_2(vrmlf,ptl2);
  write_vrml_2(vrmlf,ptl3,1.0,0.0,0.0);


  ////for rear points....
  ptl2.clear();ptl3.clear();
    for (unsigned i=0;i<pts_r0.size();i++) {
      ptl2.push_back(pts_r0[i]);
  }
  for (unsigned i=0;i<pts_r1.size();i++) {
      //vgl_point_3d <double> a(ptl1[i].x()+min_sh,ptl1[i].y()+min_sh_y,ptl1[i].z());
      vgl_point_3d <double> a(pts_r1[i].x(),pts_r1[i].y()+min_sh_y,pts_r1[i].z());
      ptl3.push_back(a);
  }


 std::string   vrml_filer ="delr-AEC-1.wrl";
      //std::string vrml_filef=out_file_dir+"//"+vr_dir+"//"+"delf-AEC-"+mode+".wrl";
      //std::string vrml_filer=out_file_dir+"//"+vr_dir+"//"+"delr-AEC-"+mode+".wrl";
      //std::string vrml_file3=out_file_dir+"//"+vr_dir+"//"+"del-AEC-all-"+mode+".wrl";

  std::ofstream vrmlr(vrml_filer.c_str());
  write_vrml_2(vrmlr,ptl2);
  write_vrml_2(vrmlr,ptl3,1.0,0.0,0.0);


  std::vector <vgl_point_3d <double> > ppppp;
  ppppp.push_back(ppp0[0]);
  ppppp.push_back(ppp1s[0]);
  ppppp.push_back(ppp0[1]);
  ppppp.push_back(ppp1s[1]);
  print_bb_and_camera(ppppp);

  exit(1);


}

