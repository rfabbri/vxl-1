// 
#include <testlib/testlib_test.h>
#include <vgl/vgl_convex.h>
#include <iostream>
#include <vnl/vnl_matrix.h>

#include <vnl/vnl_math.h>
#include <vnl/vnl_double_4x4.h>
#include <vnl/vnl_double_3x3.h>
#include <vnl/vnl_double_3.h>
#include <vul/vul_file.h>
#include <vul/vul_file_iterator.h>
#include <vul/vul_reg_exp.h>
#include <cstring>
#include <string>

struct p 
{
   //int probe_number;
   std::vector <vnl_double_3>   pt;
   int real_class_id;
   double HoC;
   double x01,x07,x79,x01_over_x07,z7_over_HoC;
   float recognized_class_id;
   //float scale;
};
typedef struct p p;

///////////////////
read_data_file(std::vector <p > & ptl,std::string argv){
  std::ifstream in(argv.c_str());
  std::cout<<argv<<std::endl;
  //in >> cam_matrix;
  if(in.fail()){
    std::cerr << "Failed to read file "<<argv << std::endl;
    return -1;
  }

  // in
  std::string hhh="";
  double x,y,z;
  double dummy;
  //std::vector<vgl_point_3d<double> >ptl;



  double temp;
  int counter=0,real_class_id;
  double a,b,c,d;

  double HoC;
  double x01,x07,x79,x01_over_x07,z7_over_HoC;
  while (!in.eof()) {

   

    in>>hhh;
    if (hhh=="recognizer") {
      p p1;
      in>>p1.HoC;
      //std::cout<<HoC;
      for (unsigned j=0;j<10;j++) {
        in >>x>>y>>z;
        vnl_double_3 xyz(x,y,z);
        p1.pt.push_back(xyz);
      //  std::cout<<xyz<<std::endl;
      }

 
      in >>p1.x01>>p1.x07>>p1.x79>>p1.x01_over_x07>>p1.z7_over_HoC;
     // std::cout<<p1.x01<<" "<<p1.x07<<" "<<p1.x79<<" "<<p1.x01_over_x07<<" "<<p1.z7_over_HoC<<std::endl;
      while (hhh!="real_class_id") {
        in>>hhh;
      }
      in>> p1.real_class_id;
      in>>hhh; 
      if (hhh=="OOOOO") std::cout<<1;
      else std::cout<<-1;
      in>>hhh;
      in>>p1.recognized_class_id; std::cout<<" "<<p1.real_class_id<<" "<<p1.recognized_class_id<<" ";

      in >>hhh>>hhh;
      if (hhh=="------------------------------") {
      in>>a>>b>>c>>d;
      std::cout<<a<<" "<<b<<" "<<c<<" "<<d<<std::endl;
      }
     // std::cout<<p1.recognized_class_id<<std::endl;
      counter++;

      vnl_double_3 xyz1(p1.x01,p1.x07,p1.x79);
      p1.pt.push_back(xyz1);
      vnl_double_3 xyz2(p1.HoC,p1.x01_over_x07,p1.z7_over_HoC);
      p1.pt.push_back(xyz2);

      ptl.push_back(p1);
    }

  }
  ///##comented out 10-2-2006##/// std::cout<<"size: "<<ptl.size()<<std::endl;
  in.close();
  
}
// learn_rec file
int main(int argc, char* argv[])
{

    if (argc!=4) {

        std::cout<<"Usage!  learn_rec <class_id> <probe_id> <read_file> "<<std::endl;
            exit(1);

    }
  std::vector <p> t;
 // read_data_file(t,"X:/CBB_/BATCH/GOOD/r-com1-all_res4.txt");
  /// read_data_file(t,"X:/CBB_/BATCH/GOOD/r-com1-all_res_TEMP.txt");
  read_data_file(t,argv[3]);
  std::cout<<t.size()<<std::endl;

  int total_counter=0,correct_counter=0;
  for (unsigned i=0;i<t.size();i++) 
  {

    int class_id;
    total_counter++;

    if (t[i].recognized_class_id<10) class_id=0;
    else if (t[i].recognized_class_id<20) class_id=1;
    else if (t[i].recognized_class_id<30) class_id=2;
    else class_id=454;
    
    if (t[i].real_class_id==class_id) correct_counter++;
    if (atoi(argv[1])==t[i].real_class_id&&argv[3]||atoi(argv[1])==3&&argv[3]) 
    {
    std::cout<<t[i].pt[atoi(argv[2])]<<std::endl;

    }
  }

  std::cout<<total_counter<<" "<<correct_counter<<std::endl;
    std::cerr<<total_counter<<" "<<correct_counter<<std::endl;

}

