...
#ifndef dbvxm_multi_scale_voxel_world_h_
#define dbvxm_multi_scale_voxel_world_h_
//:
// \file
...
class dbvxm_multi_scale_voxel_world: public vbl_ref_count
{
 public:

  //: default constructor
  dbvxm_multi_scale_voxel_world(){};

  //: construct world with parameters
  dbvxm_multi_scale_voxel_world(bvxm_world_params_sptr params){params_ = params;}
  ...
  //: equality operator
  bool operator == (dbvxm_multi_scale_voxel_world const& that) const;
  ...
  bool save_occupancy_raw(std::string filename,unsigned int scale = 0);
...
//: get the observation count of a voxel type at a specific bin
template<bvxm_voxel_type VOX_T>
unsigned int dbvxm_multi_scale_voxel_world::num_observations(unsigned int bin_idx,unsigned int scale)
{
...
//: increment the observation count of a voxel type at a specific bin
template<bvxm_voxel_type VOX_T>
void dbvxm_multi_scale_voxel_world::increment_observations(unsigned int bin_idx,unsigned int scale)
{
    assert(scale < params_->max_scale());
  // call get_grid so data will be loaded from disk if necessary.
  typedef typename bvxm_voxel_traits<VOX_T>::voxel_datatype vox_datatype;
...
template<bvxm_voxel_type VOX_T>
bvxm_voxel_grid_base_sptr dbvxm_multi_scale_voxel_world::get_grid(unsigned bin_index,unsigned int scale_idx)
{
...
template <bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::update(bvxm_image_metadata const& observation, unsigned bin_index, unsigned scale)
{
    assert(scale < params_->max_scale());
...
template<bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::update(bvxm_image_metadata const& observation,
...
template<bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::update_impl(bvxm_image_metadata const& metadata,
                                   bool return_prob,
                                   ...
                                   vil_image_view<bool> &mask, unsigned bin_index, unsigned scale)
{
  ...
  std::vector<vgl_h_matrix_2d<double> > H_img_to_plane;
  {
    ...
    {
      dbvxm_util::compute_plane_image_H(metadata.camera,params_,z,Hp2i,Hi2p,scale);
  ...
  if (!dbvxm_util::img_to_slab(metadata.img,image_slab)) {
    std::cerr << "error converting image to voxel slab of observation type for bvxm_voxel_type:" << APM_T << std::endl;
    return false;
  }
  ...
  for (unsigned z=0; z<(unsigned)grid_size.z(); ++z, ++ocp_slab_it, ++apm_slab_it, ++preX_slab_it, ++PIvisX_slab_it)
  {
    ...
    dbvxm_util::write_slab_as_image(frame_backproj,"d:/frame_backproj.tiff");

    // transform preX to voxel plane for this level
    dbvxm_util::warp_slab_bilinear(preX_accum, H_plane_to_img[z], *preX_slab_it);
    ...
    dbvxm_util::multiply_slabs(visX,PI,*PIvisX_slab_it);

    // update appearance model, using PX*visX as the weights
    dbvxm_util::multiply_slabs(visX,*ocp_slab_it,PXvisX);
    ...
    if (return_mask){
      dbvxm_util::add_slabs(PX_img,mask_slab,mask_slab);
    }
  ...
  for (unsigned z = 0; z < (unsigned)grid_size.z(); ++z, ++PIvisX_slab_it, ++preX_slab_it, ++ocp_slab_it2) {
    ...
    dbvxm_util::warp_slab_bilinear(preX_accum, H_plane_to_img[z], preX_accum_vox);

    // transform visX_sum to current level
    dbvxm_util::warp_slab_bilinear(visX_accum, H_plane_to_img[z], visX_accum_vox);
...
template<bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::expected_image(bvxm_image_metadata const& camera,
                                      ...
                                      unsigned scale)
{
  ...
  {
    vgl_h_matrix_2d<double> Hp2i, Hi2p;
    dbvxm_util::compute_plane_image_H(camera.camera,params_,z,Hp2i,Hi2p,scale);
    H_plane_to_img.push_back(Hp2i);
    H_img_to_plane.push_back(Hi2p);
  }
...
template<bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::inv_pixel_range_probability(bvxm_image_metadata const& observation,
...
template<bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::pixel_probability_density(bvxm_image_metadata const& observation,
...
//: generate the mixture of gaussians slab from the specified viewpoint. the slab should be allocated by the caller.
template<bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::mixture_of_gaussians_image(bvxm_image_metadata const& observation,
                                                  bvxm_voxel_slab_base_sptr& mog_image, unsigned bin_index, unsigned scale)
{
  ...
  for (unsigned z=0; z<(unsigned)grid_size.z(); ++z, ++ocp_slab_it, ++apm_slab_it)
  {
    ...
    if (!apm_processor.update(mog_slab, expected_slice_img, w)) {   // check if does "if (*I_it >= 0)" during update
      std::cout << "In dbvxm_multi_scale_voxel_world<APM_T>::mixture_of_gaussians_image() -- problems in appearance update\n";
      return false;
    }
  }
...
template<bvxm_voxel_type APM_T>
bool dbvxm_multi_scale_voxel_world::virtual_view(bvxm_image_metadata const& original_view,
                                    const vpgl_camera_double_sptr virtual_camera,
...
void vsl_b_write(vsl_b_ostream & os, dbvxm_multi_scale_voxel_world const &world);

//: Binary load parameters from stream.
void vsl_b_read(vsl_b_istream & is, dbvxm_multi_scale_voxel_world &world);

void vsl_print_summary(std::ostream &os, const dbvxm_multi_scale_voxel_world &world);

void vsl_b_read(vsl_b_istream& is, dbvxm_multi_scale_voxel_world* p);

void vsl_b_write(vsl_b_ostream& os, const dbvxm_multi_scale_voxel_world* &p);
