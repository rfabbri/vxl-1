...
//: Destructor
dbvxm_multi_scale_voxel_world::~dbvxm_multi_scale_voxel_world()
{
}


//: equality operator
bool dbvxm_multi_scale_voxel_world::operator == (dbvxm_multi_scale_voxel_world const& that) const
{
  return true;
}
...
vgl_plane_3d<double> dbvxm_multi_scale_voxel_world::fit_plane()
{
...
bool dbvxm_multi_scale_voxel_world::update_edges(bvxm_image_metadata const& metadata, unsigned int scale)
{
...
bool dbvxm_multi_scale_voxel_world::expected_edge_image(bvxm_image_metadata const& camera,vil_image_view_base_sptr &expected,unsigned scale)
{
  // datatype for current appearance model
  typedef bvxm_voxel_traits<EDGES>::voxel_datatype edges_datatype;
...
void vsl_b_read(vsl_b_istream& is, dbvxm_multi_scale_voxel_world* p)
{
  ...
  if (not_null_ptr)
  {
    p = new dbvxm_multi_scale_voxel_world();
    vsl_b_read(is, *p);
  }
  else
    p = 0;
