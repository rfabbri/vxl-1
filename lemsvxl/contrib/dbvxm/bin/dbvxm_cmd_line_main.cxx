...
int main(int argc, char *argv[]) {
  ...
  vul_arg<std::string> image_name(arg_list,"-image","the name with path for the input image file for the routine","");
  vul_arg<std::string> changes_file_name(arg_list,"-changes","the name with path for the binary changes file","");
  ...
  vul_arg<std::string> mask_img_name(arg_list,"-mask","the name with path for the binary mask file","");
  vul_arg<std::string> measurement_img_name(arg_list,"-meas-img","the name with path for the measurement image (e.g. glitch area difference)","");
  vul_arg<std::string> prob_map_name(arg_list,"-prob-map","the name with path for the prob map for the image (e.g. glitch probability)","");
  ...
  if (prepare_cameras_for_eth80_arg()) {
    if (!prepare_cameras_for_eth80(output_prefix())) {
      ...
      std::cout << "usage with -prepare-cameras: \n";
    ...
    if (!prepare_joint_hist(image_name(), changes_file_name())) {
      std::cout << "routine failed\n";
      std::cout << "usage with -prepare-hist: \n";
      std::cout << image_name.option() << " " << image_name.help() << std::endl;
      std::cout << changes_file_name.option() << " " << changes_file_name.help() << std::endl;
    }
    ...
    if (!collect_statistics_for_glitch_area(mask_img_name(), prob_map_name(), measurement_img_name(), measurement_max(), measurement_bin_cnt(), neighborhood_size(), not(), fit())) {
      std::cout << "routine failed\n";
      std::cout << "usage with -fit-weibull: \n";
      std::cout << mask_img_name.option() << " " << mask_img_name.help() << std::endl;
      std::cout << prob_map_name.option() << " " << prob_map_name.help() << std::endl;
      std::cout << not.option() << " " << not.help() << std::endl;
      std::cout << measurement_img_name.option() << " " << measurement_img_name.help() << std::endl;
      std::cout << measurement_max.option() << " " << measurement_max.help() << std::endl;
      std::cout << measurement_bin_cnt.option() << " " << measurement_bin_cnt.help() << std::endl;
      std::cout << neighborhood_size.option() << " " << neighborhood_size.help() << std::endl;
    }
  }
