#ifndef dbvxm_util_h_
#define dbvxm_util_h_
//:
// \file
...
template<class T>
void dbvxm_util::warp_slab_bilinear(bvxm_voxel_slab<T> const& slab_in,
                               vgl_h_matrix_2d<double> invH, bvxm_voxel_slab<T> &slab_out)
{
...
template<class T>
void dbvxm_util::warp_slab_nearest_neighbour(bvxm_voxel_slab<T> const& slab_in,
                               vgl_h_matrix_2d<double> invH, bvxm_voxel_slab<T> &slab_out)
{
    vnl_matrix_fixed<double,3,3> Hd = invH.get_matrix();
    // test if slab_in's projection is higher resolution than slab out.
...
template<class T, unsigned N>
bool dbvxm_util::img_to_slab(vil_image_view_base_sptr const image, bvxm_voxel_slab<vnl_vector_fixed<T,N> > &slab)
{
...
//:  if input image is a float image, assumes that it is already scaled to [0,1] range
template<class T>
bool dbvxm_util::img_to_slab(vil_image_view_base_sptr const image, bvxm_voxel_slab<T> &slab)
{
  // check slab is preallocated to correct size
  if ( (slab.nx() != image->ni()) || (slab.ny() != image->nj()) ) {
    std::cerr << "error: slab and image are different sizes.\n";
...
template<class T, unsigned N>
bool dbvxm_util::slab_to_img(bvxm_voxel_slab<vnl_vector_fixed<T,N> > const &slab, vil_image_view_base_sptr image)
{
...
template<class T>
bool dbvxm_util::slab_to_img(bvxm_voxel_slab<T> const& slab, vil_image_view_base_sptr image)
{
  // check image is preallocated to correct size
  if ( (slab.nx() != image->ni()) || (slab.ny() != image->nj()) )
  {
...
template<class T>
T dbvxm_util::sum_slab(bvxm_voxel_slab<T> const& slab)
{
  T sum = 0;
  typename bvxm_voxel_slab<T>::const_iterator slab_it = slab.begin();
  for (; slab_it != slab.end(); ++slab_it) {
...
template<class T>
void dbvxm_util::add_slabs(bvxm_voxel_slab<T> const& s1, bvxm_voxel_slab<T> const& s2, bvxm_voxel_slab<T> &sum)
{
  // check sizes
...
template<class T>
void dbvxm_util::multiply_slabs(bvxm_voxel_slab<T> const& s1, bvxm_voxel_slab<T> const& s2, bvxm_voxel_slab<T> &product)
{
  // check sizes
...
template<class T>
void dbvxm_util::threshold_slab_above(bvxm_voxel_slab<T> const& slab, T const& thresh, bvxm_voxel_slab<bool> &mask)
{
  // check sizes
...
template<class T>
void dbvxm_util::smooth_gaussian(bvxm_voxel_slab<T> &slab, float stdx, float stdy)
{
  if ( (stdx < 0) || (stdy < 0) ) {
...
// used for debugging
template<class T>
void dbvxm_util::write_slab_as_image(bvxm_voxel_slab<T> const& slab_in,std::string filename)
{
...
// used for debugging
template<class T, unsigned N>
void dbvxm_util::write_slab_as_image(bvxm_voxel_slab<vnl_vector_fixed<T,N> > const& slab_in,std::string filename)
{
...
template<class T>
bool dbvxm_util::generate_test_boxes(T box_min_x, T box_min_y, T box_min_z, 
                    T box_dim_x, T box_dim_y, T box_dim_z,
                    T world_dim_x, T world_dim_y, T world_dim_z,
                    std::vector<vgl_box_3d<T> >& boxes)
{
  ...
  if ((max_x > world_dim_x) || (max_y > world_dim_y) || (max_z > world_dim_z)) {
    std::cerr << "generate_boxes() -- the box is out of world boundaries!" << std::endl;
    return false;
  }
...
dbvxm_util::corners_of_box_3d(vgl_box_3d<T> box)
{
