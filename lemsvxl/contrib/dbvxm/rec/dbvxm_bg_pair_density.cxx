...
double dbvxm_bg_pair_density::operator()(const double y0, const double y1)
{
...
double dbvxm_bg_pair_density::operator()(const rgb_obs_datatype y0, const rgb_obs_datatype y1)
{
...
dbvxm_bg_pair_density::prob_density(vil_image_view<grey_obs_datatype>& obs)
{
  unsigned ni = obs.ni();
  unsigned nj = obs.nj();
...
vil_image_view<float> 
dbvxm_bg_pair_density::prob_density_non_pair(vil_image_view<grey_obs_datatype>& obs)
{
  unsigned ni = obs.ni();
  unsigned nj = obs.nj();
...
bool dbvxm_bg_pair_density::generate_mixture_image()
{
  if (!world_)
    return false;
  ...
  if (voxel_type_ == "apm_mog_rgb") {
    ...
    std::cout << "In bvxm_normalize_image_process::execute() -- input appearance model: " << voxel_type_ << " is not supported\n";
    return false;
  }
...
bool
dbvxm_bg_pair_density::generate_appearance_likelihood_map(vil_image_view<vxl_byte>& img, vil_image_view<float>& prob_map)
{
...
vil_image_view<float> 
dbvxm_bg_pair_density::prob_density_from_likelihood_map(vil_image_view<vxl_byte>& img)
{
  unsigned ni = img.ni();
  unsigned nj = img.nj();
...
vil_image_view<float> 
dbvxm_bg_pair_density::prob_density_from_likelihood_map(vil_image_view<vxl_byte>& img, vil_image_view<float>& prob_density)
{
  unsigned ni = img.ni();
  unsigned nj = img.nj();
...
bool 
dbvxm_bg_pair_density::initialize_bg_map(vil_image_view<float>& prob_density, vil_image_view<float>& map, float high_prior, float top_percentile)
{
