...
#include <iostream>
#include "dbvxm_bayesian_propagation.h"
...
bool
dbvxm_bayesian_propagation::horizontal_pass(dbvxm_fg_pair_density& fgp, dbvxm_fg_bg_pair_density& fgbgp, dbvxm_bg_pair_density& bgp)
{
...
bool 
dbvxm_bayesian_propagation::directional_pass(dbvxm_fg_pair_density& fgp, dbvxm_fg_bg_pair_density& fgbgp, dbvxm_bg_pair_density& bgp, int ii, int jj)
{
  //: for now make only one horizontal pass
...
bool 
dbvxm_bayesian_propagation::horizontal_pass_using_prob_density_as_bgp(dbvxm_fg_pair_density& fgp, 
                                                            dbvxm_fg_bg_pair_density& fgbgp)
{
  ...
  for (unsigned i = 0; i < bg_map_.ni()-1; i++) {
    for (unsigned j = 0; j < bg_map_.nj(); j++) {
      ...
      if (i == 572 && j == 335) {
        std::cout << "--------- 572, 335 -----------\n";
        std::cout << "p_b_current: " << p_b_current << " p_f_current: " << p_f_current << std::endl;
        std::cout << "p_b_current1: " << p_b_current1 << " p_f_current1: " << p_f_current1 << std::endl;
        std::cout << " p_ff: " << p_ff << " p_fb: " << p_fb;
        std::cout << " p_bf: " << p_bf << " p_bb: " << p_bb << std::endl;
        std::cout << " m_ff: " << m_ff << " m_bb: " << m_bb << " m_fb: " << m_fb << " m_bf: " << m_bf << std::endl;
        std::cout << " p_h_bb: " << p_h_bb << " p_h_bf: " << p_h_bf << " p_h_fb: " << p_h_fb << " p_h_ff: " << p_h_ff << std::endl;
        std::cout << "the highest prob hypothesis was: ";
        if (p_h_bb >= p_h_bf && p_h_bb >= p_h_ff && p_h_bb >= p_h_fb)
          std::cout << " p_h_bb\n";
        else if (p_h_bf >= p_h_bb && p_h_bf >= p_h_ff && p_h_bf >= p_h_fb)
          std::cout << " p_h_bf\n";
...
bool 
dbvxm_bayesian_propagation::vertical_pass_using_prob_density_as_bgp(dbvxm_fg_pair_density& fgp, dbvxm_fg_bg_pair_density& fgbgp)
{
  //: for now make only one horizontal pass
  for (unsigned i = 0; i < bg_map_.ni(); i++) {
...
//  jj is either -1 or 0 or 1 so can run in all 8 directions
bool 
dbvxm_bayesian_propagation::directional_pass_using_prob_density_as_bgp(dbvxm_fg_pair_density& fgp, dbvxm_fg_bg_pair_density& fgbgp, int ii, int jj)
{
  //: for now make only one horizontal pass
  for (unsigned i = 0; i < bg_map_.ni(); i++) {
    for (unsigned j = 0; j < bg_map_.nj(); j++) {
...
bool 
dbvxm_bayesian_propagation::initialize_bg_map(float high_prior, float top_percentile)
{
  //: initialize the map
  bg_map_.set_size(ni_, nj_);
  dbvxm_bg_pair_density::initialize_bg_map(prob_density_, bg_map_, high_prior, top_percentile);
  return true;
}

//: use background mixture of gaussian image
bool 
dbvxm_bayesian_propagation::run(dbvxm_fg_pair_density& fgp, dbvxm_fg_bg_pair_density& fgbgp, dbvxm_bg_pair_density& bgp, 
                                int npasses)
{
...
bool 
dbvxm_bayesian_propagation::pass_using_prob_density_as_bgp(dbvxm_fg_pair_density& fgp, dbvxm_fg_bg_pair_density& fgbgp, vil_image_view<float>& temp)
{
...
bool 
dbvxm_bayesian_propagation::run_using_prob_density_as_bgp(dbvxm_fg_pair_density& fgp, 
                                dbvxm_fg_bg_pair_density& fgbgp,
                                int npasses)
{
