...
bvxm_voxel_slab_base_sptr create_mog_image2_using_grey_processor(std::string model_dir, bvxm_voxel_world_sptr& vox_world, vil_image_view_base_sptr& expected_img)
{
  vul_file::make_directory(model_dir);
...
MAIN( test_dbvxm_detect_instance_process )
{
  unsigned ni = 200;
  unsigned nj = 200;
  ...
  //: create a GREY mog image from a known object world
  command = "rm -rf ./test_world_object_dir";
  system(command.c_str());
  vil_image_view_base_sptr obj_expected_image = new vil_image_view<unsigned char>(640,480);
  bvxm_voxel_world_sptr obj_vox_world;
  bvxm_voxel_slab_base_sptr obj_mog_image = create_mog_image2_using_grey_processor("./test_world_object_dir", obj_vox_world, obj_expected_image);
  TEST("testing world creation", obj_vox_world->get_params()->num_voxels().z(), 4);
  TEST("testing world creation", obj_vox_world->get_params()->num_voxels().x(), 40);
  TEST("testing mixture of gaussian image creation", !obj_mog_image, false);
  mog_image_ptr = dynamic_cast<bvxm_voxel_slab<mog_type>*>(obj_mog_image.ptr());
  TEST("testing mixture of gaussian image creation", !mog_image_ptr, false);
  vil_image_view<unsigned char> expected_i_obj(*obj_expected_image);
  vil_save(expected_i_obj, "./expected_object.png");
#endif
  ...
  REG_PROCESS(dbvxm_detect_instance_process, bprb_batch_process_manager);
  ...
  params->add("world_dir", "worlddir", std::string("./test_syn_world"));
  std::string command = "rm -rf ./test_syn_world";
  system(command.c_str());
  ...
  params->set_value("nx", (unsigned)30); params->set_value("ny", (unsigned)30); params->set_value("nz", (unsigned)15);
  params->set_value("minx", (unsigned)5); params->set_value("miny", (unsigned)5); params->set_value("minz", (unsigned)0);
  params->set_value("dimx", (unsigned)20); params->set_value("dimy", (unsigned)20); params->set_value("dimz", (unsigned)10);
  params->set_value("gen2", false);   params->set_value("genImages", true); params->set_value("rand1", false); 
  params->set_value("worlddir", std::string("./test_syn_obj_world"));
  command = "rm -rf ./test_syn_obj_world";
  system(command.c_str());
  good = bprb_batch_process_manager::instance()->init_process("bvxmGenSyntheticWorldProcess");
  good = good && bprb_batch_process_manager::instance()->set_params(params);
  good = good && bprb_batch_process_manager::instance()->run_process();

  unsigned id_world_obj;
  good = good && bprb_batch_process_manager::instance()->commit_output(0, id_world_obj);
  TEST("run bvxmGenSyntheticWorldProcess", good ,true);

  brdb_query_aptr Q_w_obj = brdb_query_comp_new("id", brdb_query::EQ, id_world_obj);
  brdb_selection_sptr S_w_obj = DATABASE->select("bvxm_voxel_world_sptr_data", Q_w_obj);
  TEST("output world is in db", S_w_obj->size(), 1);

  brdb_value_sptr value_w_obj;
  TEST("output world is in db", S_w_obj->get_value(std::string("value"), value_w_obj), true);
  TEST("output world is non-null", (value_w_obj != 0) ,true);

  brdb_value_t<bvxm_voxel_world_sptr>* result_w_obj = static_cast<brdb_value_t<bvxm_voxel_world_sptr>* >(value_w_obj.ptr());
  bvxm_voxel_world_sptr obj_vox_world = result_w_obj->value();
  obj_vox_world->increment_observations<APM_MOG_GREY>(0);

  //vpgl_camera_double_sptr cam1 = create_camera();
  vpgl_camera_double_sptr cam1 = create_syn_world_camera(vox_world);
#if 1
  //: set the inputs
  brdb_value_sptr v0 = new brdb_value_t<bvxm_voxel_world_sptr>(vox_world);
  brdb_value_sptr v1 = new brdb_value_t<vpgl_camera_double_sptr>(cam1);
  brdb_value_sptr v2 = new brdb_value_t<bvxm_voxel_world_sptr>(obj_vox_world);
  brdb_value_sptr v3 = new brdb_value_t<std::string>("apm_mog_grey");
  brdb_value_sptr v4 = new brdb_value_t<unsigned>(0);

  //: inits 
  good = bprb_batch_process_manager::instance()->init_process("dbvxmDetectInstanceProcess");
...
#if 0  // run this process to test the camera
  //: set the inputs
  brdb_value_sptr v0 = new brdb_value_t<vpgl_camera_double_sptr>(cam1);
  brdb_value_sptr v1 = new brdb_value_t<unsigned>(ni);
  brdb_value_sptr v2 = new brdb_value_t<unsigned>(nj);
  brdb_value_sptr v3 = new brdb_value_t<bvxm_voxel_world_sptr>(obj_vox_world);
  brdb_value_sptr v4 = new brdb_value_t<std::string>("apm_mog_grey");
  brdb_value_sptr v5 = new brdb_value_t<unsigned>(0);
  
  //: inits 
  good = bprb_batch_process_manager::instance()->init_process("bvxmRenderExpectedImage");
  good = good && bprb_batch_process_manager::instance()->set_input(0, v0);
  good = good && bprb_batch_process_manager::instance()->set_input(1, v1);
  good = good && bprb_batch_process_manager::instance()->set_input(2, v2);
  good = good && bprb_batch_process_manager::instance()->set_input(3, v3);
  ...
  TEST("run dbvxm detect instance process", good ,true);
  ...
  vil_image_view_base_sptr exp_img_overlayed = result->value();
