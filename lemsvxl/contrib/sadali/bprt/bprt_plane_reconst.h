...
inline  void set_params(double normx,double normy,double dist){
                this->normx=normx;
                this->normy=normy;
                this->dist=dist;
        }
inline void get_params(double &a,double &b, double &d)
{
        (a)=normx;
        (b)=normy;
        (d)=dist;
...
HMatrix2D H_;
double std_;
std::vector <vgl_homg_point_2d <double> > ptlist1_,ptlist2_;

public: 
        params param_;
bprt_plane_reconst();
bprt_plane_reconst(std::vector <vtol_edge_2d_sptr >  curvs1,std::vector <vtol_edge_2d_sptr >  curvs2,int samp,double std);
~bprt_plane_reconst();
...
std::vector< vsol_spatial_object_2d_sptr> get_vsol_point_2ds(int view);
int calibrate();
}
;
