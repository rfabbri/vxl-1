// This is contrib/sadali/vidpro1_plane_reconst_process.cxx

//:
// \file

#include "vidpro1_plane_reconst_process.h"
...
//: Constructor
vidpro1_plane_reconst_process::vidpro1_plane_reconst_process()
{
  ...
  {
    std::cerr << "ERROR: Adding parameters in vidpro1_plane_reconst_process::vidpro1_plane_reconst_process()" << std::endl;
  }
}
...
bpro1_process* vidpro1_plane_reconst_process::clone() const
{
    return new vidpro1_plane_reconst_process(*this);
}


//: Return the name of this process
std::string
vidpro1_plane_reconst_process::name()
{
...
int
vidpro1_plane_reconst_process::input_frames()
{
...
int
vidpro1_plane_reconst_process::output_frames()
{
...
vidpro1_plane_reconst_process::get_input_type()
{
...
std::vector< std::string > 
vidpro1_plane_reconst_process::get_output_type()
{  
...
//: Execute the process
bool
vidpro1_plane_reconst_process::execute()
{
  // verify that the number of input frames is correct
  if ( input_data_.size() != 1 ){
    std::cout << "In vidpro1_plane_reconst_process::execute() - not exactly two"
  ...
  vidpro1_vtol_storage_sptr curr_frame_edges; 
  vidpro1_vtol_storage_sptr curr_frame_edges2;
...
//  prev_frame_image.vertical_cast(input_data_[1][0]);
  ...
  std::vector< vtol_edge_2d_sptr > datainput;
  std::vector< vtol_edge_2d_sptr > datainput2;
  vtol_topology_object_sptr intertol;


  
 std::set<vtol_topology_object_sptr>::const_iterator itr;
 ...
 std::set<vtol_topology_object_sptr>::const_iterator itr2;
 for(itr2=curr_frame_edges->begin(); itr2!=curr_frame_edges->end();++itr2)
 {
  ...
  R[0][0] = 1/2;               R[0][1] = 0;        R[0][2] = std::sqrt(3.0f)/2; 
  R[1][0] = 0;                 R[1][1] = 1;        R[1][2] = 0;
  R[2][0] = std::sqrt(3.0f)/-2;        R[2][1] = 0;        R[2][2] = 1/2;
 ...
 // std::vector<vsol_spatial_object_2d_sptr> point=Reconstructor.get_vsol_point_2ds(1);
...
//: Finish
bool
vidpro1_plane_reconst_process::finish()
{
