...
//-----------------------------------------------------------------------------
mygui_yong_star_tableau2::mygui_yong_star_tableau2(const char* n) : name_(n), style_(vgui_style::new_style())
{
  style_->rgba[0] = 1.0f;
  style_->rgba[1] = 1.0f;
  style_->rgba[2] = 0.0f;
...
//-----------------------------------------------------------------------------
bool mygui_yong_star_tableau2::handle(vgui_event const& e)
{
    ...
    if(e.type ==  vgui_KEY_DOWN && e.key == 'r' && e.modifier == vgui_CTRL)
    {
        if(current_selected_star_ != -1)
        {
            ...
            this->star_list_[current_selected_star_]->set_orientation(ori + 0.1);
            this->draw();
        }    
    }
    if(e.type ==  vgui_KEY_DOWN && e.key == 'l' && e.modifier == vgui_CTRL)
    {
        if(current_selected_star_ != -1)
        {
            ...
            this->star_list_[current_selected_star_]->set_arm_size(arm_s + 1);
            this->draw();
        }    
    }
...
bool mygui_yong_star_tableau2::draw()
{
    ...
    for (vcl_vector<yong_star_sptr>::iterator vit = this->star_list_.begin();    vit != this->star_list_.end(); vit++, j++)
    {
        ...
        {
            x1 = x + cos(ori+i*(2*MY_PI/arm_n))*arm_s;
            y1 = y + sin(ori+i*(2*MY_PI/arm_n))*arm_s;

            if(i < (arm_n - 1))
            {
                x2 = x + cos(ori+(i+1)*(2*MY_PI/arm_n))*arm_s;
                y2 = y + sin(ori+(i+1)*(2*MY_PI/arm_n))*arm_s;
            }
            else
            {
                x2 = x + cos(ori)*arm_s;
