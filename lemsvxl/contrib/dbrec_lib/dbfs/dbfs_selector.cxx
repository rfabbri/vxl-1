...
dbfs_selector::dbfs_selector(dbfs_measure_sptr ms, std::vector<int>& features, int class_cnt) : measurer_(ms), features_(features), class_cnt_(class_cnt), mutual_infos_calculated_(class_cnt, false), arr_(class_cnt, std::vector<std::pair<int, float> >())
{
}
//: initializes the feature ids from 0 to feature_cnt-1 in the features_ vector
dbfs_selector::dbfs_selector(dbfs_measure_sptr ms, int feature_cnt, int class_cnt) : measurer_(ms), class_cnt_(class_cnt), mutual_infos_calculated_(class_cnt, false), arr_(class_cnt, std::vector<std::pair<int, float> >())
{
  for (int i = 0; i < feature_cnt; i++)
    features_.push_back(i);
}

bool sort_by_mi(const std::pair<int, float>& lhs, const std::pair<int, float>& rhs )
{
...
void dbfs_selector::select_features(int class_id)
{
  ...
  for (unsigned i = 0; i < features_.size(); i++) {
    ...
    std::pair<int, float> id_mi_pair(features_[i], mi);
    arr_[class_id].push_back(id_mi_pair);
    std::cout << "(" << features_[i] << ", " << mi << ") ";
  }
  std::sort(arr_[class_id].begin(), arr_[class_id].end(), sort_by_mi);
...
void dbfs_selector::get_top_features(int class_id, int k, std::vector<int>& best_features)
{
  if (!mutual_infos_calculated_[class_id])
    select_features(class_id);
  ...
  for (int i = 0; i < s; i++) {
    std::cout << "(" << arr_[class_id][i].first << ", " << arr_[class_id][i].second << ") ";
