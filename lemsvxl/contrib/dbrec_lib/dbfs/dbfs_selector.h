...
{
public:
  dbfs_selector(dbfs_measure_sptr ms, std::vector<int>& features, int class_cnt);
  ...
  void get_top_features(int class_id, int k, std::vector<int>& best_features); 
...
protected:
  ...
  std::vector<int> features_;  // each feature needs to be given a unique identifier
  int class_cnt_;             // 2 for class&non-class (binary classifications)

  std::vector<bool> mutual_infos_calculated_;
  std::vector<std::vector<std::pair<int, float> > > arr_;
  
};

class dbfs_selector_max_class_min_non_class : public dbfs_selector {
public:
  dbfs_selector_max_class_min_non_class(dbfs_measure_sptr ms, std::vector<int>& features, int class_cnt) : dbfs_selector(ms, features, class_cnt) {}
