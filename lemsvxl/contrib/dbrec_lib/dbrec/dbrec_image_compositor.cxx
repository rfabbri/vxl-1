...
void dbrec_or_compositor::sample_locations(const vgl_point_2d<float>& loc, float scale, const std::vector<dbrec_part_sptr>& children, std::vector<vgl_point_2d<float> >& locs) const
{
  locs.clear();
  locs.push_back(loc);
...
void dbrec_or_compositor::direction_vector(const std::vector<dbrec_part_sptr>& children, vnl_vector_fixed<float,2>& v) const
{
  v(0) = 0.0f;
  v(1) = 1.0f;
}
...
dbrec_part_context_sptr dbrec_or_compositor::detect_instances(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, const std::vector<float>& comp_priors, float radius) const
{
  ...
  for (unsigned i = 0; i < children.size(); i++) {
    ...
    while (pi) {
      std::vector<dbrec_part_ins_sptr> c_ch_ins;
...
dbrec_compositor_sptr dbrec_central_compositor::xml_parse_element(bxml_data_sptr data)
{
  ...
  std::vector<dbrec_pairwise_model_sptr> class_models;
...
void dbrec_central_compositor::sample_locations(const vgl_point_2d<float>& loc, float scale, const std::vector<dbrec_part_sptr>& children, std::vector<vgl_point_2d<float> >& locs) const
{
  if (!children.size())
    return;
...
void dbrec_central_compositor::direction_vector(const std::vector<dbrec_part_sptr>& children, vnl_vector_fixed<float,2>& v) const
{
  if (!children.size())
    return;
...
void dbrec_central_compositor::draw_attributes(bsvg_document& doc, const std::vector<dbrec_part_sptr>& children, const std::vector<vgl_point_2d<float> >& locs) const
{
  if (!children.size() || children.size() != locs.size()) {
    std::cout << "Error! incompatible vector sizes in dbrec_central_compositor::draw_attributes()!\n";
    return;
  }
...
dbrec_part_context_sptr dbrec_central_compositor::detect_instances(unsigned type_id, dbrec_context_factory_sptr cf, 
   const std::vector<dbrec_part_sptr>& children, const std::vector<float>& comp_priors, float radius) const
{
  dbrec_part_context_sptr out_c = cf->new_context();
  ...
  if (!central_c)
    return out_c;
  std::vector<dbrec_part_context_sptr> children_c;
  for (unsigned i = 1; i < children.size(); i++) {
    dbrec_part_context_sptr cont = cf->get_context(children[i]->type());
    if (!cont)
      return out_c;
  ...
  std::vector<std::vector<dbrec_pairwise_model_sptr> > models;
  dbrec_pairwise_model_sptr non_class_uniform_model = new dbrec_pairwise_indep_uniform_discrete_model(radius);
  std::vector<dbrec_pairwise_model_sptr> nc_models(models_.size(), non_class_uniform_model);
  switch(comp_priors.size()) { // it is either 2 or 4
    ...
    case 3: case 0: case 1: default: { std::cout << "Error! dbrec_central_compositor::detect_instances() -- size of priors vector is not valid!\n"; return out_c; }
  }
  ...
  while (pi) {
    ...
    if (pi->pt().x() == 740 && pi->pt().y() == 511) {
      std::cout << "here found 459 and 735!\n";
    }

    std::vector<double> temp_central_likelihoods(pi->posteriors_size());
    for (unsigned jj = 0; jj < pi->posteriors_size(); jj++) {
    ...
    for (unsigned i = 1; i < children.size(); i++) {
      ...
      for (unsigned k = 0; k < out.size(); k++) {
        std::vector<double> temp(pi->posteriors_size());
        ...
        std::vector<double>::iterator it = temp.begin(); it++;
        double min = *std::min_element(it, temp.end());
        if (best_score < min) {
          best_score = min;
          best_part = out[k];
        }
...
bool dbrec_central_compositor::train_instances(dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float radius)
{
  if (!children.size() || models_.size() != children.size() - 1) // there should be a model for each non-central child
    return false;
...
void dbrec_central_compositor::visualize(std::string& doc_name, const std::vector<dbrec_part_sptr>& children) const
{
  for (unsigned i = 0; i < models_.size(); i++) {
    std::stringstream ii; ii << i;
    std::string doc_name2 = doc_name+"_model_"+ii.str();
    models_[i]->visualize(doc_name2);
  }
}

void dbrec_pairwise_discrete_compositor::visualize(std::string& doc_name, const std::vector<dbrec_part_sptr>& children) const
{
  std::string filename = doc_name+ "_model_";
  model_->visualize(filename);
}

//: sample locations of each child in the composition wrt given location
void dbrec_pairwise_compositor::sample_locations(const vgl_point_2d<float>& loc, float scale, const std::vector<dbrec_part_sptr>& children, std::vector<vgl_point_2d<float> >& locs) const
{
  if (!children.size())
    return;
/*
...
dbrec_part_context_sptr dbrec_pairwise_compositor::detect_instances_rot_inv(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float comp_class_prior, float radius)
{
  dbrec_part_context_sptr out_c = cf->new_context();
  ...
  while (pi) {
    ...
    //std::cout << "At part1 instance: " << pi->pt() << " class post: " << pi->posterior(0) << " non_class post: " << pi->posterior(1) << std::endl;
    ...
    for (unsigned k = 0; k < out.size(); k++) {
      //std::cout << "\t part2 instance: " << out[k]->pt() << " class post: " << out[k]->posterior(0) << " non_class post: " << out[k]->posterior(1) << std::endl;
    ...
    //if (best_score > 0 && best_part) { // if we've found an instance in the probe region
      //std::cout << "\t winner part2 instance: " << best_part->pt() << " ";
      ...
      if (!model_->rho_defined()) {//: if rho is undefined, there is a danger that the same part is about to be added twice so check for this
        ...
        std::vector<dbrec_part_ins_sptr> out2;
      ...
      if (!exists) {
        ...
        out_c->add_part(ci);
        //std::cout << " class post: " << best_class_likelihood/den << " ";
      }
      //std::cout << "\n";
    }
...
dbrec_part_context_sptr dbrec_pairwise_compositor::detect_instances_rot_inv(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float comp_class_prior, vil_image_view<float>& fg_prob, float radius)
{
  dbrec_part_context_sptr out_c = cf->new_context();
  ...
  while (pi) {
    ...
    if ((pi->pt().x() == 281 && pi->pt().y() == 469))
      std::cout << "here in (281, 469) class post: " << pi->posterior(0) << " non_class post: " << pi->posterior(1) << std::endl;
    ...
    for (unsigned k = 0; k < out.size(); k++) {
      ...
      if (!exists) {
        ...
        if ((pi->pt().x() == 463 && pi->pt().y() == 272))
          std::cout << "\t \t prior_i_f: " << prior_i_f << " prior_k_f: " << prior_k_f << " comp_prior_f: " << comp_prior_f << std::endl;

        double non_class_likelihood = model_->prob_density_non_class();  // this is a constant value for all instances in the radius satisfying gamma criterion

        if ((pi->pt().x() == 463 && pi->pt().y() == 272))
          std::cout << "\t \t non_class_likelihood: " << non_class_likelihood << std::endl;
        ...
        */
        ...
        if ((pi->pt().x() == 463 && pi->pt().y() == 272))
          std::cout << "\t \t class_fg_likelihood: " << class_fg_likelihood << " non_class_fg_like: " << non_class_fg_likelihood << " den: " << den << std::endl;
...
void dbrec_pairwise_compositor::collect_rot_inv_stats(dbrec_context_factory_sptr cf, int posterior_id, dbrec_part_sptr p1, dbrec_part_sptr p2, float radius, const std::string& output_file)
{
  ...
  if (hdv1.has_it_) {
    ...
    if (hdv2.has_it_) {
      ...
      std::cout << " \t found gamma_range: " << gamma_range << " rho_range: " << rho_range << std::endl;
    }
  }

  std::ofstream ofs(output_file.c_str(), std::ofstream::app);
  if (!ofs) {
    std::cout << "In dbrec_pairwise_compositor::collect_rot_inv_stats() -- problems opening file: " << output_file << std::endl;
    return;
  }
  ...
  if (pi)
    if (posterior_id >= (int)pi->posteriors_size()) {
      std::cout << "In dbrec_pairwise_compositor::collect_rot_inv_stats) -- posterior maps have not been computed!\n";
      throw 0;
    }
...
//: method to detect instances of a composition in a rotationally invariant way for the 2 case problem: class and non-class
dbrec_part_context_sptr dbrec_pairwise_discrete_compositor::detect_instances_rot_inv(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float comp_class_prior, float radius)
{
  std::cout << "In dbrec_pairwise_discrete_compositor::detect_instances_rot_inv() - not implemented yet!\n";
  throw 0;
}

//: method to detect instances of a composition in a rotationally invariant way for the 4 case problem: class-foreground, non-class foreground, class background, non-class background
dbrec_part_context_sptr dbrec_pairwise_discrete_compositor::detect_instances_rot_inv(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float comp_class_prior, vil_image_view<float>& fg_prob, float radius)
{
  std::cout << "PARSING PART: " << type_id << "!!!!!!!!!!!!!!!!!!!!!!!!!\n";
  std::cout.flush();
  dbrec_part_context_sptr out_c = cf->new_context();
...
//: method to train a compositional part's geometric models
bool dbrec_pairwise_discrete_compositor::train_instances(dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float radius)
{
  if (children.size() != 2) {
    std::cout << "In dbrec_pairwise_discrete_compositor::train_instances() -- wrong number of children!\n";
    throw 0;
  }
  ...
  dbrec_part_context_sptr part2_c = cf->get_context(p2->type());
  if (!part1_c || !part2_c) {
    std::cout << "In dbrec_pairwise_discrete_compositor::train_instances() -- part contexts have not been computed!\n";
    return false;
  }
  ...
  if (!pi) {
    std::cout << "In dbrec_pairwise_discrete_compositor::train_instances() -- no instances of part: " << pi->type() << " in its context!!!!!!!!!!!!\n";
    return true;
  }
  //: make sure the part's class posterior have been computed
...
dbrec_hierarchy_sptr populate_class_based_pairwise_discrete_compositor(dbrec_hierarchy_sptr orig_hierarchy, int radius, float angle_increment, int d_interval, int rho_interval)
{
  ...
  for (unsigned c = 0; c < orig_hierarchy->class_cnt(); c++) {
    ...
    std::vector<dbrec_part_sptr> parts = c_node_comp->children();  // this is an OR composition, use all of its parts to generate new compositional nodes
    
    //: first compute the direction vectors and sym angles if any
    std::vector<bool> has_direction_vector(parts.size(), false);
    std::vector<int> sym_angles(parts.size(), -1);
    ...
    std::vector<dbrec_part_sptr> part_pool;  
    for (unsigned i = 0; i < parts.size(); i++) {
      ...
      for (unsigned j = i; j < parts.size(); j++) {
        ...
        std::vector<dbrec_part_sptr> pp; pp.push_back(p1); pp.push_back(p2);

        std::vector<dbrec_pairwise_rot_invariant_discrete_model_sptr> prims;
        if (gamma_defined) {
...
dbrec_hierarchy_sptr populate_class_based_central_compositor_by_sampling(dbrec_hierarchy_sptr orig_hierarchy, unsigned n)
{
  ...
  for (unsigned c = 0; c < orig_hierarchy->class_cnt(); c++) {
    ...
    for (unsigned i = 0; i < parts.size(); i++) {
      ...
      if (!part_i) {
        std::cout << "In populate_class_based_central_compositor_by_sampling() -- the node is not a composition!\n";
        throw 0;
      }
      dbrec_central_compositor* c_i = dynamic_cast<dbrec_central_compositor*>(part_i->compositor().ptr());
      if (!c_i) {
        std::cout << "In populate_class_based_central_compositor_by_sampling() -- the compositor is not a central compositor!\n";
        throw 0;
      }
      std::vector<dbrec_pairwise_model_sptr>& mods = c_i->models();
      std::vector<dbrec_part_sptr>& ch = part_i->children();
      
      std::vector<std::vector<dbrec_pairwise_model_sptr> > new_mods(n, std::vector<dbrec_pairwise_model_sptr>());
      for (unsigned mm = 0; mm < mods.size(); mm++) {
        dbrec_pairwise_discrete_model* m = dynamic_cast<dbrec_pairwise_discrete_model*>(mods[mm].ptr());
        if (!m) {
          std::cout << "In populate_class_based_central_compositor_by_sampling() -- the pairwise model is not a discrete model!\n";
          throw 0;
        }
        std::vector<dbrec_pairwise_model_sptr> new_ins;
...
dbrec_hierarchy_sptr populate_class_based_central_compositor_by_indep_gaussian_model(dbrec_hierarchy_sptr orig_hierarchy, unsigned n)
{
  ...
  for (unsigned c = 0; c < orig_hierarchy->class_cnt(); c++) {
    ...
    for (unsigned i = 0; i < parts.size(); i++) {
      dbrec_composition* part_i = dynamic_cast<dbrec_composition*>(parts[i].ptr());
      if (!part_i) {
        std::cout << "In populate_class_based_central_compositor_by_indep_gaussian_model() -- the node is not a composition!\n";
        throw 0;
      }
      dbrec_central_compositor* c_i = dynamic_cast<dbrec_central_compositor*>(part_i->compositor().ptr());
      if (!c_i) {
        std::cout << "In populate_class_based_central_compositor_by_indep_gaussian_model() -- the compositor is not a central compositor!\n";
        throw 0;
      }
      std::vector<dbrec_pairwise_model_sptr>& mods = c_i->models();
      std::vector<dbrec_part_sptr>& ch = part_i->children();
      ...
      for (unsigned mm = 0; mm < mods.size(); mm++) {
        dbrec_pairwise_discrete_model* m = dynamic_cast<dbrec_pairwise_discrete_model*>(mods[mm].ptr());
        if (!m) {
          std::cout << "In populate_class_based_central_compositor_by_indep_gaussian_model() -- the pairwise model is not a discrete model!\n";
          throw 0;
        }
        std::vector<dbrec_pairwise_model_sptr> new_ins;
