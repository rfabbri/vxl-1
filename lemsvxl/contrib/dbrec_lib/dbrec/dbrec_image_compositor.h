...
public:
  ...
  virtual void sample_locations(const vgl_point_2d<float>& loc, float scale, const std::vector<dbrec_part_sptr>& children, std::vector<vgl_point_2d<float> >& locs) const = 0;
  
  //: for the central_compositor, the direction vector is the direction vector of the central part, the first child
  virtual void direction_vector(const std::vector<dbrec_part_sptr>& children, vnl_vector_fixed<float,2>& v) const = 0;

  //: draw additional attributes on the composing parts, OR compositor does nothing
  virtual void draw_attributes(bsvg_document& doc, const std::vector<dbrec_part_sptr>& children, const std::vector<vgl_point_2d<float> >& locs) const = 0;

  //: draw additional attributes on the composing parts, OR compositor does nothing
  virtual void visualize(std::string& doc_name, const std::vector<dbrec_part_sptr>& children) const = 0;

  //: method to detect instances of a composition
  virtual dbrec_part_context_sptr detect_instances(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, const std::vector<float>& comp_priors, float radius) const = 0;

  //: method to train a compositional part's geometric models
  virtual bool train_instances(dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float radius) = 0;
...
public:
  ...
  //: sample locations of each child in the composition wrt given location
  virtual void sample_locations(const vgl_point_2d<float>& loc, float scale, const std::vector<dbrec_part_sptr>& children, std::vector<vgl_point_2d<float> >& locs) const;
  
  //: for the central_compositor, the direction vector is the direction vector of the central part, the first child
  virtual void direction_vector(const std::vector<dbrec_part_sptr>& children, vnl_vector_fixed<float,2>& v) const;

  //: draw additional attributes on the composing parts, OR compositor does nothing
  virtual void draw_attributes(bsvg_document& doc, const std::vector<dbrec_part_sptr>& children, const std::vector<vgl_point_2d<float> >& locs) const {}

  //: draw additional attributes on the composing parts, OR compositor does nothing
  virtual void visualize(std::string& doc_name, const std::vector<dbrec_part_sptr>& children) const {};

  //: method to detect instances of a composition
  virtual dbrec_part_context_sptr detect_instances(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, const std::vector<float>& comp_priors, float radius) const;

  //: method to train a compositional part's geometric models
  //  an OR node shall never be trained, cause it has no models
  virtual bool train_instances(dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float radius) { return true; }
...
public:
  dbrec_central_compositor(const std::vector<dbrec_pairwise_model_sptr>& class_models) : models_(class_models) {}
  ...
  //: method to train a compositional part's geometric models
  virtual bool train_instances(dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float radius);

  std::vector<dbrec_pairwise_model_sptr>& models() { return models_; }
...
protected:
  ...
  std::vector<dbrec_pairwise_model_sptr> models_;
};
...
public:
  ...
  virtual void direction_vector(const std::vector<dbrec_part_sptr>& children, vnl_vector_fixed<float,2>& v) const  { throw 0; }

  //: draw additional attributes on the composing parts
  virtual void draw_attributes(bsvg_document& doc, const std::vector<dbrec_part_sptr>& children, const std::vector<vgl_point_2d<float> >& locs) const  { throw 0; }

  virtual void visualize(std::string& doc_name, const std::vector<dbrec_part_sptr>& children) const  { throw 0; }
  
  //: method to detect instances of a composition
  virtual dbrec_part_context_sptr detect_instances(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, const std::vector<float>& comp_priors, float radius) const { throw 0; }

  //: method to detect instances of a composition in a rotationally invariant way for the 2 case problem: class and non-class
  dbrec_part_context_sptr detect_instances_rot_inv(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float comp_class_prior, float radius);
  ...
  dbrec_part_context_sptr detect_instances_rot_inv(unsigned type_id, dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float comp_class_prior, vil_image_view<float>& fg_prob, float radius);

  //: method to train a compositional part's geometric models
  virtual bool train_instances(dbrec_context_factory_sptr cf, const std::vector<dbrec_part_sptr>& children, float radius)  { throw 0; }

  static void collect_rot_inv_stats(dbrec_context_factory_sptr cf, int posterior_id, dbrec_part_sptr p1, dbrec_part_sptr p2, float radius, const std::string& output_file_prefix);
...
public:
  ...
  //: sample locations of each child in the composition wrt given location
  virtual void sample_locations(const vgl_point_2d<float>& loc, float scale, const std::vector<dbrec_part_sptr>& children, std::vector<vgl_point_2d<float> >& locs) const { throw 0; }
  virtual void direction_vector(const std::vector<dbrec_part_sptr>& children, vnl_vector_fixed<float,2>& v) const  { throw 0; }
  //: draw additional attributes on the composing parts
  virtual void draw_attributes(bsvg_document& doc, const std::vector<dbrec_part_sptr>& children, const std::vector<vgl_point_2d<float> >& locs) const  { throw 0; }
  virtual void visualize(std::string& doc_name, const std::vector<dbrec_part_sptr>& children) const;
...
dbrec_hierarchy_sptr populate_depth_central_compositor(dbrec_hierarchy_sptr orig_hierarchy, int depth, float radius, const F& model_factory)
{
  ...
  for (unsigned i = 0; i < parts.size(); i++) {
    for (unsigned j = 0; j < parts.size(); j++) {
      std::vector<dbrec_part_sptr> pp; pp.push_back(parts[i]); pp.push_back(parts[j]);
      dbrec_pairwise_model_sptr m = model_factory.generate_model(); // one model per pair
      std::vector<dbrec_pairwise_model_sptr> models; models.push_back(m);
