...
vil_image_resource_sptr dbrec_part_context::get_posterior_map(unsigned i, unsigned ni, unsigned nj)
{
  ...
  while (pi) {
    ...
    if (ii < ni && jj < nj && ii >=0 && jj >= 0)
      ...
      std::cout << "In dbrec_part_context::get_posterior_map() -- the location of the instance is out of passed image size, ni: " << ni << " nj: " << nj << std::endl;
    }
    pi = this->next();
  }
  return vil_new_image_resource_of_view(out_v);
...
void dbrec_rtree_context::query(const vgl_box_2d<float>& box, std::vector<dbrec_part_ins_sptr>& out) 
{
  rtree_.get(box, out);
}
...
dbrec_part_context_sptr dbrec_rtree_context_factory::get_context(unsigned type)
{
  ...
  std::map<unsigned, dbrec_part_context_sptr>::iterator it = contexts_.find(type);
  if (it != contexts_.end()) {
    c = it->second;
  } 
  return c;
