...
dbrec_gaussian_appearance_model_sptr dbrec_gaussian_weibull_model_learner::fit_weibull(double& k, double& lambda)
{
  if (p_sum_ > 0.0001) {  // if different from zero
    ...
    std::cout << "p_sum = " << p_sum_ << "  x_sum = " << x_sum_ << '\n';
    std::cout << "Total response was very close to zero so no fitting is done, output a really peaked distribution around strength = 0.00001!\n";
    k = 1.001;
    lambda = 0.00001;
  }
...
void dbrec_gaussian_weibull_model_learner::visualize_model(dbrec_gaussian_appearance_model_sptr m, const std::string& name, float width, float height, float margin, int font_size)
{
  ...
  for (unsigned a = 0; a < h.nbins(); a++) {
    ...
    float prob = m->probability(std::pow(10.0f, min_val), std::pow(10.0f, max_val), 0, 0);
...
void dbrec_gaussian_weibull_model_learner::visualize_hist(const std::string& name, float width, float height, float margin, int font_size)
{
...
{
  if (!data_set_) {
    std::cout << "In dbrec_gaussian_weibull_model_learner_visitor::visit_gaussian_primitive() -- images are not set! cannot collect stats for: " << g->type() << "\n";
    return;
  }
  dbrec_part_context_sptr pc = cf_->get_context(g->type());
  if (!pc || pc->maps_size() < 2) {
    std::cout << "In dbrec_gaussian_weibull_model_learner_visitor::visit_gaussian_primitive() -- the context for type: " << g->type() << " has not been created!\n";
    return;
  } 
  ...
  if (it == learners_.end()) {
    ...
    learners_[g->type()] = std::pair<dbrec_gaussian_weibull_model_learner_sptr, dbrec_gaussian_weibull_model_learner_sptr>(l1, l2);
  } else {
    l1 = it->second.first;
    l2 = it->second.second;
  }
  ...
  if (ni != prob_img_.ni() || nj != prob_img_.nj() || ni != mask_img_.ni() || nj != mask_img_.nj()) {
    std::cout << "In dbrec_gaussian_weibull_model_learner_visitor::visit_gaussian_primitive() -- image sizes are not compatible!\n";
    return;
  }
...
{
  if (!data_set_) {
    std::cout << "In dbrec_rot_inv_gaussian_weibull_model_learner_visitor::visit_gaussian_primitive() -- images are not set! cannot collect stats for: " << g->type() << "\n";
    return;
  }
  dbrec_part_context_sptr pc = cf_->get_context(g->type());
  if (!pc || pc->maps_size() < 3) {
    std::cout << "In dbrec_rot_inv_gaussian_weibull_model_learner_visitor::visit_gaussian_primitive() -- the context for type: " << g->type() << " has not been created!\n";
    return;
  } 
...
void dbrec_rot_inv_gaussian_weibull_model_learner_visitor::print_current_models_at_each_orientation(const std::string& prefix, float angle_inc)
{
  for (map_type::iterator it = learners_.begin(); it != learners_.end(); it++) {
    ...
    std::string str_id = p->string_identifier();
    std::string name = prefix + str_id + "_class_model.svg";
    std::cout << "------------ " << str_id << " fitting class model: -----\n";
    double k1, lambda1;
    ...
    std::cout << "------------ " << str_id << " fitting non-class model: -----\n";
    double k2, lambda2;
    ...
    std::string text_file = prefix + str_id + "_fg_params_dbrec.txt";
    std::ofstream of(text_file.c_str());
    of << k1 << " " << lambda1 << std::endl;
    of << k2 << " " << lambda2 << std::endl;
    of.close();
    ...
    for (float angle = 0.0f; angle < 180.0f; angle += angle_inc) {
      ...
      std::string text_file = prefix + g_a.string_identifier() + "_fg_params_dbrec.txt";
      std::ofstream of_a(text_file.c_str());
      of_a << k1 << " " << lambda1 << std::endl;
      of_a << k2 << " " << lambda2 << std::endl;
      of_a.close();
    }
  }
}

void dbrec_gaussian_weibull_model_learner_visitor::print_current_histograms(const std::string& prefix)
{
  for (map_type::iterator it = learners_.begin(); it != learners_.end(); it++) {
    dbrec_gaussian_weibull_model_learner_sptr l1 = it->second.first;
    dbrec_gaussian_weibull_model_learner_sptr l2 = it->second.second;
    dbrec_part_sptr p = h_->get_part(it->first);
    std::string str_id = p->string_identifier();
    std::string name = prefix + "_" + str_id + "_class.svg";
    l1->visualize_hist(name);
    name = prefix + "_" + str_id + "_non_class.svg";
    l2->visualize_hist(name);
  }
}
void dbrec_gaussian_weibull_model_learner_visitor::print_current_models(const std::string& prefix)
{
  for (map_type::iterator it = learners_.begin(); it != learners_.end(); it++) {
    dbrec_gaussian_weibull_model_learner_sptr l1 = it->second.first;
    dbrec_gaussian_weibull_model_learner_sptr l2 = it->second.second;
...
void dbrec_image_construct_bg_model_visitor::visit_gaussian_primitive(dbrec_gaussian* g)
{
  ...
  std::string name = model_folder_+g->string_identifier()+"_bg_mu_img_dbrec.tiff";
...
void dbrec_image_construct_bg_model_rot_inv_visitor::visit_gaussian_primitive(dbrec_gaussian* g) 
{
  ...
  if (g->lambda0_ == g->lambda1_) {
    if (g->theta_ != 0.0) {
      std::cout << "ERROR: In dbrec_image_construct_bg_model_rot_inv_visitor::visit_gaussian_primitive() -- the primitive is isotropic but its angle is not set to be zero!!\n";
      throw 0;
    }
  ...
  for (float angle = 0.0f; angle < 180.0f; angle += angle_inc_) {
    ...
    //: write the images out to the folder
    std::string name = model_folder_+g_a.string_identifier()+"_bg_mu_img_dbrec.tiff";
    bool result = vil_save(mu_img, name.c_str());
    if ( !result ) {
      std::cerr << "Failed to save image to " << name << std::endl;
      return;
