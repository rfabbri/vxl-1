...
public:
  ...
  void write_doc(std::string& name);
...
public:
  ...
  static bool parse_composition_structure(bxml_data_sptr d, std::map<unsigned, std::pair<dbrec_part_sptr, bool> >& part_map);
...
protected:
  ...
  std::map<unsigned, std::pair<dbrec_part_sptr, bool> > part_map_;
};
...
protected:
  ...
  float space_;
  float scale_;
  std::vector<vgl_point_2d<float> > children_locs_;
...
public:
  dbrec_draw_bsvg_visitor(float w, float h, float cx, float cy, float vrad, const std::string& c) : width_(w), height_(h), 
...
const std::string file_fg_suffix = "_fg_params_dbrec.txt";
//const std::string file_fg_suffix = "_fg_params.txt";

//: helper to set the appearance models of gaussian primitives
class dbrec_parse_image_visitor_helper : public dbrec_visitor {
public:
  dbrec_parse_image_visitor_helper(const std::string& fg_appearance_model_path) : fg_app_model_path_(fg_appearance_model_path) {}
  virtual void visit_composition(dbrec_composition* c);
  virtual void visit_gaussian_primitive(dbrec_gaussian* g);
  
protected:
  std::string fg_app_model_path_;
...
public:
  ...
  dbrec_parse_image_visitor(dbrec_hierarchy_sptr h, vil_image_resource_sptr img, float class_prior, const std::vector<float>& comp_priors, const std::string& fg_appearance_model_path = "");

  //: if a context factory is available for the image for some parts of the hierarchy, use it to avoid re-extraction
  dbrec_parse_image_visitor(dbrec_hierarchy_sptr h, dbrec_context_factory_sptr cf, vil_image_resource_sptr img, float class_prior, const std::vector<float>& comp_priors, const std::string& fg_appearance_model_path = "");
  ...
  static float marginal_value(const std::vector<dbrec_gaussian_appearance_model_sptr>& app_models, const std::vector<dbrec_prior_model_sptr>& priors, float res, unsigned i, unsigned j);

protected:
  void initialize(const std::string& model_path, float class_prior);
  
  dbrec_hierarchy_sptr h_;
  vil_image_resource_sptr img_;
  std::string fg_model_path_;
  
  std::vector<float> composition_priors_; // a prior for each case of compositions need to be passed. if size 0, then class prior saved in the composition will be used
  ...
  std::vector<dbrec_prior_model_sptr> priors_; // a prior for each appearance model in the primitives need to be passed, otherwise it won't parse
  
};

const std::string file_bg_mu_suffix = "_bg_mu_img_dbrec.tiff";
const std::string file_bg_sigma_suffix = "_bg_sigma_img_dbrec.tiff";
//const std::string file_bg_mu_suffix = "_bg_mu_img.tiff";
//const std::string file_bg_sigma_suffix = "_bg_sigma_img.tiff";
...
public:
  dbrec_parse_image_with_fg_map_visitor(dbrec_hierarchy_sptr h, vil_image_resource_sptr img, const vil_image_view<float>& fg_prob, 
    float class_prior, const std::vector<float>& comp_priors, const std::string& fg_model_path, const std::string& bg_model_path);
...
public:
  ...
  dbrec_parse_image_rot_inv_visitor(dbrec_hierarchy_sptr h, vil_image_resource_sptr img, const vil_image_view<bool>& valid_region_mask, float class_prior, float theta_inc, const std::string& fg_appearance_model_path = "") 
    : dbrec_parse_image_visitor(h, img, class_prior, std::vector<float>(), fg_appearance_model_path), theta_inc_(theta_inc), valid_region_mask_(valid_region_mask) {}
  ...
  dbrec_parse_image_rot_inv_visitor(dbrec_hierarchy_sptr h, dbrec_context_factory_sptr cf, vil_image_resource_sptr img, const vil_image_view<bool>& valid_region_mask, float class_prior, float theta_inc, const std::string& fg_appearance_model_path = "")
    : dbrec_parse_image_visitor(h, cf, img, class_prior, std::vector<float>(), fg_appearance_model_path), theta_inc_(theta_inc), valid_region_mask_(valid_region_mask) {}
...
public:
  dbrec_parse_image_rot_inv_with_fg_map_visitor(dbrec_hierarchy_sptr h, vil_image_resource_sptr img, const vil_image_view<bool>& valid_region_mask, const vil_image_view<float>& fg_prob, 
    float class_prior, float theta_inc, const std::string& fg_model_path, const std::string& bg_model_path);
...
protected:
  void initialize(const std::string& fg_model_path); 
  dbrec_gaussian_appearance_model_sptr get_map(std::map<float, dbrec_gaussian_appearance_model_sptr>& bg_model_map, float angle, dbrec_gaussian* g);
