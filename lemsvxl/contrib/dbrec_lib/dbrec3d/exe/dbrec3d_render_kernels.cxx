...
{
  vul_arg<std::string> path("-path", "path to taylor kernels", "");
  ...
  std::map<std::string, bvpl_kernel_sptr> taylor_basis;
  loader.create_basis(taylor_basis);
  
  std::map<std::string, bvpl_kernel_sptr>::iterator it= taylor_basis.begin();
  it->second->print();
