...
{
  vul_arg<std::string> taylor_basis_path("-taylor_basis_path", "file to principal components' matrix ", "");
...
//    std::string filename = taylor_basis_path() + "/" + kernel_names[ki] + ".txt";
...
//    }
////    if(i!=125){
////      std::cerr << "Wrong dimension " << i << "\n";
////      return -1;
////    }
//    
//    basis.set_column(ki, basis_vector);
//    //std::cout << basis_vector << std::endl;
//  }
  
 
  
//  std::cout << inner_product(basis.get_column(0), basis.get_column(1)) << std::endl;
//  vnl_matrix<double> norm_basis = basis.normalize_columns();
//   vnl_matrix<double> inverse = vnl_matrix_inverse<double>(norm_basis);
//  std::cout << norm_basis* inverse << std::endl;
  ...
  for(unsigned i=0; i<num_means; i++){
    ...
    //std::cout << feature_approx << std::endl;
