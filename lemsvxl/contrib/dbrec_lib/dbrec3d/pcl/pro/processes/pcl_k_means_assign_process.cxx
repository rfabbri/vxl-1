...
bool pcl_k_means_assign_process(bprb_func_process& pro)
{
  ...
  //get inputs
  unsigned i = 0;
  std::string xyznormal_pcd_file = pro.get_input<std::string>(i++);
  std::string fpfh_pcd_file = pro.get_input<std::string>(i++);
  std::string means_file = pro.get_input<std::string>(i++);
  std::string output_dir = pro.get_input<std::string>(i++); 
  
  //read in the features cloud
  std::cout << "Loading: " << fpfh_pcd_file <<std::endl;
  PointCloud<FPFHSignature33>::Ptr fpfh_cloud(new PointCloud<FPFHSignature33>);
  if (pcl::io::loadPCDFile (fpfh_pcd_file, *fpfh_cloud) < 0)
    return (false);
  std::cout << " Done:" <<  fpfh_cloud->width * fpfh_cloud->height << " points\n";
  std::cout << "Available dimensions: " << getFieldsList(*fpfh_cloud).c_str ();
  
  
  //read in the point cloud
  std::cout << "Loading: " << xyznormal_pcd_file <<std::endl;
  ...
  //read the means
  std::ifstream mean_ifs(means_file.c_str());
  if(!mean_ifs.is_open()){
    std::cerr << "Error: Could not open mean_ifs: " << means_file <<  "\n";
    return false;
  }
