...
const char bof_labels_keypoint_joint::max_aposteriori(const std::vector< dbcll_euclidean_cluster_light<10> > classify_clusters, 
                                                      std::vector<double> &log_p_c_given_obj) const
{
  log_p_c_given_obj.clear();
  log_p_c_given_obj.resize(ncategories_, 0.0);
  double max_log_aposteriori = -1.0f * std::numeric_limits<double>::infinity();
  char max_class = -1;
  std::cout << "Number of categories= " << ncategories_ << "\n";
...
void bof_labels_keypoint_joint::merge_clusters(char class_id, const std::vector< dbcll_euclidean_cluster_light<10> > &class_cluster2)
{
  std::vector< dbcll_euclidean_cluster_light<10> > &class_cluster = all_class_clusters_[class_id];
...
void bof_labels_keypoint_joint::xml_read_and_combine(std::string xml_scen_prfx, std::vector< vnl_vector_fixed<double, 10> > common_means, unsigned nscenes, unsigned ncategories)
{
  for (unsigned si=0; si< nscenes; si++) {
    for(unsigned ci=0; ci < ncategories; ci++){
      std::stringstream xml_file_ss;
      xml_file_ss << xml_scen_prfx << si << "/class_" << ci << "_distribution.xml";
      if (!vul_file::exists(xml_file_ss.str()))
        continue;
      std::vector< dbcll_euclidean_cluster_light<10> > scene_ith_clusters;
...
void bof_labels_keypoint_joint::xml_read(std::string xml_path, std::vector< vnl_vector_fixed<double, 10> > common_means)
{
  ...
  for(unsigned ci=0; ci < ncategories_; ci++){
    ...
    if (!vul_file::exists(xml_file_ss.str())){
      std::cerr << "File " << xml_file_ss.str() << "doesn't exist \n";
      continue;
    }
    std::vector< dbcll_euclidean_cluster_light<10> > class_ith_clusters;
...
void bof_labels_keypoint_joint::xml_write(std::string xml_path)
{
...
void bof_labels_keypoint_joint::bsta_save_and_plot(std::string path_out)
{
  for (unsigned class_id = 0 ; class_id < all_class_clusters_.size(); class_id++) {
    
    const std::vector< dbcll_euclidean_cluster_light<10> > &class_clusters = all_class_clusters_[class_id];
    std::vector<double> counts(class_clusters.size(),0.0);
    ...
    std::ofstream hist_ofs((ss.str() + "_bsta_hist.txt").c_str());
    std::ofstream hist_arrays_ofs((ss.str() + "_hist_plot.txt").c_str());
...
{
  std::cerr << "Error: Tryinbof_labels_keypoint_jointg to read but binary io not implemented\n";
  return;
}

void vsl_print_summary(std::ostream & , const bof_labels_keypoint_joint & )
{
...
void vsl_print_summary(std::ostream& os, const bof_labels_keypoint_joint* &p)
{
  if (p==0)
    os << "NULL PTR";
  else {
