...
bool bof_k_means_train_process(bprb_func_process& pro)
{
  ...
  std::string init_mean_file = pro.get_input<std::string>(i++);
  ...
  std::string mean_out_file = pro.get_input<std::string>(i++);

 
  //read initial means
  std::ifstream mean_ifs(init_mean_file.c_str());
  if(!mean_ifs.is_open()){
    std::cerr << "Error: Could not open: " << init_mean_file << "\n";
    return false;
  }
  ...
  {
    //write means to file
    std::ofstream means_ofs(mean_out_file.c_str());
    ...
    if(means_ofs.is_open())
      ...
      std::cerr << "Could not open file: " << mean_out_file << "\n";
      return false;
    }
