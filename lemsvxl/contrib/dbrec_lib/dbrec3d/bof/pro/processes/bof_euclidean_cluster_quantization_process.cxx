...
bool bof_euclidean_cluster_quantization_process(bprb_func_process& pro)
{
  ...
  std::string mean_file = pro.get_input<std::string>(i++);
  ...
  std::string cluster_out_file = pro.get_input<std::string>(i++);  
 
  //read means
  std::ifstream mean_ifs(mean_file.c_str());
  if(!mean_ifs.is_open()){
    std::cerr << "Error: Could not open: " << mean_file << "\n";
    return false;
  }
