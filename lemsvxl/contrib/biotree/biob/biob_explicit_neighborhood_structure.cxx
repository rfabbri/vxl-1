...
typedef std::list<biob_worldpt_index> list_t;
typedef list_t::const_iterator iterator_t;

void add_nearby_points(const biob_worldpt_roster_sptr roster, worldpt pt, const std::list<biob_worldpt_index> & candidates,
                       double distance, std::list<biob_worldpt_index> & nearby_points){
...
void biob_explicit_neighborhood_structure::populate(biob_worldpt_roster_sptr roster, double resolution){
  ...
  */
  ...
  std::vector<std::list<biob_worldpt_index> > voxel_inhabitants(grid.num_points());
