...
static void test_cali_corresp(int argc, char* argv[] ){
    ...
    std::string fname = "C:\\latest_amherst\\testing_correspondences\\parameters.txt";
    cali_param par(fname);
  cali_artf_corresponder corr(par);
  std::vector<std::vector<vsol_conic_2d > >model_conics;
vnl_vector<double> fx(((par.END - par.START/par.INTERVAL)+1)*par.NUM_OF_CONICS*2);
  xscan_scan scan;
  std::vector<vil_image_resource_sptr> resources;

  std::string file_name = par.LOGFILE;
   std::FILE *fp = std::fopen(file_name.data(),"r");
  assert(fp != NULL);
...
std::vector<conic_vector_set> real_conics;

 for (unsigned i=0; i<scan.n_views(); i += par.INTERVAL) {
    std::string file_name = corr.gen_read_fname(par.CONICS_BIN_FILE_BASE, i);
...
//  std::vector<std::vector<vsol_conic_2d_sptr> > v = corr.fit_conics("\\test_artifact315.jpg");
