...
cali_vnl_least_squares_function(cali_param par,
                                std::vector<vil_image_resource_sptr> scan_resources, 
                                xscan_scan scan, 
...
{
  /*std::string txt_file = "C:\\test_images\\gen_calib\\diff.txt";
  ...
  for (unsigned i=0; i<scan.n_views(); i+=interval) {
    std::string file_name = corresp.gen_read_fname(file_base, i);
...
cali_vnl_least_squares_function::f(vnl_vector<double> const& ax, vnl_vector<double>& fx)
{
    ...
    {
        std::cout<<" ax values "<<ax[i]<<std::endl;
    }
   ...
   for (int j = 0;j < 18 ;j++)
   {
     if (par_.SETPARAM[j])
       ...
       std::cout << par_.PARAM[j] << "\n"; 

   }

   std::vector<conic_vector_set> sim_artf_conics;
   ...
   std::cout << std::endl << std::endl << "Artifact Rotation "
           << artf_rot.axis() << "," 
           << artf_rot.angle() 
          << "(" << (artf_rot.angle()/vnl_math::pi)*180  << " deg)" << " " ;
  std::cout << "Translation " << artf_tr << std::endl;

  std::cout << std::endl << std::endl << "Turntable Rotation "  
          << turn_table_rot.axis() << "," 
          << turn_table_rot.angle() 
          << "(" << (turn_table_rot.angle()/vnl_math::pi)*180  << " deg)" << " " ;
  std::cout << "Translation " << turn_table_tr << std::endl;

  std::cout<<"scale ("<<x_scale <<","<<y_scale<< ")" << " ";
  std::cout<<" principal point ("<<princp_point.x()<<","<<princp_point.y()<< ")" << std::endl;

/*
  for (int i = 0;i< static_cast<int>(x_coord_tol.size());i++)
  {
      std::cout<<" x_coord_tol: "<< "for ball " <<i+1<<" : "<<x_coord_tol[i]<<std::endl;
       std::cout<<" y_coord_tol: "<< "for ball " <<i+1<<" : "<<y_coord_tol[i]<<std::endl;
        std::cout<<" z_coord_tol: "<< "for ball " <<i+1<<" : "<<z_coord_tol[i]<<std::endl;
         std::cout<<" rad_tol: "<< "for ball " <<i+1<<" : "<<rad_tol[i]<<std::endl;
  }
  */
 ...
 /*
    std::cerr << "centers is of size: " << centers.size() << "\n"; 
    int sum = 0;
    for(int i = 0; i < centers.size(); i++)
            sum+=centers[i].size(); 
    std::cerr << "sum of center sizes: " << sum << "\n"; 
    */

               
   /*
    std::cout << "real_img_conics is of size: " << real_img_conics.size() << "\n"; 
    int sum = 0;
    for(int i = 0; i < real_img_conics.size(); i++){
            std::cout << "\treal_img_conics[" << i << "] has " << real_img_conics[i].size() << "\n";
            for(int j = 0; j < real_img_conics[i].size(); j++){
                    std::cout << "\t\treal_img_conics[" << i << "][" << j << "] has" << real_img_conics[i][j].size() << "\n";
                    sum+=real_img_conics[i][j].size();
            }
    }
    std::cout << "sum of real_img_conic sizes: " << sum << "\n"; 
    */
   ...
   /*
    std::cout << "modified_real_conic_vector is of size: " << modified_real_conic_vector.size() << "\n"; 
    for(int i = 0; i < modified_real_conic_vector.size(); i++){
            std::cout << "\tmodified_real_conic_vector[" << i << "] has " << modified_real_conic_vector[i].size() << "\n";
    }
    */
...
for (int j = 0;j<x.size();j++)
  ...
  turn_table_trans = vnl_double_3 (tt_tx, tt_ty, tt_tz);
   ...
   std::cout << std::endl << std::endl << "Artifact Rotation-->" << std::endl;
  std::cout << artf_rot.axis() << std::endl;
  std::cout << artf_rot.angle() << " deg=" << (artf_rot.angle()/vnl_math::pi)*180 << std::endl;
  std::cout << "Artifact Translation-->" << std::endl;
  std::cout << artf_trans << std::endl;

  std::cout << "Turn Table Rotation-->" << std::endl;
  std::cout << turn_table_rot.axis() << std::endl;
  std::cout << turn_table_rot.angle() << " deg=" << (turn_table_rot.angle()/vnl_math::pi)*180 << std::endl;
  std::cout << "Turn Table Translation-->" << std::endl;
  std::cout << turn_table_trans << std::endl;
...
for (int j = 0;j<x.size();j++)
  ...
  for (int k = 0;k<par_.BALL_NUMBER;k++)
  {
  std::cout << "x_coord_tol for ball "<<k+1<<" -->"<<std::endl;
  std::cout << x_coord_tol[k] <<std::endl;
 std::cout << "y_coord_tol for ball "<<k+1<<" -->"<<std::endl;
  std::cout << y_coord_tol[k] <<std::endl;
 std::cout << "z_coord_tol for ball "<<k+1<<" -->"<<std::endl;
  std::cout << z_coord_tol[k] <<std::endl;
   std::cout << "rad_tol for ball "<<k+1<<" -->"<<std::endl;
  std::cout << rad_tol[k] <<std::endl;
  }
  */
                                                ...
                                                xscan_scan& scan)
{
  ...
  dummy[i++] = scan.kk().x_scale();
   ...
   std::cout<<" x-scale " <<  scan.kk().x_scale()<<std::endl;
   std::cout<<" y-scale " << scan.kk().y_scale()<<std::endl;
  ...
  int count = 0;
   ...
   std::cout << std::endl << std::endl << "Artifact Rotation-->" << std::endl;
   std::cout << artf_rot.axis() << std::endl;
   std::cout << artf_rot.angle()  << std::endl;
   std::cout << "Artifact Translation-->" << std::endl;
  std::cout << artf_tr << std::endl;
 
   std::cout << "Turn Table Rotation-->" << std::endl;
   std::cout << tt_rot.axis() << std::endl;
  std::cout << tt_rot.angle() << std::endl;
                                                ...
                                                std::vector<double>& rad_tol)
{
        ...
        std::cout << "cali_vnl_least_squares_function::set_lsqr_fn_params_with_tolerance\n";
   vnl_vector<double>dummy;
   dummy.set_size(par_.SIZE_OF_X);
   int i=0;
    ...
    num_of_images_(num_of_images)
{
  ...
  std::cout << "constructing cali_vnl_least_squares_function without cali_param.\n";
  ...
  for (unsigned i=0; i<scan.n_views(); i+=interval) {
    std::string file_name = corresp.gen_read_fname(file_base, i);
    std::cout << "adding conic " << file_name << "\n";
    conic_vector_set saved_conics = corresp.read_conics_bin(file_name);
    std::cout << "read " << saved_conics.size() << " conics\n";
    real_img_conics.push_back(saved_conics);
  }
...
std::cout<<" size of ax " <<ax.size()<<std::endl;
 ...
 // ball_distance = cali_cylinder_artifact::ball_spacing;
  ...
  std::cout << turn_table_rot.angle() << " deg=" << (turn_table_rot.angle()/vnl_math::pi)*180 << std::endl;
  std::cout << "Turn Table Translation-->" << std::endl;
  std::cout << turn_table_tr << std::endl;

  std::cout<<" x & y scale "<<x_scale <<"  "<<y_scale<<std::endl;
  std::cout<<" principal point "<<princp_point.x()<<"  "<<princp_point.y()<<std::endl;
  ...
  xscan_orbit_base_sptr new_orbit = new xscan_uniform_orbit(orbit.rot(),
                        ...
                        turn_table_tr);
  scan_.set_orbit(new_orbit);
 // std::cout<<" scan values in vnl_least_squares_function " <<scan_<<std::endl;
  //ball_distance = cali_cylinder_artifact::ball_spacing;
 // cali_simulated_scan_resource scan_res(scan_, scan_res_, ball_indent, ball_indent_big, ball_distance);
//
//std::string path = "C:\\amherst_artifact\\parameters.txt";
//cali_param par(path);

cali_simulated_scan_resource scan_res(scan_, scan_res_,par_);
//cali_simulated_scan_resource scan_res(scan_, scan_res_);
//  std::vector<std::vector<vsol_conic_2d> > centers = scan_res.gen_ball_projections(artf_rot, artf_tr, par.INTERVAL);
   std::vector<std::vector<vsol_conic_2d> > centers = scan_res.gen_ball_projections(artf_rot, artf_tr,par_.INTERVAL);
