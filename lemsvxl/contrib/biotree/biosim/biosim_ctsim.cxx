...
rot_axis_(rotation_axis), rot_step_size_(rotation_step_size), noise_(noise_type)
{
  ...
  assert(std::fabs(length - src_org_d_) < 1e-06);

  num_views_ = std::abs(int(360.0 / rot_step_size_));
  data_.resize(sns_dim_[0], sns_dim_[1], num_views_);
  data_.fill(0.0);
...
biosim_ctsim::biosim_ctsim(xscan_dummy_scan dummy_scan, std::string noise_type)
{
...
void biosim_ctsim::write_scan(std::string scanfile)
{
...
{
  assert(std::strlen(folder) <= 248);
  // write the info file
  FILE *fp;
  char infofile[256];
  std::strcpy(infofile, folder);
  std::strcat(infofile, "\\info.txt");
  fp = std::fopen(infofile, "w");
  std::fprintf(fp, "camera pixel size: %f um\n", pix_size_);
  std::fprintf(fp, "source to origin distance: %f mm\n", src_org_d_);
  std::fprintf(fp, "source to sensor distance: %f mm\n", src_sns_d_);
  std::fprintf(fp, "image rows: %d\n", sns_dim_[1]);
  std::fprintf(fp, "image columns: %d\n", sns_dim_[0]);
  std::fprintf(fp, "principal point: (%f,%f)\n", prn_pnt_[0], prn_pnt_[1]);
  std::fprintf(fp, "initial source position: (%f,%f,%f)\n", init_src_pos_.x(), 
                                                           init_src_pos_.y(),
                                                           init_src_pos_.z());
  std::fprintf(fp, "rotation axis: (%f,%f,%f)\n", rot_axis_[0], rot_axis_[1], rot_axis_[2]);
  std::fprintf(fp, "rotation step size angle: %f\n", rot_step_size_);
  std::fprintf(fp, "number of views: %d\n", num_views_);
  ...
  std::strcpy(imgfileroot, folder);
  std::strcat(imgfileroot, "\\");
  std::strcat(imgfileroot, file);
/* have to adjust noise addition according to the new normalization rules
  ...
  for(int view=0; view < num_views_; view++)
  {
    ...
    std::strcpy(imgfile, imgfileroot);
    std::strncat(imgfile, num, 4);
    std::strcat(imgfile, ".pgm");
//    fp = std::fopen(imgfile, "w");
    ...
    for(int j=0; j < sns_dim_[1]; j++)
    {
      for(int i=0; i < sns_dim_[0]; i++)
      {
        ...
        if(imval > 65535)
          imval = 65535;
        img(i,j) = imval;
//        std::fprintf(fp, "%d ", int(data_[i][j][view]));
      }
//      std::fprintf(fp, "\n");
    }
...
void biosim_ctsim::scan(char *fname)
{
  ...
  char dummy[80];
  std::fscanf(fp, "%s\n", dummy);
  assert(!std::strcmp(dummy, "biosim_ctsim_input"));
  ...
  while(1)
  {
    ...
    else if(start_of_line == 'c') //this line is a cylinder input
    {
      ...
      std::cout << "splatting cylinder " << ++counter << std::endl;
