...
vgl_box_3d<double> 
compute_outer_box(vgl_box_3d<double> box, std::vector<xmvg_filter_descriptor> &d_list) 
{
              ...
              proc_run_args proc_args) 
{
  if (filter_type == FILTER_UNDEF) {
    std::cerr << "bioproc_filtering_example_xml: Undefined Filter Type" << std::endl;
    std::cerr << "bioproc_filtering_example_xml: valid Filter Types {gaussian, no_noise, edge_detector}" << std::endl;
    return 1;
  }
  
  if (splatting_type == SPLAT_UNDEF) {
    std::cerr << "bioproc_filtering_example_xml: Undefined Splat Type" << std::endl;
    std::cerr << "bioproc_filtering_example_xml: valid Splat Types {parallel, conebeam}" << std::endl;
    return 1;
  }

  if (algo_type == ALGO_UNDEF) {
    std::cerr << "bioproc_filtering_example_xml: Undefined Algorithm Type" << std::endl;
    std::cerr << "bioproc_filtering_example_xml: valid Algorithm Types {proc, proc_splr}" << std::endl;
    return 1;
  }
