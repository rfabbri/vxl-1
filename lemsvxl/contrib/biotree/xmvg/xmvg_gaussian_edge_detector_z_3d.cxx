// This is /contrib/biotree/xmvg/xmvg_gaussian_edge_detector_z_3d.cxx

#include "xmvg_gaussian_edge_detector_z_3d.h"

xmvg_gaussian_edge_detector_z_3d::xmvg_gaussian_edge_detector_z_3d()
{
}
...
class analytic_gauss_edge_detector_z_1
{
public:
...
class analytic_gauss_edge_detector_z_3
{
public:
  //: constructor with the fixed parameters
  analytic_gauss_edge_detector_z_3(double sigma, vnl_double_3 p) : sigma_(sigma), p_(p) 
  {
    x0 = p_.get(0);
    y0 = p_.get(1);
    z0 = p_.get(2);
...
xmvg_atomic_filter_2d<double> xmvg_gaussian_edge_detector_z_3d::splat(vgl_point_3d<double> centre, xmvg_perspective_camera<double> cam)
{
  // locals
...
//  line = cam_trans.backproject(vgl_homg_point_2d<double>(double(0), double(0), 1.0));
  ...
  analytic_gauss_edge_detector_z_1 aged_z(descriptor_.sigma(), vnl_double_3(ray_start.x(), ray_start.y(), ray_start.z()));
  ...
  for(int j=min_y; j<=max_y; j++)
  {
    for(int i=min_x; i<=max_x; i++)
    {
      ...
      aged_z.set_varying_params(theta, phi);
      double analytic_result = aged_z.evaluate();
