...
public:
  ...
  analytic_gauss_edge_detector_y_3(double sigma, vnl_double_3 p) : sigma_(sigma), p_(p) 
  {
    ...
    G = (3*y0*sigmatwo_ - std::pow(y0,3)) / sigmasix_;
  ...
  double evaluate()
  {
    ...
    double D = -std::pow(sintheta*sinphi,3) / sigmasix_;
    double E = -3*std::pow(sintheta*sinphi,2)*y0 / sigmasix_;
    double F = 3*sintheta*sinphi*(sigmatwo_-std::pow(y0,2)) / sigmasix_;
