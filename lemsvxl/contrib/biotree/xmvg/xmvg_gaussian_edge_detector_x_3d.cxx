// This is /contrib/biotree/xmvg/xmvg_gaussian_edge_detector_x_3d.cxx

#include "xmvg_gaussian_edge_detector_x_3d.h"

xmvg_gaussian_edge_detector_x_3d::xmvg_gaussian_edge_detector_x_3d()
{
}

xmvg_gaussian_edge_detector_x_3d::xmvg_gaussian_edge_detector_x_3d(xmvg_gaussian_edge_detector_descriptor & descriptor)
{
...
class analytic_gauss_edge_detector_x_1
{
public:
  //: constructor with the fixed parameters
  analytic_gauss_edge_detector_x_1(double sigma, vnl_double_3 p) : sigma_(sigma), p_(p) 
  {
  ...
  double evaluate()
  {
    ...
    double D = -sintheta * cos(phi_) / sigmasq_;

    double expo = vcl_exp((vcl_pow(B,2.0) - C) / (2*sigmasq_));
    double val = common_coeff_ * expo * (B*D - E);
    return val;
  }
protected:
  // fixed parameters
...
class analytic_gauss_edge_detector_x_3
{
public:
  //: constructor with the fixed parameters
  analytic_gauss_edge_detector_x_3(double sigma, vnl_double_3 p) : sigma_(sigma), p_(p) 
  {
    x0 = p_.get(0);
    y0 = p_.get(1);
    z0 = p_.get(2);
    ...
    G = (3*x0*sigmatwo_ - vcl_pow(x0,3)) / sigmasix_;
  ...
  double evaluate()
  {
    ...
    double D = -vcl_pow(sintheta*cosphi,3) / sigmasix_;
    double E = -3*vcl_pow(sintheta*cosphi,2)*x0 / sigmasix_;
    double F = 3*sintheta*cosphi*(sigmatwo_-vcl_pow(x0,2)) / sigmasix_;
...
xmvg_atomic_filter_2d<double> xmvg_gaussian_edge_detector_x_3d::splat(vgl_point_3d<double> centre, xmvg_perspective_camera<double> cam)
{
  // locals
...
//  line = cam_trans.backproject(vgl_homg_point_2d<double>(double(0), double(0), 1.0));
  ...
  analytic_gauss_edge_detector_x_1 aged_x(descriptor_.sigma(), vnl_double_3(ray_start.x(), ray_start.y(), ray_start.z()));
  ...
  for(int j=min_y; j<=max_y; j++)
  {
    for(int i=min_x; i<=max_x; i++)
    {
      ...
      aged_x.set_varying_params(theta, phi);
      double analytic_result = aged_x.evaluate();
...
{
  xmvg_gaussian_edge_detector_descriptor edd = detector.descriptor();
  x_write(os, static_cast<xmvg_filter_descriptor&>(edd));
}
