...
public:
  ...
  analytic_gauss_edge_detector_x_3(double sigma, vnl_double_3 p) : sigma_(sigma), p_(p) 
  {
    ...
    G = (3*x0*sigmatwo_ - std::pow(x0,3)) / sigmasix_;
  ...
  double evaluate()
  {
    ...
    double D = -std::pow(sintheta*cosphi,3) / sigmasix_;
    double E = -3*std::pow(sintheta*cosphi,2)*x0 / sigmasix_;
    double F = 3*sintheta*cosphi*(sigmatwo_-std::pow(x0,2)) / sigmasix_;
