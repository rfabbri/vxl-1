...
static void test_xmvg_no_noise_filter()
{
  ...
  TEST_NEAR("box dimension test w", nnfd.box().width(), std::sqrt(2.0)*2.0, 1e-06);
  TEST_NEAR("box dimension test h", nnfd.box().height(), std::sqrt(2.0)*2.0, 1e-06);
  ...
  std::cout <<  "All below tests are along rays in the xz-plane" << std::endl;
  ...
  TEST_NEAR("line integral test-2",
    nnf.line_integral(vgl_point_3d<double>(-2.0, 0.0, 0.0), vgl_vector_3d<double>(4.0, 0.0, 2.0), nnfd.inner_radius()), std::sqrt(5.0), 1e-06);
  TEST_NEAR("line integral test-3",
    nnf.line_integral(vgl_point_3d<double>(-2.0, 0.0, 0.0), vgl_vector_3d<double>(100.0, 0.0, -100.0), nnfd.inner_radius()), std::sqrt(2.0), 1e-06);
  ...
  TEST_NEAR("line integral test-7",
    nnf.line_integral(vgl_point_3d<double>(-1.0, 0.0, -3.0), vgl_vector_3d<double>(1.0, 0.0, 4.0), nnfd.inner_radius()), std::sqrt(17.0), 1e-06);
  TEST_NEAR("line integral test-8",
    nnf.line_integral(vgl_point_3d<double>(-1.0, 0.0, -3.0), vgl_vector_3d<double>(1.0, 0.0, 2.0), nnfd.inner_radius()), std::sqrt(11.25), 1e-06);
  TEST_NEAR("line integral test-9",
    nnf.line_integral(vgl_point_3d<double>(-1.0, 0.0, -3.0), vgl_vector_3d<double>(2.0, 0.0, 2.0), nnfd.inner_radius()), std::sqrt(2.0), 1e-06);
  ...
  TEST_NEAR("splat result center value test", res.weights().get(10,14), (4 - 2 * std::sqrt(2.0)) * f_radius, 1e-09);
...
//        std::fprintf(fp, "%d %d %d ", 0, 0, int(-5000*res.weights().get(i,j)));
//      else
//        std::fprintf(fp, "%d %d %d ", int(5000*res.weights().get(i,j)), 0, 0);
//    }
//    std::fprintf(fp, "\n");
//  } 
//  std::fclose(fp);
