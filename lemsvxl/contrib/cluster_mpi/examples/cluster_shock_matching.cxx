  ...
  try
    {
      ...
      std::string database_list, images_dir, esfs_dir, cons_dir, out_file, output_dir, output_dir_shgm, resolution, addition;
      ...
      for (int i = 1; i < argc; i++) {
        ...
        else if (arg == std::string ("-esf")) { esfs_dir = std::string(argv[++i]);}
        else if (arg == std::string ("-img")) { images_dir = std::string(argv[++i]);}
        else if (arg == std::string ("-con")) { cons_dir = std::string(argv[++i]);}
        else if (arg == std::string ("-db")) { database_list = std::string(argv[++i]);}
        else if (arg == std::string ("-outf")) { out_file = std::string(argv[++i]);}
        else if (arg == std::string ("-outd")) { output_dir = std::string(argv[++i]); save_output_img = true; }
        else if (arg == std::string ("-outshgm")) { output_dir_shgm = std::string(argv[++i]); save_shgm = true; }
        else if (arg == std::string ("-res")) { resolution = std::string(argv[++i]);}
        else if (arg == std::string ("-dbstring")) { addition = std::string(argv[++i]);}
        else if (arg == std::string ("-ds")) { sampling_ds = double(atof(argv[++i]));}
        else if (arg == std::string ("-elastic")) { elastic_splice = true;}
        else if (arg == std::string ("-normalize")) { normalize_cost = true;}
        else if (arg == std::string ("-prune")) { pruning_threshold = double(atof(argv[++i]));}
        else
        {
          std::cout << "Usage: " << argv[0] << "[-help] [-v {0|1|2}] [-esf esf directory] [-img image directory] [-con con directory] " << std::endl;
          std::cout << "[-outf out_file] [-outd directory for output images] [-outshgm directory for output shgm files (not saving if this argument does not exist)] " << std::endl;
          std::cout << "[-prune shock pruning before matching, default = -1.0 (no pruning)] [-ds shock curve sampling ds, default = 1.0] [-elastic use elastic splice cost]" << std::endl;
          std::cout << "[-normalize create the matrix with normalized costs] [-elastic use elastic splice cost] " << std::endl;
          std::cout << "[-res resolution (064)] [-dbstring additional db string to be added to end of each name] " << std::endl;
          throw -1;
        }
      }
      ...
      std::cout << "output_dir_shgm: " << output_dir_shgm << std::endl;
      ...
      std::vector<std::vector<double> > & matrix = match.get_dist_matrix();
