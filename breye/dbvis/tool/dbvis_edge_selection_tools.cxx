#include <dbvis/tool/dbvis_edge_selection_tools.h>
#include <bvis/bvis_manager.h>
#include <vgui/vgui_style.h>

dbvis_edge_selection_tool::dbvis_edge_selection_tool()
{
...
bool
dbvis_edge_selection_tool::set_tableau( const vgui_tableau_sptr& tableau )
{
...
bool
dbvis_edge_selection_tool::set_storage ( const bpro_storage_sptr& storage)
{
  if (!storage.ptr())
    return false;
...
dbvis_edge_selection_tool::tableau()
{
  return tableau_;
}

vidpro_vsol2D_storage_sptr
dbvis_edge_selection_tool::storage()
{
...
bool
dbvis_edge_selection_tool::handle( const vgui_event & e, 
                                   const bvis_view_tableau_sptr& view )
{
  ...
  if ( gesture_prune_(e) ) {
    ...
    //storage_ = bvis_manager::instance()->storage_from_tableau(tableau_);
    //this->set_storage(bvis_manager::instance()->storage_from_tableau(tableau_));
...
vcl_string 
dbvis_edge_selection_tool::name() const
{
