#include <dbvis/tool/dbvis_polyg_select_img_tool.h>
...
dbvis_polyg_select_img_tool::dbvis_polyg_select_img_tool()
...
bool
dbvis_polyg_select_img_tool::set_tableau( const vgui_tableau_sptr& tableau )
{
...
bool
dbvis_polyg_select_img_tool::set_storage ( const bpro_storage_sptr& storage)
{
  if (!storage.ptr())
    return false;
...
dbvis_polyg_select_img_tool::tableau()
{
  return tableau_;
}

vidpro_image_storage_sptr
dbvis_polyg_select_img_tool::storage()
{
  vidpro_image_storage_sptr vsol_storage;
  vsol_storage.vertical_cast(storage_);
  return vsol_storage;
}

bool
dbvis_polyg_select_img_tool::handle( const vgui_event & e, 
                                   ...
                                   )
{
  ...
  if (active_&gesture_close_polyg_(e))
  {
      ...
      vidpro_image_storage_sptr stor;
      ...
      stor.vertical_cast ( bvis_manager::instance()->storage_from_tableau( tableau_) );
     ...
     //Creating a mask for background and setting the edgemap pixels in bg to 0
      ...
      // image_tab = vgui_image_tableau_new(origedgemap);   
          ...
          vidpro_image_storage_sptr new_img_stor = vidpro_image_storage_new();
          vidpro_image_storage_sptr new_mask_stor = vidpro_image_storage_new();
          ...
          bvis_manager::instance()->repository()->store_data(new_img_stor);
          bvis_manager::instance()->add_to_display(new_img_stor);
          bvis_manager::instance()->make_tableau(new_img_stor);
          }


          if (form_mask)
          {
          bvis_manager::instance()->repository()->store_data(new_mask_stor);
             bvis_manager::instance()->add_to_display(new_mask_stor);
          bvis_manager::instance()->make_tableau(new_mask_stor);
          }
...
vcl_string 
dbvis_polyg_select_img_tool::name() const
{
  return "Image ROI Selection";
}

void dbvis_polyg_select_img_tool::activate()
{
    active_ = true;
}
void dbvis_polyg_select_img_tool::deactivate()
{
    drawnvert.clear();
    active_ = false;
...
void 
dbvis_polyg_select_img_tool::get_popup( const vgui_popup_params& params, 
                                                                         vgui_menu &menu )
{
  vcl_string on = "[x] ", off = "[ ] ";
  menu.add( ((form_mask)?on:off)+"Form Mask", 
                        bvis_tool_toggle, (void*)(&form_mask) );
   menu.add( ((cleaning_curves)?on:off)+"Prune Curves", 
                        bvis_tool_toggle, (void*)(&cleaning_curves) );
