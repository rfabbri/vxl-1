// This is file seg/dbsks/dbsks_ccm_subpix.cxx

//:
// \file

#include "dbsks_ccm_subpix.h"
        ...
        bool precompute_contour_orient_cost)
{
  ...
  if (method = dbsks_ccm_subpix::USE_CLOSEST_ORIENTED_EDGE)
  {
...
float dbsks_ccm_subpix::
f(const vcl_vector<int >& xs, const vcl_vector<int >& ys, const vcl_vector<int >& orient_bins)
{
...
{
  double search_width = this->search_window_width()/2;

  int radius = (int) (this->distance_threshold_ + this->distance_tol_near_zero_);
  //double search_height = radius;
      ...
      else
      {
        ...
        for (int p = 0; p < norient; ++p)
        {
          ...
          for (unsigned i_edge = 0; i_edge < neighbor_edgels.size(); ++i_edge)
          {
            ...
            if (vnl_math_abs(local_dx) > search_width)
            {
              continue;
            }
