...
/*	// sample griding of phi centered at the phi from prev detected graph
    ...
    params.min_r = xv->radius() - params.step_r * vnl_math::floor(double(params.num_r-1)/4);
    //params.num_r = 11;
    //params.min_r = xv->radius() -(params.num_r-1) * params.step_r /2;
...
*/
    ...
    params.step_r = vnl_math::max(0.06 * (xv->radius()), double(1)); 
    params.num_r = vnl_math::max(vnl_math::floor((xv->radius()*0.36)/params.step_r)+1,3);
    params.min_r = (xgraph_vertices_max_r_[i]+xgraph_vertices_min_r_[i])/2 - params.step_r * vnl_math::floor(double(params.num_r-1)/3);
/*
