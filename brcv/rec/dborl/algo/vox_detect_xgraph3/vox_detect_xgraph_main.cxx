...
int main(int argc, char *argv[]) 
{
  ...
  // Path to folder where we saved prev frame detection results
  int prev_ind = atoi(params->input_object_name_().c_str());
  prev_ind--;
  int ind_size = params->input_object_name_().size();
  ...
  vcl_string prev_folder = params->get_output_folder() + "/" + params->get_xgraph_prototype_name() + "+" + prev_ind_s;
  vcl_cout << "prev_folder: "<< prev_folder << vcl_endl;
  ...
  {  
    ...
    //> Set up params
    dbsks_detect_xgraph_using_edgemap m;
	...
	m.prev_image_file = params->get_prev_image_file();
	//prev image file
	...
	m.input_cemv_extension	   = params->input_cemv_extension_();
    ...
    m.xgraph_appearance_file   = params->get_appearance_prototype_name();
