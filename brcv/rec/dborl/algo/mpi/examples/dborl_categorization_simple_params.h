//:
// \file
// \brief The parameter class for dborl_categorization_simple inherits from dborl_algo_params
//        handles all the io related to input.xml, status.xml, perf.xml, params.xml thanks to inheritance
//        all other parameters needed for dborl_categorization_simple is defined here and added to param_list_ of base class in the constructor
//        
//
...
#if !defined(_dborl_categorization_simple_params_h)
#define _dborl_categorization_simple_params_h
...
class dborl_categorization_simple_params : public dborl_algo_params
{
public:
  ...
  dborl_categorization_simple_params(vcl_string algo_name) : dborl_algo_params(algo_name) { 
    ...
    input_object_path.set_values(param_list_, "io_data", "input_object_path", "the path to the input object, assumes esf is saved here", "", "", 1, dborl_parameter_system_info::INPUT_OBJECT_DIR);
    input_object_name.set_values(param_list_, "io_data", "input_object_name", "the string identifier of the input object", "", "", 1, dborl_parameter_system_info::INPUT_OBJECT_STRING_ID);
    // user will first select the dataset since its system_info_index is 0, then will select the query as the second input to the algorithm since input_object_path and input_object_name has system_info_indices 1
...
#endif // _dborl_categorization_simple_params_h)
