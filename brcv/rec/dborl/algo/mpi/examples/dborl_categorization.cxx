...
bool dborl_categorization::parse_command_line(vcl_vector<vcl_string>& argv)
{
...
bool dborl_categorization::parse_index(vcl_string index_file)
{
  ...
  if (xmlFile == NULL){
    vcl_cout << "dborl_categorization::parse_index() -- " << index_file << "-- error on opening" << vcl_endl;
    return false;
  }
...
bool dborl_categorization::parse(const char* param_file)
{
  ...
  vcl_string input_file = params_->input_object_path() + "/" + params_->input_object_name() + params_->input_extension();
  if (!vul_file::exists(input_file)) {
    vcl_cout << "dborl_categorization::parse() -- cannot find the input file: " << input_file << "\n";
    return false;
  }
  ...
  if (params_->use_shock_matching()) {  // load the .esf
    ...
    if (!sg) {
      vcl_cout << "dborl_categorization::parse() -- Problems in loading shock graph: "<< input_file << vcl_endl;
      return false;
    } 
  ...
  if (!vul_file::exists(obj_groundtruth)) {
    vcl_cout << "dborl_categorization::parse() -- cannot find ground truth description for input object: " << obj_groundtruth << vcl_endl;
    return false;
  }
...
bool dborl_categorization::initialize(vcl_vector<vcl_string>& t)
{
  if (!current_node_)
    return false;
...
bool dborl_categorization::process(vcl_string t1, float& f)
{
  if (!vul_file::exists(t1)) {
    vcl_cout << "In dborl_categorization::process() -- the input file: " << t1 << " is not found!\n";
    f = -1;
    return false;
  }
  ...
  if (params_->use_shock_matching()) {
    ...
    if (!sg) {
      vcl_cout << "dborl_categorization::process() -- Problems in loading shock graph: "<< t1 << vcl_endl;
      return false;
    } 
    ...
    if (!vul_file::exists(out_name1)) {
      ...
      if (!edit.edit()) {
        vcl_cout << "dborl_categorization::process() -- Problems in editing trees for: " << out_name1 << "\n";
        return false;
      }
      f = edit.final_cost();
...
bool dborl_categorization::finalize(vcl_vector<float>& results)
{
  ...
  if (!vul_file::exists(obj_groundtruth)) {
    vcl_cout << "dborl_categorization::finalize() -- cannot find ground truth description: " << obj_groundtruth << vcl_endl;
    return false;
  }
  ...
  vcl_string output_file = params_->output_directory() + "/categorization.xml";
  vcl_ofstream of(output_file.c_str());
  if (!of) {
    vcl_cout << "dborl_categorization::finalize() - not able to write output to: " << output_file << vcl_endl;
  } else {
    of << "<type name = \"image\">\n";
    of << "\t<description>\n";
    of << "\t\t<instance>\n";
  ...
  vcl_vector<dborl_index_node_base_sptr> children;
  ...
  vcl_string nearest_object_name = current_node_->names()[min_i];
  current_node_ = 0;
