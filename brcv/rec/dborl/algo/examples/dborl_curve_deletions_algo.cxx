...
bool dborl_curve_deletions_algo::parse_index(vcl_string index_file)
{
...
bool dborl_curve_deletions_algo::initialize()
{
  ...
  if (root_->names().size() != root_->paths().size()) {
    vcl_cout << "dborl_curve_deletions_algo::initialize() -- names and paths size not equal in index: " << params_->db_index_() << vcl_endl;
    return false;
  }
...
bool dborl_curve_deletions_algo::process()
{
  ...
  for (unsigned i = 0; i < D; i++) {
    ...
    if (!vul_file::exists(con_file)) {
      vcl_cout << "dborl_curve_deletions_algo::process() -- cannot find: " << con_file << vcl_endl;
      return false;
    }
    ...
    if (params_->delete_segments_()) {
      ...
      if (params_->add_segments_()) { // both delete and add
        ...
        while (!dbsol_curve_algs::segment_addition_and_deletion(poly, vsol_list, del_pieces, 
          ...
          counter++;
          if (counter > 100) {
            vcl_cout << "dborl_curve_deletions_algo::process() -- cannot randomize segment deletion! in: " << con_file << vcl_endl;
            return false;
          }
        }
      ...
      if (params_->add_segments_()) {  // just add
               ...
               !vsol_list.size()) 
        {
            ...
            counter++;
            if (counter > 100) {
              vcl_cout << "dborl_curve_deletions_algo::process() -- cannot randomize segment addition! in: " << con_file << vcl_endl;
              return false;
            }
        }
