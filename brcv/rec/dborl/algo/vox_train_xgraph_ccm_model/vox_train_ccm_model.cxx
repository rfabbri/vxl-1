// This is file /lemsvxl/brcv/rec/dborl/algo/vox_train_xgraph_geometry_and_ccm_models/vox_train_xgraph_ccm_model/vox_train_ccm_model.cxx
...
{
    vox_train_ccm_model_params p;

    // CCM parameters
    p.nbins_0topi                = 18;
    p.distance_threshold         = 4;
    p.distance_tol_near_zero     = 2;
...
set_training_data_info(const vox_train_ccm_model_params& p)
{
    ...
    pos_image_fnames = dborl_get_object_full_paths(this->params.index_file_name, index_query, this->params.image_ext);
...
build_biarc_sampler(dbsks_biarc_sampler& biarc_sampler)
{
    ...
    // max dx = 32
    bsp.step_dx = this->params.base_xgraph_size / 32; // heuristic
    bsp.num_dx = 33;
    bsp.min_dx = -bsp.step_dx * (bsp.num_dx-1)/2;

    // max dy = 32
    bsp.step_dy = this->params.base_xgraph_size / 32; // heuristic
    bsp.num_dy = 33;
    ...
    biarc_sampler.set_sampling_params(2*this->params.nbins_0topi, 1.5, 51);
        ...
        vcl_vector<vgl_box_2d<int > >& list_edgemap_bboxes)
{
    ...
    dborl_index_query index_query;
    index_query.add_object_name(vul_file::strip_directory(image_name));
