...
bool sort_by_size( const vcl_vector< dbdet_keypoint_match > & lhs,
                   const vcl_vector< dbdet_keypoint_match > & rhs )
{
  return lhs.size() > rhs.size();
}

//: Fill \param clusters with the clusters sorted by cluster size
vcl_vector< vcl_vector< dbdet_keypoint_match > >
dbkpr_lowe_clusterer::get_sorted_clusters() const
{
  vcl_vector< vcl_vector< dbdet_keypoint_match > > clusters(hash_);
...
bool
dbkpr_lowe_clusterer::estimate_all_affine( vcl_vector< bapl_affine_transform >& transforms, 
                                          vcl_vector< dbdet_keypoint_match >& inliers ) const
{
  vcl_vector< vcl_vector< dbdet_keypoint_match > > clusters = this->get_sorted_clusters();
  ...
  for(unsigned c=0; c<clusters.size() && clusters[c].size()>2; ++c){
    ...
    vcl_vector< dbdet_keypoint_match >& matches = clusters[c];
    ...
    matches.clear();
    for( vcl_vector< dbdet_keypoint_match >::const_iterator itr = all_matches_.begin();
    ...
    for( vcl_vector< dbdet_keypoint_match >::iterator m_itr = matches.begin();
         m_itr != matches.end();  ++m_itr ){
      bool already_found = false;
      for( vcl_vector< dbdet_keypoint_match >::iterator i_itr = inliers.begin();
                                      ...
                                      vcl_vector< dbdet_keypoint_match >& matches ) const
{
  ...
  while( num_inliers!=matches.size() ){
    ...
    if( !est.weighted_least_squares_fit(p,covar) ){
      ...
      for( vcl_vector< dbdet_keypoint_match >::iterator itr = matches.begin();
           itr != matches.end();  ++itr ){
