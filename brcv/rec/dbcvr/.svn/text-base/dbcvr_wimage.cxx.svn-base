...
dbcvr_image::dbcvr_image (int w, int h)
{
   width = w;
   height = h;
   imagesize = w*h;
...
dbcvr_image::dbcvr_image (int w, int h, void* imd)
{
   width = w;
   height = h;
   imagesize = w*h;
...
dbcvr_image::dbcvr_image (int w, int h, void* imd, bool bcopy)
{
   width = w;
   height = h;
   imagesize = w*h;
...
dbcvr_image::dbcvr_image (int w, int h, void* imd, int sx, int sy)
{
   width = w;
   height = h;
   imagesize = w*h;
...
void dbcvr_image::CopyImage (dbcvr_image* im)
{
  memcpy (imagedata, im->imagedata, im->imagesize);
}

// draw horizontal or vertical line on image im pixel by pixel...
void imHVLine (dbcvr_image* im, int x1, int y1, int x2, int y2, int color)
{
   int i;
   int w = im->width;
   unsigned char *s;
