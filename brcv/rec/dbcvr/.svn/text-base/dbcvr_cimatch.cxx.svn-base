...
void dbcvr_cimatch::allocateDPMemory (void)
{
  int j, y;
...
void dbcvr_cimatch::freeDPMemory (void)
{
  int j, y;
...
double dbcvr_cimatch::computeCost (int j1, int j2, int i1x, int i1y, int i2x, int i2y, int prevTangent)
{
  //1)CurveCost
  //1-A)StretchCost
  double dStretchCost;
...
double dbcvr_cimatch::computeImageCost (int posx, int posy, int dir)
{
...
void dbcvr_cimatch::computeCannyMagnitudeDir (void)
{
/*  int x, y;
...
double dbcvr_cimatch::computeImageCostCanny (int posx, int posy, int dir)
{
  //pos is relative
...
/*
//0: perfect edge, 1: no edge, normalized!
double dbcvr_cimatch::computeImageCost1Canny (int posx, int posy, int dir)
{
  double imageCost;
...
dbcvr_cimatch::~dbcvr_cimatch (void)
{
  freeDPMemory ();
  if (nmsbuf)
    delete nmsbuf;
...
void ResampleCurve (RESAMPLE_CURVE_TYPE resample_type, bsol_intrinsic_curve_2d* inputCurve, bsol_intrinsic_curve_2d* outputCurve)
{
  ...
  if (resample_type==RESAMPLE_CURVE_UNIT) {
    ...
    int intx = (int) outputCurve->x(0);
    int inty = (int) outputCurve->y(0);
    outputCurve->modify_vertex (0, intx, inty);
    intx = (int) outputCurve->x(1);
    inty = (int) outputCurve->y(1);
