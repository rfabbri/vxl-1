//---------------------------------------------------------------------
// This is brcv/rec/dbcvr/dbcvr_clsd_cvmatch.h
//:
// \file
// \brief closed curve matching adapted from original source code for Windows in
//        /vision/projects/kimia/curve-matching/CurveMatch/CODE/CODE-IRIX6.5/Matching-Tek/c/
//
//
// \author
...
#ifndef _dbcvr_clsd_cvmatch_h
#define _dbcvr_clsd_cvmatch_h
...
class dbcvr_clsd_cvmatch : public vbl_ref_count
{
...
public:
  dbcvr_clsd_cvmatch();
  dbcvr_clsd_cvmatch(const bsol_intrinsic_curve_2d_sptr c1, 
                     const bsol_intrinsic_curve_2d_sptr c2, 
                     double R, 
                     int template_size=3);

  dbcvr_clsd_cvmatch(const vsol_polygon_2d_sptr p1, 
                     const vsol_polygon_2d_sptr p2, 
                     double R, 
                     double rms = 0.0f,  // fit lines to the input polygon before using
                     int template_size = 3);

  virtual ~dbcvr_clsd_cvmatch(){};
  ...
  dbcvr_cv_cor_sptr get_cv_cor(int minIndex);
