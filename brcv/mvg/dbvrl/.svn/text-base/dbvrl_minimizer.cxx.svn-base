// This is algo/dbvrl/dbvrl_minimizer.cxx
//:
// \file

#include "dbvrl_minimizer.h"
...
//: Constructor
dbvrl_minimizer::dbvrl_minimizer( const vimt_image_2d_of<float>& image1,
...
dbvrl_minimizer::dbvrl_minimizer( const vimt_image_2d_of<float>& image1,
                                  ...
                                  const dbvrl_world_roi& roi, bool image1_mask ) 
...
//: The main function.
void
dbvrl_minimizer::minimize(vimt_transform_2d& xform)
{
  ...
  for(int L=from_pyramid_.hi(); L>=from_pyramid_.lo(); --L){
    ...
    dbvrl_lsqr_cost_func *cost;
    // no masks
    if(!from_mask_ && !to_mask_)
    {
      cost = new dbvrl_lsqr_cost_func(im1, im2, roi_pyramid_[L], xform);
    }
    // one mask
      ...
      if (from_mask_) {
        ...
        cost = new dbvrl_lsqr_cost_func( im1, im2, immask, roi_pyramid_[L], xform, true);
      } else {
        const vimt_image_2d_of<float>& image_mask
          = static_cast<const vimt_image_2d_of<float>& >(to_mask_pyramid_(L));    
        vimt_image_2d_of<float> immask(image_mask);
        immask.set_world2im(undo_xform*image_mask.world2im());
        cost = new dbvrl_lsqr_cost_func( im1, im2, immask, roi_pyramid_[L], xform, false);
      }
    }
    // both masks
    else
    {
      ...
      cost = new dbvrl_lsqr_cost_func( im1, im2, f_immask, t_immask, roi_pyramid_[L], xform);
    }
