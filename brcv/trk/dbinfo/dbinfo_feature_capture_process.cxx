...
#include <dbinfo/dbinfo_io.h>
#include <dbinfo/dbinfo_tracking_face_2d.h>
dbinfo_feature_capture_process::dbinfo_feature_capture_process(dbinfo_info_tracker_params & tp)
  : tracker_(tp)
{
  failure_ = false;
  first_frame_ = true;
...
dbinfo_feature_capture_process::~dbinfo_feature_capture_process()
{
}

bool dbinfo_feature_capture_process::execute()
{
  if (failure_)
    return false;
...
bool dbinfo_feature_capture_process::finish()
{
  first_frame_ = true;
  face_index_ = 0;
  failure_ = false;
  ...
  dbinfo_tracking_face_2d_sptr tf = tracker_.capture_tf();
  if(!tf)
    return false;
