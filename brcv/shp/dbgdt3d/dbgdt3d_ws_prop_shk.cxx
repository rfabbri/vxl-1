                                      ...
                                      double& tauL, double& tauR, double& simtime)
{
  ...
  if (success) { //If success, set simtime.
    ...
    #if GDT_DEBUG_MSG
    if (n_verbose_>2)
      vul_printf (vcl_cout, " Possible intersect. Sl %d Sl %d at %f (WC %d), tauL %.3f, tauR %.3f.\n",
...
void gdt_ws_manager::update_S_to_prjE (gdt_shock* S) 
{
  ...
  if (S->prjE()) {
    #if GDT_DEBUG_MSG
    if (n_verbose_>3)
      vul_printf (vcl_cout, " Update S %d (v %d) to prjE %d tauE %.3f, endtime %f.\n", 
    ...
    #if GDT_DEBUG_MSG
    if (n_verbose_>3)
      vul_printf (vcl_cout, " Update S %d (v %d) to infinity (NULL prjE).\n", 
                   S->id(), S->Snode()->id());
    #endif
    assert (S->edgeT() == GDT_HUGE);
    S->set_simT (GDT_HUGE);
                                                   ...
                                                   double& simtime)
{
  ...
  if (n_verbose_>2) {
    ...
    vul_printf (vcl_cout, "    tauL %.3f, tauR %.3f, simtime %f.\n", 
                 tauL, tauR, simtime);
    ...
    vul_printf (vcl_cout, "  leftW %s, midI %d (%.3f, %.3f), rightW %s.\n",
                lstring,
