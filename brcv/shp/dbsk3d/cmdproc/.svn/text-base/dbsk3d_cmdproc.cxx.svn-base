...
bool dbsk3d_check_cmdproc (dbsk3d_pro_vis* pv, int argc, char **argv)
{
  ...
  if (!dbsk3d_cmd_fs_file() && !dbsk3d_cmd_cms_file() && !dbsk3d_cmd_cms_file2() &&
      ...
      dbsk3d_cmd_all()==0 && dbsk3d_cmd_allg()==0 &&
  ...
  if (dbsk3d_cmd_all()!=0 || dbsk3d_cmd_allg()!=0 ||
      dbsk3d_cmd_ms()!=0 || dbsk3d_cmd_seg()!=0 || dbsk3d_cmd_ifgp()!=0 ||
      dbsk3d_cmd_reg()!=0 || dbsk3d_cmd_isc()!=0 || dbsk3d_cmd_ms()!=0 ||
      dbsk3d_cmd_trans()!=0 || dbsk3d_cmd_smrib()!=0 || dbsk3d_cmd_vlink()!=0 ||
      dbsk3d_cmd_transg()!=0 || dbsk3d_cmd_sgsm()!=0) {
    if (!dbmsh3d_cmd_fileprefix()) {
      vul_printf (vcl_cout, "ERROR:\tMissing -f PREFIX. Use -h, -hh 1,2,3, for more help.\n");
      return false;
    }
  }
...
SoSeparator* dbsk3d_cmdproc_execute (dbsk3d_pro_vis* pv)
{
      ...
      if (dbmsh3d_cmd_gui()) {        
        ...
        if (dbmsh3d_cmd_v()==0) { //-v 0: Draw the valid fs_mesh shock sheets
          ...
          if (dbmsh3d_cmd_v()==2) { //-v 2: show shock links in Queue 1          
            ...
            vul_printf (vcl_cout, "\t Queue 1 shape cost median = %f, 10%% = %f, 90%% = %f\n", 
                         pv->fs_segre()->get_Q1_median_cost(), 
                         pv->fs_segre()->get_Q1_percent_cost (0.1f),
                         pv->fs_segre()->get_Q1_percent_cost (0.9f));

            vul_printf (vcl_cout, "Visualizing %.0f to %.0f percent of A13-2-links in Queue 1.\n", 
            ...
            vul_printf (vcl_cout, "\t Queue 2 shape cost median = %f, 10%% = %f, 90%% = %f\n", 
                         pv->fs_segre()->get_Q2_percent_cost (0.5f), 
                         pv->fs_segre()->get_Q2_percent_cost (0.1f),
                         pv->fs_segre()->get_Q2_percent_cost (0.9f));

            vul_printf (vcl_cout, "Visualizing %.0f to %.0f percent of A13-links in Queue 2.\n", 
                         dbmsh3d_cmd_percent(), dbmsh3d_cmd_percent2());
          ...
          //-rmax: rmax-pruning (skip if rmax==-1)
          if (dbsk3d_cmd_rmax() != -1.0f)
            pv->run_rmax_trim_xforms (dbsk3d_cmd_rmax(), dbsk3d_cmd_gsa()!=0);
  ...
  if (pv->b_view_cms_) {
    ...
    //print statistics of MS node types.
    ///pv->ms_hypg()->print_MS_info ();
    ...
    if (dbmsh3d_cmd_gui()) {
      ...
      //-vsel 1: visualize selected curve length > threshold.
      //-n to specify the min_curve number.
      if (dbsk3d_cmd_vsel()==1) {
        const int ms_curve_min_elm = 10;
        int num_ms_curves = 50;
        if (dbmsh3d_cmd_n() != -1)
          num_ms_curves = dbmsh3d_cmd_n();
        pv->ms_hypg()->select_salient_ms_curves (ms_curve_min_elm, num_ms_curves);
        dbmsh3d_cmd_v() = 10;
      }

      //-v 0: draw the medial scaffold graph and randomly color each sheet but avoiding color close to red or blue.
      //   1: draw the medial scaffold graph and draw the whole ms_sheets in a single color (m_face_ccode_).
      ...
      //   8: distinguish A13-2 saddle and A13-3 relay curves.
      //   9: not drawing anything.
      //-v 10: draw only selected curves and nodes and valid corners.
      ...
      //-id 7: show curve id specified by -n .
        ...
        //show id at the middle of the curve.
        const vgl_point_3d<double> pt = MC->get_middle_V()->pt();
        _root->addChild (draw_text2d (id_buf, (float)pt.x(), (float)pt.y(), (float)pt.z(), SbColor (0,0,0)));
      }
      //-id 8: show vertex id specified by -n .
      else if (dbmsh3d_cmd_id()==8) {      
        int id = dbmsh3d_cmd_n();
        assert (id >= 0);
  ...
  if (dbsk3d_cmd_bsphr()==1 || dbsk3d_cmd_spd()==1 || dbsk3d_cmd_seg()==1 || dbsk3d_cmd_ifgp()==1 || 
      ...
      dbsk3d_cmd_trans()==1 || dbsk3d_cmd_smrib()==1 || dbsk3d_cmd_vlink()==1 ||      
      dbsk3d_cmd_transg()==1 || dbsk3d_cmd_sgsm()==1 || 
      dbsk3d_cmd_all()==1 || dbsk3d_cmd_allg()==1) {
  ...
  //###########################################################################
  //-seg 1-2: Run Surface Segregation.   
  ...
  if (dbsk3d_cmd_seg()!=0 || dbsk3d_cmd_reg()==1 || dbsk3d_cmd_reg()==2 || 
      ...
      dbsk3d_cmd_smrib()==1 || dbsk3d_cmd_smrib()==2 || 
      dbsk3d_cmd_vlink()==1 || dbsk3d_cmd_vlink()==2 ||
      dbsk3d_cmd_transg()==1 || dbsk3d_cmd_transg()==2 || 
      dbsk3d_cmd_sgsm()==1 || dbsk3d_cmd_sgsm()==2 || 
      dbsk3d_cmd_all()==1 || dbsk3d_cmd_all()==2 ||
      dbsk3d_cmd_allg()==1 || dbsk3d_cmd_allg()==2) {    
    if (dbmsh3d_pro_contiune == false) {
  ...
  //###########################################################################
  //-reg 1-3: Shock Regularization (Early Splice Transforms).
  if (dbsk3d_cmd_reg()!=0 || (dbsk3d_cmd_isc()>=1 && dbsk3d_cmd_isc()<=3) || 
      ...
      (dbsk3d_cmd_smrib()>=1 && dbsk3d_cmd_smrib()<=3) || 
      (dbsk3d_cmd_vlink()>=1 && dbsk3d_cmd_vlink()<=3) || 
      (dbsk3d_cmd_transg()>=1 && dbsk3d_cmd_transg()<=3) || 
      (dbsk3d_cmd_sgsm()>=1 && dbsk3d_cmd_sgsm()<=3) || 
      (dbsk3d_cmd_all()>=1 && dbsk3d_cmd_all()<=3) ||
      (dbsk3d_cmd_allg()>=1 && dbsk3d_cmd_allg()<=3)) {
  ...
  //###########################################################################
  //-isc 1-4: Identify Inside Shock Component: Bounding-box pruning and component extraction.
  if (dbsk3d_cmd_isc()!=0 || 
      (dbsk3d_cmd_ms()>=1 && dbsk3d_cmd_ms()<=4) ||     
      (dbsk3d_cmd_trans()>=1 && dbsk3d_cmd_trans()<=4) || 
      (dbsk3d_cmd_smrib()>=1 && dbsk3d_cmd_smrib()<=4) || 
      (dbsk3d_cmd_vlink()>=1 && dbsk3d_cmd_vlink()<=4) || 
      (dbsk3d_cmd_transg()>=1 && dbsk3d_cmd_transg()<=4) || 
      (dbsk3d_cmd_sgsm()>=1 && dbsk3d_cmd_sgsm()<=4) || 
      (dbsk3d_cmd_all()>=1 && dbsk3d_cmd_all()<=4) ||
      (dbsk3d_cmd_allg()>=1 && dbsk3d_cmd_allg()<=4)) {
  ...
  //###########################################################################
  //-ms 1-5: Coarse-scale Medial Scaffold Computation.
  if (dbsk3d_cmd_ms()!=0 || 
      (dbsk3d_cmd_trans()>=1 && dbsk3d_cmd_trans()<=5) || 
      (dbsk3d_cmd_smrib()>=1 && dbsk3d_cmd_smrib()<=5) || 
      (dbsk3d_cmd_vlink()>=1 && dbsk3d_cmd_vlink()<=5) || 
      (dbsk3d_cmd_transg()>=1 && dbsk3d_cmd_transg()<=5) || 
      (dbsk3d_cmd_sgsm()>=1 && dbsk3d_cmd_sgsm()<=5) || 
      (dbsk3d_cmd_all()>=1 && dbsk3d_cmd_all()<=5) ||
      (dbsk3d_cmd_allg()>=1 && dbsk3d_cmd_allg()<=5)) {
    if (dbmsh3d_pro_contiune == false) {
      pv->reset_mesh();
      pv->reset_fs_mesh();      
  ...
  //###########################################################################
  //-trans 1-6: Medial Scaffold Hypergraph Transition Regularization.
  if (dbsk3d_cmd_trans()!=0 ||     
      (dbsk3d_cmd_smrib()>=1 && dbsk3d_cmd_smrib()<=6) || 
      (dbsk3d_cmd_vlink()>=1 && dbsk3d_cmd_vlink()<=6) || 
      (dbsk3d_cmd_transg()>=1 && dbsk3d_cmd_transg()<=6) || 
      (dbsk3d_cmd_sgsm()>=1 && dbsk3d_cmd_sgsm()<=6) || 
      (dbsk3d_cmd_all()>=1 && dbsk3d_cmd_all()<=6) ||
      (dbsk3d_cmd_allg()>=1 && dbsk3d_cmd_allg()<=6)) {
    if (dbmsh3d_pro_contiune == false) {
      pv->reset_mesh();
      pv->reset_fs_mesh();
      if (pv->load_cms () == false) {
        vcl_cout << "\ndbsk3d_cmdproc_execute (ms): Error loading " << pv->dir_prefix() + ".cms (with .fs).\n";
        dbmsh3d_cmd_gui() = 0;
        return _root;
      }
    }
    ...
    //-cc (def 10): Remove shock curve with > # fs_edges.
    //-sc (def 30): Remove shock curve with > # fs_edges.
    //-nnm (def 10): Merge node-node with > # fs_edges.
    //-ncm (def 10): Merge node-curve with > # fs_edges.
    //-ccm (def 10): Merge curve-curve with > # fs_edges.
    //-cmxth (def 15): Curve merge transform validity threshold.
    //-gsa 1: re-assigne lost genes to shocks.
    //-n1 : stop at elm1 of default debug id (for splice, contract xforms).
    //-n2 : stop at elm2 of default debug id (for merge xforms).

    //option specified in -transo:
    //1: half-cost non-merge transforms: result xx-ht1.cms.
    //2: all half-cost transforms: result xx-ht2.cms.
    //3: all regular cost transforms: result xx-ht.cms.
    //4: steps 1 + 2.
    //5: steps 2 + 3.
    //6: steps 1 + 2 + 3.
    //7: all regular cost non-merge transforms: result xx-ht.cms.
    //8: step 1 + 7.

    //Iteration 1: half-cost non-merge transforms: result xx-ht1.cms.
    if (dbsk3d_cmd_transo() == 1 || dbsk3d_cmd_transo() == 4 || dbsk3d_cmd_transo() == 6 || dbsk3d_cmd_transo() == 8) {
      pv->ms_trans_regul (false, dbsk3d_cmd_tab()/2, dbsk3d_cmd_a5()/2, 
                          dbsk3d_cmd_cc()/2, dbsk3d_cmd_sc()/2, 
                          dbsk3d_cmd_nnm()/2, dbsk3d_cmd_ncm()/2, dbsk3d_cmd_ccm()/2,
                          dbsk3d_cmd_ncma1a5()/2, dbsk3d_cmd_cmxth(),
                          dbsk3d_cmd_gsa()!=0, dbmsh3d_cmd_n1(), dbmsh3d_cmd_n2());
      if (dbmsh3d_cmd_ofile()) {
        pv->save_cms (pv->dir_prefix() + "-ht1");
        pv->save_fs (pv->dir_prefix() + "-ht1");
      }
    }

    //Iteration 2: all half-cost transforms: result xx-ht2.cms.
    if (dbsk3d_cmd_transo() == 2 || dbsk3d_cmd_transo() == 4 || dbsk3d_cmd_transo() == 6) {
      pv->ms_trans_regul (true, dbsk3d_cmd_tab()/2, dbsk3d_cmd_a5()/2, 
                          dbsk3d_cmd_cc()/2, dbsk3d_cmd_sc()/2, 
                          dbsk3d_cmd_nnm()/2, dbsk3d_cmd_ncm()/2, dbsk3d_cmd_ccm()/2,
                          dbsk3d_cmd_ncma1a5()/2, dbsk3d_cmd_cmxth(),
                          dbsk3d_cmd_gsa()!=0, dbmsh3d_cmd_n1(), dbmsh3d_cmd_n2());      
      if (dbmsh3d_cmd_ofile()) {
        pv->save_cms (pv->dir_prefix() + "-ht2");
        pv->save_fs (pv->dir_prefix() + "-ht2");
      }
    }

    //Iteration 3: all regular-cost transforms: result xx-ht.cms.
    if (dbsk3d_cmd_transo() == 3 || dbsk3d_cmd_transo() == 5 || dbsk3d_cmd_transo() == 6) {
      pv->ms_trans_regul (true, dbsk3d_cmd_tab(), dbsk3d_cmd_a5(), 
                          dbsk3d_cmd_cc(), dbsk3d_cmd_sc(), 
                          dbsk3d_cmd_nnm(), dbsk3d_cmd_ncm(), dbsk3d_cmd_ccm(),
                          dbsk3d_cmd_ncma1a5(), dbsk3d_cmd_cmxth(),
                          dbsk3d_cmd_gsa()!=0, dbmsh3d_cmd_n1(), dbmsh3d_cmd_n2());    
      if (dbmsh3d_cmd_ofile()) {
        pv->save_cms (pv->dir_prefix() + "-ht");
        pv->save_fs (pv->dir_prefix() + "-ht");
      }
    }
    
    
    //Iteration 7: regular-cost non-merge transforms: result xx-ht1.cms.
    if (dbsk3d_cmd_transo() == 7 || dbsk3d_cmd_transo() == 8) {
      pv->ms_trans_regul (false, dbsk3d_cmd_tab(), dbsk3d_cmd_a5(), 
                          dbsk3d_cmd_cc(), dbsk3d_cmd_sc(), 
                          dbsk3d_cmd_nnm(), dbsk3d_cmd_ncm(), dbsk3d_cmd_ccm(),
                          dbsk3d_cmd_ncma1a5(), dbsk3d_cmd_cmxth(),
                          dbsk3d_cmd_gsa()!=0, dbmsh3d_cmd_n1(), dbmsh3d_cmd_n2());
    ...
    if (dbmsh3d_cmd_gui()) {
      ...
      if (pv->ms_vis_param_.sheets_ctable_.size() == 0)
        ...
        //handle the case of additional ms_sheets (updating the sheets_ctable_)
        if (pv->ms_vis_param_.sheets_ctable_.size() != pv->ms_hypg()->sheet_id_counter())
          generate_shock_color_table (dbmsh3d_cmd_cseed(), pv->ms_hypg()->sheet_id_counter(), 
                                      pv->ms_vis_param_.sheets_ctable_);
  ...
  if (dbsk3d_cmd_transv()!=0) {
    ...
    //     -v 2 only show ms_graph, -v 8: not showing anything.
    //-cseed: color seed (default DBMSH3D_SHEET_COLOR_SEED: 622).
    //-idv 1: draw individual fs_faces
    if (dbsk3d_cmd_transv() == 1) {
      //draw other shock sheets in a single color (yellow).
      if (dbmsh3d_cmd_v() == 0)
        dbmsh3d_cmd_v() = 1; 
      pv->m_face_ccode_ = COLOR_DARKYELLOW;
    }
    ///_root->addChild (pv->vis_ms_hypg(dbmsh3d_cmd_v(), dbmsh3d_cmd_cseed(), dbmsh3d_cmd_idv()!=0));
    ...
    if (dbsk3d_cmd_transv() == 1) {
      ...
      int n_trans = 2;
      if (dbmsh3d_cmd_n() > 0)
        n_trans = dbmsh3d_cmd_n();
      vis = vis_SH_elms_n_SCxforms (pv->ms_hypg(), n_trans, pv->ms_vis_param_.curve_width_,
                                    pv->ms_vis_param_.ball_size_, dbmsh3d_cmd_id()>5);
      _root->addChild (vis);
    }
      ...
      //-n : A5 MC id.
      if (dbmsh3d_cmd_n() == -1) {
        vul_printf (vcl_cout, "error: specify MC id by -n .\n");
      }
      else {
    ...
    else if (dbsk3d_cmd_transv() == 4) {
      //-transv 4: all merge transform candidates.
      //-tab: Remove shock tabs with > # generators.
      //-a5: Remove A5 with > # A5 fs_edges.
      //-cc: Remove shock curve with > # fs_edges.
      //-sc: Remove shock curve with > # fs_edges.
      //-nnm: Merge node-node with > # fs_edges.
      //-ncm: Merge node-curve with > # fs_edges.
      //-ccm: Merge curve-curve with > # fs_edges.
      //-cmxth: Curve merge transform validity threshold.
      vis = vis_all_merge_xforms (pv->ms_hypg_trans(), dbsk3d_cmd_tab(), dbsk3d_cmd_a5(), 
                                  dbsk3d_cmd_cc(), dbsk3d_cmd_sc(), 
                                  dbsk3d_cmd_nnm(), dbsk3d_cmd_ncm(), dbsk3d_cmd_ccm(),
                                  dbsk3d_cmd_ncma1a5(), dbsk3d_cmd_cmxth(),
                                  pv->ms_vis_param_.curve_width_, pv->cube_size_);
      _root->addChild (vis);      
    }

  }
  //###########################################################################
  //-smrib 1-7: Medial Scaffold Rib Curve Smoothing.
  if (dbsk3d_cmd_smrib()!=0 ||
      (dbsk3d_cmd_vlink()>=1 && dbsk3d_cmd_vlink()<=7) || 
      (dbsk3d_cmd_transg()>=1 && dbsk3d_cmd_transg()<=7) || 
      (dbsk3d_cmd_sgsm()>=1 && dbsk3d_cmd_sgsm()<=7) || 
      (dbsk3d_cmd_all()>=1 && dbsk3d_cmd_all()<=7) ||
      (dbsk3d_cmd_allg()>=1 && dbsk3d_cmd_allg()<=7)) {
    if (dbmsh3d_pro_contiune == false) {
      pv->reset_mesh();
      pv->reset_fs_mesh();
      if (pv->load_cms() == false) {
        dbmsh3d_cmd_gui() = 0;
        vcl_cout << "\ndbsk3d_cmdproc_execute (smrib): Error loading " << pv->dir_prefix() + ".\n";
        return _root;
      }
    }
    ...
    if (dbmsh3d_pro_contiune==false && dbmsh3d_cmd_gui()) {
      ...
      if (dbmsh3d_pro_contiune==false) { 
        //View (-2dx, 0): Medial Scaffold Before Transition.
        vis = pv->vis_ms_hypg ();
        _root->addChild (shift_output (vis, float(-pv->view_dx_*2), 0, 0));
        vis = pv->vis_ptset();
        _root->addChild (shift_output (vis, float(-pv->view_dx_*2), 0, 0));
    ...
    //Perform MS rib curve smoothing according to the option specifie.
    // -smribo 0: skip this step.
    // -smribo 1: only smooth the rib curve within each fine-scale face.
    // -smribo 2: DCS smoothing the rib curves.
    // -smribo 3: Gaussian smoothing the rib curves.
    // -smribo 4: Step 1 + 2.
    // -smribo 5: Step 1 + 2 + 3.

    //-smi : # of smoothing iterations (def. 10).
    int n_step = dbmsh3d_cmd_smi();
    if (n_step < 0)
      n_step = 10;
    ...
    //-n: steps for Gaussian smoothing (def 3)
    int gaussian_iter = dbmsh3d_cmd_n();
    if (gaussian_iter < 0)
      gaussian_iter = 3;
    //-gsr: G_sigma_ratio
    float sigma = dbmsh3d_cmd_gsr();
    //-grr: G_radius_ratio    

    if (dbsk3d_cmd_smribo()==1 || dbsk3d_cmd_smribo()==4 || dbsk3d_cmd_smribo()==5) {
      //Smooth the rib curve of each fine-scale face.
      pv->smooth_ms_hypg_ribs_within_face (DCS_psi, n_step, rib_ratio);
    }
    if (dbsk3d_cmd_smribo()==2 || dbsk3d_cmd_smribo()==4 || dbsk3d_cmd_smribo()==5) {
      //DCS smooth the rib curve.
      pv->smooth_ms_hypg_ribs_DCS (DCS_psi, n_step, rib_ratio);
    }
    if (dbsk3d_cmd_smribo()==3 || dbsk3d_cmd_smribo()==5) {
      //Gaussian smooth the rib curve.
      pv->smooth_ms_hypg_ribs_gaussian (gaussian_iter, sigma);
    }
    ...
    //-ofile 2: Save xx-smrib.sg.
    if (dbmsh3d_cmd_ofile()) //>1
      pv->save_sg ();

    if (dbmsh3d_cmd_gui()) {
      //-all : View (-3dx, 0): Medial Scaffold Hypergraph After Regularization.
      if (pv->ms_vis_param_.sheets_ctable_.size() == 0)
        vis = pv->vis_ms_hypg ();
      else
        vis = pv->vis_ms_hypg_ctable ();
  ...
  //###########################################################################
  //-vlink 1-8: Medial Scaffold Rib Curve Smoothing.
  if (dbsk3d_cmd_vlink()!=0 ||
      (dbsk3d_cmd_transg()>=1 && dbsk3d_cmd_transg()<=8) || 
      (dbsk3d_cmd_sgsm()>=1 && dbsk3d_cmd_sgsm()<=8) || 
      (dbsk3d_cmd_all()>=1 && dbsk3d_cmd_all()<=8)) {
    if (dbmsh3d_pro_contiune == false) {
      pv->reset_mesh();
      pv->reset_fs_mesh();
      if (pv->load_cms() == false) {
        dbmsh3d_cmd_gui() = 0;
        vcl_cout << "\ndbsk3d_cmdproc_execute (vlink): Error loading " << pv->dir_prefix() + ".\n";
        return _root;
      }
    }

    //Draw the input medial scaffold before adding virtual links.
    if (dbmsh3d_pro_contiune==false && dbmsh3d_cmd_gui()) {
      pv->compute_dx_dy_dz ();
      pv->compute_vertex_node_size ();
    ...
    //Adding virtual links to the MS hypergraph.
    //-vlink 1:
    //-tab (def 50): Remove shock tabs with > # generators.
    //-a5 (def 30): Remove A5 with > # A5 fs_edges.
    //-cc (def 10): Remove shock curve with > # fs_edges.
    //-sc (def 30): Remove shock curve with > # fs_edges.
    //The input threshold is enlarged by some factor (to include more potential transitions).
    pv->add_MS_virtual_curves (dbsk3d_cmd_tab()*30, dbsk3d_cmd_a5()*3, dbsk3d_cmd_cc()*3, dbsk3d_cmd_sc()*3);

    //Build scaffold_graph_stand_alone again after the transition.
    pv->build_ms_graph_sa_from_cms ();
    dbmsh3d_pro_contiune = true;

    pv->set_dir_file (pv->dir_file() + "-vl");
    if (dbmsh3d_cmd_ofile()) {
      pv->save_cms ();
      pv->save_fs ();
    }

    //-ofile 2: Save xx-vl.sg.
    if (dbmsh3d_cmd_ofile()) //>1
      pv->save_sg ();

    if (dbmsh3d_cmd_gui()) {
      //-all : View (-4dx, 0): Medial Scaffold Hypergraph After Regularization.
      if (pv->ms_vis_param_.sheets_ctable_.size() == 0)
        vis = pv->vis_ms_hypg ();
      else
        vis = pv->vis_ms_hypg_ctable ();
      _root->addChild (shift_output (vis, (float)(-pv->view_dx_*4), 0, 0));
  ...
  //###########################################################################
  //-transg 1-9: Medial Scaffold Graph Transition Regularization.
  if (dbsk3d_cmd_transg()!=0 ||     
      (dbsk3d_cmd_sgsm()>=1 && dbsk3d_cmd_sgsm()<=9) || 
      (dbsk3d_cmd_allg()>=1 && dbsk3d_cmd_allg()<=9)) {
    if (dbmsh3d_pro_contiune == false) {
      pv->reset_mesh();
      pv->reset_fs_mesh();
    ...
    //-a5 (def 30): Run A5 Graph Transitions
    if (dbsk3d_cmd_a5() > 0) {
    ...
    //remove dummy nodes.
    //pv->graph_trans_remove_dummy_nodes ();
  ...
  //###########################################################################
  //-sgsm 1-10: Smooth the Mesial Scaffold Curves.
  if (dbsk3d_cmd_sgsm()!=0 || dbsk3d_cmd_allg()!=0) {
    if (dbmsh3d_pro_contiune == false) {
      pv->reset_mesh();
      pv->reset_fs_mesh();
    ...
    if (dbmsh3d_cmd_gui()) {
      //-all : View (-4dx, 0): Scaffold Graph After Smoothing.
      vis = pv->vis_ms_graph_sa ();
      _root->addChild (shift_output (vis, (float)(-pv->view_dx_*4), 0, 0));
    }
  }
  ...
  if (dbsk3d_cmd_bktseg()) {
    ...
    if (dbmsh3d_pro_contiune == false) {
      ...
      if (dbmsh3d_cmd_gui())
        _root->addChild (draw_ifs (input_pts, finalM_faces, COLOR_SILVER, true));
    }
  }
  ...
  if (dbsk3d_cmd_bktpst()==1) {
    ...
    if (bkt_recover_vid_richmesh (dbmsh3d_cmd_fileprefix()) == false) {
      dbmsh3d_cmd_gui() = 0;
      vul_printf (vcl_cout, "ERROR in recovery vertex id to a rich mesh!\n");
    }
  }
  //-bktpst 2: 
  //  Read in the prefix-bktinfo.txt
  ...
  //  Produce the 'candidate stitching' mesh: prefix_stitch_cand.ply.  
    ...
    if (bkt_pre_stitch_buckets (dbmsh3d_cmd_fileprefix(), dbmsh3d_cmd_bktbr()) == false) {
      dbmsh3d_cmd_gui() = 0;
      vul_printf (vcl_cout, "ERROR in pre sitich buckets!\n");
    }
  }
  ...
  if (dbsk3d_cmd_merge() == 1) { //-merge 1 -f prefix
    ...
    if (bkt_merge_final_surface(dbmsh3d_cmd_fileprefix(), pv->mesh()) == false) {
      dbmsh3d_cmd_gui() = 0;
      vul_printf (vcl_cout, "ERROR in merging the final surface mesh!\n");
    }
  ...
  //############### Save Shock Related Output Files ############### 
  if (pv->fs_mesh()->vertexmap().size()!=0) {
    vcl_string dirfile;
    //-ovfs :
    if (dbsk3d_cmd_fs_vtk_ofile()) {
      dirfile = dbul_get_dir_file (dbsk3d_cmd_fs_vtk_ofile());
      dirfile += ".vtk";
      vul_printf (vcl_cout, "Writing fs_mesh to VTK file (*.vtk) %s...\n", dirfile.c_str());
      pv->save_fs_vtk (dirfile);
    }
  }
  /* DUPLICATE with a similar section in dbmsh3d_cmdproc.cxx
  if (pv->mesh()->vertexmap().size()!=0) {
    ...
    if (dbmsh3d_cmd_ply2_ofile()) {
      ...
      dirfile += ".ply2";
      vul_printf (vcl_cout, "Writing PLY2 mesh file (*.ply2) %s...\n", dirfile.c_str());
