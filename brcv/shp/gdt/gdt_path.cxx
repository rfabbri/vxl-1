...
{
  vul_printf (vcl_cerr, "Geodesic path: length %lf, to source %d\n", 
  ...
  for (unsigned int i=0; i<I_tau_pairs_.size(); i++) {
    ...
    if (i>0) {
      ...
      cumu_len += path_len;
      vul_printf (vcl_cerr, "       len (%d-%d): %lf, cumulative len: %lf\n", i-1, i, path_len, cumu_len);
    }

    //: print the [i]-th point
    vul_printf (vcl_cerr, "     Point[%d] on edge %d with tau %lf, (%.2f, %.2f, %.2f)\n", 
                 i, I->edge()->id(), tau, 
