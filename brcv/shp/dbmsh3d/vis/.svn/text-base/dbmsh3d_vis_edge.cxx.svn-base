...
{
  const dbmsh3d_vertex* sV = E->sV();
  const dbmsh3d_vertex* eV = E->eV();
  ...
  if (user_defined_class) {
    dbmsh3dvis_edge_SoLineSet* lineSet = new dbmsh3dvis_edge_SoLineSet (E);
...
SoSeparator* draw_edge (dbmsh3d_edge* E, const SbColor& color, const float width,
                        const bool user_defined_class)
{
