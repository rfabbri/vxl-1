//---------------------------------------------------------------------
// This is brcv/shp/dbmsh3d/dbmsh3d_pt_set.cxx
//:
// \file
...
dbmsh3d_pt_set* dbmsh3d_pt_set::clone ()
{
  ...
  PS2->set_vertex_id_counter (vertex_id_counter_);
  assert (PS2->vertexmap().size() == vertexmap_.size());
...
bool detect_bounding_box (dbmsh3d_pt_set* pt_set, vgl_box_3d<double>& bbox)
{ 
...
{  
  ...
  for (; it != pt_set->vertexmap().end(); it++) {
    ...
    v->get_pt().set (P.x()*scale + C2.x(), P.y()*scale + C2.y(), P.z()*scale + C2.z());
  }
}
...
void sample_skip_points (dbmsh3d_pt_set* pt_set, const unsigned int skip)
{
  ...
  for (; it != pt_set->vertexmap().end(); it++, c++) {
    ...
    if (c % skip != 0)
      erase_id.push_back (V->id());
  }

  //Go through erase_id and erase nodes from pt_set.
  for (unsigned int i=0; i<erase_id.size(); i++) {
    c = erase_id[i];
    pt_set->vertexmap().erase (c);
  }
}
