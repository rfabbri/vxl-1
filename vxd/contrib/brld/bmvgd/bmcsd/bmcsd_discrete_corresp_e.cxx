...
bool  bmcsd_discrete_corresp_e::
add_unique(const bmcsd_attributed_object_e &e, unsigned i, 
    std::list<bmcsd_attributed_object_e>::iterator *itr
    )
{
  std::list<bmcsd_attributed_object_e>::iterator 
  ...
  if (i >= corresp_.size()) {
    std::cout << "bmcsd_discrete_corresp_e:: resizing list\n";
    corresp_.resize(i+1);
  }
...
std::ostream&  operator<<(std::ostream& s, const bmcsd_attributed_object_e &c)
{
  c.print_summary(s);
  return s;
}

std::ostream&  operator<<(std::ostream& s, const bmcsd_discrete_corresp_e &c)
{
  s << "===========  bmcsd_discrete_corresp_e ===========\n";
  s << "# of Domain objects = num_objs0 + 1 = " << c.corresp_.size() << std::endl;
  ...
  for (i=0; i < c.corresp_.size(); ++i) {
    if (i < max_domain_elts) {
      s << "obj_id: " << i << " #corresp: " << c.corresp_[i].size() << std::endl;

      std::list< bmcsd_attributed_object_e >::const_iterator  itr;
  ...
  s << "Total domain objects with some correspondents: " << count_nonzero << std::endl;
  s << "Total number of corresps: " << total_n_corresp << std::endl;
  s << "Avg. number of corresps per domain object: " << (float)total_n_corresp/(float)c.corresp_.size() << std::endl;
...
b_read(vsl_b_istream &is)
{
  ...
  switch (ver)
  {
    ...
    default:
        std::cerr << "I/O ERROR: bmcsd_attributed_object_e::b_read(vsl_b_istream&)\n"
...
b_write(vsl_b_ostream &os) const
{
  ...
  for (unsigned i=0; i < corresp_.size(); ++i) {
    vsl_b_write<bmcsd_attributed_object_e>(os, corresp_[i]);
  }
}
...
b_read(vsl_b_istream &is)
{
  ...
  {
    case 1: {
      std::cerr << "Warning: this is an old correspondence file...\n"
       << "...it is incompatible with some newer tools\n";
      unsigned size;
      vsl_b_read(is, size);
      corresp_.resize(size);
      for (unsigned i=0; i < size; ++i)
        vsl_b_read<bmcsd_attributed_object_e>(is, corresp_[i]);
    } break;
    ...
    default:
        std::cerr << "I/O ERROR: bmcsd_discrete_corresp_e::b_read(vsl_b_istream&)\n"
             << "           Unknown version number "<< ver << '\n';
    is.is().clear(std::ios::badbit); // Set an unrecoverable IO error on stream
    return;
  }
...
version() const
{
  return 4; // bumped to 4 for enhanced curve sketch, even though it is the same
            // as 3 for now
}
...
bmcsd_discrete_corresp_e::
is_a() const 
{return "bmcsd_discrete_corresp_e";}

//: remove every element whose cost is above threshold
//\todo test
void bmcsd_discrete_corresp_e::
...
void bmcsd_discrete_corresp_e::
compare_and_print( const bmcsd_discrete_corresp_e *gt) const
{
  ...
  std::cout << "# of objects in image 1 having ground-truth among any candidates:\n\t" 
      ...
      for (itr = corresp_[i].begin(); itr != corresp_[i].end(); ++itr) {
        ...
        if (result != gt->corresp_[i].end()) {
          if (vnl_math_isfinite(result->cost_)) {
            ...
            std::cout << "Warning: non-finite cost in ground-truth corresp\n";
          }
        }
      }
      if (has_finite)
  ...
  std::cout << "# objects in image 1 having correct candidates ranked among top 50% matches:\n\t" 
    << 100.0*(float)n_top_50/(float)n_gt << "% ("  << n_top_50 << " out of " << n_gt  << ")." << std::endl;

  std::cout << "# objects in image 1 having correct candidates in top 5 matches:\n\t" 
    << 100.0*(float)n5/(float)n_gt << "% ("  << n5 << " out of " << n_gt  << ")." << std::endl;
...
//for each object p1 in image 1
  ...
  for (unsigned p1_idx=0; p1_idx < this->corresp_.size(); ++p1_idx) {
    std::list<bmcsd_attributed_object_e>::const_iterator  result;
    ...
    if (result != this->corresp_[p1_idx].end()) { // found
      ...
      std::list< bmcsd_attributed_object_e >::const_iterator  itr = corresp_[p1_idx].begin();
...
// found.
std::list<bmcsd_attributed_object_e>::const_iterator bmcsd_discrete_corresp_e::
find_right_corresp_mincost(unsigned p1_idx, const bmcsd_discrete_corresp_e *gt) const
{
  ...
  unsigned i = p1_idx;
  std::list<bmcsd_attributed_object_e>::const_iterator min_cost_itr = corresp_[i].end();
...
//
bool bmcsd_discrete_corresp_e::
is_gt_among_top5(unsigned p1_idx, const bmcsd_discrete_corresp_e *gt) const
{
...
{
  //  std::cout << "Using lowe's criterion with ratio " << ratio << std::endl;
  sort();
  for (unsigned i=0; i < n0(); ++i) {
    ...
    bmcsd_attributed_object_e attr = corresp_[i].back();
    ...
    //    std::cout << "second cost: " << attr_second.cost()
    //      << " first cost: "<< attr.cost() << std::endl;
    if (ratio*attr_second.cost() < attr.cost()) {
    //      std::cout << "Inserting unambiguous\n";
      corresp_[i].push_back(attr);
    }
  }
}
