...
set_original_curve_sizes(const vcl_vector<vcl_vector<unsigned> > &original_curve_sizes)
{
  original_curve_sizes_ = original_curve_sizes;
}

unsigned mw_odt_curve_stereo::
...
void mw_odt_curve_stereo::
set_sseq(const vcl_vector<dbbl_subsequence_set> &sseq)
{
    sseq_ = sseq;
}

vcl_vector<dbbl_subsequence_set> mw_odt_curve_stereo::
get_sseq()
{
    return sseq_;
}
...
match_using_orientation_dt_extras(vcl_vector<unsigned long> *votes_ptr, vcl_vector<vcl_vector<vcl_set<int> > > &inlierEdgelsPerCandidate,
                                  vcl_vector<vcl_vector<unsigned> > &inlierViews, vcl_vector<vcl_vector<unsigned> > &edge_support_count_per_candidate,
				  vcl_vector<vcl_vector<vcl_vector<int> > > *edge_index_chain_per_candidate_ptr)
{
#ifndef NDEBUG
  ...
  if(edge_index_chain_per_candidate_ptr==NULL) {
    vcl_vector<vcl_vector<vcl_vector<int> > > *dummy_pointer;
    edge_index_chain_per_candidate_ptr = dummy_pointer;
  }

  vcl_vector<vcl_vector<vcl_vector<int> > > &edge_index_chain_per_candidate = *edge_index_chain_per_candidate_ptr;
  ...
  vcl_vector<dbbl_subsequence_set> sseq = this->get_sseq();

  for (unsigned ic=0; ic < num_candidates(); ++ic) {
    unsigned origID_u = sseq[this->v1()][crv_candidates(ic)].orig_id();
    if(vcl_find(usedCurves_[this->v1()].begin(), usedCurves_[this->v1()].end(), origID_u) == usedCurves_[this->v1()].end()) {
      ...
      vcl_vector<vcl_set<int> > inlierEdgelsPerConfView;
      vcl_vector<unsigned> curInlierViews;
      vcl_vector<vcl_vector<int> > edge_index_chain_per_conf_view(nviews()-2);
      ...
      vcl_fill(edge_support_count_per_candidate[ic].begin(),edge_support_count_per_candidate[ic].end(),0);
	...
	// Compute match cost
	unsigned d_vote;
	vcl_set<int> cur_inliers;
	vcl_vector<int> edge_index_chain(curve_3d.size(),-1);
	if (sels_.empty()) {
...
bool mw_odt_curve_stereo::
match_using_orientation_dt_extras(vcl_vector<unsigned long> *votes_ptr, vcl_vector<vcl_vector<vcl_set<int> > > &inlierEdgelsPerCandidate,
                                  vcl_vector<vcl_vector<unsigned> > &inlierViews, vcl_set<int> curve_ids, 
				  vcl_vector<vcl_vector<unsigned> > &edge_support_count_per_candidate)
{
  ...
  for (unsigned ic=0; ic < num_candidates(); ++ic) {
    ...
    if((curve_ids.find(origID) != curve_ids.end()) && 
       (vcl_find(usedCurves_[this->v1()].begin(), usedCurves_[this->v1()].end(), origID_u) == usedCurves_[this->v1()].end())) {
      //Anil: Adding a threshold for minimum number of inlier views Apr 18th, 2014
      //Anil: Also adding vector of inlier views per candidate, and inlier edgels per confirmation view for each candidate
      unsigned num_inlier_views = 0; 
      vcl_vector<vcl_set<int> > inlierEdgelsPerConfView;
...
#ifdef MW_VERBOSE_DEBUG
    vcl_cout << "\t\tinliers on view[" << vv << "] = " << d_vote << vcl_endl;
#endif
  }
                                           ...
                                           const dbdif_rig &rig, dbdif_1st_order_curve_3d *crv_ptr, vcl_vector<bool> &flags, unsigned &recon_shift)
{
  ...
  vcl_vector<bool> secondAnchorFlags;
  ...
  {
      //vcl_cout << "FIRST: " << vsol_flags_[v0()][curve_id][i] << ", SECOND: " << secondAnchorFlags[i-ini_id_sub];
      //vcl_cout << ", JOINT: " << (vsol_flags_[v0()][curve_id][i] || secondAnchorFlags[i-ini_id_sub]) << vcl_endl;
    ...
    ) const
{
  ...
  vcl_string out_fname = "alignment.txt";
  ...
  if(dummyFlag)
  {
      ...
      out_file << numAlignmentSamples << vcl_endl;
  }
  ...
  for (unsigned di0=0; di0 + ini_id_sub <= end_id_sub; ++di0) {
    ...
    if(dummyFlag)
    {
        ...
        out_file << firstSample << " " << dummyID << " " << lineCoef[0] << " " << lineCoef[1] << " " << lineCoef[2] << vcl_endl;
    }
    ...
    )
{
  ...
  for (unsigned di0=0; di0 + ini_id_sub <= end_id_sub; ++di0) {
    ...
    if(dummyFlag)
    {
        unsigned firstSample = ini_id_sub + di0;
        out_file << firstSample << " " << dummyID << " " << lineCoef[0] << " " << lineCoef[1] << " " << lineCoef[2] << vcl_endl;
        //vcl_cout << "SHIFT: " << ini_id_sub << vcl_endl;
        //vcl_cout << "END: " << end_id_sub << vcl_endl;
        //vcl_cout << "EP CONTAINER SIZE: " << ep(second_view-1).size() << vcl_endl;
        //vcl_cout << "SPANNED SIZE: " << end_id_sub - ini_id_sub + 1 << vcl_endl;
    }
    ...
    )
{
  ...
  vcl_vector<vsol_spatial_object_2d_sptr> curvesIntermediate;
  ...
  dbsol_save_cem(curvesIntermediate, vcl_string("intermediate_v0.cemv"));
  ...
  )
{
  ...
  for (unsigned i=0; i < vsols.size(); ++i) {
    ...
    vcl_vector<dbecl_episeg_sptr> eps = factory.convert_curve_using_tangents(dc, tgts[i], &ss_partition, print, i);
  ...
  bool track_supporting_edges)
{
  ...
  for (unsigned c=0; c < ncurves; ++c) {
    ...
    vcl_vector<vcl_vector<vcl_set<int> > > inlierEdgelsPerCandidate;
    vcl_vector<vcl_vector<unsigned> > inlierViews;
    vcl_vector<vcl_vector<unsigned> > edge_support_count_per_candidate;
...
//        vcl_cout << "TOTAL NUMBER OF SUPPORT FOR FIRST CANDIDATE: " << inlierEdgelsPerCandidate[0].size() << vcl_endl;
  ...
  bool track_supporting_edges)
{
  ...
  vcl_vector<dbbl_subsequence_set> sseq = s.get_sseq();
  if(isFirstRun)
  {
    for (unsigned c=0; c < ncurves; ++c) {
      unsigned origID = sseq[s.v0()][c].orig_id();
      if(vcl_find(s.usedCurves_[s.v0()].begin(), s.usedCurves_[s.v0()].end(), origID) == s.usedCurves_[s.v0()].end()) {
	...
	vcl_vector<unsigned long> votes;
	vcl_vector<vcl_vector<vcl_set<int> > > inlierEdgelsPerCandidate(s.num_candidates());
	vcl_vector<vcl_vector<unsigned> > inlierViews(s.num_candidates());
	vcl_vector<vcl_vector<unsigned> > edge_support_count_per_candidate(s.num_candidates());
	vcl_vector<vcl_vector<vcl_vector<int> > > edge_index_chain_per_candidate(s.num_candidates());
...
reconstruct_from_corresp_attr(
    ...
    vcl_vector<dbdif_1st_order_curve_3d> *crv3d_ptr,    vcl_vector< dbmcs_curve_3d_attributes > *attr_ptr,
    unsigned seed_id
    )
{
  ...
  for (unsigned c=0; c < ncurves; ++c) {
    ...
    vcl_vector<bool> certaintyFlags;
    unsigned ini_shift;
    ...
    //Anil: Propagating the edgel support data from mw_discrete_corresp to dbmcs_curve_3d_attributes
    ...
    vcl_vector<vcl_map<unsigned,unsigned> > mate_curve_weights(s.nviews()-2);

    for (unsigned v=0; v+2<s.nviews(); ++v)
    {
      vcl_set<int> curViewSupport = corresp[c].front().supportingStructures_[v];
      unsigned numInlierEdgels = curViewSupport.size();

      for (vcl_set<int>::iterator edgeit=curViewSupport.begin(); edgeit != curViewSupport.end(); ++edgeit)
      {
	int curEdgel = *edgeit;
	...
	if(curCurve > -1){
	  ...
	  if(mate_curve_weights[v].find(u_curCurve) == mate_curve_weights[v].end())
	    mate_curve_weights[v].insert(vcl_pair<unsigned,unsigned>(u_curCurve,1));
	  else
    ...
    //Anil: Propagating the inlier view data from mw_discrete_corresp to dbmcs_curve_3d_attributes
    ...
    //Anil: Propagating the edge support count vector from mw_discrete_corresp to dbmcs_curve_3d_attributes
    //Also propagating the offset to convert hypothesis indices to image curve indices using subseqence set
    //vcl_cout << "SHIFT: " << sseq[s.v0()][c].ini() << "+" << ini_shift << vcl_endl;
    ...
    //Anil: Propagating the original ID of the unbroken image curve from mw_odt_curve_stereo to 
    //      dbmcs_curve_3d_attributes using dbbl_subsequence_set
    attr.back().orig_id_v0_ = sseq[s.v0()][c].orig_id();
    attr.back().orig_id_v1_ = sseq[s.v1()][corresp[c].front().id()].orig_id();

    //Anil: Propagating the ID of the broken image curve (before pruning) from mw_odt_curve_stereo to 
    //      dbmcs_curve_3d_attributes using dbbl_subsequence_set
    attr.back().int_id_v0_ = sseq[s.v0()][c].int_id();
    attr.back().int_id_v1_ = sseq[s.v1()][corresp[c].front().id()].int_id();

    //Anil: Adding the certainty flags to the attributes
    ...
    )
{
  ...
  for (vcl_list<mw_attributed_object>::const_iterator m=corresp[c].begin(); m != corresp[c].end(); ++m) {  
    ...
    //assert (found);
    if(!found)
      vcl_cout << "ERROR: Candidate index could not be located during reconstruction!" << vcl_endl;
