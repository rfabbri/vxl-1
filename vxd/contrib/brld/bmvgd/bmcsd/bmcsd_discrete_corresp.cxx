...
{
  std::list<bmcsd_attributed_object>::iterator 
  ...
  if (i >= corresp_.size()) {
    std::cout << "bmcsd_discrete_corresp:: resizing list\n";
    corresp_.resize(i+1);
  }
...
std::ostream&  operator<<(std::ostream& s, const bmcsd_match_attribute &c)
{
...
std::string bmcsd_match_attribute::
is_a() const 
{return "bmcsd_match_attribute";}

std::ostream&  operator<<(std::ostream& s, const bmcsd_attributed_object &c)
{
  c.print_summary(s);
  return s;
}

std::ostream&  operator<<(std::ostream& s, const bmcsd_discrete_corresp &c)
{
...
b_read(vsl_b_istream &is)
{
  ...
  switch (ver)
  {
    ...
    default:
        std::cerr << "I/O ERROR: bmcsd_match_attribute::b_read(vsl_b_istream&)\n"
...
b_read(vsl_b_istream &is)
{
  ...
  switch (ver)
  {
    ...
    default:
        std::cerr << "I/O ERROR: bmcsd_attributed_object::b_read(vsl_b_istream&)\n"
             << "           Unknown version number "<< ver << '\n';
    is.is().clear(std::ios::badbit); // Set an unrecoverable IO error on stream
    return;
  }
...
b_read(vsl_b_istream &is)
{
  ...
  switch (ver)
  {
    ...
    default:
        std::cerr << "I/O ERROR: bmcsd_discrete_corresp::b_read(vsl_b_istream&)\n"
             << "           Unknown version number "<< ver << '\n';
    is.is().clear(std::ios::badbit); // Set an unrecoverable IO error on stream
    return;
  }
...
//for each object p1 in image 1
  ...
  for (unsigned p1_idx=0; p1_idx < this->corresp_.size(); ++p1_idx) {
    std::list<bmcsd_attributed_object>::const_iterator  result;
    ...
    if (result != this->corresp_[p1_idx].end()) { // found
      ...
      std::list< bmcsd_attributed_object >::const_iterator  itr = corresp_[p1_idx].begin();
...
// found.
std::list<bmcsd_attributed_object>::const_iterator bmcsd_discrete_corresp::
find_right_corresp_mincost(unsigned p1_idx, const bmcsd_discrete_corresp *gt) const
{
  ...
  unsigned i = p1_idx;
  std::list<bmcsd_attributed_object>::const_iterator min_cost_itr = corresp_[i].end();
