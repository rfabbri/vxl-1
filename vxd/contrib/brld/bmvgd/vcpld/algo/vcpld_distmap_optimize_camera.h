...
class vcpld_distmap_optimize_camera_lsqr : public vnl_least_squares_function
{
  ...
  void lookup_distance(const vgl_homg_point_2d<double> &pt, double e[]) const
  {
    ...
    const std::vector<sdet_edgel*> &ev = em_->edge_cells.begin()[l];
#ifndef NDEBUG
    if (ev.size() == 0) {
      std::cout << "label: " << l 
        << " px: " << px << " py: " << py << std::endl;

      std::cout << "label_: " << label_.top_left_ptr()[l] << std::endl;
      std::cout << "DT: "<< dt_(p_i,p_j) << std::endl;
      std::cout << "DT(label): "<< dt_.top_left_ptr()[l] << std::endl;
      // const std::vector<sdet_edgel*> &ev = em_->edge_cells.begin()[l];
    }
    assert(ev.size());
#endif
  ...
  {
    std::vector<double> ev((unsigned)(fx.size()*0.5));
    double e=0;
    for (unsigned i=0; i+1 < fx.size(); i+= 2) {
      ev[i/2] = std::sqrt(fx[i]*fx[i] + fx[i+1]*fx[i+1]);
      e += ev[i/2];
//      std::cout << "err: " << ei << std::endl;
    }
