...
public:
  ...
  // information
  // 
  // \return std::vector with partition into episegs for which all tangents with
  // epipolar lines are either greater or smaller than or equal \p tau_dtheta as
  // set in set_tangent_threshold.
  std::vector<becld_episeg_sptr> convert_curve_using_tangents(
...
protected:
  ...
  void link_episegs(std::vector<becld_episeg_sptr>& segs) const;
};
...
//
class becld_delta_angle_predicate : public std::unary_function<unsigned, bool> {
public:
...
private:
  ...
  const std::vector<double> &tangents_;
  becld_epipole_sptr epipole_;
