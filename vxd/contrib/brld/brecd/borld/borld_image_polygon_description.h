...
{
public:
  std::map<std::string, std::vector<vsol_polygon_2d_sptr> > data_;

  virtual unsigned get_type() { return borld_image_data_description_type::polygonal_outline; }

  void add_polygon(std::string cat, vsol_polygon_2d_sptr p);
  std::vector<vsol_polygon_2d_sptr>& get_polygon_vector(std::string cat);

  bool category_exists(std::string cat) { return data_.find(cat) != data_.end(); }

  std::map<std::string, std::vector<vsol_polygon_2d_sptr> >& get_category_map() { return data_; }
