...
{
  std::map<std::string, std::vector<vsol_box_2d_sptr> >& map = box_data->get_category_map();

  //: add each category and counts one by one
  for (std::map<std::string, std::vector<vsol_box_2d_sptr> >::iterator i = map.begin(); i != map.end(); i++) 
...
{
  std::map<std::string, std::vector<vsol_polygon_2d_sptr> >& map = poly_data->get_category_map();

  //: add each category and counts one by one
  for (std::map<std::string, std::vector<vsol_polygon_2d_sptr> >::iterator i = map.begin(); i != map.end(); i++) 
    category_list_[i->first] = (i->second).size();
...
borld_image_description::borld_image_description(borld_image_mask_description_sptr mask_data, std::vector<std::string>& cats)
{
...
{
  std::cout << "IMPLEMENT: borld_image_description::b_read()\n";
}

void borld_image_description::b_write()
{
  std::cout << "IMPLEMENT: borld_image_description::b_write()\n";
}
...
void borld_image_description::write_xml(std::ostream& os)
{
  os << "<type name = \"image\">\n";
  os << "\t<description>\n";
  ...
  if (category_data_->cast_to_image_polygon_description()) {
    ...
    category_data_->cast_to_image_bbox_description()->write_xml(os); 
      ...
      for (std::map<std::string, int>::iterator iter = category_list_.begin(); iter != category_list_.end(); iter++) {
