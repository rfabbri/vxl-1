...
{ 
  ...
  if (desc->category_data_->cast_to_image_bbox_description()) {
    ...
    //std::cout << "image polygon desc\n";

    if (desc->category_data_->cast_to_image_polygon_description()->category_exists(model_category)) {
      std::vector<vsol_polygon_2d_sptr>& polys = desc->category_data_->cast_to_image_polygon_description()->get_polygon_vector(model_category);
      //std::cout << "model_cat: " << model_category << " size of polys: " << polys.size() << "\n";
    ...
    std::cout << "WARNING: description boxes could not be determined!!!!\n";
    return false;
  }
  return true;
}
