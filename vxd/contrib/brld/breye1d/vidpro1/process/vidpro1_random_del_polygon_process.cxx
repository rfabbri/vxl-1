...
vidpro1_random_del_polygon_process::vidpro1_random_del_polygon_process() : bpro1_process(), first_frame_(true)
{
  ...
  {
    std::cerr << "ERROR: Adding parameters in vidpro1_random_del_polygon_process::vidpro1_random_del_polygon_process()" << std::endl;
  }
}
...
std::vector< std::string > vidpro1_random_del_polygon_process::get_input_type()
{
...
std::vector< std::string > vidpro1_random_del_polygon_process::get_output_type()
{
  std::vector< std::string > to_return;
  to_return.push_back( "vsol2D" );
  to_return.push_back( "vsol2D" );
  ...
  if (delete_segments) {
    if (add_lines) {  // both delete and add
      ...
      if (!pieces.size() || !del_pieces.size()) {
        std::cout << "vidpro1_random_del_polygon_process::execute() - both deleted and non deleted pieces are empty!\n";
        return false;
      }
      ...
      for (unsigned i = 0; i<pieces.size(); i++) {
        //std::cout << "remaining piece: " << i << " size: " << pieces[i]->size() << std::endl;
      ...
      for (unsigned i = 0; i<del_pieces.size(); i++) {
        std::cout << "deleted piece: " << i << " size: " << del_pieces[i]->size() << std::endl;
    ...
    if (add_lines) {
      ...
      std::cout << "there are " << pieces.size() << " pieces\n";
