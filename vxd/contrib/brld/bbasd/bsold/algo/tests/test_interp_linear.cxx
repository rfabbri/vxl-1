...
MAIN( test_interp_linear )
{
   ...
   TEST_NEAR("Total Length",c.length(),1+std::sqrt(2.0), 
   ...
   TEST("Point at arclength sqrt(2)",almost_equal3(c.point_at(std::sqrt(2.0))->get_p(),
                                                   pts[1]->get_p()),true);
   ...
   TEST_NEAR("Total Length",c.length(),2+std::sqrt(2.0), 
              std::numeric_limits<double>::epsilon()*100);
