...
bool almost_equal(vgl_point_2d<double> a, vgl_point_2d<double> b) {
  return (std::fabs(a.x()-b.x()) < bgld_arc::near_zero_value &&
          std::fabs(a.y()-b.y()) < bgld_arc::near_zero_value);
}
...
MAIN( test_arc )
{
  ...
  std::cout << "construct upper hemisphere of unit circle:" << std::endl;
  ...
  TEST("start_angle() ", std::fabs(a.start_angle()-0.0)<tolerance, true);
  TEST("end_angle() ", std::fabs(a.end_angle()-PI)<tolerance, true);
  ...
  TEST("tangent_angle_at() start", std::fabs(a.tangent_angle_at(0.0)-PIOVERTWO)<tolerance, true);
  TEST("tangent_angle_at() end", std::fabs(a.tangent_angle_at(1)-3*PIOVERTWO)<tolerance, true);
  ...
  TEST("tangent_angle_at() mid", 
      std::fabs(a.tangent_angle_at_length(0.5*a.length())-PI)<tolerance, true);

  vgl_vector_2d<double> midtan = a.tangent_at_length(0.5*a.length());

  //std::cout << "tangent vector at mid point: " << midtan << std::endl;
  TEST("tangent_at_length() mid", std::fabs(atan2(midtan.y(),midtan.x())-PI)<tolerance, true);
  //std::cout << "tangent vector at mid point angle: " << atan2(midtan.y(),midtan.x()) << std::endl;
  ...
  std::cout << "Testing arc in reverse direction\n";
  bgld_arc d(end, start, other);
  TEST("reverse arc curvature() ", d.curvature(), 1.0);
  TEST("reverse arc center() ", d.center() == center, true);
  TEST("reverse arc start_angle() ", std::fabs(d.start_angle()-PI)<tolerance, true);
  TEST("reverse arc end_angle() ", std::fabs(d.end_angle()-0.0)<tolerance, true);
  ...
  TEST("reverse arc tangent_angle_at() ", 
      std::fabs(a.tangent_angle_at_length((1.0/4.0)*a.length())-3.0*PI/4.0)<tolerance, true);

  midtan = a.tangent_at_length((1.0/4.0)*a.length());
  TEST("reverse arc tangent_at() ", std::fabs(atan2(midtan.y(),midtan.x())-3.0*PI/4.0)<tolerance, true);

  std::cout << "try to construct an arc from linear points: " << std::endl;
  ...
  //std::cout << "f center " << f.center() << std::endl;
  //std::cout << "f curv " << f.curvature() << std::endl;
  TEST("third arc curvature() ", std::fabs(f.curvature()-1.0/2.0) < tolerance, true);
  ...
  TEST_NEAR("Length", a2.length(), 8*std::sqrt(2.0), tolerance);
