...
MAIN( test_poly_line )
{
  ...
  TEST("tangent_angle_at() start", std::fabs(a.tangent_angle_at(0.0)-0.0)<NearZeroValue, true);
  TEST("tangent_angle_at() start", std::fabs(a.tangent_angle_at(1)-0.0)<NearZeroValue, true);

  vgl_vector_2d<double> midtan = a.tangent_at(0.5);

  TEST("tangent_at() mid", std::fabs(atan2(midtan.y(),midtan.x())-0.0)<NearZeroValue, true);

  midtan = a.tangent_at_length(1.0);
  TEST("tangent_at_length() mid", std::fabs(atan2(midtan.y(),midtan.x())-0.0)<NearZeroValue, true);
