...
mid_pt() const 
{
  ...
  if (std::abs(this->k1()) >= bgld_biarc_e_k){
...
point_at( double s) const
{
  ...
  if ( s <= this->len1() ){
    ...
    if (std::abs(this->k2()) >= bgld_biarc_e_k){
...
compute_biarc_params(){
  ...
  if ( (std::fabs(tdiff) < bgld_biarc_e_angle) || 
    (std::fabs(tdiff - vnl_math::pi) < bgld_biarc_e_angle) || 
    (std::fabs(tdiff - 2*vnl_math::pi) < bgld_biarc_e_angle) )
  {
    // straight line (still need to check mod 2*pi)
    if (std::fabs(std::fmod(psi-t0,2*vnl_math::pi)) < bgld_biarc_e_angle){ 
      k1 = 0;
      k2 = 0;
      d1 = d;
      d2 = 0;
    ...
    if ( ((std::fabs(k3) > bgld_biarc_e_k) || (std::fabs(k4)>bgld_biarc_e_k)) && std::fabs(k4-k3) > bgld_biarc_e_k ){
      if (std::fabs(k3)<bgld_biarc_e_k){
        k3 = 0;
        d3 = d * std::sin((t2+t0)/2-psi)/ std::sin((t2-t0)/2);
        d4 = this->compute_arclength(t0,t2,k4);
      }
      else {
        if (std::fabs(k4) < bgld_biarc_e_k){
          k4 = 0;
