...
compute_end_pt( double k0, double gamma, double len, bool normalized ) const {
  ...
  if ( std::fabs(gamma) < bgld_eulerspiral_e_gamma ){ 
    if ( std::fabs(k0) < bgld_eulerspiral_e_k ){
      //straight line
...
//
  ...
  const std::string bgld_eulerspiral_lookup_table::file_path = std::string(BRLD_LIB_DIR);
#else
  const std::string bgld_eulerspiral_lookup_table::file_path = std::string();
#endif

const std::string bgld_eulerspiral_lookup_table::
file_name = std::string("bgld_eulerspiral_lookup_table.bvl");
...
bgld_eulerspiral_lookup_table::bgld_eulerspiral_lookup_table(){
  std::string full_path = bgld_eulerspiral_lookup_table::file_path + std::string("/") +
