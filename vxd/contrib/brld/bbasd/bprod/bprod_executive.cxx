...
std::ostream& operator<<(std::ostream& os, bprod_signal s){
  switch(s){
...
{
  public:
    executive_debug(bprod_executive* exec, std::ostream& os) : exec_(exec), os_(os) {}
    ...
    virtual void notify_enter(const bprod_process* const pro, unsigned int timestamp)
    {
      ...
      std::string pname = exec_->process_name(pro);
      ...
      for(unsigned i=0; i<process_stack_.size(); ++i)
        os_ << "  ";
      os_ << "enter: "<< pname << "  timestamp = " << timestamp <<std::endl;
    ...
    virtual void notify_exit(const bprod_process* const pro, unsigned int /*timestamp*/)
    {
      ...
      std::string pname = name_stack_.back();
      display_mutex_.lock();
#if BPROD_HAS_PTHREADS
      os_ << pthread_self();
#endif
    ...
    virtual void notify_post_exec(const bprod_process* const pro)
    {
      ...
      std::map<const bprod_process* const, long int>::iterator f = runtimes_.find(pro);
      if( f == runtimes_.end() )
        runtimes_[pro] = t;
      else
        f->second += t;
  ...
  private:
    ...
    std::vector<const bprod_process*> process_stack_;
    std::vector<std::string> name_stack_;
    std::map<const bprod_process* const, long int> runtimes_;
    vul_timer timer_;
    std::ostream& os_;
...
bprod_process_sptr
bprod_executive::find(const std::string& name) const
{
  typedef std::map<std::string, bprod_process_sptr >::const_iterator Itr;
...
bprod_executive::run_step() const
{
  ++time_;
  bprod_signal retval = BPROD_VALID;
  typedef std::set<bprod_process_sptr >::const_iterator Itr;
...
#ifndef NDEBUG
  if(sinks_.empty()){
    std::cerr<< "WARNING: There are no data sinks in this process graph" << std::endl;
  }
#endif
}
...
bprod_executive::enable_debug()
{
  if(debug_)
    delete debug_;
  debug_ = new executive_debug(this,std::cerr);
}
