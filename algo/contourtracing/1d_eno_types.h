 ...
 * $Header: /vision/projects/cvsroot2svn/cvsroot/lemsvxlsrc/algo/contourtracing/1d_eno_types.h,v 1.3 2004-10-15 16:37:27 mleotta Exp $
 * $Log: not supported by cvs2svn $
 * Revision 1.2  2003/11/12 22:01:35  mcchang
 * MC..finished. it compiles
 *
 * Revision 1.1  2003/10/29 15:32:52  mcchang
 * Ming
 *
 * Revision 1.1.1.1  2003/03/31 18:17:18  mcchang
 * no message
 *
 */
...
/**************************************************
  ...
  @member@ label - label of the data at the given zero crossings.
                   Each object in the image is given a different
       label.
  @end@
  **/
...
/**************************************************
  ...
  @member@ jump - indicate whether the shock in the interval is a simple
  discontinuity or a jump discontinuity. The interpolation values are 
  different on either side of a jump discontinuity.
...
int enoimage_compute_shocks_tbs(ENO_Interval_Image_t *eno_image,double const *data);
...
ENO_Interval_Image_t *enoimage_from_data(double *surface,
                                         int height,int width);
ENO_Interval_Image_t *enoimage_from_data_tbs(double *surface,
                                         int height,int width);
...
int enointerv_compute_interpolants_along_all_gridlines(ENO_Interval_t *interval,
                                                       ...
                                                       int len,int stride);
int  enointerv_place_shock_in_interval(ENO_Interval_t *prev,
               ENO_Interval_t *cur,
               ENO_Interval_t *next,
               double const *data, int stride, 
               int index);
int  enointerv_place_jump_in_interval(ENO_Interval_t *prev,
              ENO_Interval_t *cur,
              ENO_Interval_t *next,
              double const *data, int stride, 
              int index);
...
int enointerv_place_shocks_along_gridline_tbs(ENO_Interval_t *interval,double const *data,
                int len, int stride);
...
int eno_just_interp_print(ENO_Interpolant_t *interp,FILE *fp);

void eno_intv_copy(ENO_Interval_t *src, ENO_Interval_t *dest);
int
eno_just_interv_print(ENO_Interval_t *interval, FILE *fp);

/* eno_interpolant.c */

double eno_interpolation(ENO_Interval_t *ei, double loc);
...
int
enointerp_compute_linear_polynomial(ENO_Interpolant_t *interp,
            double x_loc, 
            double const data[kENODataLength]);

int
enointerp_compute_polynomial_one_sided(ENO_Interpolant_t *interp,
               double x_loc, 
               double const data[kENODataLength],
               int forward);
...
int
enointerp_make_interpolant_one_sided(ENO_Interpolant_t *interp,
             double start,double end,
             double const data[kENODataLength],
             int forward);
int
enointerp_make_linear_interpolant(ENO_Interpolant_t *interp,
          double start,double end,
          double const data[kENODataLength]);
...
int enoshock_make_shock(ENO_Interval_t *prev,ENO_Interval_t *cur,
                        ENO_Interval_t *next);
int
enoshock_check_for_shock_tbs(ENO_Interval_t *prev,ENO_Interval_t *cur,
           ENO_Interval_t *next);
...
int enozerox_place_jump_zerox_in_interval(ENO_Interval_t *prev,
            ENO_Interval_t *cur,
            ENO_Interval_t *next);
/* eno_label_zerox.c*/
...
void compute_interpolated_array(ENO_Interval_t *ei_array, 
        double *interp_array,
        double offset, 
        int height, int width);


void eno_zoom(double *surface, double *surface_high_res, 
        int height, int width, int zoom);

int enoimage_alloc_data(ENO_Interval_Image_t *enoimage,int height,int width);
